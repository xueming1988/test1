                                        ...
                                        mDNSInterfaceID InterfaceID, mDNSServiceCallback Callback, void *Context, mDNSu32 flags)
{
    ...
    mDNSu32 i;
    mDNSu32 hostTTL;
    mDNSu32 standardTTL = kStandardTTL;
    AuthRecType artype;
    ...
    domainname lpbonjour, airplay, hap;

    MakeDomainNameFromDNSNameString(&lpbonjour, kLinkplayBonjourSrvType);
    MakeDomainNameFromDNSNameString(&airplay, kAirplayBonjourSrvType);
    MakeDomainNameFromDNSNameString(&airplay, kHapBonjourSrvType);

    //Use short TTL for Airplay/Hap/Linkplay bonjour.
    //So these services can be shutdown quickly.
    if (SameDomainName(type, &lpbonjour) 
        || SameDomainName(type, &airplay)
        || SameDomainName(type, &hap)) {
        standardTTL = kLinkplayBonjourSmallTTL;
    }
    ...
    mDNS_SetupResourceRecord(&sr->RR_ADV, mDNSNULL, InterfaceID, kDNSType_PTR, standardTTL, kDNSRecordTypeAdvisory, artype, ServiceCallback, sr);
    mDNS_SetupResourceRecord(&sr->RR_PTR, mDNSNULL, InterfaceID, kDNSType_PTR, standardTTL, kDNSRecordTypeShared,   artype, ServiceCallback, sr);
    ...
    mDNS_SetupResourceRecord(&sr->RR_TXT, mDNSNULL, InterfaceID, kDNSType_TXT, standardTTL, recordType, artype, ServiceCallback, sr);
    ...
    for (i=0; i<NumSubTypes; i++)
    {
        ...
        mDNS_SetupResourceRecord(&sr->SubTypes[i], mDNSNULL, InterfaceID, kDNSType_PTR, standardTTL, kDNSRecordTypeShared, artype, ServiceCallback, sr);
