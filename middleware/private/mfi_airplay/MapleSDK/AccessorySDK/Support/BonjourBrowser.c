...
static void _BonjourBrowser_Stop(void* inArg)
{
    ...
    if (wasStarted)
        bb_ulog(kLogLevelTrace, "Stopping browse for %s\n", browser->serviceType);
...
// Set up a shared connection for DNS-SD operations.
    ...
    if (background) {
        ...
        check_noerr(err);
        if (!err)
            inBrowser->detailedBrowseRef = service;
    }
    ...
    void* inContext)
{
    ...
    for (nextService = &browser->services; (service = *nextService) != NULL; nextService = &service->nextForBrowser) {
        if (service->ifindex != inIfIndex)
            continue; // Interface must match (zero/all is not allowed here).
        if (stricmp(service->name, inName) != 0)
            continue; // Name must match.
        if (stricmp(service->type, inType) != 0)
            continue; // Type must match.
        if (stricmp(service->domain, inDomain) != 0)
            continue; // Domain must match.
        break;
    }
    if (add) {
...
static void _BonjourBrowser_HandleError(BonjourBrowserRef me, OSStatus inError)
{
    ...
    if (!me->retryTimer)
        _BonjourBrowser_PostEvent(me, kBonjourBrowserEventType_Restarted, NULL, NULL);
    ...
    *type = '\0';
    ...
    for (i = 0; i < n; ++i) {
        ...
        *domain = '\0';
        ...
        nameSize = snprintf(name, sizeof(name), "%s%s", type, domain);
        check(nameSize > 0 && (size_t)nameSize < sizeof(name));
        if (nameSize <= 0 || (size_t)nameSize >= sizeof(name))
            continue;
...
static CFDictionaryRef _BonjourDevice_CreateDictionary(BonjourDeviceRef inDevice, OSStatus* outErr)
{
    ...
    if (stricmp(bestService->type, "_airport._tcp.") == 0) {
        ...
        while (ParseCommaSeparatedNameValuePair(src, end, keyStr, sizeof(keyStr) - 1, &keyLen, NULL,
                   valueStr, sizeof(valueStr), &valueLen, NULL, &src)
            == kNoErr) {
            keyStr[keyLen] = '\0';
    ...
    if (p2pOnly)
        CFDictionarySetValue(newDeviceInfo, CFSTR(kBonjourDeviceKey_P2POnly), kCFBooleanTrue);
...
static BonjourServiceRef _BonjourDevice_GetBestService(BonjourDeviceRef inDevice)
{
    ...
    for (service = inDevice->services; service; service = service->nextForDevice) {
        if (strcmp(service->domain, "local.") == 0) {
            if (!service->isWiFi)
                return (service); // Prefer wired interfaces over all others.
            if (!firstNonP2P && !service->isP2P)
                firstNonP2P = service;
            if (!firstLocal)
                firstLocal = service;
        }
    ...
    BonjourServiceRef* outService)
{
    ...
    if (NetTransportTypeIsP2P(obj->transportType)) {
        obj->isP2P = true;
        obj->isWiFi = true;
    } else if (obj->transportType == kNetTransportType_WiFi)
        obj->isWiFi = true;
    ...
    void* inContext)
{
    ...
    if (add)
        _BonjourService_AddRData(service, inRData, inRDataSize);
    else
        _BonjourService_RemoveRData(service, inRData, inRDataSize);
...
static CFDictionaryRef _BonjourService_CreateDictionary(BonjourServiceRef inService, OSStatus* outErr)
{
    ...
    if (inService->isP2P)
        CFDictionarySetValue(serviceDict, CFSTR(kBonjourDeviceKey_P2P), kCFBooleanTrue);
    ...
    if (inService->isWiFi)
        CFDictionarySetValue(serviceDict, CFSTR(kBonjourDeviceKey_WiFi), kCFBooleanTrue);
...
static OSStatus _BonjourService_GetDeviceID(BonjourServiceRef inService, char* inKeyBuf, size_t inKeyMaxLen)
{
    ...
    if ((stricmp(inService->type, "_airplay._tcp.") == 0) || (stricmp(inService->type, "_mfi-config._tcp.") == 0)) {
    ...
    } else if ((inService->browser->flags & kBonjourBrowserFlag_StandardID) || (stricmp(inService->type, "_hap._tcp.") == 0)) {
        // This key should already be in AA:BB:CC:DD:EE:FF format, but validate and normalize to handle case issues, etc.
...
OSStatus BonjourDevice_GetDNSName(CFDictionaryRef inDeviceInfo, uint64_t inFlags, char* inBuf, size_t inMaxLen)
{
    ...
    if (firstWiredInfo)
        bestInfo = firstWiredInfo;
    else if (firstLocalInfo)
        bestInfo = firstLocalInfo;
    else
        bestInfo = firstInfo;
...
int64_t BonjourDevice_GetInt64(CFDictionaryRef inDeviceInfo, const char* inTXTKey, int inBase, OSStatus* outErr)
{
    ...
    if (IsTrueString(valuePtr, valueLen)) {
        value.s64 = 1;
        n = 1;
    } else if (IsFalseString(valuePtr, valueLen)) {
        value.s64 = 0;
        n = 1;
    } else if (inBase == 0)
        n = SNScanF(valuePtr, valueLen, "%lli", &value.s64);
    else if (inBase == 16)
        n = SNScanF(valuePtr, valueLen, "%llx", &value.u64);
    else if (inBase == 10)
        n = SNScanF(valuePtr, valueLen, "%lld", &value.s64);
    else if (inBase == 8)
        n = SNScanF(valuePtr, valueLen, "%llo", &value.u64);
    else {
        dlogassert("Unsupported base (%d)", inBase);
        err = kParamErr;
        goto exit;
    }
    ...
    if (valuePtr) {
        value2.s64 = 0;
        ++valuePtr;
        if (IsTrueString(valuePtr, SIZE_MAX)) {
            value2.s64 = 1;
            n = 1;
        } else if (IsFalseString(valuePtr, SIZE_MAX)) {
            value2.s64 = 0;
            n = 1;
        } else if (inBase == 0)
            n = SNScanF(valuePtr, SIZE_MAX, "%lli", &value2.s64);
        else if (inBase == 16)
            n = SNScanF(valuePtr, SIZE_MAX, "%llx", &value2.u64);
        else if (inBase == 10)
            n = SNScanF(valuePtr, SIZE_MAX, "%lld", &value2.s64);
        else if (inBase == 8)
            n = SNScanF(valuePtr, SIZE_MAX, "%llo", &value2.u64);
        else {
            dlogassert("Unsupported base (%d)", inBase);
            err = kParamErr;
            goto exit;
...
OSStatus BonjourDevice_MergeInfo(CFDictionaryRef* ioDeviceInfo, CFDictionaryRef inNewDeviceInfo)
{
    ...
    if (newServices)
        mergedServices = CFArrayCreateMutableCopy(NULL, 0, newServices);
    else
        mergedServices = CFArrayCreateMutable(NULL, 0, &kCFTypeArrayCallBacks);
    ...
    if (p2pOnly)
        CFDictionarySetValue(mergedInfo, CFSTR(kBonjourDeviceKey_P2POnly), kCFBooleanTrue);
    else
        CFDictionaryRemoveValue(mergedInfo, CFSTR(kBonjourDeviceKey_P2POnly));

    if (oldDeviceInfo)
        CFRelease(oldDeviceInfo);
...
OSStatus BonjourDevice_RemoveInterfaceInfo(CFDictionaryRef* ioDeviceInfo, const char* inIfName, Boolean inRemoveOthers)
{
    ...
    while (n-- > 0) {
        ...
        *ifname = '\0';
        ...
        if ((strcmp(ifname, inIfName) == 0) != !inRemoveOthers)
            continue;
    ...
    if (n > 0)
        CFDictionarySetValue(newInfo, CFSTR(kBonjourDeviceKey_Services), newServices);
    else
        CFDictionaryRemoveValue(newInfo, CFSTR(kBonjourDeviceKey_Services));
...
OSStatus BonjourBrowser_Test(void)
{
    ...
    dict = (CFMutableDictionaryRef)CFCreateF(&err,
        ...
        "%kO=%D"
        "}",
        CFSTR(kBonjourDeviceKey_TXT), "\x07"
                                      "x=12345",
        8);
    require_noerr(err, exit);
    ...
    dict = (CFMutableDictionaryRef)CFCreateF(&err,
        ...
        "%kO=%D"
        "}",
        CFSTR(kBonjourDeviceKey_TXT), "\x09"
                                      "x=0x12345",
        10);
    require_noerr(err, exit);
    s64 = BonjourDevice_GetInt64(dict, "x", 0, &err);
    ...
    dict = (CFMutableDictionaryRef)CFCreateF(&err,
        ...
        "%kO=%D"
        "}",
        CFSTR(kBonjourDeviceKey_TXT), "\x0B"
                                      "x=12345,678",
        12);
    require_noerr(err, exit);
    s64 = BonjourDevice_GetInt64(dict, "x", 0, &err);
    ...
    dict = (CFMutableDictionaryRef)CFCreateF(&err,
        ...
        "%kO=%D"
        "}",
        CFSTR(kBonjourDeviceKey_TXT), "\x0F"
                                      "x=0x12345,0x678",
        16);
    require_noerr(err, exit);
    s64 = BonjourDevice_GetInt64(dict, "x", 0, &err);
...
exit:
    ...
    BonjourBrowser_Forget(&browser);
    if (context.doneSem)
        arc_safe_dispatch_release(context.doneSem);
    if (queue)
...
exit:
    if (outErr)
        *outErr = result ? kNoErr : kNotFoundErr;
    return (result);
}
