...
#if (!AES_UTILS_USE_COMMON_CRYPTO && !AES_UTILS_USE_GLADMAN_AES && !AES_UTILS_USE_WICED && !AES_UTILS_USE_WINDOWS_API && TARGET_NO_OPENSSL)
...
#if (DEBUG)
    if (inSrc != inDst)
        check_ptr_overlap(inSrc, inLen, inDst, inLen);
#endif
...
{
#if (AES_UTILS_USE_COMMON_CRYPTO)
    if (inContext->cryptor)
        CCCryptorRelease(inContext->cryptor);
#elif (AES_UTILS_USE_WINDOWS_API)
    if (inContext->key)
        CryptDestroyKey(inContext->key);
    if (inContext->provider)
        CryptReleaseContext(inContext->provider, 0);
#endif
...
#elif (AES_UTILS_USE_WICED)
    if (inEncrypt)
        aes_setkey_enc(&inContext->ctx, (unsigned char*)inKey, kAES_CBCFrame_Size * 8);
    else
        aes_setkey_dec(&inContext->ctx, (unsigned char*)inKey, kAES_CBCFrame_Size * 8);
    inContext->encrypt = inEncrypt;
#elif (AES_UTILS_USE_WINDOWS_API)
    OSStatus err;
...
#else
    if (inEncrypt)
        AES_set_encrypt_key(inKey, kAES_CBCFrame_Size * 8, &inContext->key);
    else
        AES_set_decrypt_key(inKey, kAES_CBCFrame_Size * 8, &inContext->key);
...
OSStatus AES_CBCFrame_Update(AES_CBCFrame_Context* inContext, const void* inSrc, size_t inSrcLen, void* inDst)
{
    ...
    if (len > 0) {
        ...
        memcpy(iv, inContext->iv, kAES_CBCFrame_Size); // Use local copy so original IV is not changed.
        if (inContext->encrypt)
            aes_crypt_cbc(&inContext->ctx, AES_ENCRYPT, len, iv, (unsigned char*)src, dst);
        else
            aes_crypt_cbc(&inContext->ctx, AES_DECRYPT, len, iv, (unsigned char*)src, dst);
#elif (AES_UTILS_USE_WINDOWS_API)
        BOOL good;
        DWORD dwLen;
        ...
        dwLen = (DWORD)len;
        if (inContext->encrypt)
            good = CryptEncrypt(inContext->key, 0, false, 0, dst, &dwLen, dwLen);
        else
            good = CryptDecrypt(inContext->key, 0, false, 0, dst, &dwLen);
        err = map_global_value_errno(good, good);
        require_noerr(err, exit);
#else
        uint8_t iv[kAES_CBCFrame_Size];
...
#elif (AES_UTILS_USE_WICED)
        if (inContext->encrypt)
            aes_crypt_cbc(&inContext->ctx, AES_ENCRYPT, len, iv, (unsigned char*)src1, dst);
        else
            aes_crypt_cbc(&inContext->ctx, AES_DECRYPT, len, iv, (unsigned char*)src1, dst);
#elif (AES_UTILS_USE_WINDOWS_API)
        BOOL good;
        DWORD dwLen;
...
#elif (AES_UTILS_USE_WICED)
        if (inContext->encrypt)
            aes_crypt_cbc(&inContext->ctx, AES_ENCRYPT, i, iv, buf, dst);
        else
            aes_crypt_cbc(&inContext->ctx, AES_DECRYPT, i, iv, buf, dst);
#elif (AES_UTILS_USE_WINDOWS_API)
        {
            BOOL good;
            DWORD dwLen;
...
#elif (AES_UTILS_USE_WICED)
        if (inContext->encrypt)
            aes_crypt_cbc(&inContext->ctx, AES_ENCRYPT, len, iv, (unsigned char*)src2, dst);
        else
            aes_crypt_cbc(&inContext->ctx, AES_DECRYPT, len, iv, (unsigned char*)src2, dst);
#elif (AES_UTILS_USE_WINDOWS_API)
        BOOL good;
        DWORD dwLen;
...
#elif (AES_UTILS_USE_WICED)
    if (inMode == kAES_ECB_Mode_Encrypt)
        aes_setkey_enc(&inContext->ctx, (unsigned char*)inKey, kAES_ECB_Size * 8);
    else
        aes_setkey_dec(&inContext->ctx, (unsigned char*)inKey, kAES_ECB_Size * 8);
...
// inSrc and inDst may be the same, but otherwise, the buffers must not overlap.
    ...
    if ((inLen % kAES_ECB_Size) != 0)
        dlogassert("ECB doesn't support non-block-sized operations (%zu bytes)", inLen);
#endif
...
static OSStatus _AES_GCM_TestPerfOne(size_t inLen, size_t inCount, Boolean inBad)
{
    ...
    require_action(inCount > 0, exit, err = kParamErr);
    ...
    for (i = 0; i < inCount; ++i) {
        ...
        if (inBad)
            require_action(err != kNoErr, exit, err = kIntegrityErr);
        else
            require_noerr(err, exit);
    }
    ticks = UpTicks() - ticks;
...
OSStatus AESUtils_Test(int inPrint, int inPerf)
{
    ...
    if (inPrint)
        printf("AES-CTR Test Vector F.5.1\n");
    ...
    if (inPrint)
        printf("AES-CTR Test Vector F.5.2\n");
    ...
    if (inPrint)
        printf("F.2.1 CBC-AES128.Encrypt test vector\n");
    ...
    if (inPrint)
        printf("F.1.1 ECB-AES128.Encrypt test vector\n");
    ...
    if (inPrint)
        printf("F.1.2 ECB-AES128.Decrypt test vector\n");
