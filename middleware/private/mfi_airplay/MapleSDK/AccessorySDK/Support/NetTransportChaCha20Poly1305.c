...
#else
#define nt_post_read_dlog(LEVEL, ...) \
    do {                              \
    } while (0)
#endif
...
#else
#define nt_pre_write_dlog(LEVEL, ...) \
    do {                              \
    } while (0)
#endif
...
#else
#define nt_post_write_dlog(LEVEL, ...) \
    do {                               \
    } while (0)
#endif
    ...
    const uint8_t inWriteNonce[8])
{
    ...
    if (inReadNonce)
        memcpy(ctx->readNonce, inReadNonce, 8);
    memcpy(ctx->writeKey, inWriteKey, 32);
    if (inWriteNonce)
        memcpy(ctx->writeNonce, inWriteNonce, 8);
...
exit:
    ...
    if (((err == EWOULDBLOCK) || (err == kConnectionErr)) && (len > 0))
        err = kNoErr;
    if (outLen)
        *outLen = len;
    return (err);
...
static OSStatus _NetTransportWriteV(iovec_t** ioArray, int* ioCount, void* inContext)
{
    ...
    for (;;) {
        ...
        if (n > 0)
            nt_post_write_dlog(kLogLevelMax, "-- Post-encrypt write body final:\n%1.1H\n", dst, (int)n, 256);
        dst += n;
