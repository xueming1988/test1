/*
	Copyright (C) 2000-2019 Apple Inc. All Rights Reserved. Not to be used or disclosed without permission from Apple.
*/
...
static CF_RETURNS_RETAINED CFTypeRef createIfUnique(CFTypeRef anObject, void* info);
    ...
    va_list inArgs)
{
    ...
    if (inParent) {
        ...
        if (typeID == CFArrayGetTypeID())
            array = (CFMutableArrayRef)inParent;
        else if (typeID == CFDictionaryGetTypeID())
            dict = (CFMutableDictionaryRef)inParent;
        else {
    ...
    for (c = *fmt; c != '\0'; c = *(++fmt)) {
        ...
        case 'i': // %i: Integer (e.g. "<integer>-3</integer>")
            ...
            if (longFlag == 2)
                s64 = va_arg(inArgs, int64_t);
            else
                s64 = va_arg(inArgs, int);
            if (suppress) {
                ForgetCF(&key);
                break;
            }
    ...
    if (inLen > 0) {
        ...
        if (inType == CFDictionaryGetTypeID()) {
            if (isMutable)
                ...
                obj = CFDictionaryCreate(NULL, NULL, NULL, 0, &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks);
            require_action(obj, exit, err = kNoMemoryErr);
        } else if (inType == CFArrayGetTypeID()) {
            if (isMutable)
                obj = CFArrayCreateMutable(NULL, 0, &kCFTypeArrayCallBacks);
            else
                obj = CFArrayCreate(NULL, NULL, 0, &kCFTypeArrayCallBacks);
            require_action(obj, exit, err = kNoMemoryErr);
        } else {
    ...
    OSStatus* outErr)
{
    ...
    while (INIGetNext(src, end, &flags, &namePtr, &nameLen, &valuePtr, &valueLen, &src)) {
        if (flags & kINIFlag_Section) {
            ...
            if (valuePtr && inSectionNameKey && (inSectionNameKey != kINISectionDotted) && (inSectionNameKey != kINISectionNested)) {
    ...
    FILE* inFile)
{
    ...
    if (0) {
    } // Empty if to simplify conditionalize code below.
    ...
    void* inResponseContext)
{
    ...
    if (inResponseQueue)
        arc_safe_dispatch_retain(inResponseQueue);
...
char* CFObjectGetPropertyCStringSync(
    CFTypeRef inObject,
    dispatch_queue_t inQueue,
    CFObjectCopyPropertyFunc inFunc,
    CFObjectFlags inFlags,
...
char* CFGetCString(CFTypeRef inObj, char* inBuf, size_t inMaxLen)
{
    ...
    } else if (inObj == kCFBooleanTrue)
        strlcpy(inBuf, "true", inMaxLen);
    else if (inObj == kCFBooleanFalse)
        strlcpy(inBuf, "false", inMaxLen);
    else if (typeID == CFDataGetTypeID()) {
        const uint8_t* src;
        const uint8_t* end;
        char* dst;
...
double CFGetDouble(CFTypeRef inObj, OSStatus* outErr)
{
    ...
    if (typeID == CFNumberGetTypeID()) {
        ...
        if (IsTrueString(tempStr, kSizeCString))
            value = 1;
        else if (IsFalseString(tempStr, kSizeCString))
            value = 0;
        else if (sscanf(tempStr, "%lf", &value) == 1) {
        } else if (sscanf(tempStr, "%lli", (long long*)&s64) == 1)
            value = (double)s64;
        else {
            err = kFormatErr;
            goto exit;
...
int64_t CFGetInt64(CFTypeRef inObj, OSStatus* outErr)
{
    ...
    if (typeID == CFNumberGetTypeID()) {
        if (CFNumberIsFloatType((CFNumberRef)inObj)) {
            ...
            if (tempDouble < INT64_MIN) {
                value = INT64_MIN;
                err = kRangeErr;
                goto exit;
            } else if (tempDouble > INT64_MAX) {
                value = INT64_MAX;
                err = kRangeErr;
                goto exit;
            } else
                value = (int64_t)tempDouble;
        } else {
            CFNumberGetValue((CFNumberRef)inObj, kCFNumberSInt64Type, &value);
        }
    } else if (((CFBooleanRef)inObj) == kCFBooleanTrue)
        value = 1;
    else if (((CFBooleanRef)inObj) == kCFBooleanFalse)
        value = 0;
    else if (typeID == CFStringGetTypeID()) {
        char tempStr[128];
        ...
        if (IsTrueString(tempStr, kSizeCString))
            ...
            value = 0;
        else if (sscanf(tempStr, "%lli", (long long*)&value) != 1) {
            err = kFormatErr;
            goto exit;
        }
    } else if (typeID == CFDataGetTypeID()) {
...
OSStatus CFGetUUIDEx(CFTypeRef inObj, const uint8_t* inBaseUUID, uint8_t outUUID[16])
{
    ...
    if (typeID == CFStringGetTypeID()) {
        ...
        if (n == 16) {
            if (uuidPtr)
                memcpy(uuidPtr, CFDataGetBytePtr((CFDataRef)inObj), 16);
...
//    CFArrayCreateMap
//===========================================================================================================================
CFArrayRef CFArrayCreateMap(CFArrayRef array, CFArrayMapper createMap, void* info)
{
    CFIndex i;
    CFTypeRef aMappedObject;

    CFMutableArrayRef mapResults = CFArrayCreateMutable(NULL, 0, &kCFTypeArrayCallBacks);

    for (i = 0; i < CFArrayGetCount(array); i++) {
        CFTypeRef obj = CFArrayGetValueAtIndex(array, i);
        aMappedObject = createMap(obj, info);
        if (aMappedObject) {
            CFArrayAppendValue(mapResults, aMappedObject);
            CFRelease(aMappedObject);
        }
    }
...
//===========================================================================================================================
//    CFArrayCreateDifference
//===========================================================================================================================
CFArrayRef CFArrayCreateDifference(CFArrayRef minuend, CFArrayRef subtrahend)
{
    CFArrayRef result;
    ...
    if (!minuend) {
        ...
        result = (CFArrayRef)CFRetain(minuend);
    } else {
        result = CFArrayCreateMap(minuend, createIfUnique, (void*)subtrahend);
    }

    return result;
}

static CFTypeRef createIfUnique(CFTypeRef anObject, void* info)
{
    CFArrayRef set = (CFArrayRef)info;
    return CFArrayContainsValue(set, CFRangeMake(0, CFArrayGetCount(set)), anObject) ? NULL : CFRetain(anObject);
}
    ...
    int* outMicros)
{
    ...
    if (outMicros)
        *outMicros = (int)(modf(t, &d) * 1000000);
    err = kNoErr;
    ...
    OSStatus* outErr)
{
    ...
    require_action_quiet(inDict, exit, err = kNotFoundErr);
    obj = CFDictionaryGetValue(inDict, inKey);
    require_action_quiet(obj, exit, err = kNotFoundErr);

    typeID = CFGetTypeID(obj);
    if (typeID == CFDataGetTypeID()) {
        result = CFDataGetBytePtr((CFDataRef)obj);
        len = (size_t)CFDataGetLength((CFDataRef)obj);
    } else if (typeID == CFStringGetTypeID()) {
        const char* utf8Ptr;
        char* utf8Buf;

        err = CFStringGetOrCopyCStringUTF8((CFStringRef)obj, &utf8Ptr, &utf8Buf, &len);
        require_noerr_quiet(err, exit);
        result = (const uint8_t*)utf8Ptr;
        buf = (void*)utf8Buf;
    } else {
        err = kUnsupportedDataErr;
        goto exit;
...
OSStatus CFUtilsTest(int inPrint)
{
    ...
    *tempStr = '\0';
    ...
    // CFDictionaryCreateWithINIBytes
    {
        static const char* const kCFUtilsINITest1 = "name  = MyName\n"
                                                    "model = MyModel\n"
                                                    "\n"
        ...
        require_action(CFEqualNullSafe(CFDictionaryGetCFString(dict, CFSTR("propertyName1"), NULL),
                           CFSTR("section1Value2")),
            exitINI, err = -1);
        require_action(CFEqualNullSafe(CFDictionaryGetCFString(dict, CFSTR("propertyName2"), NULL),
                           CFSTR("15 00 25 01 95 08 75 01 81 02 01 29 08 45")),
            exitINI, err = -1);
        require_action(CFEqualNullSafe(CFDictionaryGetCFString(dict, CFSTR("propertyName3"), NULL),
                           CFSTR("4a0913fc-9676-4cb2-9219-f2cf7074af82")),
            exitINI, err = -1);
        ...
        require_action(CFEqualNullSafe(CFDictionaryGetCFString(dict, CFSTR("propertyName6"), NULL),
                           CFSTR("685073b1-71c5-404b-abb8-aa4ea73404f3")),
            exitINI, err = -1);
