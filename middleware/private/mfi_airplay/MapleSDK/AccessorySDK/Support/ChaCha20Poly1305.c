...
#endif
static const uint32_t kChaCha20Constants[4]
...
size_t chacha20_final(chacha20_state* inState, void* inDst)
{
    ...
    n = inState->leftover;
    if (n)
        _chacha20_xor(inState, (uint8_t*)inDst, inState->buffer, n);
...
static void _chacha20_xor(chacha20_state* inState, uint8_t* inDst, const uint8_t* inSrc, size_t inLen)
{
    ...
    for (;;) {
        ...
        if (!++j12)
            ++j13; // Stopping at 2^70 bytes per nonce is the caller's responsibility.
...
static void _chacha20_xor(chacha20_state* inState, uint8_t* out, const uint8_t* in, size_t inlen)
{
    ...
    for (iters = 0; iters < inlen / (BPI * 64); iters++) {
...
#endif
#if GPR_TOO
        x0 = kChaCha20Constants[0];
        x1 = kChaCha20Constants[1];
        x2 = kChaCha20Constants[2];
        x3 = kChaCha20Constants[3];
        x4 = kp[0];
        x5 = kp[1];
        x6 = kp[2];
        x7 = kp[3];
        ...
        x10 = kp[6];
        x11 = kp[7];
        x12 = (inState->state[12]) + ((unsigned)(BPI * iters + (BPI - 1)));
        x13 = inState->state[13];
        x14 = np[0];
        x15 = np[1];
#endif
...
OSStatus chacha20_test(int inPerf)
{
    ...
    chacha20_all_64x64((const uint8_t*)"\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f"
...
poly1305_donna_mul:
    t[0] = mul32x32_64(state->h0, state->r0) + mul32x32_64(state->h1, state->s4) + mul32x32_64(state->h2, state->s3) + mul32x32_64(state->h3, state->s2) + mul32x32_64(state->h4, state->s1);
    t[1] = mul32x32_64(state->h0, state->r1) + mul32x32_64(state->h1, state->r0) + mul32x32_64(state->h2, state->s4) + mul32x32_64(state->h3, state->s3) + mul32x32_64(state->h4, state->s2);
    t[2] = mul32x32_64(state->h0, state->r2) + mul32x32_64(state->h1, state->r1) + mul32x32_64(state->h2, state->r0) + mul32x32_64(state->h3, state->s4) + mul32x32_64(state->h4, state->s3);
    t[3] = mul32x32_64(state->h0, state->r3) + mul32x32_64(state->h1, state->r2) + mul32x32_64(state->h2, state->r1) + mul32x32_64(state->h3, state->r0) + mul32x32_64(state->h4, state->s4);
    t[4] = mul32x32_64(state->h0, state->r4) + mul32x32_64(state->h1, state->r3) + mul32x32_64(state->h2, state->r2) + mul32x32_64(state->h3, state->r1) + mul32x32_64(state->h4, state->r0);
...
OSStatus poly1305_test(int inPerf)
{
    ...
    memset(result, 0, sizeof(result));
    poly1305(result, (const uint8_t*)"Cryptographic Forum Research Group", 34, (const uint8_t*)"\x85\xD6\xBE\x78\x57\x55\x6D\x33\x7F\x44\x52\xFE\x42\xD5\x06\xA8"
...
size_t chacha20_poly1305_encrypt(chacha20_poly1305_state* inState, const void* inSrc, size_t inLen, void* inDst)
{
    ...
    if (!inState->padded) {
        n = inState->aadLen & 15;
        if (n > 0)
            poly1305_update(&inState->poly1305, kZero64, 16 - n);
        inState->padded = true;
    }
    ...
    uint8_t outAuthTag[16])
{
    ...
    if (inNonceLen == 8)
        chacha20_poly1305_init_64x64(&state, inKey, inNoncePtr);
    else if (inNonceLen == 12)
        chacha20_poly1305_init_96x32(&state, inKey, inNoncePtr);
    else {
        DEBUG_HALT();
    }
    ...
    if (inAADLen > 0)
        poly1305_update(&state.poly1305, inAADPtr, inAADLen);
    n = inAADLen & 15;
    if (n > 0)
        poly1305_update(&state.poly1305, kZero64, 16 - n);

    if (inPlaintextLen > 0)
        poly1305_update(&state.poly1305, ciphertextPtr, inPlaintextLen);
    n = inPlaintextLen & 15;
    if (n > 0)
        poly1305_update(&state.poly1305, kZero64, 16 - n);
    ...
    const uint8_t inAuthTag[16])
{
    ...
    if (inCiphertextLen > 0)
        poly1305_update(&state.poly1305, inCiphertextPtr, inCiphertextLen);
...
static OSStatus chacha20_poly1305_test_one(const chacha20_poly1305_test_vector* inVector, int inPrint)
{
    ...
    if (use96)
        chacha20_poly1305_encrypt_all_96x32(keyPtr, noncePtr, aadPtr, aadLen, ptPtr, ptLen, buf, tag);
    else
        chacha20_poly1305_encrypt_all_64x64(keyPtr, noncePtr, aadPtr, aadLen, ptPtr, ptLen, buf, tag);
    ...
    if (use96)
        err = chacha20_poly1305_decrypt_all_96x32(keyPtr, noncePtr, aadPtr, aadLen, ctPtr, ctLen, buf, tagPtr);
    else
        err = chacha20_poly1305_decrypt_all_64x64(keyPtr, noncePtr, aadPtr, aadLen, ctPtr, ctLen, buf, tagPtr);
    require_noerr(err, exit);
    ...
    memset(tag, 'b', sizeof(tag));
    if (use96)
        chacha20_poly1305_init_96x32(&state, keyPtr, noncePtr);
    else
        chacha20_poly1305_init_64x64(&state, keyPtr, noncePtr);
    ...
    if (use96)
        ...
        chacha20_poly1305_init_64x64(&state, keyPtr, noncePtr);
    for (i = 0; i < aadLen; ++i)
        chacha20_poly1305_add_aad(&state, &aadPtr[i], 1);
    n = 0;
    for (i = 0; i < ptLen; ++i)
        n += chacha20_poly1305_encrypt(&state, &ptPtr[i], 1, &buf[n]);
    n += chacha20_poly1305_final(&state, &buf[n], tag);
    require_action(n == ctLen, exit, err = kSizeErr);
    require_action(memcmp(buf, ctPtr, ctLen) == 0, exit, err = kMismatchErr);
    require_action(memcmp(tag, tagPtr, tagLen) == 0, exit, err = kMismatchErr);
    ...
    n = 0;
    for (i = 0; i < ctLen; ++i)
        n += chacha20_poly1305_decrypt(&state, &ctPtr[i], 1, &buf[n]);
    err = -1;
    n += chacha20_poly1305_verify(&state, &buf[n], tagPtr, &err);
    require_noerr(err, exit);
    require_action(n == ptLen, exit, err = kSizeErr);
