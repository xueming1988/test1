...
#if (!defined(DISPATCH_LITE_THREAD_PRIORITY))
#define DISPATCH_LITE_THREAD_PRIORITY -1
#endif
...
DEBUG_STATIC OSStatus __LibDispatch_AddSocket(dispatch_source_t inSource, long* outEvents);
DEBUG_STATIC OSStatus __LibDispatch_RemoveSocket(dispatch_source_t inSource, long* outEvents);
DEBUG_STATIC OSStatus __LibDispatch_LookupSocket(SOCKET inSock, dispatch_source_type_t inType, dispatch_source_t* outSource);
...
static int gDispatchThreadPool_Priority = DISPATCH_LITE_THREAD_PRIORITY;
#if (!TARGET_OS_WINDOWS)
...
static FDRef gDispatchSelect_CommandReadFD = kInvalidFD;
static FDRef gDispatchSelect_CommandWriteFD = kInvalidFD;
...
#if (CLOCK_MONOTONIC_SUPPORTED)
    pthread_condattr_t condattr;
    pthread_condattr_init(&condattr);
    err = pthread_condattr_setclock(&condattr, CLOCK_MONOTONIC);
    require_noerr(err, exit);

    err = pthread_cond_init(&gDispatchThreadPool_Condition, &condattr);
...
// Tear down select() support.
...
#if (DISPATCH_LITE_USE_SELECT)
    if (IsValidSocket(gDispatchSelect_CommandWriteFD)) {
        ...
        pkt.source = NULL;
        n = write(gDispatchSelect_CommandWriteFD, &pkt, sizeof(pkt));
        err = map_global_value_errno(n == (ssize_t)sizeof(pkt), n);
        check_noerr(err);
    ...
    ForgetFD(&gDispatchSelect_CommandReadFD);
    ForgetFD(&gDispatchSelect_CommandWriteFD);
...
//===========================================================================================================================
//	LibDispatch_SetThreadPriority
//===========================================================================================================================

void LibDispatch_SetThreadPriority(int inPriority)
{
    gDispatchThreadPool_Priority = inPriority;
}
...
DEBUG_STATIC void* __LibDispatch_WorkThread(void* inArg)
{
    ...
    if (gDispatchThreadPool_Priority >= 0) {
        SetCurrentThreadPriority(gDispatchThreadPool_Priority);
    }
...
void dispatch_suspend(dispatch_object_t inObj)
{
    ...
    if (magic == kDispatchQueue_MagicGood)
        __dispatch_queue_suspend((dispatch_queue_t)inObj);
    else if (magic == kDispatchSource_MagicGood)
        __dispatch_source_suspend((dispatch_source_t)inObj);
    else {
        dlogassert("bad object '%C' %p", magic, inObj);
        goto exit;
    }
...
void dispatch_resume(dispatch_object_t inObj)
{
    ...
    magic = *((const uint32_t*)inObj);
    if (magic == kDispatchQueue_MagicGood)
        __dispatch_queue_resume((dispatch_queue_t)inObj);
    else if (magic == kDispatchSource_MagicGood)
        __dispatch_source_resume((dispatch_source_t)inObj);
    else {
        dlogassert("bad object '%C' %p", magic, inObj);
        goto exit;
    }
...
dispatch_queue_t dispatch_get_global_queue(long inPriority, unsigned long inFlags)
{
    ...
    if (inPriority == DISPATCH_QUEUE_PRIORITY_DEFAULT)
        queue = gDispatchConcurrentQueues[1];
    else if (inPriority > DISPATCH_QUEUE_PRIORITY_DEFAULT)
        queue = gDispatchConcurrentQueues[2];
    else
        queue = gDispatchConcurrentQueues[0];
    require(queue, exit);
...
{
    if (inQueue == DISPATCH_CURRENT_QUEUE_LABEL) {
        inQueue = dispatch_get_current_queue();
    }
    return (inQueue ? inQueue->label : "");
}
...
DEBUG_STATIC void __dispatch_queue_serial_drain(void* inContext)
{
    ...
    if (queue->busy)
        goto exit; // Exit if another thread is already draining the queue.
    queue->busy = true;
    ...
    uint64_t inLeeway)
{
    ...
    if (inSource->u.timer.intervalMs == 0)
        inSource->u.timer.intervalMs = 1; // Round up to at least 1 millisecond.
...
unsigned long dispatch_source_get_data(dispatch_source_t inSource)
{
    ...
    if (inSource->type == DISPATCH_SOURCE_TYPE_READ)
        return ((unsigned long)inSource->u.rw.avail);
    else if (inSource->type == DISPATCH_SOURCE_TYPE_TIMER)
        return (0); // $$$ TO DO: fire count since last callback.
    else
        dlogassert("'%s' doesn't make sense for source type %d", __ROUTINE__, inSource->type);
...
uintptr_t dispatch_source_get_handle(dispatch_source_t inSource)
{
    ...
    if (inSource->type == DISPATCH_SOURCE_TYPE_READ)
        return ((uintptr_t)inSource->u.rw.fd);
    else if (inSource->type == DISPATCH_SOURCE_TYPE_WRITE)
        return ((uintptr_t)inSource->u.rw.fd);
    else if (inSource->type == DISPATCH_SOURCE_TYPE_SIGNAL)
        return ((uintptr_t)inSource->u.sig.sig);
    else
        dlogassert("'%s' doesn't make sense for source %d", __ROUTINE__, inSource->type);
...
{
    if (atomic_add_and_fetch_32(&inGroup->outstanding, -1) == 0) {
        if (inGroup->notifyQueue)
            dispatch_async_f(inGroup->notifyQueue, inGroup, __dispatch_group_notify_callback);
        else
            dispatch_semaphore_signal(inGroup->sem);
    }
}
...
dispatch_semaphore_t dispatch_semaphore_create(long inValue)
{
    ...
    err = pthread_cond_init(&obj->condition, &condattr);
    pthread_condattr_destroy(&condattr);
#else
...
long dispatch_semaphore_wait(dispatch_semaphore_t inSem, dispatch_time_t inTimeout)
{
    ...
    if (inTimeout == DISPATCH_TIME_FOREVER) {
        ...
        deadline.tv_sec = (int32_t)(nowTV.tv_sec + (inTimeout / 1000));
        deadline.tv_nsec = (int32_t)((nowTV.tv_usec * 1000) + ((inTimeout % 1000) * kNanosecondsPerMillisecond));
#else
        (void)nowTV;
        clock_gettime(CLOCK_MONOTONIC, &deadline);
        deadline.tv_sec += (inTimeout / 1000);
        deadline.tv_nsec += ((inTimeout % 1000) * kNanosecondsPerMillisecond);
#endif
...
DEBUG_STATIC void __LibDispatch_PlatformArmSourceAndUnlock(dispatch_source_t inSource)
{
    ...
    if (inSource->type == DISPATCH_SOURCE_TYPE_READ) {
        ...
        if (((int64_t)inSource->u.timer.start) < 0) // Wall clock time.
        {
            ...
            if (milliseconds > tempStart)
                milliseconds = 0; // Start is in the past...force to now.
        } else {
            milliseconds = inSource->u.timer.start - __dispatch_milliseconds();
            if (milliseconds > inSource->u.timer.start)
                milliseconds = 0; // Start is in the past...force to now.
        }
        if (milliseconds <= 0)
            milliseconds = 1; // 0 confuses some kqueue implementations.
...
DEBUG_STATIC OSStatus __LibDispatch_SelectEnsureInitialized(void)
{
    ...
    err = map_global_noerr_errno(err);
    require_noerr(err, exit);
    gDispatchSelect_CommandReadFD = pipeFDs[0];
    gDispatchSelect_CommandWriteFD = pipeFDs[1];
...
DEBUG_STATIC void __LibDispatch_SelectDrain(void* inArg)
{
    ...
    for (;;) {
        ...
        if (curr)
            timeoutPtr = UpTicksToTimeValTimeout(curr->u.timer.expireTicks, &timeout);
        else
            timeoutPtr = NULL;
        ...
        maxFD = gDispatchSelect_CommandReadFD;
        FD_SET(gDispatchSelect_CommandReadFD, &gDispatchSelect_ReadSet);
        writeSetPtr = NULL;
        ...
        if (!err) {
            ...
            if (FD_ISSET(gDispatchSelect_CommandReadFD, &gDispatchSelect_ReadSet)) {
                ...
                for (next = &gDispatchSelect_ReadWriteList; (curr = *next) != NULL;) {
                    ...
                    if ((curr->type == DISPATCH_SOURCE_TYPE_READ) && FD_ISSET(curr->u.rw.fd, &gDispatchSelect_ReadSet)) {
                        *next = curr->armedNext;
                        dispatch_async_f(curr->queue, curr, __LibDispatch_SelectHandleReadWriteEvent);
                    } else if ((curr->type == DISPATCH_SOURCE_TYPE_WRITE) && FD_ISSET(curr->u.rw.fd, &gDispatchSelect_WriteSet)) {
                        *next = curr->armedNext;
                        dispatch_async_f(curr->queue, curr, __LibDispatch_SelectHandleReadWriteEvent);
                    } else {
                        next = &curr->armedNext;
        ...
        } else if ((err != kTimeoutErr) && (err != EINTR)) {
            gcd_ulog(kLogLevelWarning, "### select() failed: %#m\n", err);
            sleep(1);
        }
...
DEBUG_STATIC OSStatus __LibDispatch_SelectHandleCommand(const dispatch_select_packet* inPkt)
{
    ...
    if (inPkt == NULL) {
        n = read(gDispatchSelect_CommandReadFD, (char*)&pkt, sizeof(pkt));
        err = map_global_value_errno(n >= 0, n);
        require_noerr(err, exit);
    ...
    case kDispatchCommandArmSource:
        if (inPkt->source->type == DISPATCH_SOURCE_TYPE_TIMER) {
            ...
            if (((int64_t)inPkt->source->u.timer.start) < 0) // Wall clock time.
            {
                ...
                if (milliseconds > inPkt->source->u.timer.start)
                    milliseconds = 0; // Start is in the past...force to now.
            }
            ...
            *next = inPkt->source;
        } else if ((inPkt->source->type == DISPATCH_SOURCE_TYPE_READ) || (inPkt->source->type == DISPATCH_SOURCE_TYPE_WRITE)) {
    ...
    case kDispatchCommandQuit:
        ...
        check(gDispatchSelect_QuitSem);
        if (gDispatchSelect_QuitSem)
            dispatch_semaphore_signal(gDispatchSelect_QuitSem);
        err = kEndingErr;
        goto exit;
...
DEBUG_STATIC LRESULT CALLBACK __LibDispatch_IOThreadWinProc(HWND inWindow, UINT inMsg, WPARAM inWParam, LPARAM inLParam)
{
    ...
    case kLibDispatchWindowEvent_Socket:
        ...
        if (err)
            gcd_ulog(kLogLevelNotice, "### socket %d error %#m, event 0x%X\n", sock, err, event);

        type = (event == FD_WRITE) ? DISPATCH_SOURCE_TYPE_WRITE : DISPATCH_SOURCE_TYPE_READ;
        err = __LibDispatch_LookupSocket(sock, type, &source);
        if (!err)
...
DEBUG_STATIC OSStatus __LibDispatch_AddSocket(dispatch_source_t inSource, long* outEvents)
{
    ...
    hash = (DWORD)(inSource->u.rw.fd % countof(gDispatchSocketHashTable));
    for (next = &gDispatchSocketHashTable[hash]; (curr = *next) != NULL; next = &curr->next) {
        if (curr->sock == inSource->u.rw.fd) {
            break;
        }
    }
    if (curr) {
        if (inSource->type == DISPATCH_SOURCE_TYPE_READ) {
            if (curr->readSource && (curr->readSource != inSource)) {
                gcd_ulog(kLogLevelWarning, "socket %d read source already set %{ptr} -> %{ptr}\n",
                    inSource->u.rw.fd, curr->readSource, inSource);
                err = kAlreadyInUseErr;
                goto exit;
            }
            curr->readSource = inSource;
        } else {
            if (curr->writeSource && (curr->writeSource != inSource)) {
                gcd_ulog(kLogLevelWarning, "socket %d write source already set %{ptr} -> %{ptr}\n",
                    inSource->u.rw.fd, curr->writeSource, inSource);
                err = kAlreadyInUseErr;
                goto exit;
            }
            curr->writeSource = inSource;
        }
    } else {
        curr = (DispatchSocketHashRef)calloc(1, sizeof(*curr));
        require_action(curr, exit, err = ENOMEM);
        curr->sock = inSource->u.rw.fd;
        if (inSource->type == DISPATCH_SOURCE_TYPE_READ) {
            gcd_ulog(kLogLevelChatty, "adding socket %d for read source %p\n", inSource->u.rw.fd, inSource);
            curr->readSource = inSource;
        } else {
            gcd_ulog(kLogLevelChatty, "adding socket %d for read source %p\n", inSource->u.rw.fd, inSource);
            curr->readSource = inSource;
    ...
    *outEvents = 0;
    if (curr->readSource)
        *outEvents |= FD_READ | FD_ACCEPT | FD_CONNECT | FD_CLOSE;
    if (curr->writeSource)
        *outEvents |= FD_WRITE;
    err = kNoErr;
...
DEBUG_STATIC OSStatus __LibDispatch_RemoveSocket(dispatch_source_t inSource, long* outEvents)
{
    ...
    if (curr->readSource == inSource) {
        curr->readSource = NULL;
    } else if (curr->writeSource == inSource) {
        curr->writeSource = NULL;
    } else {
        gcd_ulog(kLogLevelNotice, "### socket %d no matching read or write source?\n", inSource->u.rw.fd);
    }
    *outEvents = 0;
    if (curr->readSource)
        *outEvents |= FD_READ | FD_ACCEPT | FD_CONNECT | FD_CLOSE;
    if (curr->writeSource)
        *outEvents |= FD_WRITE;
    if (!curr->readSource && !curr->writeSource) {
        *next = curr->next;
        free(curr);
    }
    err = kNoErr;
...
DEBUG_STATIC OSStatus __LibDispatch_LookupSocket(SOCKET inSock, dispatch_source_type_t inType, dispatch_source_t* outSource)
{
    ...
    if (!curr) {
        gcd_ulog(kLogLevelNotice, "### socket %d msg when not in list (stale, queued msg?)\n", inSock);
        err = kNotFoundErr;
        goto exit;
    }
    if (inType == DISPATCH_SOURCE_TYPE_READ) {
        if (!curr->readSource) {
            gcd_ulog(kLogLevelNotice, "### socket %d msg with NULL read source\n", inSock);
            err = kValueErr;
            goto exit;
        }
        *outSource = curr->readSource;
    } else {
        if (!curr->writeSource) {
            gcd_ulog(kLogLevelNotice, "### socket %d msg with NULL write source\n", inSock);
            err = kValueErr;
            goto exit;
        }
        *outSource = curr->writeSource;
    }
    err = kNoErr;
...
DEBUG_STATIC void __LibDispatch_PlatformArmSourceAndUnlock(dispatch_source_t inSource)
{
    ...
    if ((inSource->type == DISPATCH_SOURCE_TYPE_READ) || (inSource->type == DISPATCH_SOURCE_TYPE_WRITE)) {
        ...
        err = __LibDispatch_AddSocket(inSource, &events);
        require_noerr(err, exit);

        err = WSAAsyncSelect(inSource->u.rw.fd, gDispatchIOThreadWindow, kLibDispatchWindowEvent_Socket, events);
...
exit:
    if (err)
        gcd_ulog(kLogLevelNotice, "### source %{ptr} arm failed %#m\n", inSource, err);
    pthread_mutex_unlock(inSource->queue->lockPtr);
}
...
DEBUG_STATIC void __LibDispatch_PlatformDisarmSourceAndUnlock(dispatch_source_t inSource)
{
    ...
    if ((inSource->type == DISPATCH_SOURCE_TYPE_READ) || (inSource->type == DISPATCH_SOURCE_TYPE_WRITE)) {
        ...
        __LibDispatch_RemoveSocket(inSource, &events);
...
void DispatchLite_Test1(void* inContext)
{
    ...
    if ((x == 5) && (gDispatchLiteTest1 == 0))
        gDispatchLiteTest1 += x;
    else if ((x == 50) && (gDispatchLiteTest1 == 5))
        gDispatchLiteTest1 += x;
    else if ((x == 500) && (gDispatchLiteTest1 == 55))
        gDispatchLiteTest1 += x;
    else if ((x == 5000) && (gDispatchLiteTest1 == 555))
        gDispatchLiteTest1 += x;
    else {
...
void DispatchLite_Test2(void* inContext)
{
    ...
    if (x == 5)
        require(dispatch_get_current_queue() == dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), exit);
    else if (x == 50)
        require(dispatch_get_current_queue() == dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), exit);
    else if (x == 500)
        require(dispatch_get_current_queue() == dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), exit);
    else
        require(dispatch_get_current_queue() == dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), exit);
