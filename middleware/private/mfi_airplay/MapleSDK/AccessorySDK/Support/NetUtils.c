/*
	Copyright (C) 2006-2019 Apple Inc. All Rights Reserved. Not to be used or disclosed without permission from Apple.
*/
...
exit:
    if (outDrainedPackets)
        *outDrainedPackets = drained;
    return (err);
}
...
static OSStatus _GetPrimaryMACAddress(uint8_t outMAC[6])
{
    ...
    for (ia = iaList; ia; ia = ia->ifa_next) {
        ...
        if (!(ia->ifa_flags & IFF_UP))
            continue; // Skip inactive.
        if (ia->ifa_flags & IFF_LOOPBACK)
            continue; // Skip loopback.
        if (!ia->ifa_addr)
            continue; // Skip no addr.
        if (ia->ifa_addr->sa_family != AF_PACKET)
            continue; // Skip non-AF_PACKET.
...
exit:
    ...
    if (context.results)
        freeaddrinfo_dnssd(context.results);
    return ((int)err);
}
    ...
    void* inContext)
{
    ...
    if (inErrorCode == -65791)
        goto exit; // Ignore mStatus_ConfigChanged
    require(inAddr, exit);
#if (defined(AF_INET6))
    ...
    if (inErrorCode == kDNSServiceErr_NoError) {
        ...
        if (sip->sa.sa_family == AF_INET)
            newAddr->ai_addrlen = (socklen_t)sizeof(struct sockaddr_in);
#if (defined(AF_INET6))
        else if (sip->sa.sa_family == AF_INET6)
            newAddr->ai_addrlen = (socklen_t)sizeof(struct sockaddr_in6);
#endif
        else {
...
#if (defined(AF_INET6))
                if (!((tempSIP->sa.sa_family == AF_INET6) && IN6_IS_ADDR_LINKLOCAL(&tempSIP->v6.sin6_addr)) && !((tempSIP->sa.sa_family == AF_INET) && !SockAddrIsLinkLocal(tempSIP)))
#else
    ...
    void* inContext)
{
    ...
    for (ptr = inHostList; *ptr != '\0'; ptr = end) {
        ...
        if (err == kNoErr) {
            ...
            if (!IsValidSocket(nativeSock))
                continue;

            if (inCallBack)
                inCallBack(inSock, nativeSock, inContext);
    ...
    while (remaining > 0) {
        ...
        if (n > 0) {
            ...
            if (err == EWOULDBLOCK) {
                ...
                dlogassert("send/write() error: %#m", err);
                goto exit;
            }
        }
    }
    ...
    for (;;) {
        ...
        if (n > 0) {
            ...
            if (err == EWOULDBLOCK) {
                ...
                dlogassert("writev() error: %zd, %#m", n, err);
                goto exit;
            }
        }
    }
    ...
    int32_t inTimeoutSecs)
{
    ...
    maxFD = -1;
    if (inNativeSock > maxFD)
        maxFD = inNativeSock;
#if (TARGET_OS_POSIX)
    if (inSock->recvCancel > maxFD)
        maxFD = inSock->recvCancel;
#else
    if (inSock->cancelSock > maxFD)
        maxFD = inSock->cancelSock;
#endif
    do {
...
#if (TARGET_OS_POSIX)
    if (n > 0)
        err = FD_ISSET(inSock->recvCancel, &readSet) ? kCanceledErr : kNoErr;
#else
    if (n > 0)
        err = FD_ISSET(inSock->cancelSock, &readSet) ? kCanceledErr : kNoErr;
#endif
    else if (n == 0)
        err = kTimeoutErr;
    else {
...
OSStatus TCPConnect(const char* inHostList, const char* inDefaultService, int inSeconds, SocketRef* outSock)
{
    ...
    if (inDefaultService)
        defaultPort = (int)strtoul(inDefaultService, NULL, 10);
    else
        defaultPort = 0;
    ...
    for (ptr = inHostList; *ptr != '\0'; ptr = end) {
        ...
        if (err == kNoErr) {
            ...
            if (SockAddrGetPort(&sip) == 0)
                SockAddrSetPort(&sip, defaultPort);
    ...
    for (;;) {
        ...
        if (inSeconds >= 0) {
            ...
            if (deadlineTicks > nowTicks)
                inSeconds = (int)((deadlineTicks - nowTicks) / UpTicksPerSecond());
            else
                inSeconds = 0;
        }
    }
...
OSStatus SocketCloseGracefully(SocketRef inSock, int inTimeoutSecs)
{
    ...
    for (;;) {
        ...
        n = select((int)(inSock + 1), &readSet, NULL, NULL, &timeout);
        if (n == 0) {
            dlogassert("timeout waiting for graceful close");
            break;
        }
        if (n < 0) {
            err = global_value_errno(n);
    ...
    char* outIfName)
{
    ...
    if (outLen)
        *outLen = (size_t)n;
    if (outFromLen)
        *outFromLen = msg.msg_namelen;
    if (outTicks)
        *outTicks = SocketGetPacketUpTicks(&msg);
    if (outIfIndex)
        *outIfIndex = SocketGetPacketReceiveInterface(&msg, outIfName);
    else if (outIfName)
        SocketGetPacketReceiveInterface(&msg, outIfName);
...
OSStatus SocketSetBoundInterface(SocketRef inSock, int inFamily, uint32_t inIfIndex)
{
    ...
    if (0) {
        ...
        err = setsockopt(inSock, IPPROTO_IPV6, IPV6_BOUND_IF, &inIfIndex, (socklen_t)sizeof(inIfIndex));
#endif
#if (defined(IP_BOUND_IF))
    else if (inFamily == AF_INET)
        err = setsockopt(inSock, IPPROTO_IP, IP_BOUND_IF, &inIfIndex, (socklen_t)sizeof(inIfIndex));
#endif
    else {
        (void)inSock;
        (void)inFamily;
...
// Get the max size from the system. If _PC_SOCK_MAXBUF ever becomes supported, we could start using that.
...
#elif (TARGET_OS_QNX)
    {
        ...
        check_noerr(err);
        if (err || (value <= 4096))
            value = 128 * 1024; // Default to 128 KB.
    }
#elif (TARGET_OS_WINDOWS)
    value = 256 * 1024; // Default to 256 KB.
#else
    ...
    NetTransportType* outTransportType)
{
    ...
    if (!inIfName) {
        ...
        for (ifa = ifaList; ifa; ifa = ifa->ifa_next) {
            if (!ifa->ifa_addr)
                continue; // Skip if address not valid.
            if (SockAddrCompareAddrEx(ifa->ifa_addr, &sip, true) != 0)
                continue; // Skip if address doesn't match.

            inIfName = ifa->ifa_name;
            if (outIfName)
                strlcpy(outIfName, inIfName, IF_NAMESIZE + 1);
            break;
        }
...
#endif
        if (outExtendedFlags)
            *outExtendedFlags = eflags;
    }
    if (outOtherFlags)
        *outOtherFlags = otherFlags;
    if (outTransportType) {
        if (0) {
        }
#if (defined(IFM_IEEE80211))
        else if (IFM_TYPE(ifmedia) == IFM_IEEE80211)
            *outTransportType = kNetTransportType_WiFi;
#endif
#if (defined(IFEF_DIRECTLINK))
        else if (eflags & IFEF_DIRECTLINK)
            *outTransportType = kNetTransportType_DirectLink;
#endif
        else
            *outTransportType = kNetTransportType_Ethernet;
    }
    err = kNoErr;
...
{
    return (SocketSetKeepAliveEx(inSock, inIdleSecs, inIdleSecs, inMaxUnansweredProbes));
}

//===========================================================================================================================
OSStatus SocketSetKeepAliveEx(SocketRef inSock, int inIdleSecs, int inIntervalSecs, int inMaxUnansweredProbes)
{
    OSStatus err;
    int option;
...
OSStatus SocketSetNotSentLowWatermark(SocketRef inSock, size_t inSize)
{
    ...
    err = map_socket_noerr_errno(inSock, err);
    if (!err)
        dlog(kLogLevelVerbose, "Socket %d TCP_NOTSENT_LOWAT: %zu\n", inSock, inSize);
    else
        dlog(kLogLevelNotice, "### TCP_NOTSENT_LOWAT failed: %#m\n", err);
...
OSStatus SocketSetP2P(SocketRef inSock, int inAllow)
{
    ...
    err = map_socket_noerr_errno(inSock, err);
    if (!err)
        dlog(kLogLevelVerbose, "Socket %d P2P: %s\n", inSock, inAllow ? "allow" : "prevent");
    else
        dlog(kLogLevelNotice, "### SO_RECV_ANYIF failed: %#m\n", err);
    return (err);
#else
    (void)inSock;
    (void)inAllow;
...
OSStatus SocketSetQoS(SocketRef inSock, int inQoS)
{
    ...
    if (inQoS == kSocketQoS_Default)
        diffServ = 0x00; // WMM=best effort, DSCP=0b000000 (CS0).
    else if (inQoS == kSocketQoS_Background)
        diffServ = 0x20; // WMM=background,  DSCP=0b001000 (CS1).
    else if (inQoS == kSocketQoS_Video)
        diffServ = 0x80; // WMM=video,       DSCP=0b100000 (CS4).
    else if (inQoS == kSocketQoS_Voice)
        diffServ = 0xC0; // WMM=voice,       DSCP=0b110000 (CS6).
    else if (inQoS == kSocketQoS_AirPlayAudio)
        diffServ = 0x80; // WMM=video,       DSCP=0b100000 (CS4).
    else if (inQoS == kSocketQoS_AirPlayScreenAudio)
        diffServ = 0xC0; // WMM=voice,       DSCP=0b110000 (CS6).
    else if (inQoS == kSocketQoS_AirPlayScreenVideo)
        diffServ = 0x80; // WMM=video,       DSCP=0b100000 (CS4).
    else if (inQoS == kSocketQoS_NTP)
        diffServ = 0xC0; // WMM=voice,       DSCP=0b110000 (CS6).
    else {
        dlogassert("Bad QoS value: %d", inQoS);
        err = kParamErr;
        goto exit;
    }
    ...
    if (family == AF_INET) {
        ...
        err = map_socket_noerr_errno(inSock, err);
        if (err)
            dlog(kLogLevelNotice, "### Set IP_TOS=0x%x failed: %#m\n", diffServ, err);
#else
        ...
        err = map_socket_noerr_errno(inSock, err);
        if (err)
            dlog(kLogLevelNotice, "### Set IPV6_TCLASS=0x%x failed: %#m\n", diffServ, err);
#else
...
// Set SO_TRAFFIC_CLASS. This is used by the driver for mbuf prioritization.
...
#if (defined(SO_TRAFFIC_CLASS) && SO_TRAFFIC_CLASS)
    {
        ...
        if (inQoS == kSocketQoS_Default)
            trafficClass = SO_TC_BE;
        else if (inQoS == kSocketQoS_Background)
            trafficClass = SO_TC_BK;
        else if (inQoS == kSocketQoS_Video)
            trafficClass = SO_TC_VI;
        else if (inQoS == kSocketQoS_Voice)
            trafficClass = SO_TC_VO;
        else if (inQoS == kSocketQoS_AirPlayAudio)
            trafficClass = SO_TC_AV;
        else if (inQoS == kSocketQoS_AirPlayScreenAudio)
            trafficClass = SO_TC_VO;
        else if (inQoS == kSocketQoS_AirPlayScreenVideo)
            trafficClass = SO_TC_VI;
        else if (inQoS == kSocketQoS_NTP)
            trafficClass = SO_TC_CTL;
        else {
            dlogassert("Bad QoS value: %d", inQoS);
            err = kParamErr;
            goto exit;
        ...
        err = map_socket_noerr_errno(inSock, err);
        if (err)
            dlog(kLogLevelNotice, "### Set SO_TRAFFIC_CLASS=%d failed: %#m\n", trafficClass, err);
    }
#endif
    err = kNoErr;
...
int SockAddrGetPort(const void* inSA)
{
    ...
    if (sa->sa_family == AF_INET)
        ...
        dlogassert("unknown family: %d", sa->sa_family);
        port = -1;
    }
    return (port);
}
...
void SockAddrSetPort(void* inSA, int inPort)
{
    ...
    sa = (const struct sockaddr*)inSA;
    if (sa->sa_family == AF_INET)
        ((struct sockaddr_in*)sa)->sin_port = htons((u_short)inPort);
#if (defined(AF_INET6))
    else if (sa->sa_family == AF_INET6)
        ((struct sockaddr_in6*)sa)->sin6_port = htons((u_short)inPort);
#endif
    else
        dlogassert("unknown family: %d", sa->sa_family);
}
...
void SockAddrCopy(const void* inSrc, void* inDst)
{
    ...
    if (src->sa_family == AF_INET)
        memmove(inDst, src, sizeof(struct sockaddr_in));
#if (defined(AF_INET6))
    else if (src->sa_family == AF_INET6)
        memmove(inDst, src, sizeof(struct sockaddr_in6));
#endif
    else
        dlogassert("unknown family: %d", src->sa_family);
}
...
#define IN6_IS_ADDR_V4MAPPED(a) ( \
    ((a)->__u6_addr.__u6_addr32[0] == 0) && ((a)->__u6_addr.__u6_addr32[1] == 0) && ((a)->__u6_addr.__u6_addr32[2] == ntohl((uint32_t)0x0000ffff)))

#undef IN6_IS_ADDR_V4COMPAT
#define IN6_IS_ADDR_V4COMPAT(a) ( \
    ((a)->__u6_addr.__u6_addr32[0] == 0) && ((a)->__u6_addr.__u6_addr32[1] == 0) && ((a)->__u6_addr.__u6_addr32[2] == 0) && ((a)->__u6_addr.__u6_addr32[3] != 0) && ((a)->__u6_addr.__u6_addr32[3] != ntohl((uint32_t)1)))
#endif
