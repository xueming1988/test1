...
{
    return (_InterlockedExchangeAdd((long*)inPtr, -inVal));
}
...
{
    return (_InterlockedExchangeAdd((long*)inPtr, inVal) + inVal);
}
...
STATIC_INLINE int32_t atomic_add_and_fetch_32(int32_t* inPtr, int32_t inVal)
{
    return ((inVal >= 0) ? (((int32_t)atomic_fetchadd_32((volatile u_int*)inPtr, (u_int)inVal)) + inVal) : (((int32_t)atomic_fetchadd_32((volatile u_int*)inPtr, (u_int)-inVal)) + inVal));
}
...
{
    return (atomic_add_and_fetch_32(inPtr, inVal) - inVal);
}
...
{
    return (atomic_add_and_fetch_32(inPtr, -inVal));
}
...
#define atomic_read_barrier() __asm__ __volatile__("" \
                                                   :  \
                                                   :  \
                                                   : "memory")
#define atomic_write_barrier() __asm__ __volatile__("" \
                                                    :  \
                                                    :  \
                                                    : "memory")
#define atomic_read_write_barrier() __asm__ __volatile__("" \
                                                         :  \
                                                         :  \
                                                         : "memory")
...
{
    if (inVal >= 0)
        return ((int32_t)atomic_add_value((unsigned*)inPtr, (unsigned)inVal));
    else
        return ((int32_t)atomic_sub_value((unsigned*)inPtr, (unsigned)-inVal));
}
...
#define atomic_spinlock_lock(SPIN_LOCK_PTR)                             \
    do {                                                                \
        while (!atomic_bool_compare_and_swap_32((SPIN_LOCK_PTR), 0, 1)) \
            atomic_yield();                                             \
    } while (0)
