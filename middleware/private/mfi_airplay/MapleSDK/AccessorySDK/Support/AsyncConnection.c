...
#define AsyncConnectionProgress(CNX, PHASE, DETAILS)                     \
    do {                                                                 \
        if ((CNX)->progressFunc)                                         \
            (CNX)->progressFunc((PHASE), (DETAILS), (CNX)->progressArg); \
    } while (0)
...
typedef struct {
    uint8_t c[256];
} domainname; // Up to 255 bytes of length-prefixed domainlabels
...
exit:
    if (err)
        _AsyncConnection_Complete(connection, kInvalidSocketRef, err);
}
...
static void _AsyncConnection_Complete(AsyncConnectionRef inConnection, SocketRef inSock, OSStatus inError)
{
    ...
    if (handlerFunc)
        handlerFunc(inSock, inError, inConnection->handlerArg);
...
exit:
    if (obj)
        _AsyncConnection_ReleaseOperation(obj);
    return (err);
}
    ...
    void* inContext)
{
    ...
    if ((port == 0) || (operation->defaultPort < 0))
        port = operation->defaultPort;
    ...
    int inDefaultPort)
{
    ...
    flags = 0;
    if (inConnection->flags & kAsyncConnectionFlag_SuppressUnusable)
        flags |= kDNSServiceFlagsSuppressUnusable_compat;
    ...
    int inDefaultPort)
{
    ...
    if (inParentOp)
        inParentOp->dnsResolveSecs = CFAbsoluteTimeGetCurrent() - inParentOp->dnsResolveStartTime;
...
#if (defined(AF_INET6))
    if ((((const sockaddr_ip*)inAddr)->sa.sa_family == AF_INET6) && (inConnection->ipv6DelayNanos > 0)) {
    ...
    uint64_t inDelayNanos)
{
    ...
    obj->ifIndex = inIfIndex;
    if (inDefaultPort < 0)
        SockAddrSetPort(&obj->addr, -inDefaultPort);
    else if (SockAddrGetPort(&obj->addr) == 0)
        SockAddrSetPort(&obj->addr, inDefaultPort);
    obj->defaultPort = inDefaultPort;
    obj->nativeSock = kInvalidSocketRef;
    ...
    int inDefaultPort)
{
    ...
    if (inConnection->flags & kAsyncConnectionFlag_P2P)
        SocketSetP2P(obj->nativeSock, true);
    ...
    if (err == 0) {
        _AsyncConnection_ConnectHandler(obj);
        goto exit;
    }
...
exit:
    if (err)
        _AsyncConnection_ErrorHandler(inOperation, err);
}
...
static char* ConvertDomainLabelToCString_withescape(const domainlabel* const label, char* ptr, char esc)
{
    ...
    if (len > MAX_DOMAIN_LABEL)
        return (NULL); // If illegal label, abort
    ...
    SocketRef* outSock)
{
    ...
    if (inWaitCallBack) {
        for (;;) {
            ...
            if (!dispatch_semaphore_wait(context.doneSem, timeout))
                break;
...
exit:
    ...
    if (context.queue)
        arc_safe_dispatch_release(context.queue);
    return (err);
}
