...
DEBUG_STATIC void _IPCAgent_Finalize(void* inContext)
{
    ...
    if (agent->quitSem)
        dispatch_semaphore_signal(agent->quitSem);
...
exit:
    ForgetSocket(&sock);
    if (err)
        dlog(kLogLevelWarning, "### [DebugIPC] Start failed: %#m\n", err);
}
...
exit:
    if (peMem)
        free(peMem);
    if (err)
        dlog(kLogLevelWarning, "[DebugIPC] ### Packet receive error: %#m\n", err);
}
...
DEBUG_STATIC OSStatus _IPCAgent_ProcessMessageBytes(IPCAgentRef inAgent, const void* inMsg, size_t inLen)
{
    ...
    if (inAgent->handlerFunc)
        inAgent->handlerFunc(plistObj, inAgent->handlerContext);
    err = kNoErr;
...
exit:
    if (context.agent)
        IPCAgent_DeleteSync(context.agent);
    if (context.lock)
        arc_safe_dispatch_release(context.lock);
    if (context.msgSem)
        arc_safe_dispatch_release(context.msgSem);
    if (context.msgs)
        CFRelease(context.msgs);
    return (err);
}
...
DEBUG_STATIC void _IPCAgent_PerformHandler(CFDictionaryRef inMsg, void* inContext)
{
    ...
    dispatch_semaphore_signal(context->lock);
    if (wasEmpty)
        dispatch_semaphore_signal(context->msgSem);
}
...
OSStatus DebugIPC_PerformV(IPCMessageHandlerFunc inHandler, void* inContext, const char* inFormat, va_list inArgs)
{
    ...
    if (!inHandler)
        inHandler = _DebugIPC_ShowHandler;
...
static void _DebugIPC_MessageHandler(CFDictionaryRef inMsg, void* inContext)
{
    ...
    if (mutableResponse)
        CFDictionarySetValue(mutableResponse, kDebugIPCKey_ResponseType, opcode);
