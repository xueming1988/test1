...
typedef OSStatus (*NetSocket_ReadFunc)(
    NetSocketRef inSock,
    size_t inMinSize,
    size_t inMaxSize,
    void* inBuffer,
...
#define NetTransportTypeIsWiFi(X) ( \
    ((X) == kNetTransportType_WiFi) || ((X) == kNetTransportType_WiFiDirect))

#define NetTransportTypeIsWired(X) ( \
    ((X) == kNetTransportType_Ethernet) || ((X) == kNetTransportType_USB) || ((X) == kNetTransportType_DirectLink))

#define NetTransportTypeIsWireless(X) ( \
    ((X) == kNetTransportType_WiFi) || ((X) == kNetTransportType_WiFiDirect) || ((X) == kNetTransportType_BTLE))

#define NetTransportTypeToString(X) ( \
    ((X) == kNetTransportType_Ethernet) ? "Enet" : ((X) == kNetTransportType_WiFi) ? "WiFi" : ((X) == kNetTransportType_WiFiDirect) ? "WFD" : ((X) == kNetTransportType_USB) ? "USB" : ((X) == kNetTransportType_DirectLink) ? "Direct" : ((X) == kNetTransportType_BTLE) ? "BTLE" : "?")
...
/*!	@function	SocketSetKeepAlive
    @abstract   Enables/disables TCP keep alive and configures the time between probes and the max probes before giving up.
	 ...
	 @param     inSock                    Socket to set keep-alive options for.
	 @param     inIdleSecs                Number of idle seconds before a keep-alive probe is sent.
	 @param     inIntervalSecs            Interval between keep-alives after the initial probe is sent.
	 @param     inMaxUnansweredProbes    Max number of unanswered probes before a connection is terminated.
 */
