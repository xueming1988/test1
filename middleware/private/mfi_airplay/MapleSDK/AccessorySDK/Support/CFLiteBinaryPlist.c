...
static void _FlattenPlist(CFBinaryPlistContext* ctx, CFTypeRef inObj)
{
    ...
    if ((type == gCFStringType) || (type == gCFNumberType) || (type == gCFBooleanType) || (type == gCFDataType) || (type == gCFDateType)) {
...
CFPropertyListRef CFBinaryPlistV0CreateWithData(const void* inPtr, size_t inLen, OSStatus* outErr)
{
    ...
    require_action_quiet(
        (trailer.offsetIntSize == 1) || (trailer.offsetIntSize == 2) || (trailer.offsetIntSize == 4) || (trailer.offsetIntSize == 8), exit, err = kMalformedErr);
    require_action_quiet(
        (trailer.objectRefSize == 1) || (trailer.objectRefSize == 2) || (trailer.objectRefSize == 4) || (trailer.objectRefSize == 8), exit, err = kMalformedErr);
    ...
    require_action_quiet(trailer.numObjects <= (((size_t)(end - (src + trailer.offsetTableOffset))) / trailer.offsetIntSize), exit, err = kCountErr);
    ...
    OSStatus* outErr)
{
    ...
    case kCFLBinaryPlistMarkerInt:
        ...
        if (u32 > 8) {
            for (u128.hi = 0; u32 > 8; --u32)
                u128.hi = (u128.hi << 8) | *ptr++;
            for (u128.lo = 0; u32 > 0; --u32)
                u128.lo = (u128.lo << 8) | *ptr++;
            obj = CFNumberCreate(NULL, kCFNumberSInt128Type_compat, &u128);
        } else {
            for (v64.u64 = 0; u32 > 0; --u32)
                v64.u64 = (v64.u64 << 8) | *ptr++;
    ...
    size_t* outOffset)
{
    ...
    case 1:
        ...
        break;
    case 2:
        refnum = ReadBig16(ptr);
        break;
    case 4:
        refnum = ReadBig32(ptr);
        break;
    case 8:
        refnum = ReadBig64(ptr);
        break;
    default:
        err = kInternalErr;
        goto exit;
    ...
    *ioPtr = ptr + ctx->objectRefSize;
    ...
    case 1:
        offset = Read8(offsetPtr);
        break;
    case 2:
        offset = ReadBig16(offsetPtr);
        break;
    case 4:
        offset = ReadBig32(offsetPtr);
        break;
    case 8:
        offset = ReadBig64(offsetPtr);
        break;
    default:
        err = kInternalErr;
        goto exit;
...
static const uint8_t kV0Test4[] = // Recursion test from <rdar://problem/49839018>.
    {
      0x62, 0x70, 0x6C, 0x69, 0x73, 0x74, 0x30, 0x30, 0xD1, 0x00, 0x02, 0x59, 0x71, 0x75, 0x61, 0x6C,
      0x69, 0x66, 0x69, 0x65, 0x72, 0xA1, 0x03, 0x5A, 0x74, 0x78, 0x74, 0x41, 0x69, 0x72, 0x50, 0x6C,
      0x61, 0x79, 0x08, 0x0B, 0x15, 0x17, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x01, 0x00, 0x00,
...
OSStatus CFLiteBinaryPlistTest(void)
{
    ...
    plist2 = (CFMutableDictionaryRef)CFBinaryPlistV0CreateWithData(kV0Test4, sizeof(kV0Test4), &err);
    require_action(plist2 == NULL, exit, err = kUnexpectedErr);
    require_action(err != kNoErr, exit, err = kUnexpectedErr);
