/*
	Portions Copyright (C) 2012-2019 Apple Inc. All Rights Reserved.
*/
...
#define SHA1_FF0(a, b, c, d, e, i)                                           \
    e = (ROTL32(a, 5) + SHA1_F0(b, c, d) + e + W[i] + UINT32_C(0x5a827999)); \
    b = ROTL32(b, 30);
#define SHA1_FF1(a, b, c, d, e, i)                                           \
    e = (ROTL32(a, 5) + SHA1_F1(b, c, d) + e + W[i] + UINT32_C(0x6ed9eba1)); \
    b = ROTL32(b, 30);
#define SHA1_FF2(a, b, c, d, e, i)                                           \
    e = (ROTL32(a, 5) + SHA1_F2(b, c, d) + e + W[i] + UINT32_C(0x8f1bbcdc)); \
    b = ROTL32(b, 30);
#define SHA1_FF3(a, b, c, d, e, i)                                           \
    e = (ROTL32(a, 5) + SHA1_F3(b, c, d) + e + W[i] + UINT32_C(0xca62c1d6)); \
    b = ROTL32(b, 30);
...
OSStatus SHA1_Test(void)
{
    ...
    SHA1_Init_compat(&ctx);
    for (i = 0; i < len; ++i)
        SHA1_Update_compat(&ctx, &buf[i], 1);
    SHA1_Final_compat(digest, &ctx);
    require_action(memcmp(digest, kSHA1_NISTTestVector3Result, SHA_DIGEST_LENGTH) == 0, exit, err = -1);
...
#define SHA256_S(x, n) ROTR32((x), (n))
...
static void _SHA256_Compress(SHA256_CTX_compat* ctx, const uint8_t* inPtr);
...
int SHA256_Update_compat(SHA256_CTX_compat* ctx, const void* inData, size_t inLen)
{
    ...
    while (inLen > 0) {
        if ((ctx->curlen == 0) && (inLen >= SHA256_BLOCK_SIZE)) {
            _SHA256_Compress(ctx, src);
            ctx->length += (SHA256_BLOCK_SIZE * 8);
            src += SHA256_BLOCK_SIZE;
            inLen -= SHA256_BLOCK_SIZE;
        } else {
            n = Min(inLen, SHA256_BLOCK_SIZE - ctx->curlen);
            memcpy(ctx->buf + ctx->curlen, src, n);
            ctx->curlen += n;
            src += n;
            inLen -= n;
...
{
    SHA256_CTX_compat ctx;

    SHA256_Init_compat(&ctx);
    SHA256_Update_compat(&ctx, inData, inLen);
    SHA256_Final_compat(outDigest, &ctx);
    return (outDigest);
}
...
// "abc"
#define kSHA256_LibTomTestVector1Result                                \
    "\xBA\x78\x16\xBF\x8F\x01\xCF\xEA\x41\x41\x40\xDE\x5D\xAE\x22\x23" \
    "\xB0\x03\x61\xA3\x96\x17\x7A\x9C\xB4\x10\xFF\x61\xF2\x00\x15\xAD"

// "abcdefghbcdefghicdefghijdefghijkefghijklfghijklmghijklmnhijklmnoijklmnopjklmnopqklmnopqrlmnopqrsmnopqrstnopqrstu"
#define kSHA256_LibTomTestVector2Result                                \
...
#define kSHA256_WikipediaTestVector3Result                             \
    "\xEF\x53\x7F\x25\xC8\x95\xBF\xA7\x82\x52\x65\x29\xA9\xB6\x3D\x97" \
    "\xAA\x63\x15\x64\xD5\xD7\x89\xC2\xB7\x65\x44\x8C\x86\x35\xFB\x6C"

// 1,000,000 of 'a'
#define kSHA256_TestVector4Result                                      \
...
OSStatus SHA256_Test(void)
{
    ...
    memset(digest, 0, sizeof(digest));
    ptr = SHA256_compat("abc", 3, digest);
    require_action(ptr == digest, exit, err = -1);
    require_action(memcmp(digest, kSHA256_LibTomTestVector1Result, SHA256_DIGEST_LENGTH) == 0, exit, err = -1);

    memset(digest, 0, sizeof(digest));
    ptr = SHA256_compat("abcdefghbcdefghicdefghijdefghijkefghijklfghijklmghijklmnhijklmnoijklmnopjklmnopqklmnopqrlmnopqrsmnopqrstnopqrstu", 112, digest);
    require_action(ptr == digest, exit, err = -1);
    require_action(memcmp(digest, kSHA256_LibTomTestVector2Result, SHA256_DIGEST_LENGTH) == 0, exit, err = -1);

    // Test vector from NIST.

    memset(digest, 0, sizeof(digest));
    ptr = SHA256_compat("The quick brown fox jumps over the lazy dog.", 44, digest);
    require_action(ptr == digest, exit, err = -1);
    require_action(memcmp(digest, kSHA256_WikipediaTestVector3Result, SHA256_DIGEST_LENGTH) == 0, exit, err = -1);
    ...
    require_action(buf, exit, err = -1);
    memset(buf, 'a', len);
    ptr = SHA256_compat(buf, len, digest);
    require_action(ptr == digest, exit, err = -1);
    require_action(memcmp(digest, kSHA256_TestVector4Result, SHA256_DIGEST_LENGTH) == 0, exit, err = -1);

    // Partial update tests.

    memset(digest, 0, sizeof(digest));
    SHA256_Init_compat(&ctx);
    SHA256_Update_compat(&ctx, &buf[0], 1);
    SHA256_Update_compat(&ctx, &buf[1], 4);
    SHA256_Update_compat(&ctx, &buf[5], 100);
    SHA256_Update_compat(&ctx, &buf[105], len - 105);
    SHA256_Final_compat(digest, &ctx);
    require_action(memcmp(digest, kSHA256_TestVector4Result, SHA256_DIGEST_LENGTH) == 0, exit, err = -1);

    memset(digest, 0, sizeof(digest));
    SHA256_Init_compat(&ctx);
    for (i = 0; i < len; ++i)
        SHA256_Update_compat(&ctx, &buf[i], 1);
    SHA256_Final_compat(digest, &ctx);
    require_action(memcmp(digest, kSHA256_TestVector4Result, SHA256_DIGEST_LENGTH) == 0, exit, err = -1);
...
OSStatus SHA512_Test(void)
{
    ...
    SHA512_Init_compat(&ctx);
    for (i = 0; i < len; ++i)
        SHA512_Update_compat(&ctx, &buf[i], 1);
    SHA512_Final_compat(digest, &ctx);
    require_action(memcmp(digest, kSHA512_TestVector4Result, SHA512_DIGEST_LENGTH) == 0, exit, err = -1);
...
int SHA3_Update_compat(SHA3_CTX_compat* ctx, const void* inData, size_t inLen)
{
    ...
    if (ctx->buffered > 0) {
        ...
        ctx->buffered += n;
        if (ctx->buffered < SHA3_BLOCK_SIZE)
            goto exit;
        src += n;
        inLen -= n;
    ...
    uint8_t* outKey)
{
    ...
    if (inPasswordLen == kSizeCString)
        inPasswordLen = strlen((const char*)inPasswordPtr);
    if (inSaltLen == kSizeCString)
        inSaltLen = strlen((const char*)inSaltPtr);
