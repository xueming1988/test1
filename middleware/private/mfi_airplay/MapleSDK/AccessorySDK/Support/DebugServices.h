...
*/
#if (DEBUG)
#define debug_track_min(VAR, VALUE) \
    do {                            \
        if ((VALUE) < (VAR)) {      \
            VAR = (VALUE);          \
        }                           \
    } while (0)
#define debug_track_max(VAR, VALUE) \
    do {                            \
        if ((VALUE) > (VAR)) {      \
            VAR = (VALUE);          \
        }                           \
...
#else
#define debug_increment_wrap(VAR, WRAP) \
    do {                                \
    } while (0)
#endif
...
#else
#define debug_increment_saturate(VAR, MAX) \
    do {                                   \
    } while (0)
#endif
...
#else
#define debug_add_saturate(VAR, VALUE, MAX) \
    do {                                    \
    } while (0)
#endif
...
#define check_ptr_overlap(P1, P1_SIZE, P2, P2_SIZE)                                                            \
    do {                                                                                                       \
        check(!(((uintptr_t)(P1) >= (uintptr_t)(P2)) && ((uintptr_t)(P1) < (((uintptr_t)(P2)) + (P2_SIZE))))); \
        check(!(((uintptr_t)(P2) >= (uintptr_t)(P1)) && ((uintptr_t)(P2) < (((uintptr_t)(P1)) + (P1_SIZE))))); \
                                                                                                               \
    } while (0)
...
#else
#if (TARGET_HAS_C99_VA_ARGS)
#define dlogc(CATEGORY_PTR, LEVEL, ...) \
    do {                                \
    } while (0)
#elif (TARGET_HAS_GNU_VA_ARGS)
#define dlogc(CATEGORY_PTR, LEVEL, ARGS...) \
    do {                                    \
    } while (0)
#else
...
#define dlogcv ulogv
#else
#define dlogcv(CATEGORY_PTR, LEVEL, FORMAT, ARGS) \
    do {                                          \
    } while (0)
#endif
...
#else
#define debug_stack_trace(LEVEL) \
    do {                         \
    } while (0)
#endif
