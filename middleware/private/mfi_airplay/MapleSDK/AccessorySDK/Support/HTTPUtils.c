...
OSStatus HTTPHeader_Parse(HTTPHeader* ioHeader)
{
    ...
    if (c == ' ') // Requests have a space after the method. Responses have '/' after the protocol.
    {
        ...
        ++ptr;
        if (strnicmpx(src, len, "GET") == 0)
            ioHeader->method = kHTTPMethod_GET;
        else if (strnicmpx(src, len, "POST") == 0)
            ioHeader->method = kHTTPMethod_POST;
        else if (strnicmpx(src, len, "PUT") == 0)
            ioHeader->method = kHTTPMethod_PUT;
        else if (strnicmpx(src, len, "DELETE") == 0)
            ioHeader->method = kHTTPMethod_DELETE;
    ...
    } else // Response
    {
        ...
        x = 0;
        for (c = 0; (ptr < end) && ((c = *ptr) >= '0') && (c <= '9'); ++ptr)
            x = (x * 10) + (c - '0');
    ...
    if (err)
        ioHeader->persistent = (Boolean)(strnicmpx(ioHeader->protocolPtr, ioHeader->protocolLen, "HTTP/1.0") != 0);
    else
        ioHeader->persistent = (Boolean)(strnicmpx(value, len, "close") != 0);
    ...
    const char** outNext)
{
    ...
    if (inHeaderLen == kSizeCString)
        inHeaderLen = strlen(inHeaderPtr);
    src = inHeaderPtr;
    end = src + inHeaderLen;
    ...
    for (;;) {
        ...
        } else if ((lineLen <= matchLen) || (linePtr[matchLen] != ':') || (strnicmp(linePtr, inName, matchLen) != 0)) {
            continue;
        }
        ...
        valueEnd = lineEnd;
        while ((valuePtr < valueEnd) && (((c = *valuePtr) == ' ') || (c == '\t')))
            ++valuePtr;
...
OSStatus HTTPParseByteRangeRequest(const char* inStr, size_t inLen, int64_t* outStart, int64_t* outEnd)
{
    ...
    {
        x = 0;
        for (ptr = ++src; (ptr < end) && ((c = *ptr) >= '0') && (c <= '9'); ++ptr)
            x = (x * 10) + (c - '0');
    ...
    const char** outSrc)
{
    ...
    while ((src < end) && isspace_safe(*src))
        ++src; // Skip whitespace before name.
    err = HTTPParseToken(src, end, &namePtr, &nameEnd, &src);
    require_noerr_quiet(err, exit);
    while ((src < end) && isspace_safe(*src))
        ++src; // Skip whitespace after name.
    ...
    if ((src < end) && (*src == '=')) {
        ++src;
        while ((src < end) && isspace_safe(*src))
            ++src; // Skip whitespace before value.
        if ((src < end) && (*src == '\"')) // Quoted String
        {
            valuePtr = ++src;
            while ((src < end) && ((*src != '"') || (src[-1] == '\\')))
                ++src;
        ...
        while ((src < end) && isspace_safe(*src))
            ++src; // Skip whitespace after value.
    }
    ...
    const char** outSrc)
{
    ...
    if (outTokenPtr)
        *outTokenPtr = (const char*)inSrc;
    if (outTokenEnd)
        *outTokenEnd = (const char*)src;
    if (outSrc)
    ...
    size_t inKeyLen)
{
    ...
    timestampPtr = src;
    for (nonceSecs = 0; (src < end) && isdigit_safe(*src); ++src)
        nonceSecs = (nonceSecs * 10) + (*src - '0');
...
DEBUG_STATIC OSStatus __HTTPAddBasicAuth(HTTPClientAuthorizationInfoRef inAuthInfo)
{
    ...
    err = HTTPHeader_AddFieldF(inAuthInfo->requestHeader, "Authorization", "%s", fullValue);
    free(fullValue);
    require_noerr(err, exit);
...
DEBUG_STATIC OSStatus __HTTPAddDigestAuth(HTTPClientAuthorizationInfoRef inAuthInfo)
{
    ...
    err = HTTPHeader_AddFieldF(inAuthInfo->requestHeader, "Authorization", "Digest "
                                                                           "username=\"%s\", "
                                                                           "realm=\"%.*s\", "
                                                                           "nonce=\"%.*s\", "
                                                                           "uri=\"%.*s\", "
...
HTTPStatus HTTPVerifyAuthorization(HTTPAuthorizationInfoRef ioAuthInfo)
{
    ...
    if (strnicmpx(schemePtr, schemeLen, "Basic") == 0) {
        ...
        *p++ = '\0';
        ...
        status = ioAuthInfo->copyPasswordFunction(ioAuthInfo, &password);
        require_quiet(status == kHTTPStatus_OK, exit);

        match = (strncmpx(ioAuthInfo->requestPasswordPtr, ioAuthInfo->requestPasswordLen, password) == 0);
        ioAuthInfo->badMatch = match;
        free(password);
        if (!match) {
            dlog(kLogLevelNotice, "%s: ### bad password: \"%.*s\"\n",
                __ROUTINE__, (int)ioAuthInfo->requestPasswordLen, ioAuthInfo->requestPasswordPtr);
    ...
    // 			response="7809A64F92C06A0E91FCE8FBFAD78AE2"
        ...
        require_action(ioAuthInfo->isValidNonceFunction, exit, status = kHTTPStatus_InternalServerError);
        if (!ioAuthInfo->isValidNonceFunction(ioAuthInfo)) {
            dlog(kLogLevelNotice, "%s: ### bad nonce: \"%.*s\"\n",
                __ROUTINE__, (int)ioAuthInfo->requestNonceLen, ioAuthInfo->requestNoncePtr);
            status = kHTTPStatus_Unauthorized;
            goto exit;
        }
        ...
        require_action(ioAuthInfo->copyPasswordFunction, exit, status = kHTTPStatus_InternalServerError);
        status = ioAuthInfo->copyPasswordFunction(ioAuthInfo, &password);
        require(status == kHTTPStatus_OK, exit);
        ...
        if (strnicmpx(ioAuthInfo->requestResponsePtr, ioAuthInfo->requestResponseLen, ourDigest) != 0) {
            dlog(kLogLevelNotice, "%s: ### bad digest:\n"
                                  "    ours:   %s\n"
                                  "    theirs: %.*s\n",
                __ROUTINE__, ourDigest, (int)ioAuthInfo->requestResponseLen, ioAuthInfo->requestResponsePtr);
            status = kHTTPStatus_Unauthorized;
            goto exit;
        }
    }
    ...
    const void* inBodyPtr, size_t inBodyLen)
{
    ...
    if (!inRequestCategory)
        inRequestCategory = &log_category_from_name(HTTPRequests);
    if (!inResponseCategory)
        inResponseCategory = &log_category_from_name(HTTPResponses);
    if (!log_category_enabled(inRequestCategory, kLogLevelInfo) && !log_category_enabled(inResponseCategory, kLogLevelInfo)) {
        return;
    }
    ...
    for (ptr = src; (ptr < end) && ((c = *ptr) != ' ') && (c != '/'); ++ptr) {
    } // Detect request or response.
    if (c == ' ') {
        isRequest = 1;
        category = inRequestCategory;
    } else {
        isRequest = 0;
        category = inResponseCategory;
    }
    ...
    } else if (log_category_enabled(category, kLogLevelInfo)) // Single line
    {
        if (inHeaderPtr) {
            ...
            if (isRequest)
            {
                if (NULL == strstr(src, "/feedback"))
                {
                    ulog(inRequestCategory, kLogLevelMax, "HTTP Request:  %.*s\n", (int)(eol - src), src);
                }
            }
            else
            {
                if (NULL == strstr(src, "200 OK"))
                    ulog(inResponseCategory, kLogLevelMax, "HTTP Response: %.*s\n\n", (int)(eol - src), src);
            }
        }
    }
}
