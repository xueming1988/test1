...
#if (LOGUTILS_CF_ENABLED)
    if (gLogCFInitialized || gLogUtilsInitializing)
        return (kNoErr); // Avoid recursion.
...
void LogUtils_Finalize(void)
{
    ...
    if (dnc)
        CFNotificationCenterRemoveEveryObserver(dnc, gLogCFNotificationObserver);
#endif
...
static void _LogUtils_EnsureCFNotificationsInitialized(void)
{
    ...
    if (gLogCFNotificationInitialized)
        return;
    dnc = CFNotificationCenterGetDistributedCenter();
    if (!dnc)
        return;
...
static void _LogUtils_ReadCFPreferences(LogControlFlags inFlags)
{
    ...
    if (cfStr) {
        if ((CFGetTypeID(cfStr) == CFStringGetTypeID()) && ((inFlags & kLogControlFlags_Defaults) || !CFEqualNullSafe(cfStr, gLogCFLastControlPref))) {
...
static OSStatus _LogUtils_WriteCFPreferences(void)
{
    ...
    for (action = gLogActionList; action; action = action->next) {
        ...
        if (((stricmp(action->variable, "output") == 0) || (stricmp(action->variable, "output2") == 0)) && (stricmp_prefix(action->value, "callback") == 0)) {
            continue;
        }
...
static OSStatus _LogControlLocked(const char* inCmd, LogControlFlags inFlags)
{
    ...
    while (*inCmd != '\0') {
        ...
        if (!(inFlags & kLogControlFlags_Internal) && ((strnicmpx(variablePtr, variableLen, "output") == 0) || (strnicmpx(variablePtr, variableLen, "output2") == 0)) && (strnicmp_prefix(valuePtr, valueLen, "callback") == 0)) {
            continue;
        }
        ...
        for (actionNext = &gLogActionList; (action = *actionNext) != NULL; actionNext = &action->next) {
            if ((strnicmpx(namePtr, nameLen, action->name) == 0) && (strnicmpx(variablePtr, variableLen, action->variable) == 0)) {
                break;
            }
        }
        if (!action) {
    ...
    if (!(inFlags & kLogControlFlags_FromPrefs)) {
        if (isDefault)
            _LogUtils_ReadCFPreferences(inFlags | kLogControlFlags_Defaults);
        else if (persist)
            _LogUtils_WriteCFPreferences();
    }
#endif
...
exit:
    if (actionNew)
        _LogUtils_FreeAction(actionNew);
    return (err);
}
...
void LogCategory_Remove(LogCategory* inCategory)
{
    ...
    for (next = &gLogCategoryList; (curr = *next) != NULL; next = &curr->next) {
        if (curr == inCategory) {
            if (curr->output1)
                --curr->output1->refCount;
            if (curr->output2)
                --curr->output2->refCount;
            *next = curr->next;
...
static OSStatus _LogCategory_ApplyActions(LogCategory* inCategory)
{
    ...
    for (action = gLogActionList; action; action = action->next) {
        ...
        for (category = gLogCategoryList; category; category = category->next) {
            if (inCategory && (inCategory != category))
                continue;
            if (!RegexMatch(action->name, category->name))
                continue;
    ...
    for (action = gLogActionList; action; action = action->next) {
        ...
        while (valueSrc < valueEnd) {
            ...
            if (valueSrc < valueEnd)
                ++valueSrc;

            if (strnicmpx(valueTok, valueLen, "none") == 0)
                flags = kLogFlags_None;
            else if (strnicmpx(valueTok, valueLen, "time") == 0)
                flags |= kLogFlags_PrintTime;
            else if (strnicmpx(valueTok, valueLen, "pid") == 0)
                flags |= kLogFlags_PrintPID;
            else if (strnicmpx(valueTok, valueLen, "program") == 0)
                flags |= kLogFlags_PrintProgram;
            else if (strnicmpx(valueTok, valueLen, "category") == 0)
                flags |= kLogFlags_PrintCategory;
            else if (strnicmpx(valueTok, valueLen, "level") == 0)
                flags |= kLogFlags_PrintLevel;
            else if (strnicmpx(valueTok, valueLen, "prefix") == 0)
                flags |= kLogFlags_PrintPrefix;
            else if (strnicmpx(valueTok, valueLen, "function") == 0)
                flags |= kLogFlags_PrintFunction;
            else
                continue;
        }
        ...
        for (category = gLogCategoryList; category; category = category->next) {
            ...
            if (flags & kLogFlags_PrintPrefix) {
                ...
                if (!valueEnd)
                    valueEnd = category->name + strlen(category->name);
    ...
    for (action = gLogActionList; action; action = action->next) {
        if (strnicmpx(action->variable, SIZE_MAX, "output") == 0)
            outputID = 1;
        else if (strnicmpx(action->variable, SIZE_MAX, "output2") == 0)
            outputID = 2;
        else
            continue;
...
static OSStatus _LogCategory_ApplyAction_Output(LogCategory* inCategory, int inOutputID, LogAction* inAction)
{
    ...
    if (inOutputID == 1)
        outputAddr = &inCategory->output1;
    else if (inOutputID == 2)
        outputAddr = &inCategory->output2;
    else {
        err = kParamErr;
        goto exit;
    }
...
static char* _LULevelToString(LogLevel inLevel, char* inBuf, size_t inLen)
{
    ...
    if (diff > 0)
        SNPrintF(inBuf, inLen, "%s+%u", name, diff);
    else if (diff < 0)
        SNPrintF(inBuf, inLen, "%s-%u", name, -diff);
    else
        SNPrintF(inBuf, inLen, "%s", name);
    return (inBuf);
}
...
LogLevel LUStringToLevel(const char* inStr)
{
    ...
    if (ptr != inStr) {
        ...
        x = 0;
        for (++ptr; isdigit_safe(c = *ptr); ++ptr)
            x = (x * 10) + (c - '0');
...
int LogPrintV(LogCategory* inCategory, const char* inFunction, LogLevel inLevel, const char* inFormat, va_list inArgs)
{
    ...
    if (!(inLevel & kLogLevelFlagContinuation)) {
        ...
        if (inLevel & kLogLevelFlagFunction)
            flags |= kLogFlags_PrintFunction;
        ...
        if (flags & (kLogFlags_PrintProgram | kLogFlags_PrintPID | kLogFlags_PrintCategory | kLogFlags_PrintLevel | kLogFlags_PrintPrefix | kLogFlags_PrintFunction)) {
...
#if (TARGET_OS_POSIX)
        if (_LogOutput_IsStdErrMappedToDevNull())
...
static void _LogOutputCallBack_Writer(LogPrintFContext* inContext, LogOutput* inOutput, const char* inStr, size_t inLen)
{
    if (inOutput->config.callback.func)
        inOutput->config.callback.func(inContext, inStr, inLen, inOutput->config.callback.arg);
}
...
static OSStatus _LogOutputFile_Setup(LogOutput* inOutput, const char* inParams)
{
    ...
    if (inOutput->config.file.logFilePtr && (inOutput->config.file.logFilePtr != stderr) && (inOutput->config.file.logFilePtr != stdout)) {
        fclose(inOutput->config.file.logFilePtr);
    }
    inOutput->config.file.logFilePtr = NULL;
...
static OSStatus _LogOutputSysLog_Setup(LogOutput* inOutput, const char* inParams)
{
    ...
    while (*inParams != '\0') {
        ...
        if (strnicmpx(namePtr, nameLen, "level") == 0) {
            ...
            if (level == kLogLevelUninitialized)
                continue;
            else if (level >= kLogLevelEmergency)
                priority = LOG_EMERG;
            else if (level >= kLogLevelAlert)
                priority = LOG_ALERT;
            else if (level >= kLogLevelCritical)
                priority = LOG_CRIT;
            else if (level >= kLogLevelError)
                priority = LOG_ERR;
            else if (level >= kLogLevelWarning)
                priority = LOG_WARNING;
            else if (level >= kLogLevelNotice)
                priority = LOG_NOTICE;
...
static void _LogOutputWindowsEventLog_Writer(LogPrintFContext* inContext, LogOutput* inOutput, const char* inStr, size_t inLen)
{
    ...
    if (inContext->level <= kLogLevelNotice)
        type = EVENTLOG_INFORMATION_TYPE;
    else if (inContext->level <= kLogLevelWarning)
