    ...
    size_t* outEncodedSize)
{
    ...
    if (inSourceSize == kSizeCString)
        inSourceSize = strlen((const char*)inSourceData);

    if (inFlags & kBase64Flag_URLCharSet)
        table = kBase64EncodeTable_URL;
    else
        table = kBase64EncodeTable;
...
size_t Base64DecodedSize(const void* inEncodedData, size_t inEncodedSize)
{
    ...
    while (src < end) {
        ...
        if (b == kBase64InvalidFlag)
            goto exit;
        if (b == kBase64IgnoreFlag)
            continue;
        if (b == kBase64PadFlag)
            break;
...
OSStatus Base64DecodeCopy(const void* inEncodedData, size_t inEncodedSize, void* outDecodedData, size_t* outDecodedSize)
{
    ...
    n = Base64DecodedMaxSize(inEncodedSize);
    dst = (uint8_t*)malloc(n + 1);
    require_action(dst, exit, err = kNoMemoryErr);
    ...
    require_noerr(err, exit);
    check(n <= Base64DecodedMaxSize(inEncodedSize));
    dst[n] = '\0';
...
OSStatus Base64UtilsTest(Boolean inPrint)
{
    ...
    //if( inPrint ) dlogmem( kLogLevelMax, decoded, size );
    if (inPrint)
        dlog(kLogLevelMax, "\n");
...
exit:
    dlog(kLogLevelMax, "%s: %s\n", __ROUTINE__, (err == kNoErr) ? "PASSED" : "FAILED");
    return (err);
}
    ...
    Boolean inPrint)
{
    ...
    if (inPrint)
        dlog(kLogLevelMax, "*** TEST VECTOR ***\n");

    // Encode the source.

    if (inLineEndings)
        size = Base64EncodedLinesMaxSize(inSrcSize, inIndent, inLineEndings);
    else
        size = Base64EncodedMaxSize(inSrcSize);
    require_action(size >= strlen(inEncoded), exit, err = kResponseErr);

    memset(encoded, 0xFF, sizeof(encoded));
    if (inFlags != kBase64Flags_None) {
        if (inLineEndings)
            Base64EncodeLinesEx(inSrc, inSrcSize, inIndent, inLineEndings, inFlags, encoded, sizeof(encoded), &size);
        else
            Base64EncodeEx(inSrc, inSrcSize, inFlags, encoded, sizeof(encoded), &size);
    } else {
        if (inLineEndings)
            Base64EncodeLines(inSrc, inSrcSize, inIndent, inLineEndings, encoded, sizeof(encoded), &size);
        else
            Base64Encode(inSrc, inSrcSize, encoded, sizeof(encoded), &size);
    }
    encodedSize = size;
    if (inPrint)
        dlog(kLogLevelMax, "%.*s\n\n", (int)size, encoded);
    //if( inPrint ) dlogmem( kLogLevelMax, encoded, size );
    if (inPrint)
        dlog(kLogLevelMax, "\n");
    ...
    require_noerr(err, exit);
    // if( inPrint ) dlogmem( kLogLevelMax, decoded, n );
    if (inPrint)
        dlog(kLogLevelMax, "\n");
    require_action(n == inSrcSize, exit, err = kResponseErr);
