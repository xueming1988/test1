...
OSStatus CLIParse(CLIOption inOptions[], CLIFlags inFlags)
{
    ...
    for (; !gEndedOptions && (gArgI < gArgC); ++gArgI) {
        ...
        {
            if (!(inFlags & kCLIFlags_ReorderArgs))
                break;
...
static OSStatus _CLIParseLongOption(CLIOption inOptions[], const char* inArg)
{
    ...
    for (;;) {
        for (option = inOptions; option->type != kCLIOptionType_End; ++option) {
            if (!CLIOption_IsOption(option))
                continue;
            if (!option->longName)
                continue;
            if (strnicmp(option->longName, namePtr, nameLen) != 0)
                continue;
            if (option->longName[nameLen] != '\0')
                continue;

            if (*nameEnd != '\0')
                gOptionPtr = nameEnd + 1;
...
static OSStatus _CLIGetValue(CLIOption* inOption, CLIValueFlags inFlags)
{
    ...
    case kCLIOptionType_CallBack:
        ...
        if (unset)
            ...
            err = _CLIGetArg(inOption, inFlags, &arg);
            if (!err)
                err = inOption->optionCallBack(inOption, arg, 0);
        }
        break;
    ...
    if (!err)
        inOption->flags |= kCLIOptionFlags_Specified;
...
static void _CLIOptionError(const CLIOption* inOption, const char* inReason, CLIValueFlags inFlags)
{
    ...
    if (inFlags & kCLIValueFlags_Short)
        fprintf(stderr, "'%c' ", inOption->shortName);
    else if (inFlags & kCLIValueFlags_Unset)
        fprintf(stderr, "'no-%s' ", inOption->longName);
    else
...
static CLIOption* _CLIFindOption(CLIOption* inOptions, CLIOptionType inType, CLIFindFlags inFlags, const char* inName)
{
    ...
    if (inOptions) {
        for (option = inOptions; option->type != kCLIOptionType_End; ++option) {
            if (((inType == kCLIOptionType_Any) || (inType == option->type) || ((inType == kCLIOptionType_AnyOption) && CLIOption_IsOption(option))) && (!(inFlags & kCLIFindFlag_NonGlobal) || !(option->flags & kCLIOptionFlags_GlobalOnly)) && ((inName == NULL) || (stricmp(option->longName, inName) == 0))) {
                return (option);
            }
        }
    }
...
static OSStatus _CLICheckMissingOptions(CLIOption* inOption)
{
    ...
    for (option = inOption->subOptions; option && (option->type != kCLIOptionType_End); ++option) {
        if (!(option->flags & kCLIOptionFlags_Required))
            continue;
        if (option->flags & kCLIOptionFlags_Specified)
            continue;
...
static OSStatus _CLIPrepareForMetaCommand(CLIOption* inOptions)
{
    ...
    for (option = inOptions; option->type != kCLIOptionType_End; ++option) {
        if (option->type != kCLIOptionType_Command)
            continue;
        if (!(option->flags & kCLIOptionFlags_MetaCommand))
            continue;
        if (stricmp(option->longName, gProgramName) != 0)
            continue;
...
void CLIHelpCommand(const char* inCmd)
{
    ...
    if (inCmd) {
        ...
        if (option)
            _CLIHelp_PrintCommand(option, true);
        else
...
OSStatus _CLIHelpOption(CLIOption* inOption, const char* inArg, int inUnset)
{
    ...
    if (inArg) {
        ...
        if (option)
            _CLIHelp_PrintCommand(option, false);
        else
            fprintf(stderr, "error: unknown command '%s'.\n", inArg);
...
static void _CLIHelp_PrintSummary(Boolean inFull)
{
    ...
    for (command = gRootOptions; command->type != kCLIOptionType_End; ++command) {
        if (command->type != kCLIOptionType_Command)
            continue;
        if (!inFull && (command->flags & kCLIOptionFlags_NotCommon))
            continue;
    ...
    if (nShownOptions || nShownCommands)
        fputc('\n', stderr);
    if (hidden)
        fprintf(stderr, "See '%s help' for a full list of commands.\n", gProgramName);
    if (nShownCommands)
        fprintf(stderr, "See '%s help <command>' for more info about a command.\n\n", gProgramName);
    else if (hidden)
        fputc('\n', stderr);
}
...
static void _CLIHelp_PrintUsageLine(CLIOption* inOption, const char* inPrefix, int inIndent)
{
    ...
    if (root) {
        ...
        if (inOption->parentOption) {
            ...
            if (inOption->parentOption->subOptions && _CLIFindOption(inOption->parentOption->subOptions, kCLIOptionType_AnyOption, kCLIFindFlags_None, NULL)) {
        ...
        if (inOption->subOptions) {
            if (_CLIFindOption(inOption->subOptions, kCLIOptionType_Command, kCLIFindFlags_None, NULL)) {
                ...
                for (option = inOption->subOptions; option->type != kCLIOptionType_End; ++option) {
                    ...
                    if (option->type != kCLIOptionType_Argument)
                        continue;
...
static int _CLIHelp_PrintOptions(const CLIOption* inOptions, int inIndent, const char* inLabel, CLIPrintOptionsFlags inFlags)
{
    ...
    widest = 0;
    for (option = inOptions; option->type != kCLIOptionType_End; ++option) {
        if (!CLIOption_IsOption(option) && (option->type != kCLIOptionType_Argument))
            continue;
        if ((inFlags & kCLIPrintOptionsFlag_NonGlobal) && (option->flags & kCLIOptionFlags_GlobalOnly))
            continue;
...
static int _CLIHelp_PrintOptionName(const CLIOption* inOption, FILE* inFile)
{
    ...
    if (inOption->shortName != '\0') {
        ...
        if (inOption->longName)
            width += FPrintF(inFile, ", ");
    }
    if (inOption->type == kCLIOptionType_Argument) {
        ...
        if (inOption->longName)
            width += FPrintF(inFile, "--%s", inOption->longName);
...
static char** _CLIInteractiveCompleter(const char* inText, int inStart, int inEnd)
{
    ...
    if (inStart == 0)
        return (rl_completion_matches(inText, _CLIInteractiveGenerator));
    return (NULL);
}
...
static char* _CLIInteractiveGenerator(const char* inText, int inState)
{
    ...
    for (command = (inState == 0) ? gRootOptions : gCLIInteractiveNextCommand;
        ...
        if (command->type != kCLIOptionType_Command)
            continue;
        if (strnicmp(command->longName, inText, len) != 0)
            continue;
...
error:
    errorStr = NULL;
    if (arg)
        AppendPrintF(&errorStr, "error: bad %s: '%s'. It must be ", inLabel, arg);
    else
        AppendPrintF(&errorStr, "error: no %s specified. It must be ", inLabel);
    ...
    for (;;) {
        ...
        if (!name)
            break;
        if ((stricmp(name, kCLIArg_AnyTrueish) == 0))
            name = kAnyTrueMessage;
        else if ((stricmp(name, kCLIArg_AnyFalseish) == 0))
            name = kAnyFalseMessage;
        else if ((stricmp(name, kCLIArg_AnyInt) == 0))
            name = "an integer";
        else
            (void)va_arg(args, int);
