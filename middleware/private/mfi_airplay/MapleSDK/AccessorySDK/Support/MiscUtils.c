    ...
    for (;;) {
        ...
        *slash = '\0';
        ...
        if ((err < 0) && (errno != EEXIST)) {
			 ...
			 * them. So chmod().
			 */
            if ((mode & ~(S_IRWXU | S_IRWXG | S_IRWXO)) != 0 && chmod(buf, mode) == -1) {
                err = errno;
                return err ? err : -1;
            }
        }
...
char* NormalizePath(const char* inSrc, size_t inLen, char* inDst, size_t inMaxLen, uint32_t inFlags)
{
    ...
    if (!(inFlags & kNormalizePathDontExpandTilde) && (src < end) && (*src == '~')) {
        ...
        if (replacePtr)
            while ((dst < lim) && (replacePtr < replaceEnd))
                *dst++ = *replacePtr++;
        else
            src = inSrc;
    }
    while ((dst < lim) && (src < end))
        *dst++ = *src++;
    *dst = '\0';
...
OSStatus RollLogFiles(FILE** ioLogFile, const char* inEndMessage, const char* inBaseName, int inMaxFiles)
{
    ...
    if (ioLogFile && *ioLogFile) {
        if (inEndMessage)
            fprintf(*ioLogFile, "%s", inEndMessage);
        fclose(*ioLogFile);
        *ioLogFile = NULL;
    }
...
OSStatus RunProcessAndCaptureOutputEx(const char* inCmdLine, char** outResponse, size_t* outLen)
{
    ...
    *outResponse = (char*)DataBuffer_GetPtr(&dataBuf);
    if (outLen)
        *outLen = DataBuffer_GetLen(&dataBuf);
...
#if (TARGET_OS_POSIX && !TARGET_OS_NETBSD)
//===========================================================================================================================
//	systemf
//===========================================================================================================================
...
int systemf(const char* inLogPrefix, const char* inFormat, ...)
{
    ...
    if (WIFEXITED(status))
        ...
        err = POSIXSignalToOSStatus(WTERMSIG(status));
    else if (WIFSTOPPED(status))
        err = POSIXSignalToOSStatus(WSTOPSIG(status));
    else if (WIFCONTINUED(status))
        err = POSIXSignalToOSStatus(SIGCONT);
    else
        err = kUnknownErr;
...
exit:
    if (base)
        mach_vm_deallocate(task, base, len * 2);
    return (err);
}
#endif
