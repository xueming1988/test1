...
#define CFLNumberTypeIsFloatType(TYPE) \
    (((TYPE) == kCFLNumberFloat32Type) || ((TYPE) == kCFLNumberFloat64Type) || ((TYPE) == kCFLNumberFloatType) || ((TYPE) == kCFLNumberDoubleType))
#endif
...
//---------------------------------------------------------------------------------------------------------------------------
/*!	@typedef	CFLUUIDRef
     @abstract	The type of a reference to a CFLUUID object.
     */
typedef struct CFLUUID* CFLUUIDRef;

//---------------------------------------------------------------------------------------------------------------------------
/*!	@function	CFLUUIDGetTypeID
     @abstract	Obtains the Core Foundation type identifier for CFLUUID objects.
     @result		The Core Foundation type identifier for CFLUUID objects.
     */
CFLTypeID CFLUUIDGetTypeID(void);

//---------------------------------------------------------------------------------------------------------------------------
/*!	@function	CFLUUIDCreate
     @abstract	Creates a CFLUUID object using the specified data components.
     */
CFLUUIDRef CFLUUIDCreate(const void* inUUID, OSStatus* outErr);

//---------------------------------------------------------------------------------------------------------------------------
/*!	@function	CFLUUIDGetUUIDPtr
     @abstract	Returns a pointer to the internal 16-byte UUID data.
     @discussion	Note that this pointer is only valid for as long as the CFLUUID object is valid.
     */
