/*
	Copyright (C) 2004-2019 Apple Inc. All Rights Reserved. Not to be used or disclosed without permission from Apple.
*/
...
CFDictionaryRef CFDictionaryCreateCopy(CFAllocatorRef inAllocator, CFDictionaryRef inDict)
{
    return (CFDictionaryCreateMutableCopy(inAllocator, 0, inDict));
}
...
#if (CFCOMPAT_HAS_UNICODE_SUPPORT)
    utf8Ptr = NULL;
    if ((inEncoding == kCFStringEncodingWindowsLatin1) || (inEncoding == kCFStringEncodingISOLatin1)) {
        err = latin1_to_utf8_copy(inBytes, (size_t)inSize, &utf8Ptr, &utf8Len, '/', 0);
        require_noerr(err, exit);

        inBytes = utf8Ptr;
        inSize = (CFIndex)utf8Len;
    } else if ((inEncoding == kCFStringEncodingUTF16) || (inEncoding == kCFStringEncodingUTF16BE) || (inEncoding == kCFStringEncodingUTF16LE)) {
        int flags;

        if (inEncoding == kCFStringEncodingUTF16BE)
            flags = UTF_BIG_ENDIAN;
        else if (inEncoding == kCFStringEncodingUTF16LE)
            flags = UTF_LITTLE_ENDIAN;
        else
            flags = 0;
...
#else
    require_action((inEncoding == kCFStringEncodingUTF8) || (inEncoding == kCFStringEncodingASCII), exit, err = kUnsupportedErr);
#endif
    ...
    CFIndex* outUsedBufLen)
{
    ...
    if (inRange.length > inMaxBufLen)
        inRange.length = inMaxBufLen;
    if (inBuffer)
        memmove(inBuffer, str + inRange.location, inRange.length);
    nConverted = inRange.length;

exit:
    if (outUsedBufLen)
        *outUsedBufLen = nConverted;
    return (nConverted);
}
...
CFComparisonResult CFStringCompare(CFStringRef inS1, CFStringRef inS2, CFOptionFlags inOptions)
{
    ...
    if (inOptions & kCFCompareNumerically)
        n = TextCompareNatural(s1, len1, s2, len2, ignoreCase);
    else if (ignoreCase)
        n = stricmp(s1, s2);
    else
        n = strcmp(s1, s2);
    if (n < 0)
        ...
        cmp = kCFCompareGreaterThan;
    else
        cmp = kCFCompareEqualTo;
    ...
    CFRange* outRange)
{
    ...
    if (inSearchOptions & kCFCompareCaseInsensitive)
        foundPtr = strncasestr(sourcePtr, searchStr, sourceLen);
    else
        foundPtr = strnstr(sourcePtr, searchStr, sourceLen);
    if (foundPtr) {
        if (outRange) {
...
{
    return (CFLUUIDGetTypeID());
}

//===========================================================================================================================

CFUUIDRef CFUUIDCreate(CFAllocatorRef inAllocator)
{
    (void)inAllocator;

    return (CFLUUIDCreate(NULL, NULL));
}

//===========================================================================================================================

CFUUIDRef CFUUIDCreateFromUUIDBytes(CFAllocatorRef inAllocator, CFUUIDBytes inBytes)
{
    (void)inAllocator;

    return (CFLUUIDCreate(&inBytes, NULL));
}

//===========================================================================================================================

CFUUIDBytes CFUUIDGetUUIDBytes(CFUUIDRef inUUID)
{
    return (*((const CFUUIDBytes*)CFLUUIDGetUUIDPtr(inUUID)));
}
    ...
    CFErrorRef* outError)
{
    ...
    if (plist) {
        if (outFormat)
            *outFormat = kCFPropertyListBinaryFormat_v1_0;
        goto exit;
    }
#endif
exit:
