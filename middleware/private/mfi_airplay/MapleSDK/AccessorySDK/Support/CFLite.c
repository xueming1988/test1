...
typedef struct CFLUUID CFLUUID;
struct CFLUUID {
    CFLObject base;
    uint8_t uuid[16];
};
...
#define CFLValidType(OBJ) ((CFLObjectGetType(OBJ) >= kCFLTypeMin) && (CFLObjectGetType(OBJ) < gCFLRuntimeClassTableCount))
...
#define CFLDataRoundUpSize(X) \
    (((X) < 256U) ? 256U : ((X) < 4096U) ? 4096U : AlignUp((X), 32768U))
...
static Boolean _CFLUUIDEqual(CFLObjectRef inLeft, CFLObjectRef inRight);
static CFLHashCode _CFLUUIDHash(CFLObjectRef inObject);
static void _CFLUUIDFree(CFLObjectRef inObject);
static OSStatus _CFLUUIDCopy(CFLUUIDRef inSrc, CFLUUIDRef* outDst);
...
static const CFLRuntimeClass kCFLRuntimeClassTable[] = {
    ...
    { "CFLUUID", _CFLUUIDFree, _CFLUUIDEqual, _CFLUUIDHash },
};
...
OSStatus CFLGetTypeID(CFLObjectRef inObject, CFLTypeID* outTypeID)
{
    ...
    if (outTypeID)
        *outTypeID = CFLObjectGetType(inObject);
    err = kNoErr;
...
OSStatus CFLGetRetainCount(CFLObjectRef inObject, CFLIndex* outCount)
{
    ...
    if (outCount)
        *outCount = CFLIsConstantObject(inObject) ? kCFLConstantRefCount : obj->retainCount;
    err = kNoErr;
...
void CFLRelease(CFLObjectRef inObject)
{
    ...
    if (!CFLIsConstantObject(obj)) {
        if (atomic_add_and_fetch_32(&obj->retainCount, -1) == 0) {
            ...
            if (gCFLRuntimeClassTable[obj->type].freeObj)
                gCFLRuntimeClassTable[obj->type].freeObj(obj);
...
OSStatus CFLCopy(CFLObjectRef inSrc, CFLObjectRef* outDst)
{
    ...
    } else if (typeID == CFLStringGetTypeID()) // String
    {
        ...
        require_noerr(err, exit);
    } else if (typeID == CFLUUIDGetTypeID()) // UUID
    {
        err = _CFLUUIDCopy((CFLUUIDRef)inSrc, (CFLUUIDRef*)outDst);
        require_noerr(err, exit);
    } else {
        err = kTypeErr;
...
OSStatus CFLArrayGetValueAtIndex(CFLArrayRef inObject, CFLIndex inIndex, void* outValue)
{
    ...
    if (outValue)
        *((void**)outValue) = object->storage[inIndex];
    err = kNoErr;
...
OSStatus CFLArraySetValueAtIndex(CFLArrayRef inObject, CFLIndex inIndex, const void* inValue)
{
    ...
    if (object->callbacks.retain)
        object->callbacks.retain(kCFLAllocatorDefault, inValue);
    if (object->callbacks.release)
        object->callbacks.release(kCFLAllocatorDefault, object->storage[inIndex]);
...
OSStatus CFLArrayInsertValueAtIndex(CFLArrayRef inObject, CFLIndex inIndex, const void* inValue)
{
    ...
    require_action(CFLValidObjectType(inObject, kCFLTypeArray), exit, err = kBadReferenceErr);
    object = (CFLArray*)inObject;
    if (inIndex == kCFLIndexEnd)
        inIndex = object->count;
    require_action((inIndex >= 0) && (inIndex <= object->count), exit, err = kRangeErr);
...
OSStatus CFLArrayRemoveValueAtIndex(CFLArrayRef inObject, CFLIndex inIndex)
{
    ...
    if (object->callbacks.release)
        object->callbacks.release(kCFLAllocatorDefault, oldValue);
    err = kNoErr;
...
OSStatus CFLArrayRemoveAllValues(CFLArrayRef inObject)
{
    ...
    for (i = 0; i < n; ++i) {
        if (object->callbacks.release)
            object->callbacks.release(kCFLAllocatorDefault, object->storage[i]);
    }
    if (object->storage) {
        free(object->storage);
        object->storage = NULL;
...
OSStatus CFLDataGetDataPtr(CFLDataRef inObject, void* outDataPtr, size_t* outSize)
{
    ...
    if (outDataPtr)
        *((void**)outDataPtr) = object->data;
    if (outSize)
        *outSize = object->usedSize;
    err = kNoErr;
...
OSStatus CFLDataAppendData(CFLDataRef inObject, const void* inData, size_t inSize)
{
    ...
    if (inData)
        memcpy(object->data + object->usedSize, inData, inSize);
    object->usedSize += inSize;
    err = kNoErr;
...
static Boolean _CFLDateEqual(CFLObjectRef inLeft, CFLObjectRef inRight)
{
    ...
    if ((l->date.year == r->date.year) && (l->date.month == r->date.month) && (l->date.day == r->date.day) && (l->date.hour == r->date.hour) && (l->date.minute == r->date.minute) && (l->date.second == r->date.second)) {
        return (true);
    }
    return (false);
}
    ...
    CFLDictionaryRef* outRef)
{
    ...
    require_action(outRef, exit, err = kParamErr);
    if (inCapacity <= 0)
        inCapacity = 101; // Default to a reasonable prime number so modular hashes work well.
    else
        inCapacity = _CFLNextPrime(inCapacity);
...
exit:
    if (object)
        CFLRelease((CFLDictionaryRef)object);
    return (err);
}
...
OSStatus CFLDictionaryGetValue(CFLDictionaryRef inObject, const void* inKey, void* outValue)
{
    ...
    if (err == kNoErr) {
        if (outValue)
            *((const void**)outValue) = (*node)->value;
    }
...
OSStatus CFLDictionarySetValue(CFLDictionaryRef inObject, const void* inKey, const void* inValue)
{
    ...
    if (!found) {
        ...
        *node = newNode;
        ...
        if (object->keyCallBacks.retain)
            object->keyCallBacks.retain(kCFLAllocatorDefault, inKey);
    }

    // Retain the new value then release any existing value.

    if (object->valueCallBacks.retain)
        object->valueCallBacks.retain(kCFLAllocatorDefault, inValue);
    if (found) {
        if (object->valueCallBacks.release)
            object->valueCallBacks.release(kCFLAllocatorDefault, (*node)->value);
    }
    (*node)->value = inValue;
    err = kNoErr;
...
OSStatus CFLDictionaryRemoveValue(CFLDictionaryRef inObject, const void* inKey)
{
    ...
    if (err == kNoErr) {
        ...
        if (object->keyCallBacks.release)
            object->keyCallBacks.release(kCFLAllocatorDefault, (*node)->key);
        if (object->valueCallBacks.release)
            object->valueCallBacks.release(kCFLAllocatorDefault, (*node)->value);
    ...
    if (object->count > 0) {
        ...
        for (i = 0; i < n; ++i) {
            ...
            for (node = object->buckets[i]; node; node = node->next) {
                if (keys)
                    keys[iTotal] = (void*)node->key;
                if (values)
                    values[iTotal] = (void*)node->value;
                ++iTotal;
            }
        }
    }
...
OSStatus CFLDictionaryGetKeysAndValues(CFLDictionaryRef inObject, const void** ioKeys, const void** ioValues)
{
    ...
    if (object->count > 0) {
        ...
        for (i = 0; i < n; ++i) {
            ...
            for (node = object->buckets[i]; node; node = node->next) {
                if (ioKeys)
                    ioKeys[iTotal] = (void*)node->key;
                if (ioValues)
                    ioValues[iTotal] = (void*)node->value;
                ++iTotal;
            }
        }
    }
...
static Boolean _CFLDictionaryEqual(CFLObjectRef inLeft, CFLObjectRef inRight)
{
    ...
    if ((l->keyCallBacks.equal != r->keyCallBacks.equal) || (l->valueCallBacks.equal != r->valueCallBacks.equal)) {
        err = kMismatchErr;
        goto exit;
    }
...
OSStatus CFLNumberCreate(CFLAllocatorRef inAllocator, CFLNumberType inType, const void* inValue, CFLNumberRef* outRef)
{
    ...
    if (inType == kCFLNumberSInt8Type)
        object->value.s64 = *((int8_t*)inValue);
    else if (inType == kCFLNumberSInt16Type)
        object->value.s64 = *((int16_t*)inValue);
    else if (inType == kCFLNumberSInt32Type)
        object->value.s64 = *((int32_t*)inValue);
    else if (inType == kCFLNumberSInt64Type)
        object->value.s64 = *((int64_t*)inValue);
    else if (inType == kCFLNumberSInt128Type)
        object->value.s128 = *((int128_compat*)inValue);
    else if (inType == kCFLNumberCharType)
        object->value.s64 = *((char*)inValue);
    else if (inType == kCFLNumberShortType)
        object->value.s64 = *((short*)inValue);
    else if (inType == kCFLNumberIntType)
        object->value.s64 = *((int*)inValue);
    else if (inType == kCFLNumberLongType)
        object->value.s64 = *((long*)inValue);
    else if (inType == kCFLNumberLongLongType)
        object->value.s64 = *((long long*)inValue);
    else if (inType == kCFLNumberCFIndexType)
        object->value.s64 = *((CFLIndex*)inValue);
#if (CFL_FLOATING_POINT_NUMBERS)
    else if (inType == kCFLNumberFloat32Type)
        object->value.f64 = *((Float32*)inValue);
    else if (inType == kCFLNumberFloat64Type)
        object->value.f64 = *((Float64*)inValue);
    else if (inType == kCFLNumberFloatType)
        object->value.f64 = *((float*)inValue);
    else if (inType == kCFLNumberDoubleType)
        object->value.f64 = *((double*)inValue);
#endif
    else {
        dlogassert("bad number type %d", inType);
        err = kParamErr;
        goto exit;
    }
...
exit:
    if (object)
        CFLRelease((CFLNumberRef)object);
    return (err);
}
...
CFLIndex CFLNumberGetByteSize(CFLNumberRef inObject)
{
    ...
    case kCFLNumberSInt8Type:
        return ((CFLIndex)sizeof(int8_t));
    case kCFLNumberSInt16Type:
        return ((CFLIndex)sizeof(int16_t));
    case kCFLNumberSInt32Type:
        return ((CFLIndex)sizeof(int32_t));
    case kCFLNumberSInt64Type:
        return ((CFLIndex)sizeof(int64_t));
    case kCFLNumberSInt128Type:
        return ((CFLIndex)sizeof(int128_compat));
    case kCFLNumberCharType:
        return ((CFLIndex)sizeof(char));
    case kCFLNumberShortType:
        return ((CFLIndex)sizeof(short));
    case kCFLNumberIntType:
        return ((CFLIndex)sizeof(int));
    case kCFLNumberLongType:
        return ((CFLIndex)sizeof(long));
    case kCFLNumberLongLongType:
        return ((CFLIndex)sizeof(long long));
    case kCFLNumberCFIndexType:
        return ((CFLIndex)sizeof(CFLIndex));
#if (CFL_FLOATING_POINT_NUMBERS)
    case kCFLNumberFloat32Type:
        return ((CFLIndex)sizeof(Float32));
    case kCFLNumberFloat64Type:
        return ((CFLIndex)sizeof(Float64));
    case kCFLNumberFloatType:
        return ((CFLIndex)sizeof(float));
    case kCFLNumberDoubleType:
        return ((CFLIndex)sizeof(double));
#endif
...
OSStatus CFLNumberGetValue(CFLNumberRef inObject, CFLNumberType inType, void* outValue)
{
    ...
    case kCFLNumberCFIndexType:
        if (inType == kCFLNumberSInt8Type)
            *((int8_t*)outValue) = (int8_t)object->value.s64;
        else if (inType == kCFLNumberSInt16Type)
            *((int16_t*)outValue) = (int16_t)object->value.s64;
        else if (inType == kCFLNumberSInt32Type)
            *((int32_t*)outValue) = (int32_t)object->value.s64;
        else if (inType == kCFLNumberSInt64Type)
            *((int64_t*)outValue) = (int64_t)object->value.s64;
        else if (inType == kCFLNumberSInt128Type) {
            ...
            *((char*)outValue) = (char)object->value.s64;
        else if (inType == kCFLNumberShortType)
            *((short*)outValue) = (short)object->value.s64;
        else if (inType == kCFLNumberIntType)
            *((int*)outValue) = (int)object->value.s64;
        else if (inType == kCFLNumberLongType)
            *((long*)outValue) = (long)object->value.s64;
        else if (inType == kCFLNumberLongLongType)
            *((long long*)outValue) = object->value.s64;
        else if (inType == kCFLNumberCFIndexType)
            *((CFLIndex*)outValue) = (CFLIndex)object->value.s64;
#if (CFL_FLOATING_POINT_NUMBERS)
        else if (inType == kCFLNumberFloat32Type)
            *((Float32*)outValue) = (Float32)object->value.s64;
        else if (inType == kCFLNumberFloat64Type)
            *((Float64*)outValue) = (Float64)object->value.s64;
        else if (inType == kCFLNumberFloatType)
            *((float*)outValue) = (float)object->value.s64;
        else if (inType == kCFLNumberDoubleType)
            *((double*)outValue) = (double)object->value.s64;
#endif
        else {
            dlogassert("bad number type %d", inType);
            err = kParamErr;
    ...
    case kCFLNumberSInt128Type:
        if (inType == kCFLNumberSInt8Type)
            *((int8_t*)outValue) = (int8_t)object->value.s128.lo;
        else if (inType == kCFLNumberSInt16Type)
            *((int16_t*)outValue) = (int16_t)object->value.s128.lo;
        else if (inType == kCFLNumberSInt32Type)
            *((int32_t*)outValue) = (int32_t)object->value.s128.lo;
        else if (inType == kCFLNumberSInt64Type)
            *((int64_t*)outValue) = (int64_t)object->value.s128.lo;
        else if (inType == kCFLNumberSInt128Type)
            *((int128_compat*)outValue) = object->value.s128;
        else if (inType == kCFLNumberCharType)
            *((char*)outValue) = (char)object->value.s128.lo;
        else if (inType == kCFLNumberShortType)
            *((short*)outValue) = (short)object->value.s128.lo;
        else if (inType == kCFLNumberIntType)
            *((int*)outValue) = (int)object->value.s128.lo;
        else if (inType == kCFLNumberLongType)
            *((long*)outValue) = (long)object->value.s128.lo;
        else if (inType == kCFLNumberLongLongType)
            *((long long*)outValue) = (long long)object->value.s128.lo;
        else if (inType == kCFLNumberCFIndexType)
            *((CFLIndex*)outValue) = (CFLIndex)object->value.s128.lo;
#if (CFL_FLOATING_POINT_NUMBERS)
            ...
            if (inType == kCFLNumberFloat32Type)
                *((Float32*)outValue) = (Float32)f64;
            else if (inType == kCFLNumberFloat64Type)
                *((Float64*)outValue) = (Float64)f64;
            else if (inType == kCFLNumberFloatType)
                *((float*)outValue) = (float)f64;
            else if (inType == kCFLNumberDoubleType)
                *((double*)outValue) = (double)f64;
            else {
                dlogassert("bad number type %d", inType);
                err = kParamErr;
                goto exit;
    ...
    case kCFLNumberDoubleType:
        if (inType == kCFLNumberSInt8Type)
            *((int8_t*)outValue) = (int8_t)object->value.f64;
        else if (inType == kCFLNumberSInt16Type)
            *((int16_t*)outValue) = (int16_t)object->value.f64;
        else if (inType == kCFLNumberSInt32Type)
            *((int32_t*)outValue) = (int32_t)object->value.f64;
        else if (inType == kCFLNumberSInt64Type)
            *((int64_t*)outValue) = (int64_t)object->value.f64;
        else if (inType == kCFLNumberSInt128Type) {
            int128_compat* const p128 = (int128_compat*)outValue;
            Float64 f64;
            ...
            *((char*)outValue) = (char)object->value.f64;
        else if (inType == kCFLNumberShortType)
            *((short*)outValue) = (short)object->value.f64;
        else if (inType == kCFLNumberIntType)
            *((int*)outValue) = (int)object->value.f64;
        else if (inType == kCFLNumberLongType)
            *((long*)outValue) = (long)object->value.f64;
        else if (inType == kCFLNumberLongLongType)
            *((long long*)outValue) = (long long)object->value.f64;
        else if (inType == kCFLNumberCFIndexType)
            *((CFLIndex*)outValue) = (CFLIndex)object->value.f64;
        else if (inType == kCFLNumberFloat32Type)
            *((Float32*)outValue) = (Float32)object->value.f64;
        else if (inType == kCFLNumberFloat64Type)
            *((Float64*)outValue) = object->value.f64;
        else if (inType == kCFLNumberFloatType)
            *((float*)outValue) = (float)object->value.f64;
        else if (inType == kCFLNumberDoubleType)
            *((double*)outValue) = (double)object->value.f64;
        else {
            dlogassert("bad number type %d", inType);
            err = kParamErr;
            goto exit;
...
exit:
    if (object)
        CFLRelease((CFLStringRef)object);
    return (err);
}
...
OSStatus CFLStringGetLength(CFLStringRef inObject, CFLIndex* outLength)
{
    ...
    if (CFLIsConstantObject(inObject)) {
        if (outLength)
            *outLength = (CFLIndex)strlen(CFLGetConstantStringPtr(object));
    } else {
        if (outLength)
            *outLength = (CFLIndex)object->size;
    }
    err = kNoErr;
...
OSStatus CFLStringSetText(CFLStringRef inObject, const void* inText, size_t inSize)
{
    ...
    if (inSize == kSizeCString)
        ...
        inSize = _CFLstrnlen((const char*)inText, inSize);
...
OSStatus CFLStringAppendText(CFLStringRef inObject, const void* inText, size_t inSize)
{
    ...
    obj->data = tmp;
    if (inText && (inSize > 0))
        memcpy(obj->data + obj->size, inText, inSize);
    obj->data[newSize] = '\0';
    obj->size = newSize;
    err = kNoErr;
...
CFLTypeID CFLUUIDGetTypeID(void)
{
    return (kCFLTypeUUID);
}
...
CFLUUIDRef CFLUUIDCreate(const void* inUUID, OSStatus* outErr)
{
    CFLUUID* object = NULL;
    OSStatus err;

    object = (CFLUUID*)calloc(1U, sizeof(CFLUUID));
    require_action(object, exit, err = kNoMemoryErr);

    object->base.signature = kCFLSignatureValid;
    object->base.type = kCFLTypeUUID;
    object->base.retainCount = 1;

    if (inUUID)
        memcpy(object->uuid, inUUID, 16);
    else
        UUIDGet(object->uuid);
    err = kNoErr;
...
const uint8_t* CFLUUIDGetUUIDPtr(CFLUUIDRef inObject)
{
    require(CFLValidObjectType(inObject, kCFLTypeUUID), exit);
    return (((const CFLUUID*)inObject)->uuid);
...
static Boolean _CFLUUIDEqual(CFLObjectRef inLeft, CFLObjectRef inRight)
{
    CFLUUID* const l = (CFLUUID*)inLeft;
    CFLUUID* const r = (CFLUUID*)inRight;

    // The upper-level CFLEqual routine will have already validated the objects and performed pointer equality testing
    // so it is unnecessary here because this should never get called unless those tests failed. Just assert instead.

    check(CFLValidObjectType(inLeft, kCFLTypeUUID));
    check(CFLValidObjectType(inRight, kCFLTypeUUID));
    check_string(inLeft != inRight, "object ptrs match...should have passed earlier equality testing");
    return (memcmp(l->uuid, r->uuid, 16) == 0);
}
...
static CFLHashCode _CFLUUIDHash(CFLObjectRef inObject)
{
    // The upper-level CFLHashObject routine will have already validated parameters so just assert here.

    check(CFLValidObjectType(inObject, kCFLTypeUUID));
    return (_CFLHashFNV1a(((CFLUUID*)inObject)->uuid, 16));
}
...
static OSStatus _CFLUUIDCopy(CFLUUIDRef inSrc, CFLUUIDRef* outDst)
{
    ...
    obj = CFLUUIDCreate(inSrc->uuid, &err);
    require_noerr(err, exit);
    *outDst = obj;
...
static char* CFLRandomString(size_t inMinChars, size_t inMaxChars, char* outString)
{
    ...
    while (ptr < end)
        *ptr++ = kCFLRandomCharSet[Random32() % sizeof_string(kCFLRandomCharSet)];
    *ptr = '\0';
    return (outString);
}
...
OSStatus CFLiteTest(int inPrint)
{
    ...
    for (n = 3; n < countof(values); ++n)
        require_action(values[n] == NULL, exit, err = kResponseErr);
    ...
    require_noerr(err, exit);
    require_action((dateComponents.year == 2002) && (dateComponents.month == 10) && (dateComponents.day == 9) && (dateComponents.hour == 3) && (dateComponents.minute == 44) && (dateComponents.second == 27),
    ...
    for (i = 0; i < 333; ++i) {
        snprintf(tempString, sizeof(tempString), "This is a test of the dictionary hash function %d", (int)i);
    ...
    if (inPrint)
        printf("\tdictionary tests: %d entries, table size 31 -> minDepth %d, maxDepth %d\n", (int)i, minDepth, maxDepth);
