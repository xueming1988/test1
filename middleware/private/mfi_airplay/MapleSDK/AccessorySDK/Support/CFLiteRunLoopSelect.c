    ...
    for (;;) {
        ...
        for (rls = inRL->sourceList; rls; rls = rls->next) {
            if (rls->type == kCFRunLoopSourceType_Socket) {
                ...
                if ((sock->callbackTypes & kCFSocketReadCallBack) || ((sock->callbackTypes & kCFSocketConnectCallBack) && sock->connecting)) {
                    FD_SET(sock->nativeSock, &readSet);
                    if (sock->nativeSock > maxFD)
                        maxFD = sock->nativeSock;
                }
                if ((sock->callbackTypes & kCFSocketWriteCallBack) || ((sock->callbackTypes & kCFSocketConnectCallBack) && sock->connecting)) {
...
void CFRunLoopAddSource(CFRunLoopRef inRL, CFRunLoopSourceRef inSource, CFStringRef inMode)
{
    ...
    if (inSource->context.schedule)
        inSource->context.schedule(inSource->context.info, inRL, inMode);
    CFRunLoopUnlock();
}
...
void CFRunLoopRemoveSource(CFRunLoopRef inRL, CFRunLoopSourceRef inSource, CFStringRef inMode)
{
    ...
    if (inRL) {
        ...
        for (next = &inRL->sourceList; (curr = *next) != NULL; next = &curr->next) {
            if (curr == inSource) {
                *next = curr->next;
                if (curr->context.cancel)
                    curr->context.cancel(curr->context.info, inRL, inMode);
                break;
            }
        }
    }
    ...
    const CFSocketContext* inContext)
{
    ...
    obj->flags = kCFSocketAutomaticallyReenableReadCallBack | kCFSocketAutomaticallyReenableAcceptCallBack | kCFSocketAutomaticallyReenableDataCallBack | kCFSocketCloseOnInvalidate;
...
DEBUG_STATIC void __CFSocketHandler(CFSocketRef inSock, CFOptionFlags inCallBackType)
{
    ...
    if (inSock->connecting) {
        ...
        if (connectErr == 0)
            connectErr = socketErr;
        if (connectErr)
            callbackData = &connectErr;
    ...
    CFRunLoopUnlock();
    if (call)
        inSock->callback(inSock, inCallBackType, NULL, callbackData, inSock->context.info);
}
    ...
    const CFFileDescriptorContext* inContext)
{
    ...
    if (inCloseOnInvalidate)
        flags |= kCFSocketCloseOnInvalidate;
    else
        flags &= ~((CFOptionFlags)kCFSocketCloseOnInvalidate);
    ...
    void* inContext)
{
    ...
    callbackTypes = 0;
    if (inType & kCFSocketReadCallBack)
        callbackTypes |= kCFFileDescriptorReadCallBack;
    if (inType & kCFSocketWriteCallBack)
        callbackTypes |= kCFFileDescriptorWriteCallBack;
...
void CFFileDescriptorEnableCallBacks(CFFileDescriptorRef inDesc, CFOptionFlags inCallBackTypes)
{
    ...
    callbackTypes = 0;
    if (inCallBackTypes & kCFFileDescriptorReadCallBack)
        callbackTypes |= kCFSocketReadCallBack;
    if (inCallBackTypes & kCFFileDescriptorWriteCallBack)
