...
#define AIRTUNES_HAVE_MDNS_GETADDRINFO 1
#else
...
OSStatus AirTunesDACPClient_StopSession(AirTunesDACPClientRef inClient)
{
    ...
    require_action_quiet(inClient, exit, err = kParamErr);
    require_action(inClient->magic == kAirTunesDACPClient_Magic, exit, err = kBadReferenceErr);
...
exit:
    if (newRequest)
        _AirTunesDACPClient_FreeRequest(newRequest);
    if (locked)
        pthread_mutex_unlock(&inClient->mutex);
    return (err);
}
...
DEBUG_STATIC void _AirTunesDACPClient_FreeRequest(AirTunesDACPRequestRef inRequest)
{
    if (inRequest->command)
        free(inRequest->command);
    free(inRequest);
}
...
DEBUG_STATIC void* _AirTunesDACPClient_Thread(void* inArg)
{
    ...
    OSStatus err;
    AirTunesDACPRequestRef request = NULL;
    ...
    for (;;) {
        ...
        if (request->internal) {
            ...
            if (err) {
                if (me->userCallBack)
                    me->userCallBack(err, me->userContext);
            }
        }
...
// If we don't have Bonjour support built into getaddrinfo then use Bonjour to get the address list.
    ...
    header->len = 0;
    for (;;) {
        err = SocketReadHTTPHeader(nativeSock, header);
        if (err == kNoErr)
            break;
        if (err == EWOULDBLOCK)
            err = kNoErr;
    ...
    require_action(IsHTTPStatusCode_Success(header->statusCode), exit, err = header->statusCode);
...
exit:
    ...
    if (!err)
        dlog(kLogLevelVerbose, "DACP command \"%s\" completed @ %N\n", inCommand);
    else
        dlog(kLogLevelVerbose, "DACP command \"%s\" failed    @ %N: %#m\n", inCommand, err);
    return (err);
}
...
#if (defined(AF_INET6))
        if ((inAddr->sa_family == AF_INET6) && IN6_IS_ADDR_LINKLOCAL(&((const struct sockaddr_in6*)inAddr)->sin6_addr)) {
