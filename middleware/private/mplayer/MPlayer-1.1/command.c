...
int run_command(MPContext *mpctx, mp_cmd_t *cmd)
{
    ...
    if (!set_property_command(mpctx, cmd))
        ...
        case MP_CMD_PLAY_TREE_STEP:{
				...
				//fprintf(stderr, "****************************MPCOMMAND MP_CMD_PLAY_TREE_STEP n=%d, force=%d\n", n, force);
#ifdef CONFIG_GUI
                if (use_gui) {
                    int i = 0;
                    if (n > 0)
...
//wiimu add>>>
        case MP_CMD_SHUFFLE:
            mp_msg(MSGT_CPLAYER, MSGL_V,"received shuffle\n");
            if (mpctx->playtree && mpctx->playtree->child) {
                mpctx->playtree->child->flags |= PLAY_TREE_RND;
            }
			//fprintf(stderr,"received shuffle\n");
            break;

        case MP_CMD_NOSHUFFLE:
            mp_msg(MSGT_CPLAYER, MSGL_V,"received noshuffle\n");
            if (mpctx->playtree && mpctx->playtree->child) {
                mpctx->playtree->child->flags &= ~PLAY_TREE_RND;
            }
			//fprintf(stderr,"received noshuffle\n");
            break;
        case MP_CMD_LOOPLIST:
            mp_msg(MSGT_CPLAYER, MSGL_V,"received looplist\n");
            if (mpctx->playtree_iter && mpctx->playtree_iter->root) {
                //translate to mplayer behave: -1 stop loop to 0 stop loop
                mpctx->playtree_iter->root->loop = (cmd->args[0].v.i==0) ? -1 : 0;
            }
			//fprintf(stderr,"received looplist %d\n", cmd->args[0].v.i);
            break;
        case MP_CMD_LOOPSINGLE:
            mp_msg(MSGT_CPLAYER, MSGL_V,"received loopsingle\n");
            if (mpctx) {
                mpctx->loop_times = cmd->args[0].v.i;
            }
			//fprintf(stderr,"received loopsingle %d\n",	mpctx->loop_times);
            break;
        case MP_CMD_INSERTNEW:{
   			    mp_msg(MSGT_GLOBAL, MSGL_V, "wmdebug: insertnew %s %d\n", cmd->args[0].v.s, cmd->args[1].v.i);
                play_tree_t *e = play_tree_new();
                play_tree_add_file(e, cmd->args[0].v.s);

                if (cmd->args[1].v.i == 1)   // append
                    play_tree_append_entry(mpctx->playtree_iter->tree, e);
                else if (cmd->args[1].v.i == -1)
                {                           //insert to next
                    play_tree_insert_entry(mpctx->playtree_iter->tree, e);
                }
                else if (cmd->args[1].v.i == -2)
                {                           //insert to next n
                    play_tree_insert_N_entry(mpctx->playtree_iter->tree, e, play_insert_count);
                    play_insert_count++;
                }

				play_tree_iter_count_file(mpctx->playtree_iter);
                brk_cmd = 1;
            }
            break;
//wiimu add<<<
