...
int av_append_packet(AVIOContext *s, AVPacket *pkt, int size)
{
    int ret;
    int old_size;
	av_log(NULL, AV_LOG_ERROR, "av_append_packet \r\n");	
    if (!pkt->size)
    ...
    if (ret < 0)
        return ret;
	av_log(NULL, AV_LOG_ERROR, "avio_read \r\n");	
...
AVInputFormat *av_probe_input_format3(AVProbeData *pd, int is_opened, int *score_ret)
{
	...
	av_log(NULL, AV_LOG_ERROR, "av_probe_input_format3 \r\n");	
	...
	av_log(NULL, AV_LOG_ERROR, "av_probe_input_format3 nodat = %d\n", nodat);	
    ...
    while ((fmt1 = av_iformat_next(fmt1))) {
        ...
        if (fmt1->read_probe) {
            ...
            if(fmt1->extensions && av_match_ext(lpd.filename, fmt1->extensions)) {
				av_log(NULL, AV_LOG_ERROR, "av_probe_input_format3  av_match_ext %s\n", fmt1->extensions);	
                score = 50;// FFMAX(score, nodat ? AVPROBE_SCORE_MAX/4-1 : 1);
            }
        } else if (fmt1->extensions) {
            if (av_match_ext(lpd.filename, fmt1->extensions)) {
				av_log(NULL, AV_LOG_ERROR, "av_probe_input_format3  av_match_ext2 %s \n", fmt1->extensions);	
                score = 50;
            }
        }
        if (score > score_max) {
                          ...
                          unsigned int offset, unsigned int max_probe_size)
{
	...
	// av_log(NULL, AV_LOG_ERROR, "av_probe_input_buffer %s \r\n",filename);
    if (!max_probe_size) {
        ...
        *fmt = av_probe_input_format2(&pd, 1, &score);
        if(*fmt){
            if(score <= AVPROBE_SCORE_MAX/4){ //this can only be true in the last iteration
                av_log(logctx, AV_LOG_ERROR, "Format %s detected only with low score of %d, misdetection possible!\n", (*fmt)->name, score);
            }else
                av_log(logctx, AV_LOG_ERROR, "Format %s probed with size=%d and score=%d\n", (*fmt)->name, probe_size, score);
        }
...
static int init_input(AVFormatContext *s, const char *filename, AVDictionary **options)
{
    ...
    if (s->pb) {
        ...
        if (!s->iformat)
            ...
            av_log(NULL, AV_LOG_ERROR, "Custom AVIOContext makes no sense and "
...
int ff_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    int ret, i;
    AVStream *st;
//	av_log(NULL, AV_LOG_ERROR, "ff_read_packet \r\n");	
    ...
    for(;;){
        ...
        if (pktl) {
            *pkt = pktl->pkt;
            if(s->streams[pkt->stream_index]->request_probe <= 0){
                av_log(s, AV_LOG_ERROR, "stream %d request_probe = %d\n", pkt->stream_index, s->streams[pkt->stream_index]->request_probe);
        ...
        if (ret < 0) {
            if (!pktl || ret == AVERROR(EAGAIN))
                return ret;
            av_log(s, AV_LOG_ERROR, "stream %d read packet ret = %d\n", pkt->stream_index, ret);
            for (i = 0; i < s->nb_streams; i++)
        ...
        if(pkt->stream_index >= (unsigned)s->nb_streams){
            av_log(s, AV_LOG_ERROR, "Invalid stream index %d, total streams %d\n", pkt->stream_index, s->nb_streams);
            continue;
        }
...
int avformat_find_stream_info(AVFormatContext *ic, AVDictionary **options)
{
	...
	//av_log(ic, AV_LOG_ERROR, "avformat_find_stream_info nb_streams=%d ++++\n", ic->nb_streams);
    ...
    for(i=0;i<ic->nb_streams;i++) {
        ...
        AVDictionary *thread_opt = NULL;
        st = ic->streams[i];
		av_log(ic, AV_LOG_ERROR, "avformat_find_stream_info%d codec=%d ++++\n", st->codec->codec_type);
        if (st->codec->codec_type == AVMEDIA_TYPE_VIDEO ||
...
//wiimu patch>>>		
        //don't scan amr file under codec libopencore_amrnb
        if(st && st->codec && st->codec->codec && !strcmp(st->codec->codec->name, "libopencore_amrnb"))
        {
#if	defined(ASR_QQ_MODULE) 
        	g_fc_nofade = 1;
#endif        	
            break;
        }
//wiimu patch<<<
    	...
    	//av_log(ic, AV_LOG_WARNING, "[stream info %lld] streaminfo nbstreams=%d ++++\n", tickSincePowerOn(), ic->nb_streams);
        ...
        if (i == ic->nb_streams) {
            ...
            if (!(ic->ctx_flags & AVFMTCTX_NOHEADER)) {
                ...
                ret = count;
                av_log(ic, AV_LOG_WARNING, "All info found\n");
                flush_codecs = 0;
                break;
            }
        }
        ...
        /* NOTE: a new stream can be added there if no header in file
           (AVFMTCTX_NOHEADER) */
		//av_log(ic, AV_LOG_WARNING, "[stream info %lld] read_frame_internal\n", tickSincePowerOn() );
...
//                     av_log(NULL, AV_LOG_ERROR, "%f\n", dts);
        ...
        */
		av_log(ic, AV_LOG_WARNING, "[stream info %lld] try_decode_frame\n", tickSincePowerOn() );
        try_decode_frame(st, pkt, (options && i < orig_nb_streams ) ? &options[i] : NULL);

        st->codec_info_nb_frames++;
        count++;
		
		av_log(ic, AV_LOG_WARNING, "[stream info %lld] streaminfo frames=%d, count=%d ----\n", tickSincePowerOn(),
			st->codec_info_nb_frames, count);
    }
    ...
    if (flush_codecs) {
		...
		av_log(ic, AV_LOG_WARNING, "[stream info %lld] flush_codecs ++++\n", tickSincePowerOn() );
	...
	av_log(ic, AV_LOG_WARNING, "[stream info %lld] streaminfo end1\n", tickSincePowerOn() );
    estimate_timings(ic, old_offset);

    compute_chapters_end(ic);
	av_log(ic, AV_LOG_WARNING, "[stream info %lld] streaminfo end2\n", tickSincePowerOn() );
...
AVStream *avformat_replace_stream(AVFormatContext *s, AVCodec *c)
{
    if(s->nb_streams) {
        av_log(s, AV_LOG_ERROR, "avformat_replace_stream\n"); 
        return s->streams[0];
    }
    else
...
AVStream *avformat_new_stream(AVFormatContext *s, AVCodec *c)
{
    ...
    //av_log(s, AV_LOG_ERROR, "avformat_new_stream\n");
    ...
    //av_log(s, AV_LOG_ERROR, "nstreams=%d\n", s->nb_streams);
    return st;
}
