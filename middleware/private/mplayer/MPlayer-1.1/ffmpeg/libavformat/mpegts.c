                                  ...
                                  uint32_t stream_type, uint32_t prog_reg_desc)
{
    ...
    if ((prog_reg_desc == AV_RL32("HDMV") ||
        ...
        if (pes->stream_type == 0x83) {
            ...
            av_log(pes->stream, AV_LOG_ERROR, "mpegts find new subtitle stream in mpegts_set_stream_info\n");
            sub_st = avformat_new_stream(pes->stream, NULL); //revert to orginal code
            if (!sub_st) {
                av_free(sub_pes);
                return AVERROR(ENOMEM);
            }
        ...
        case MPEGTS_HEADER:
            ...
            if (pes->data_index == PES_START_SIZE) {
                ...
                if (pes->header[0] == 0x00 && pes->header[1] == 0x00 &&
                    ...
                    if (!pes->st) {
                        av_log(pes->stream, AV_LOG_ERROR, "mpegts find new stream in TS head (not in PMT)\n");
                        pes->st = avformat_replace_stream(ts->stream, NULL);  //keep this change for TuneIn ads
                        if (!pes->st)
                            return AVERROR(ENOMEM);
                        pes->st->id = pes->pid;
...
static void pmt_cb(MpegTSFilter *filter, const uint8_t *section, int section_len)
{
    ...
    for(;;) {
        ...
        if (ts->pids[pid] && ts->pids[pid]->type == MPEGTS_PES) {
            pes = ts->pids[pid]->u.pes_filter.opaque;
            if (!pes->st) {
                av_log(pes->stream, AV_LOG_ERROR, "mpegts find new stream in PES\n");
                st = avformat_replace_stream(pes->stream, NULL);
                pes->st->id = pes->pid;
            }
            st = pes->st;
            ...
            if (pes) {
                av_log(pes->stream, AV_LOG_ERROR, "mpegts find new PES stream which type is not 0x13\n");
                st = avformat_replace_stream(pes->stream, NULL);
                st->id = pes->pid;
            }
        } else {
            ...
            if (idx >= 0) {
                ...
                av_log(pes->stream, AV_LOG_ERROR, "mpegts find new stream ######\n");
                st = avformat_replace_stream(ts->stream, NULL);
                st->id = pid;
                st->codec->codec_type = AVMEDIA_TYPE_DATA;
            }
        }
...
static int mpegts_read_header(AVFormatContext *s)
{
    ...
    if (s->iformat == &ff_mpegts_demuxer) {
        ...
        /* only read packets */
        av_log(NULL, AV_LOG_ERROR, "mpegts find new stream in mpegts_read_header\n");
        st = avformat_replace_stream(s, NULL);
        if (!st)
            goto fail;
