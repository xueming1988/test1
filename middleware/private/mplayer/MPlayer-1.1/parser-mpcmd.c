...
//wiimu >>>
extern int fadein_startPoint;
extern int fadein_time;
...
//wiimu add>>>
            else if(strcasecmp(opt, "playliststartindex") == 0) {
                int l;
                char *end = NULL;
                l = (i + 1 < argc) ? strtol(argv[i + 1], &end, 0) : 0;
                if(!end || *end != '\0') {
                    mp_msg(MSGT_CFGPARSER, MSGL_ERR, "playliststartindex must be integer", argv[i + 1]);
                    tmp = ERR_OUT_OF_RANGE;
                } else {
                    play_tree_t *pt = root ? root : last_entry;
                    l = l <= 0 ? -1 : l;
                    pt->startindex = l;
                    tmp = 1;
                }
            }
			else if(strcasecmp(opt, "fadeinsp") == 0) {
                char *end = NULL;
                fadein_startPoint = (i + 1 < argc) ? strtol(argv[i + 1], &end, 0) : 0;
                if(!end || *end != '\0') {
                    mp_msg(MSGT_CFGPARSER, MSGL_ERR, "fadeinsp must be integer", argv[i + 1]);
                    tmp = ERR_OUT_OF_RANGE;
                } else {
                    tmp = 1;
                }
            } else if(strcasecmp(opt, "fadeintime") == 0) {
                char *end = NULL;
                fadein_time = (i + 1 < argc) ? strtol(argv[i + 1], &end, 0) : 0;
                if(!end || *end != '\0') {
                    mp_msg(MSGT_CFGPARSER, MSGL_ERR, "fadeintime must be integer", argv[i + 1]);
                    tmp = ERR_OUT_OF_RANGE;
                } else {
                    tmp = 1;
                }
            } 
				else if(strcasecmp(opt, "fddis") == 0) {
                char *end = NULL;
                int fc_nofade = 0;
                fc_nofade = (i + 1 < argc) ? strtol(argv[i + 1], &end, 0) : 0;
                if(fc_nofade) {
                    g_fc_enable = 0;
                }
                if(!end || *end != '\0') {
                    mp_msg(MSGT_CFGPARSER, MSGL_ERR, "fddis must be integer", argv[i + 1]);
                    tmp = ERR_OUT_OF_RANGE;
                } else {
                    tmp = 1;
                }
            } 
			else if(strcasecmp(opt, "streamtype") == 0) {
                char *end = NULL;
                int stream_type = 0;
                stream_type = (i + 1 < argc) ? strtol(argv[i + 1], &end, 0) : 0;
                if(stream_type > 0) {
                ...
                if(!end || *end != '\0') {
                    mp_msg(MSGT_CFGPARSER, MSGL_ERR, "streamtype must be integer", argv[i + 1]);
                    tmp = ERR_OUT_OF_RANGE;
                } else {
                    tmp = 1;
                }
