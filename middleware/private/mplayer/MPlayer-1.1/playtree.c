...
//wiimu add >>>
void
play_tree_insert_N_entry(play_tree_t* pt, play_tree_t* entry, int step) {
  play_tree_t* iter;
  ...
  for(iter = pt ; iter->next != NULL && (step-- > 0); iter = iter->next)
    /* NOTHING */;
...
play_tree_iter_step(play_tree_iter_t* iter, int d,int with_nodes) {
  ...
  if(iter->mode == PLAY_TREE_ITER_RND)
	...
	//fprintf(stderr, "***********************play_tree_iter_step mode=%d, pt=%d, d=%d, loop=%d, withnode=%d\n", 
	//		iter->mode, pt, d, iter->root->loop, with_nodes);
  if(pt == NULL) { // No next
    // Must we loop?
    if (iter->mode == PLAY_TREE_ITER_RND) {
      if (iter->root->loop == 0)
...
play_tree_iter_up_step(play_tree_iter_t* iter, int d,int with_nodes) {
  ...
  {
 	//fprintf(stderr, "play_tree_iter_up_step return end 1\n");
    return PLAY_TREE_ITER_END;
  }
#ifdef MP_DEBUG
...
void play_tree_iter_count_file(play_tree_iter_t* iter)
{
	int total=0, count=0; 
	play_tree_t * pt;
	pt = iter->tree;
	for(count=1; pt->prev != NULL ; pt=pt->prev) {
		count++;
	}
	for(total=1; pt->next != NULL ; pt=pt->next) {
		total++;
	}
	//mp_msg(MSGT_CPLAYER, MSGL_INFO, "wmdebug: [index/total]:%d/%d\n", count, total);
	//mp_msg(MSGT_CPLAYER, MSGL_INFO, "[index/total]:%d/%d\n", count, total);
...
play_tree_iter_get_file(play_tree_iter_t* iter, int d) {
  ...
  play_tree_iter_count_file(iter);
