...
parse_asx(play_tree_parser_t *p)
{
    ...
    //<asx should be line 0
    //but some asx file is not standard, we need to skip several top line
    int line_limitation = 5; 
    char *line = NULL;

    mp_msg(MSGT_PLAYTREE, MSGL_V, "Trying asx...\n");
    fprintf(stderr, "Trying asx..\n");
    ...
    while(1) {
        ...
        if(!comments) {
            if(line[0] != '<') {
                ...
                if(line_limitation-- <= 0) {
                    //we parsed "n" line but can't find <ASX
                    return NULL;
                }
                continue;
            }
    ...
    fprintf(stderr, "Detected asx format\n");
...
{
    char *line;
    play_tree_t *list = NULL, *entry = NULL, *last_entry = NULL, *temp_entry = NULL;
    ...
    if(strcasecmp(line, "#EXTM3U"))
        return NULL;
    fprintf(stderr, "--------------------parse m3u enter\n");
    ...
    while((line = play_tree_parser_get_line(p)) != NULL) {
        ...
        fprintf(stderr, "----parse m3u line %s\n", line);
        if(line[0] == '#') {
            if(strncasecmp(line, "#EXTINF:", 8) == 0) {
				entry->duration = strtol(line + 8, &line, 10);
               fprintf(stderr, "[M3U] #EXTINF, Duration: %dsec  Title: %s\n", entry->duration, line + 2);
            }
            if(strncasecmp(line, "#EXT-X-TARGETDURATION:", 22) == 0) {
                fprintf(stderr, "[M3U] #EXT-X-TARGETDURATION, !!!!!!!!!!!!special handle #EXT-X-TARGETDURATION, why\n");
				//Mark: why we need to handle this special case?
                play_tree_t * special_entry = play_tree_new();
                play_tree_add_file(special_entry, p->stream->url);
				if(entry) play_tree_free(entry, 0);
				entry = NULL;
                return special_entry;
            }
			if(strncasecmp(line, "#EXT-X-STREAM-INF:", strlen("#EXT-X-STREAM-INF:")) == 0) {
				const char* s_program_id = strstr(line, "PROGRAM-ID=");
				const char* s_bandwidth = strstr(line, "BANDWIDTH=");
				if(s_program_id)
					entry->program_id = strtol(s_program_id + strlen("PROGRAM-ID="), NULL, 10);
				if(s_bandwidth)
					entry->bandwidth = strtol(s_bandwidth+ strlen("BANDWIDTH="), NULL, 10);
				fprintf(stderr, "[M3U] #EXT-X-STREAM-INF, program_id=%d, bandwidth=%d\n", entry->program_id, entry->bandwidth);
			}
            continue;
        }
		...
		temp_entry = list;
		while(temp_entry) {
			if(temp_entry->program_id >= 0 && entry->bandwidth > 0 && temp_entry->bandwidth > 0 
				&& temp_entry->program_id == entry->program_id) {
				fprintf(stderr, "[M3U] same program id stream, ignore it\n");
				play_tree_free(entry, 0);
				entry = NULL;
				break;
			}
...
embedded_playlist_parse(char *line)
{
    ...
    //add new playtree
    mp_msg(MSGT_PLAYTREE, MSGL_ERR, "Adding playlist %s to element entryref\n", line);
    ...
    mp_msg(MSGT_PLAYTREE, MSGL_ERR, "embedded_playlist_parse 0x%x\n", entry);
    return entry;
}
...
parse_textplain(play_tree_parser_t *p)
{
    ...
    wiimu_log(1, 0, 0, 0, "Trying plaintext playlist...");
    ...
    while((line = play_tree_parser_get_line(p)) != NULL) {
        ...
        embedded = 0;
        wiimu_log(1, 0, 0, 0, "Trying plaintext playlist %s", line);
        if(demuxer_type_by_filename(line) == DEMUXER_TYPE_PLAYLIST) {
            wiimu_log(1, 0, 0, 0, "This is pls +++");
            entry = embedded_playlist_parse(line);
            embedded = 1;
            wiimu_log(1, 0, 0, 0, "This is pls --- 0x%x", entry);
            //break;
        }

        if(!embedded) {       //regular file link
            wiimu_log(1, 0, 0, 0, "This is file +++");
            entry = play_tree_new();
            play_tree_add_file(entry, line);
            wiimu_log(1, 0, 0, 0, "This is file --- 0x%x", entry);
        }
    ...
    wiimu_log(1, 0, 0, 0, "parse_textplain list =  0x%x\n", list);
...
parse_playlist_file(char *file)
{
    ...
    mp_msg(MSGT_PLAYTREE, MSGL_ERR, "Parsing playlist file %s...\n", file);
...
play_tree_parser_get_play_tree(play_tree_parser_t *p, int forced)
{
    ...
    while(play_tree_parser_get_line(p) != NULL) {
        ...
        if(forced) {
            ...
            wiimu_log(1, 0, 0, 0, "parse_textplain finised with 0x%x\n", tree);
            if(tree) break;
        }
        break;
    }
    ...
    if(tree)
        mp_msg(MSGT_PLAYTREE, MSGL_ERR, "Playlist successfully parsed\n");
    else
