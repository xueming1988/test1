...
//wiimu add>>>
  { MP_CMD_SHUFFLE, "shuffle", 0, { {-1,{0}} } },
  { MP_CMD_NOSHUFFLE, "noshuffle", 0, { {-1,{0}} } },
  { MP_CMD_LOOPLIST, "looplist", 1, { {MP_CMD_ARG_INT,{0}}, {MP_CMD_ARG_INT,{0}}, {-1,{0}} } },
  { MP_CMD_LOOPSINGLE, "loopsingle", 1, { {MP_CMD_ARG_INT,{0}}, {MP_CMD_ARG_INT,{0}}, {-1,{0}} } },
  { MP_CMD_INSERTNEW, "insertnew", 2, { {MP_CMD_ARG_STRING, {0}}, {MP_CMD_ARG_INT,{1}}, {-1,{0}} } },
//wiimu add<<<
...
mp_input_default_cmd_func(int fd,char* buf, int l) {
  ...
  while(1) {
    int r = read(fd,buf,l);
    // Error ?
    //printf("mp_input_default_cmd_func %s ",buf);
    if(r < 0) {
      if(errno == EINTR)
	continue;
...
static int mp_read_message_queue(int fd,char* buf, int l)
{
	static int read_once = 0;
	int msg_id;
	mplay_msg  msg={0};
//	fprintf(stderr,"mp_read_message_queue \r\n");
	if(read_once > 0)
	{
		return MP_INPUT_NOTHING;
	}
	msg_id = msgget(0x8678,0666);
	if(msg_id != -1)
	{
		read_once = 1;
		int ret = msgrcv(msg_id, (void *)&msg, 64, 0, IPC_NOWAIT);
		if(ret > 0)
		{
			strcpy(buf,msg.msgbuf);
			return strlen(buf);
		}
	}else
	{
...
mp_input_init(void) {
  ...
  if(in_file) {
	...
	mp_input_add_cmd_fd(-1,0,mp_read_message_queue,NULL);
