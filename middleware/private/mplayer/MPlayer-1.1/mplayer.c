...
#include "wmrm_api.h"
extern wmrm_snd_t g_wmrm_sound;
extern int g_wmrm_used;
#endif


#ifndef OPENWRT_AVS
extern FadeCache g_fadeCache;
#endif
extern int g_fc_enable;
extern int g_fc_nofade;
...
//wiimu add >>>
int play_insert_count = 0;
double firstPts = 0;
int g_stream_type = e_stream_type_none;
#define IMUZOP_STATUS_EX_SET(val)  \
do { \
    if (g_wiimu_shm && g_stream_type > e_stream_type_none && g_stream_type < e_stream_type_max) { \
        g_wiimu_shm->imuzo_status_ex[g_stream_type] = val; \
    } \
} while(0)

#define IMUZOP_ERROR_EX_SET(val)  \
do { \
    if (g_wiimu_shm && g_stream_type > e_stream_type_none && g_stream_type < e_stream_type_max) { \
        g_wiimu_shm->imuzo_error_ex[g_stream_type] = val; \
    } \
} while(0)
//wiimu add <<<
...
static int loop_mode_max_try = 1000;
...
float stream_cache_min_percent      = 10.0;
float stream_cache_seek_min_percent = 20.0;
#endif
...
static void mvUtil_HexToBin(char *in, char *out, int *outLen)
{
    //hex \D7ַ\FB\B4\AEת\B3\C9 binary\D7ַ\FB\B4\AE
    //'6','e' -> 0x6e
    char buf[3];
    int i;
    int temp;
...
//            }
    ...
    if(mask & INITIALIZED_AO) {
        ...
        if(mpctx->audio_out)
            mpctx->audio_out->uninit(brk_cmd ? 1 : mpctx->eof ? 0 : 1);
...
extern int send_run;
extern void multicast_send_quit();
extern void m3u8_file_remove(void);
...
void exit_player_with_rc(enum exit_reason how, int rc)
{
    ...
    IMUZOP_STATUS_EX_SET(e_status_ex_exit);
    ...
    {
    if(g_fc_enable)
        fadeCache_uninit(&g_fadeCache, 0);
    }
#endif
    int snd_type = focus_manage_stream_type_map(g_stream_type);
    if(snd_type > 0) {
        WMUtil_Snd_Ctrl_SetProcessMute(g_wiimu_shm, 1, snd_type, 0);
    } else {
...
void SetHttpErrorCode(int error_code)
{
    if(shmem_mplayer) {
        share_context *pshare = (share_context *)shmem_mplayer;
        pshare->http_error_code = error_code;
        printf("errorcode HTTP %d \r\n", error_code);
    }
}
...
void SetErrorType(int error_type)
{
    if(shmem_mplayer) {
        share_context *pshare = (share_context *)shmem_mplayer;
        pshare->err_type = error_type;
        printf("error_type %d \r\n", error_type);
    }
    IMUZOP_ERROR_EX_SET(error_type);
}
...
{
    pid_t pid;
    wiimu_log(1, 0, 0, 0, "imuzop child_sighandler %d", x);
...
static void exit_sighandler(int x)
{
    ...
    wiimu_log(1, 0, 0, 0, "imuzop exit_sighandler %d", x);
    share_context *pshare = (share_context *)shmem_mplayer;
    if(pshare)
        pshare->status = 2;
...
#endif
    {
    g_fadeCache.status = STATUS_QUIT;
    g_fadeCache.cache_run = 0;
    }
#endif
    //Never process lock on signal handle
    int snd_type = focus_manage_stream_type_map(g_stream_type);
    if(snd_type > 0) {
        g_wiimu_shm->snd_ctrl[snd_type].mute = 1;
    } else {
        g_wiimu_shm->snd_ctrl[SNDSRC_MPLAYER].mute = 1;
    }
...
{
    mp_cmd_t *cmd;
    wiimu_log(1, 0, 0, 0, "libmpdemux_was_interrupted ++++++++++++++");
...
{
    double a_pts = written_audio_pts(sh_audio, d_audio) - playback_speed *
                   audio_out->get_delay();
    a_pts = a_pts < 0 ? 0 : a_pts;
    return a_pts;
}
...
static int is_at_end(MPContext *mpctx, m_time_size_t *end_at, double pts)
{
    switch(end_at->type) {
        case END_AT_TIME:
            return end_at->pos <= pts;
        case END_AT_SIZE:
            return end_at->pos <= stream_tell(mpctx->stream);
    }
    return 0;
}
...
float timing_sleep(float time_frame)
{
#ifdef HAVE_RTC
    if(rtc_fd >= 0) {
        // -------- RTC -----------
...
static int fill_audio_out_buffers(void)
{
    ...
    int timeout = 0;
    sh_audio_t *const sh_audio = mpctx->sh_audio;
    static int print_decode_ts = 1;

    // Haiyong: workaround
    //          Belkin Dive EQ will change this value to 0, we don't know the root cause.
    //          Change the value to static or volatile type can workaround the issue. 
    //          But we don't know if there is any other value changed. Seem AVS(has tts playback) and WMRM have similar issues.    
    static volatile float audio_empty_min = 0.17; //Alex:0.17 is for A98 platform, I think we need to define different value for differnt HW platform.
...
#endif
    {
    if(g_fc_enable && !g_fc_nofade)
        audio_empty_min = 0.04;
    }
#else
    audio_empty_min = 0.14;
#endif
    ...
    while(bytes_to_write) {
        ...
        t = GetTimer();
        if(!format_change && !audio_eof) {
            res = mp_decode_audio(sh_audio, playsize);
            if(print_decode_ts) {
                wiimu_log(1, 0, 0, 0, "[TICK]%s  decode_audio first\n", __FUNCTION__);
                print_decode_ts = 0;
            }
            format_change = res == -2;
        }
        ...
        if(res == -3) {
            audio_eof = 1;
            //last
        } else if(!format_change && res < 0) { // EOF or error
    ...
    {
                if(g_fc_enable && !g_fc_nofade)
                    fadeCache_eof(&g_fadeCache);
    }
#endif
                audio_eof = 1;
                if(sh_audio->a_out_buffer_len == 0) {
                    while(1) {
                        if(mpctx->audio_out->get_delay() < audio_empty_min)
                            return 0;
                        usleep(10000);
                    }
                }
        ...
        if(playsize) {
            ...
            }else if((format_change || audio_eof) && mpctx->audio_out->get_delay() < audio_empty_min)
            {
...
#endif
                if(sh_audio->a_out_buffer_len == 0) {
                    while(!mp_input_check_interrupt(0)) {
                        double a_pos;
                        if(mpctx->sh_audio)
                            a_pos = playing_audio_pts(mpctx->sh_audio, mpctx->d_audio, mpctx->audio_out);

                        if(shmem_mplayer && a_pos) {
                            share_context *pshare = (share_context *)shmem_mplayer;
                            pshare->status = 1;
                            pshare->postion = a_pos;
                            pshare->duration = demuxer_get_time_length(mpctx->demuxer);
                            pshare->offset_pts = pshare->postion - firstPts;
                        }
                        if(a_pos) {
                            IMUZOP_STATUS_EX_SET(e_status_ex_playing);
                        }

                        //wiimu_log(1, 0, 0, 0, "[TICK]%s a_pos=%f delay =%f\n", __FUNCTION__, a_pos, mpctx->audio_out->get_delay());
                        if(mpctx->audio_out->get_delay() < audio_empty_min) {
                            return 0;
                        }
                        usleep(10000);
...
int main(int argc, char *argv[])
{
    ...
    run_expire_check_thread("imuzop", EXPIRE_DAYS, 3, NULL, 30);
#endif

#if 0 //def  AUDIO_SRC_OUTPUT
    fprintf(stderr, "sched Policy %d\n", sched_getscheduler(0));
    ...
    {
        char *file_url = argv[argc - 1];
		if (file_url == NULL ||  strlen(file_url) >= 8192) {
			exit_player_with_rc(EXIT_NONE, 0);
		}
		if (strncmp(file_url, "hex://", strlen("hex://")) == 0) {
            char real_url[8192] = {0};
            hex2ascii(file_url + strlen("hex://"), real_url, strlen(file_url) - strlen("hex://"), 8192);
            wiimu_log(1, 0, 0, 0, "in_url:%.10s, out_url:%.50s", file_url, real_url);
            strncpy(file_url, real_url, strlen(file_url));
        } else {
            wiimu_log(1, 0, 0, 0, "[TICK]in_url:%.50s at %lld\n", file_url, tickSincePowerOn());
        }
		
        if(strncmp(file_url, "http://127.0.0.1", strlen("http://127.0.0.1")) == 0)
        {
            exit_player_with_rc(EXIT_NONE, 0);
        }			
    }
    ...
    //fprintf(stderr, "imuzop pid = %d \n", (int)getpid());

    parse_cfgfiles(mconfig);

    mpctx->playtree = m_config_parse_mp_command_line(mconfig, argc, argv);
    //fprintf(stderr, "!!!!!!!g_stream_type=%d!!!!!!\n", g_stream_type);

//wiimu add>>>
    if(g_stream_type == e_stream_type_none) {
        //huaijing
        int shmid = shmget(34387, 0, 0);
        if(shmid != -1)
            shmem_mplayer = shmat(shmid, (const void *)0, 0);
        else
            fprintf(stderr, "!!!!!!!imuzop shmget failed");
        ...
        IMUZOP_STATUS_EX_SET(e_status_ex_started);
    }
    int snd_type = focus_manage_stream_type_map(g_stream_type);
    if(snd_type > 0) {
    	if (snd_type != SNDSRC_NOTIFY) {
    		WMUtil_Snd_Ctrl_SetProcessMute(g_wiimu_shm, 1, snd_type, 1);
    	}
    } else {
        WMUtil_Snd_Ctrl_SetProcessMute(g_wiimu_shm, 1, SNDSRC_MPLAYER, 1);
    }
//wiimu add<<<
    ...
    if(mpctx->playtree == NULL) {
        opt_exit = 1;
        if (shmem_mplayer) {
            mp_msg(MSGT_CPLAYER, MSGL_INFO, "open stream fail\n");
            share_context *pshare = (share_context *)shmem_mplayer;
            pshare->mp_openstream_fail_flag =1;
        }
    } else {
...
//wiimu add>>>
                if(mpctx->playtree->startindex > 0) {
                    if(play_tree_iter_step(mpctx->playtree_iter, mpctx->playtree->startindex, 0) == PLAY_TREE_ITER_ENTRY) {
                    } else {
                        play_tree_iter_free(mpctx->playtree_iter);
                        mpctx->playtree_iter = NULL;
                        mp_msg(MSGT_CPLAYER, MSGL_FATAL, "playliststartindex cause file no found, exit!\n");
                        exit(-1);
                    }
                }
//wiimu add<<<
    ...
    //fprintf(stderr, "imuzop enter2 at %d \n", (int)tickSincePowerOn());
    ...
    if(shmem_mplayer) {
        share_context *pshare = (share_context *)shmem_mplayer;
        pshare->loopmode = 0;/* default none loop */
		pshare->err_type = 0;
    }
    SetErrorType(0); //clear error type
...
// =================== GUI idle loop (STOP state) ===========================
    ...
    if(filename) {
        mp_msg(MSGT_CPLAYER, MSGL_INFO, MSGTR_Playing, filename_recode(filename));
    }
...
//============ Open & Sync STREAM --- fork cache2 ====================
    ...
    //fprintf(stderr, "imuzop open_stream at %d \n", (int)tickSincePowerOn());
    //g_wiimu_shm->imuzop_gapless = 1;//test only
    ...
    if(mpctx->file_format == DEMUXER_TYPE_PLAYLIST) {
        ...
        mp_msg(MSGT_CPLAYER, MSGL_V, "Parsing playlist %s...\n",
               filename_recode(filename));
        fprintf(stderr, "Parsing playlist %s...\n",
                filename_recode(filename));
        entry      = parse_playtree(mpctx->stream, 1);
...
goto_enable_cache:
    if(stream_cache_size > 0) {
        ...
        //fprintf(stderr, "imuzop enable_cache at %d \n", (int)tickSincePowerOn());
...
//============ Open DEMUXERS --- DETECT file type =======================
    //fprintf(stderr, "imuzop demux open at %d \n", (int)tickSincePowerOn());
...
//wiimu add>>>
    //only play audio only stream
#ifndef NAVER_LINE	
    if(mpctx->sh_video) {
        printf("\n-------------------Video Stream inside, escape from this file!----------------------\n");
        goto goto_next_file; // exit_player(MSGTR_Exit_error);
    }
#endif	
//wiimu add<<<
...
//================ SETUP AUDIO ==========================
        if(mpctx->sh_audio) {
            ...
            if(mpctx->sh_audio && mpctx->sh_audio->codec)
                mp_msg(MSGT_IDENTIFY, MSGL_ERR, "ID_AUDIO_CODEC=%s\n", mpctx->sh_audio->codec->name);
        }
...
//==================== START PLAYING =======================
        ...
        while(!mpctx->eof) {
            ...
            if(g_wiimu_shm->imuzop_gapless)
                mpctx->demuxer->movi_end =  g_wiimu_shm->imuzop_end_pos;
            ...
            if(!mpctx->sh_video) {
                ...
                if(!quiet) {
                    ...
                    if(shmem_mplayer && a_pos) {
                        ...
                        pshare->duration = demuxer_get_time_length(mpctx->demuxer);
                        pshare->offset_pts = pshare->postion - firstPts;
                        if(pshare->duration > 20 && pshare->offset_pts >= pshare->duration) {
							 /* sometimes audio dosen't complete playing, dont't end it immediately */
                            if(next_ts == 0)
                            {
                                next_ts = tickSincePowerOn();
                            }else if(tickSincePowerOn() > next_ts + 2000){
								wiimu_log(1, 0, 0, 0, "goto NEXT_ENTRY position=%lf, duration=%lf pts=%lf", pshare->postion, pshare->duration,
									pshare->offset_pts);
								mpctx->eof = PT_NEXT_ENTRY;
                            }
                        }
                    ...
                    //fprintf(stderr, "eof %d, end=%lld (%d %lld), duration=%f, apos=%f\n",mpctx->eof, mpctx->demuxer->movi_end,
                    //  g_wiimu_shm->imuzop_gapless, g_wiimu_shm->imuzop_end_pos,
                    //  (float)demuxer_get_time_length(mpctx->demuxer), (float)a_pos);
                }
...
//============================ Handle PAUSE ===============================
            ...
            if(mpctx->eof == 1 && mpctx->loop_times >= 0) {
                //fprintf(stderr, "loop_times = %d, eof = %d\n", mpctx->loop_times, mpctx->eof);
            ...
            //  printf("rel_seek_secs %f abs_seek_pos %d \r\n",rel_seek_secs,abs_seek_pos);
...
goto_next_file:  // don't jump here after ao/vo/getch initialization!
    ...
    //fprintf(stderr,"[play_next_file 1] eof=%d\n", mpctx->eof);
    ...
    if(mpctx->eof == PT_NEXT_ENTRY || mpctx->eof == PT_PREV_ENTRY) {
        ...
        if(play_tree_iter_step(mpctx->playtree_iter, mpctx->play_tree_step, 0) == PLAY_TREE_ITER_ENTRY) {
            //fprintf(stderr,"[play_next_file 2] play_tree_iter_step return PLAY_TREE_ITER_ENTRY\n");
            mpctx->eof = 1;
        } else {
            play_tree_iter_free(mpctx->playtree_iter);
            mpctx->playtree_iter = NULL;
            //fprintf(stderr,"[play_next_file 2] play_tree_iter_step return NULL\n");
        }
        ...
        if(mpctx->playtree_iter) {
            if(play_tree_iter_up_step(mpctx->playtree_iter, mpctx->eof, 0) == PLAY_TREE_ITER_ENTRY) {
                //fprintf(stderr,"[play_next_file 3] play_tree_iter_step return PLAY_TREE_ITER_ENTRY\n");
                mpctx->eof = 1;
            } else {
                play_tree_iter_free(mpctx->playtree_iter);
                mpctx->playtree_iter = NULL;
                //fprintf(stderr,"[play_next_file 3] play_tree_iter_step return NULL\n");
            }
        }
    } else if(mpctx->eof == PT_STOP) {
        play_tree_iter_free(mpctx->playtree_iter);
        mpctx->playtree_iter = NULL;
        //fprintf(stderr,"[play_next_file 4] play_tree_iter_step PT_STOP\n");
    } else { // NEXT PREV SRC
        mpctx->eof = mpctx->eof == PT_PREV_SRC ? -1 : 1;
        //fprintf(stderr,"[play_next_file 5]\n");
    }
    ...
    while(mpctx->playtree_iter != NULL) {
        ...
        if(filename == NULL) {
            if(play_tree_iter_step(mpctx->playtree_iter, mpctx->eof, 0) != PLAY_TREE_ITER_ENTRY) {
                //fprintf(stderr,"[play_next_file 6] play_tree_iter_step return NULL\n");
                play_tree_iter_free(mpctx->playtree_iter);
                mpctx->playtree_iter = NULL;
            }
        } else {
            //fprintf(stderr,"[play_next_file 6] filename=%s\n", filename);
            break;
        }
    }

    //fprintf(stderr, "[play_next_file] mpctx->playtree_iter = %x, player_idle_mode=%d, eof=%d\n", mpctx->playtree_iter,
    //player_idle_mode, mpctx->eof);


    if(shmem_mplayer) {
        share_context *pshare = (share_context *)shmem_mplayer;
        keepLooping = pshare->loopmode;
        fprintf(stderr, "[Mplayer] #### loopmode=[%d] ####.\n", keepLooping);
    }
...
//wiimu add <<<
        ...
        fprintf(stderr, "[Mplayer] #### scast loop playing ####.\n");
        mpctx->eof = 0;
    ...
    exit_player_with_rc(EXIT_EOF, 0);
    //fprintf(stderr, "imuzop leave at %d \n", (int)tickSincePowerOn());
    return 1;
}
