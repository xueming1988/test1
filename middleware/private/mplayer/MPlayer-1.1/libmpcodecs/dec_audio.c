...
extern void SetErrorType(int error_type);
			  ...
			  char **audio_fm_list)
{
	...
	mp_msg(MSGT_DECAUDIO, MSGL_ERR, MSGTR_CantFindAudioCodec,
	       sh_audio->format);
	SetErrorType(-1); //PLAYER_NO_DECODER -1
	return 0;   // failed
    }
...
int mp_decode_audio(sh_audio_t *sh_audio, int minlen)
{
	...
	if (res < 0)
	    return res;
	if(mp_input_check_interrupt(0) != 0)
	{
		printf("mp_decode_audio break \r\n");
		return 0;
	}
