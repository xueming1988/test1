...
extern const demuxer_desc_t demuxer_desc_dra;
...
const demuxer_desc_t *const demuxer_list[] = {
    ...
    &demuxer_desc_ogg,
#endif
#ifdef CONFIG_NOT_MINIMIZE_SIZE
    ...
    &demuxer_desc_dra,
#endif
    &demuxer_desc_mov,  //AAC
#ifdef CONFIG_LIVE555
    &demuxer_desc_rtp,
#endif
#ifdef CONFIG_FFMPEG
    &demuxer_desc_lavf,//APE
#endif
                                    ...
                                    int dvdsub_id, char *filename)
{
    ...
    fprintf(stderr, "++++[%lld] demux_open_stream file_format=%d, force=%d, id=%d, stream=%s\n", 
			tickSincePowerOn(), file_format, force, audio_id,
            filename);
    // If somebody requested a demuxer check it
    if(file_format == 0)
        file_format = demuxer_type_by_filename(filename);
    fprintf(stderr, "demux_open_stream file_format=%d\n", file_format);
    if(file_format) {
        if((demuxer_desc = get_demuxer_desc_from_type(file_format))) {
            if(demuxer_desc->name)
                fprintf(stderr, "---- demux_open_stream, %s\n", demuxer_desc->name);
            ...
            fprintf(stderr, "[1] [%lld] fformat = %d type=%d\n", tickSincePowerOn(), fformat, demuxer_desc->type);
            if(fformat != 0) {
                if(fformat == demuxer_desc->type) {
                    ...
                    if(!demuxer->desc->open
                        ...
                        demuxer = demux2;
                        fprintf(stderr, "---- demux_open_stream %d, quit ok 4\n", file_format);
                        goto dmx_open;
                    }
                } else {
            ...
            if(demuxer)
                free_demuxer(demuxer);
            demuxer = NULL;
            //  fprintf(stderr, "---- demux_open_stream, quit2\n");
            //  return NULL;//demux_open_stream(stream, fformat, force, audio_id, video_id, dvdsub_id, filename);
        }
    }

    // Test demuxers with safe file checks
    fprintf(stderr, "[%lld] Test demuxers with safe file checks\n", tickSincePowerOn());
            ...
            if((fformat = demuxer_desc->check_file(demuxer)) != 0) {
                if(fformat == demuxer_desc->type) {
                    ...
                    if(!demuxer->desc->open
                            || (demux2 = demuxer->desc->open(demuxer))) {
                        demuxer = demux2;
                        fprintf(stderr, "---- demux_open_stream %d, quit ok 5\n", file_format);
                        goto dmx_open;
                    }
                } else {
                    if(fformat == DEMUXER_TYPE_PLAYLIST) {
                        fprintf(stderr, "---- demux_open_stream, quit with playlist 3\n");
                    ...
                    free_demuxer(demuxer);
                    demuxer = NULL;
                    fprintf(stderr, "Test %d demuxer with safe file check\n", i);
                    /*
                        ...
                        if (demuxer)
                        {
                            //fprintf(stderr, "---- demux_open_stream, quit ok 4\n");
                            return demuxer; // done!
                        }
                        */
                    file_format = fformat;
                        ...
                        if(fformat != 0) {
                            if(fformat == demuxer_desc->type) {
                                ...
                                if(!demuxer->desc->open
                                        || (demux2 = demuxer->desc->open(demuxer))) {
                                    demuxer = demux2;
                                    fprintf(stderr, "---- demux_open_stream %d, quit ok 6\n", file_format);
                                    goto dmx_open;
                                }
                            }
                        }
    ...
    if(file_format == DEMUXER_TYPE_UNKNOWN && filename
        ...
        if(file_format != DEMUXER_TYPE_UNKNOWN) {
            // we like recursion :)
            fprintf(stderr, "Extension_parsing, format=%d\n", file_format);
            if((demuxer_desc = get_demuxer_desc_from_type(file_format))) {
                demuxer = new_demuxer(stream, demuxer_desc->type, audio_id, video_id, dvdsub_id, filename);
                if(demuxer_desc->check_file)
                    fformat = demuxer_desc->check_file(demuxer);
                ...
                if(fformat != 0) {
                    if(fformat == demuxer_desc->type) {
                        ...
                        if(!demuxer->desc->open
                                || (demux2 = demuxer->desc->open(demuxer))) {
                            demuxer = demux2;
                            fprintf(stderr, "---- demux_open_stream %d, quit ok 7\n", file_format);
                            goto dmx_open;
                        }
                    }
                }
            ...
            //if (demuxer)
            //{
            //fprintf(stderr, "---- demux_open_stream, quit ok 5\n");
            //  return demuxer; // done!
            //}
    ...
    fprintf(stderr, "[%lld] try all other demuxers\n", tickSincePowerOn());
    for(i = 0; (demuxer_desc = demuxer_list[i]); i++) {
        if(!demuxer_desc->safe_check && demuxer_desc->check_file) {
            fprintf(stderr, " demux_open_stream %d %d ++++ \n", i, demuxer_desc->type);
            if(demuxer_desc->name)
                fprintf(stderr, "   demux_open_stream %d %s \n", i, demuxer_desc->name);
            demuxer = new_demuxer(stream, demuxer_desc->type, audio_id,
                                  video_id, dvdsub_id, filename);
            if((fformat = demuxer_desc->check_file(demuxer)) != 0) {
                if(fformat == demuxer_desc->type) {
                    ...
                    if(!demuxer->desc->open
                            || (demux2 = demuxer->desc->open(demuxer))) {
                        demuxer = demux2;
                        fprintf(stderr, "----[%lld] demux_open_stream, format= %d, quit ok 8\n", tickSincePowerOn(), file_format);
                        goto dmx_open;
                    }
                } else {
                    if(fformat == DEMUXER_TYPE_PLAYLIST) {
                        fprintf(stderr, "---- demux_open_stream, quit with playlist 6\n");
                        return demuxer; // handled in mplayer.c
                    }
                    // Format changed after check, recurse
                    free_demuxer(demuxer);
                    ...
                    fprintf(stderr, "demux_open_stream not match, file_format=%d, continue check\n", file_format);
                    file_format = fformat;
                    if((demuxer_desc = get_demuxer_desc_from_type(file_format))) {
                        demuxer = new_demuxer(stream, demuxer_desc->type, audio_id, video_id, dvdsub_id, filename);
                        if(demuxer_desc->check_file)
                        ...
                        if(fformat != 0) {
                            if(fformat == demuxer_desc->type) {
                                ...
                                if(!demuxer->desc->open
                                    ...
                                    fprintf(stderr, "---- demux_open_stream format = %d, quit ok 9\n", file_format);
                                    goto dmx_open;
                                }
                            }
                        }
                    ...
                    //fprintf(stderr, "  recursice 5\n");
                    //demuxer = demux_open_stream(stream, fformat, force,
                    //                             audio_id, video_id,
                    //                            dvdsub_id, filename);
                    //if (demuxer)
                    //{
                    //fprintf(stderr, "---- demux_open_stream, quit ok 7\n");
                    //  return demuxer; // done!
                    //}
                    file_format = DEMUXER_TYPE_UNKNOWN;
                }
            ...
            if(demuxer)
                free_demuxer(demuxer);
            demuxer = NULL;
            fprintf(stderr, " demux_open_stream %d ---- \n", i);
        }
    }

    fprintf(stderr, "---- demux_open_stream fail, quit\n");
    return NULL;
    ...
    for(n = 0; info && info[2 * n] != NULL; n++) {
        if(!strcasecmp(opt, info[2 * n])) {
            ...
            if(shmem_mplayer) {
                ...
                if(strcasecmp(opt, "title") == 0)
                    strcpy(pshare->title, opt);
                else if(strcasecmp(opt, "artist") == 0 || strcasecmp(opt, "author") == 0)
                    strcpy(pshare->artist, opt);
                else if(strcasecmp(opt, "album") == 0)
                    strcpy(pshare->album, opt);
                else if(strcasecmp(opt, "track") == 0)
                    strcpy(pshare->track, opt);
                else if(strcasecmp(opt, "genre") == 0)
                    strcpy(pshare->genre, opt);
                else if(strcasecmp(opt, "year") == 0)
                    strcpy(pshare->year, opt);
            }
            free(info[2 * n + 1]);
...
double demuxer_get_time_length(demuxer_t *demuxer)
{
    ...
    if(demux_control
        ...
        if(sh_video && sh_video->i_bps && sh_audio && sh_audio->i_bps)
            ...
            //  printf("movi_end %lld movi_start %lld i_bps %d \r\n",demuxer->movi_end,demuxer->movi_start,sh_audio->i_bps);
            get_time_ans = (double)(demuxer->movi_end -
                                    demuxer->movi_start) / sh_audio->i_bps;
            //  printf("get_time_ans %f \r\n",get_time_ans);
        } else
            get_time_ans = 0;
    }
    return get_time_ans;
