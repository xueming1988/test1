...
void ao_rtsp_afterPlaying(void* /*clientData*/) {
  // End by closing the media:
    	printf("ao_rtsp_afterPlaying  rtsp_init %d \n",rtsp_init);
  if(rtsp_init)
  {
...
void udpSend(char* IP,int Port,char* send_buf,int send_buf_len)
{
  /* socket文件描述符 */  
  int sock_fd;  
...
static void demux_rtsp_exit_sighandler(int x){
  // close stream
  printf("demux rtsp rtsp_exit_sighandler %d\n",x);
  ao_rtsp_afterPlaying(NULL);
  exit(0);
}


extern "C" int ao_run_rstp(char ** pbuffer,unsigned int * psize,
						unsigned int * pwprt,
						unsigned int * prprt,
  ...
  do {
	...
	printf("ao_run_rstp psize %d  plen %d \n",*psize,*pwprt);
	
	signal(SIGTERM,demux_rtsp_exit_sighandler);
	signal(SIGKILL,demux_rtsp_exit_sighandler);
	signal(SIGINT,demux_rtsp_exit_sighandler);
	signal(SIGABRT,demux_rtsp_exit_sighandler);
	signal(SIGALRM,demux_rtsp_exit_sighandler);
    signal(SIGTERM, demux_rtsp_exit_sighandler); // kill
    signal(SIGHUP, demux_rtsp_exit_sighandler); // kill -HUP  /  xterm closed
    signal(SIGINT, demux_rtsp_exit_sighandler); // Interrupt from keyboard
    signal(SIGQUIT, demux_rtsp_exit_sighandler); // Quit from keyboard
    signal(SIGPIPE, demux_rtsp_exit_sighandler); // Some window managers cause this
#ifdef CONFIG_SIGHANDLER
    // fatal errors:
    signal(SIGBUS, demux_rtsp_exit_sighandler); // bus error
#ifndef __WINE__                      // hack: the Wine executable will crash else
    signal(SIGSEGV, demux_rtsp_exit_sighandler); // segfault
#endif
    signal(SIGILL, demux_rtsp_exit_sighandler); // illegal instruction
    signal(SIGFPE, demux_rtsp_exit_sighandler); // floating point exc.
    signal(SIGABRT, demux_rtsp_exit_sighandler); // abort()
#endif

	printf("signal init  \n");

#if 1
		MplayerFileSource* pcmSource
		  = MplayerFileSource::createNew(*env,pbuffer,psize,pwprt,prprt,pmutex,0 );
		if (pcmSource == NULL) {
			printf("MplayerFileSource createNew failed \n");
			return -1;
		}
		...
		if (sessionState.source == NULL) {
		  printf("MplayerFileSource Unable to create a little->bit-endian order \n");
		  return -1;
		}
		...
		// Create (and start) a 'RTCP instance' for this RTP sink:
		  const unsigned estimatedSessionBandwidth = (44100*32)/1000;
	  ...
	  sessionState.sink->startPlaying(*sessionState.source, ao_rtsp_afterPlaying, NULL);
