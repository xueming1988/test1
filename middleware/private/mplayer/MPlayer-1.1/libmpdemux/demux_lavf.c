...
static demuxer_t* demux_open_lavf(demuxer_t *demuxer){
	...
	if(priv->avif->name)
	{
		mp_msg(MSGT_HEADER,MSGL_ERR,"priv->avif->name %s, url=%s\n",priv->avif->name, demuxer->stream->url);
		if(strcmp("hls,applehttp",priv->avif->name) == 0 || strcmp("aac",priv->avif->name) == 0 || strstr(priv->avif->name,"m4a"))
		{
			//mp_msg(MSGT_HEADER,MSGL_ERR,"set probe size from %d to 1000, opt_analyzeduration from %d to 1\n",opt_probesize, opt_analyzeduration);
    		//opt_probesize = 1000;
    		//opt_analyzeduration = 1;
		}
	}
