...
static int demux_audio_open(demuxer_t* demuxer) {
  ...
  while(n < 30000 && !s->eof) {
    ...
    if( hdr[0] == 'R' && hdr[1] == 'I' && hdr[2] == 'F' && hdr[3] == 'F' ) {
      ...
      frmt = WAV;
      break;
    } else if( found_WAVE && hdr[0] == 'b' && hdr[1] == 'e' && hdr[2] == 'x' && hdr[3] == 't' ) {
		uint32_t size = 0;
		int i;
		for(i = 0; i < 4; i++) {
			uint8_t data = stream_read_char(s);
	...
	else if( hdr[0] == 0x4d && hdr[1] == 0x41 && hdr[2] == 0x43 && hdr[3] == 0x20 ) {
		 frmt = APE;
		 if (!mp3_hdrs || mp3_hdrs->cons_hdrs < 3)
		   break;
	   }
    ...
    found_WAVE = found_WAVE ? found_WAVE : hdr[0] == 'W' && hdr[1] == 'A' && hdr[2] == 'V' && hdr[3] == 'E';
  ...
  switch(frmt) {
  case APE:
	fprintf(stderr, "demux raw audio, find APE\n");
  	sh_audio->format = DEMUXER_TYPE_LAVF;
...
//	    get_flac_metadata (demuxer);
  ...
  //fprintf(stderr, "[demux audio _open] --\n");
  if(APE == frmt)
