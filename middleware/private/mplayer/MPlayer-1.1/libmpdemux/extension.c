...
static struct {
    ...
    { "ashx", DEMUXER_TYPE_PLAYLIST },
    { "smil", DEMUXER_TYPE_PLAYLIST },
    { "ram", DEMUXER_TYPE_PLAYLIST },
    { "m3u8", DEMUXER_TYPE_PLAYLIST },
    ...
    { "ts", DEMUXER_TYPE_MPEG_TS },
#if HAVE_LIBDRADEC
    { "dra", DEMUXER_TYPE_DRA  },
#endif
...
int demuxer_type_by_filename(char *filename)
{
    ...
    char* param = strrchr(dupfilename, '?');
    if(param) param[0] = 0;
    char *extension = strrchr(dupfilename, '.');
    ...
    if(extension) {
        ...
        for(i = 0 ; i < (sizeof(extensions_table) / sizeof(extensions_table[0])) ; i++) {
            int n = strlen(extensions_table[i].extension);
            if( strlen(extension) < n) continue;
            if(!strncasecmp(extension, extensions_table[i].extension, n)) {
