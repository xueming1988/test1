...
#endif
//	fprintf(stderr, "[mov_check_file] 1, len=%lld\n", len);
	...
	//	char* cc = &tmp;
	 // fprintf(stderr, "[mov_check_file] 2, fourcc=%c%c%c%c\n", cc[0], cc[1], cc[2],cc[3]);
	  switch(tmp) {
	    ...
	    case MOV_FOURCC('m','m','p','4'):
	      mp_msg(MSGT_DEMUX,MSGL_INFO,"ISO: File Type Major Brand: Mobile ISO/IEC 14496-1 (MPEG-4 system)\n");
     	      break;
	    case MOV_FOURCC('d','r','a','1'):
	      mp_msg(MSGT_DEMUX,MSGL_INFO,"ISO: File Type Major Brand: dra\n");
     	      return 0;
	    default:
	      tmp = be2me_32(tmp);
...
//skip_chunk:
	...
	fprintf(stderr, "[mov_check_file] end, flag=%d\n", flags);
...
//			    printf("######## audio format = %d ########\n",esds.objectTypeId);
			...
			if(atom_len > 8) {
				...
				if(atom_len >= 36) {
					char swap = sh->codecdata[32];
					sh->codecdata[32] = sh->codecdata[35];
					sh->codecdata[35] = swap;
					swap = sh->codecdata[33];
					sh->codecdata[33] = sh->codecdata[34];
					sh->codecdata[34] = swap;
					memcpy(&sh->samplerate, &sh->codecdata[32], 4);
					mp_msg(MSGT_DEMUX, MSGL_ERR, "MOV2: Found alac atom (%d), samplerate=%d\n", atom_len, sh->samplerate);
				}
			}
		      } break;
...
//wiimu >>>
    ...
    {
		fprintf(stderr, "------%s: stream detect EOF, return!!!-----\n", __func__);
    	return 0; //EOF
	}
//wiimu <<<
