...
//huaijing add resample int
#define AUDIO_SAMPLE_MAX    (32767)
#define AUDIO_SAMPLE_MIN    (-32768)

static int ResampleintFixed(af_data_t* c,af_data_t* l, af_resample_t* s)
{
    int CurrPos = s->pos;
    unsigned int ClientRate = c->rate;
    unsigned int BaseRate = l->rate;
    unsigned int BaseRateInv = s->BaseRateInv;
	int len = 0;
	//fprintf(stderr,"BaseRate %d ClientRate %d \r\n",BaseRate,ClientRate);
    int CurrSamp0 = s->c_0;
    int CurrSamp1 = s->c_1;
    int PrevSamp0 = s->p_0;
    int PrevSamp1 = s->p_1;
    unsigned char *pCurrData =  ((unsigned char*)c->audio);
    unsigned char *pCurrDataEnd = pCurrData+c->len;
	char *pcmDest = ((int32_t*)l->audio);
    while (1)
    {
        while (CurrPos < 0)
        {
            ...
            CurrSamp0 = *(short *)pCurrData;
            pCurrData +=2;
            CurrSamp1 = *(short *)pCurrData;
            pCurrData +=2;
        }
...
static int control(struct af_instance_s* af, int cmd, void* arg)
{
  ...
  case AF_CONTROL_COMMAND_LINE:{
	...
	s->pos= -rate;//huaijing
	s->BaseRateInv = ((unsigned int)((((long long)1<<32)/rate)));//huaijing
	s->c_0=s->c_1=s->p_0=s->p_1=0;//huaijing
