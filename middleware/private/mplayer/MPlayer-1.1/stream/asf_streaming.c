...
static int asf_streaming_start( stream_t *stream, int *demuxer_type) {
	...
	!strcasecmp(proto, "mmshttp"))
    {
		...
		if( fd==-100) //redirect
		{
			//fd = asf_streaming_start(stream, demuxer_type);
			mp_msg(MSGT_NETWORK,MSGL_ERR,"%s-%d: fd %d=====debug\n",__func__,__LINE__,fd);
			return fd;
		}
		mp_msg(MSGT_NETWORK,MSGL_ERR,"  ===> ASF/HTTP failed\n");
	}
	...
	if( 	!strcasecmp(content_type, "application/octet-stream") ||
		...
		if( http_hdr->body_size>sizeof(ASF_obj_header_t) ) {
			if( asf_header_check( http_hdr )==0 ) {
				...
				return ASF_Unknown_e;//ASF_Redirector_e;
			}
		} else {
...
static int asf_http_parse_response(stream_t *stream,asf_http_streaming_ctrl_t *asf_http_ctrl, HTTP_header_t *http_hdr ) {
	...
	switch( http_hdr->status_code ) {
		...
		case 307: // Temporarily (since HTTP/1.1)
			...
			if (next_url != NULL && rd_url != NULL) {
				mp_msg(MSGT_NETWORK,MSGL_ERR,"Redirected: Using this url instead %s\n",next_url);
...
//printf("read: %d\n", i );
		...
		ret = asf_http_parse_response(stream,asf_http_ctrl, http_hdr);
		if( ret<0 ) {
...
static int open_s(stream_t *stream,int mode, void* opts, int* file_format) {
	...
	ret = asf_streaming_start(stream, file_format);
