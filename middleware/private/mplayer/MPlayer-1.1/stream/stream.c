...
#endif
	mp_msg(MSGT_OPEN,MSGL_ERR, "sinfo->open failed s->url(%s)\n",s->url);
    free(s->url);
    free(s);
    return NULL;
  }
  ...
  for(i = 0 ; auto_open_streams[i] ; i++) {
	...
	*file_format = DEMUXER_TYPE_UNKNOWN;
	...
	if(r == STREAM_REDIRECTED && redirected_url) {
	  mp_msg(MSGT_OPEN,MSGL_ERR, "[%s] open %s redirected to %s\n",
...
int stream_read_internal(stream_t *s, void *buf, int len)
{
  ...
  //mp_msg(MSGT_NETWORK,MSGL_V,"stream_read_internal %d \n",len);
  switch(s->type){
  case STREAMTYPE_STREAM:
#ifdef CONFIG_NETWORKING
    if( s->streaming_ctrl!=NULL && s->streaming_ctrl->streaming_read ) {
	//  printf("s->end_pos %lld \r\n",s->end_pos);
	  if(s->end_pos &&(s->pos + len > s->end_pos))
	  {
		 len = s->end_pos-s->pos;
	  }	
      len=s->streaming_ctrl->streaming_read(s->fd, buf, len, s->streaming_ctrl);
	 //  printf("s->end_pos %lld \r\n",s->end_pos);
	 //   printf("s->streaming_ctrl->status %d \r\n",s->streaming_ctrl->status);
...
int stream_seek_long(stream_t *s,off_t pos){
  int res;
	...
	//fprintf(stderr, "seek_long to 0x%X\n",(unsigned int)pos);
...
static stream_t* hls_stream = NULL;
extern int read_hls_data(void *opaque, uint8_t *buf, int buf_size);
static int hls_fill_buffer(stream_t *s, char *buffer, int max_len)
{
    int r = read_hls_data(s->priv, buffer, max_len);
    if (AVERROR_EOF == r) {
        s->eof = 1;
    }
    return (r <= 0) ? -1 : r;
...
int hls_cache_read(void *opaque, uint8_t *buf, int size) 
{
	 int ret = -1;
	if(hls_stream)
    	ret = stream_read(hls_stream, buf, size);
	
    return ret;
}

void hls_stream_cache_enable(void *opaque)
{

	hls_stream = new_stream(-1,STREAMTYPE_FILE);
	hls_stream->fill_buffer = hls_fill_buffer;
	hls_stream->priv = (void *)opaque;
	
	stream_enable_cache(hls_stream, 1024 * 1024ull,
			1024 * 1024ull * (3.0 / 100.0),
			1024 * 1024ull * (3.0 / 100.0));


}
void hls_stream_cache_free(void)
{	
	if(hls_stream)
		free_stream(hls_stream);
	hls_stream = NULL;
}
