...
const mime_struct_t mime_type_table[] = {
	...
	{ "audio/scpls", DEMUXER_TYPE_PLAYLIST },
	{ "audio/x-mpegurl", DEMUXER_TYPE_PLAYLIST },
	{ "audio/x-pls", DEMUXER_TYPE_PLAYLIST },
	{ "application/pls+xml", DEMUXER_TYPE_PLAYLIST },
	// Real Media
...
http_send_request( URL_t *url, off_t pos ) {
	...
	if (network_useragent)
	    ...
	    snprintf(str, sizeof(str), "User-Agent: %s", "MPlayer 1.1");
        http_set_field(http_hdr, str);
	...
	//http_set_field( http_hdr, "ache-Control: no-cache");
	http_set_field( http_hdr, "Connection: close");//for Qplay set Keep-Alive
	if (proxy)
...
//	printf("http_hdr->buffer %s",http_hdr->buffer);
...
http_seek( stream_t *stream, off_t pos ) {
	...
	mp_msg(MSGT_NETWORK,MSGL_ERR,"http_seek %lld\n",pos);
	//fprintf(stderr, ">>>>>>>>>>>>>>>>>>>>>>> stream http seek enter\n");
...
static int http_get_line( int fd, char *line, int line_size,streaming_ctrl_t *stream_ctrl)
{
    ...
    for(;;) {
		ch = 0;
        len = nop_streaming_read(fd,&ch,1,stream_ctrl);
		if(len == 0)
			return 0;
		//printf("ch %c \r\n",ch);
        if (ch < 0)
...
//modify by huaijing
int
nop_streaming_read_chunked( int fd, char *buffer, int size, streaming_ctrl_t *stream_ctrl ) 
{
	int len=0;
	...
	len = nop_streaming_read(fd,buffer,size,stream_ctrl);
...
//	mp_msg(MSGT_NETWORK,MSGL_ERR,"nop_streaming_read ++ %d %d \n",size,stream_ctrl->buffer_size);
...
//printf("buffer %s \r\n",buffer);
	...
	//mp_msg(MSGT_NETWORK,MSGL_ERR,"nop_streaming_read %d \n",len);
...
nop_streaming_seek( int fd, off_t pos, streaming_ctrl_t *stream_ctrl ) {
	//fprintf(stderr, ">>>>>>>>>>>>>nop_streaming_seek unsupport\n");
	//return -1;
	return 0;
}
