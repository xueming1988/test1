...
typedef struct {
  ...
  int64_t cache_limit;  // we should fill buffer only if space>=fill_limit
...
static int cache_read(cache_vars_t *s, unsigned char *buf, int size,int *ispause)
{
  int total=0;
  int sleep_count = 0;
  int64_t last_max = s->max_filepos;
  int input_ret = 0;
  int pause_try_wait = 0;
  *ispause = 0;
  while(size>0){
    int64_t pos,newb,len;

 // printf("CACHE2_READ: %lld <= %lld <= %lld <= %lld \n",s->min_filepos,s->read_filepos,s->max_filepos,s->stream->end_pos);
		...
		input_ret = stream_check_interrupt(READ_SLEEP_TIME);
		if (input_ret == 1) {
			s->eof = 1;
			break;
		}else if(input_ret == 2)
		{
			if(log_count++ == 100)
			{
				mp_msg(MSGT_CACHE,MSGL_STATUS, "[%lld] cache read in pausing state\n", tickSincePowerOn());
				log_count = 0;
			}
			*ispause = 1;
			usleep(10000);
		...
		//huaijing cache modify 20150210
		if(s->stream->end_pos > 0)
	    {
	    	while(1)
	    	{
				if(s->max_filepos - s->read_filepos >= 0 &&  s->max_filepos - s->read_filepos < s->cache_limit
					&& s->stream->end_pos - s->max_filepos > s->cache_limit)
				{
					if(log_count++ == 10)
					{
						mp_msg(MSGT_CACHE,MSGL_STATUS, "[%lld] Cache fill: %5.2f%% (%lld bytes)\n", tickSincePowerOn(),
...
extern long long master_get_ntp_time(void);
long debug_last_ts  = 0;
...
#if 0
 	{
 		long debug_ts = master_get_ntp_time();
	  if(debug_ts > debug_last_ts + 2000 )
	  {
		   mp_msg(MSGT_CACHE, MSGL_WARN, "cache_pos=%d, read_pos=%d, buffered=%d\n",
		   		(int)s->max_filepos, (int)s->read_filepos, (int)(s->max_filepos-s->read_filepos));
		   debug_last_ts = debug_ts;
	  }
 	}
#endif
  return len;
...
static cache_vars_t* cache_init(int64_t size,int sector){
  ...
  //printf("sector = %d size = %lld \r\n",sector,size);
  s->buffer_size=num*sector;
  //printf(" s->buffer_size = %lld\r\n",s->buffer_size);
...
static void exit_sighandler(int x){
  wiimu_log(1,0,0,0, "cache2 exit_sighandler %d", x);
...
int stream_enable_cache(stream_t *stream,int64_t size,int64_t min,int64_t seek_limit){
  ...
  //printf("1 seek_limit = %lld mini size = %lld \r\n",seek_limit,min);
  ...
  if (min <= 0)
    min = 1;
  s->cache_limit = min;
  //printf("3 seek_limit = %lld mini size = %lld \r\n",s->seek_limit,min);
...
#else
  {
    ...
    while(s->read_filepos <s->min_filepos || s->max_filepos -s->read_filepos < min)
	{
		if(log_count++ == 5)
		{
			mp_msg(MSGT_CACHE,MSGL_STATUS, "[TICK %lld] Cache fill: %5.2f%% (%lld bytes)\n", tickSincePowerOn(),
			    100.0*(float)(s->max_filepos-s->read_filepos)/(float)(s->buffer_size),
			  	  s->max_filepos-s->read_filepos	);
			log_count = 0;
		}
	...
	//time out
	if(GetTimerMS() - startTime > CACHE_TIMEOUT)
	{
		break;
	}
    input_ret = stream_check_interrupt(PREFILL_SLEEP_TIME);
	if(input_ret == 1) {
	  res = 0;
	  goto err_out;
        }
    ...
    mp_msg(MSGT_CACHE,MSGL_STATUS, "[TICK %lld] stream_enable_cache, fill: %5.2f%% (%lld bytes)\n", tickSincePowerOn(),
        100.0*(float)(s->max_filepos-s->read_filepos)/(float)(s->buffer_size),
        s->max_filepos-s->read_filepos);
...
int cache_stream_fill_buffer(stream_t *s){
  ...
  len=cache_read(s->cache_data,s->buffer, sector_size,&pause);
