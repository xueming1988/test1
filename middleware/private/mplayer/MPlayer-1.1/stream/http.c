...
extern void SetHttpErrorCode(int error_code);
static unsigned int my_read_I(int fd, char *buffer, int len, streaming_ctrl_t *sc);

static int http_get_line( int fd, char *line, int line_size,streaming_ctrl_t *stream_ctrl)
{
    char ch;
    char *q;
	int len = 0;
    ...
    for(;;) {
		ch = 0;
        //len = scast_streaming_read(fd,&ch,1,stream_ctrl);
		len = my_read_I(fd, &ch, 1, stream_ctrl);
		if(len == 0){
			printf("%d: ch %02x \r\n",__LINE__,ch);
			return 0;
		}
...
static int scast_streaming_read_chunked(int fd,char * buffer,int size,streaming_ctrl_t * stream_ctrl)
{
	int len=0;
	...
	if (stream_ctrl->chunksize >= 0) {
		if (!stream_ctrl->chunksize) {
			...
			for(;;) {
				do {
					if ((err = http_get_line(fd, line, sizeof(line),stream_ctrl)) <= 0)
					return err;	
				} while (!*line);    /* skip CR LF from last chunk */
				stream_ctrl->chunksize = strtoll(line, NULL, 16);
				// av_dlog(NULL, "Chunked encoding data size: %"PRId64"'\n", stream_ctrl->chunksize);

				//printf("stream_ctrl->chunksize %d \n",stream_ctrl->chunksize);
				if (!stream_ctrl->chunksize)
				{
				printf("stream_ctrl->chunksize %d \n",stream_ctrl->chunksize);
		...
		size = FFMIN(size,  stream_ctrl->chunksize);
	}
	len = my_read_I(fd,buffer,size,stream_ctrl);
...
static unsigned int my_read_I(int fd, char *buffer, int len, streaming_ctrl_t *sc) {
  unsigned int pos = 0;
  int cp_len = sc->buffer_size - sc->buffer_pos;
...
static int my_read(int fd, char *buffer, int len, streaming_ctrl_t *sc)
{
   return scast_streaming_read_chunked(fd,buffer,len,sc);
}
/**
...
static void SendMsgQueue(char *msg)
{
	...
	if(g_msg_id != -1)
	{
		ret=msgsnd(g_msg_id, &msgbuf,sizeof(struct msgmbuf)-sizeof(long),IPC_NOWAIT);
		if(ret == -1)
       {
            printf("msgsnd message err\n");
            return;
       }
	}
}
 ...
 * \param sc streaming_ctrl_t whose buffer is consumed before reading from fd
 */
static  int scast_meta_read(int fd, streaming_ctrl_t *sc) {
  unsigned char tmp = 0;
  int metalen;
  int nlen = 0;
  int no_ascii_count = 0;
  nlen = my_read(fd, &tmp, 1, sc);
  if(nlen != 1)
  {
	 fprintf(stderr,"my_read error %d \r\n",nlen);
	 return -1;
  }
  metalen = tmp * 16;
  if (metalen > 0) {
    ...
    nlen = my_read(fd, info, metalen, sc);
	if(nlen < 0)
	{
		fprintf(stderr,"my_read error %d \r\n",nlen);
		free(info);
    ...
    info[nlen] = 0;
/*
	if(no_ascii_count >= nlen/3)
	{
		fprintf(stderr,"ICY Info error %d \r\n",no_ascii_count);
		free(info);
		return -1;
	}
*/
    mp_msg(MSGT_DEMUXER, MSGL_INFO, "\nICY Info: %s\n", info);
#ifndef IHEART_MODULE_REMOVE
	SendMsgQueue(info);
#endif
    free(info);
  }
...
static int scast_set_loop_flag()
{
	static char *tshmem = NULL;
	
	fprintf(stderr,"scast_streaming_seek, set scast flag\r\n");

	if (NULL == tshmem)
	{
		int shmid=shmget( 34387, 0, 0);
		if(shmid != -1)
			tshmem = shmat( shmid,( const void* )0,0 );

		if (NULL == tshmem){
			fprintf(stderr,"[%s,%d], fail to set scast flag, errno:%d,%s\r\n", errno, strerror(errno));
		}
	}
	
	if (tshmem)
	{
		share_context *pshare = (share_context *)tshmem;
		pshare->loopmode = 1;
	}
...
static int scast_streaming_seek( stream_t *stream, off_t pos ) 
{
	scast_set_loop_flag();
	return -1;
	...
	fprintf(stderr,"scast_streaming_seek \r\n");
	...
	if( http_hdr ) {
		 scast_data_t *scast_data = stream->streaming_ctrl->data;
		if(!scast_data->is_ultravox)
		{
			int metaint = atoi(http_get_field(http_hdr, "Icy-MetaInt"));
			if(metaint > 0)
	...
	stream->streaming_ctrl->buffer_pos = 0;//reset buffer pos
	stream->pos = pos;
	return 1;
#endif	
}
...
static int scast_streaming_read(int fd, char *buffer, int size,
  ...
  while (done < size) { // now comes the metadata
    ...
    {
      meta_len = scast_meta_read(fd, sc); // read and display metadata
      if(meta_len < 0)
      {
      	 fprintf(stderr,"scast_meta_read error %d \r\n",meta_len);
		 break;
      }
    }
   // if(sc->chunksize > 0)
...
static int scast_streaming_start(stream_t *stream) {
  ...
  stream->seek = scast_streaming_seek;
...
static int nop_streaming_start( stream_t *stream ) {
	...
	stream->streaming_ctrl->streaming_read = nop_streaming_read_chunked;//huaijing fixed chunked mode
...
http_build_request( HTTP_header_t *http_hdr ) {
	...
	ptr = http_hdr->buffer;
	// Add the method line
	ptr += sprintf( ptr, "%s %s HTTP/1.%d\r\n", http_hdr->method, uri, 1);//http_hdr->http_minor_version );
	field = http_hdr->first_field;
	// Add the field
	while( field!=NULL ) {
		ptr += sprintf( ptr, "%s\r\n", field->field_name );
...
http_debug_hdr( HTTP_header_t *http_hdr ) {
	...
	//fprintf(stderr,"--- HTTP DEBUG HEADER --- START ---\n");
	//fprintf(stderr,"protocol:           [%s]\n", http_hdr->protocol );
	//fprintf(stderr,"http minor version: [%d]\n", http_hdr->http_minor_version );
	...
	//	field = field->next;
	//}
	//fprintf(stderr,"--- HTTP DEBUG HEADER --- END ---\n");
}
...
static int http_streaming_start(stream_t *stream, int* file_format) {
	...
	do
	{
		...
		if( http_hdr!=NULL && http_hdr->status_code==200 ) {
			...
			const char *TransferEncode = http_get_field(http_hdr,"Transfer-Encoding");
            int length;
			content_length = http_get_field(http_hdr, "Content-Length");
            const char *dlna = http_get_field(http_hdr, "contentFeatures.dlna.org");
			if (content_length) {
				length = atoll(content_length);
				seekable = 1;//huaijing
			}
		    if (accept_ranges)
			{
				...
				if(!seekable)
					seekable = strncmp(accept_ranges,"*/*",3)==0;
		    }
            ...
            else if(length > 0 && dlna && (strncmp(dlna, "DLNA", 4) == 0))
            {
				// HACK for sumsang DLNA server incorrectly claiming not to support seeking
				mp_msg(MSGT_NETWORK, MSGL_WARN, "Broken webserver, incorrectly claims to not support Accept-Ranges\n");
				seekable = 1;
            }
		
			if(TransferEncode && strncmp(TransferEncode,"chunked",7) == 0)
			{
				printf("chunked \r\n");
				stream->streaming_ctrl->chunksize = 0;
			}
			
			if(!seekable)
			{
				if (server && strstr(server, "UPnP"))
					seekable = 1;
			}
		}
		...
		if( !strcasecmp(http_hdr->protocol, "ICY") ||
			...
			switch( http_hdr->status_code ) {
				case 200: { // OK
					...
					//fprintf(stderr, "[Http file format check] %d --\n", *file_format);
					res = STREAM_ERROR;
					goto out;
				}
			...
			case 200: // OK
				...
				if( content_type!=NULL ) {
					...
					for (i = 0; mime_type_table[i].mime_type != NULL; i++) {
						if( !strncasecmp( content_type, mime_type_table[i].mime_type, strlen(mime_type_table[i].mime_type) ) ) {
			...
			case 307: // Temporarily (since HTTP/1.1)
				...
				if( next_url!=NULL ) {
					...
					if (strcasecmp(url->protocol, "http")) {
						mp_msg(MSGT_NETWORK,MSGL_ERR,"Supported http %d redirect to %s protocol\n", http_hdr->status_code, url->protocol);
						res = STREAM_REDIRECTED;
						goto err_out;
					}
					if (is_ultravox)  {
...
err_out:
	...
	fd = -1;
	if(http_hdr)
		SetHttpErrorCode(http_hdr->status_code);
	http_free( http_hdr );
	http_hdr = NULL;
out:
...
static int fixup_open(stream_t *stream,int seekable) {
	...
	wiimu_log(1,0,0,0, ">>>>>>>>>>>>>>>>>>fixup_open, %d-%d %d", is_icy, is_ultravox, seekable);
...
static int open_s1(stream_t *stream,int mode, void* opts, int* file_format) {
	...
	stream->streaming_ctrl->bandwidth = network_bandwidth;
	stream->streaming_ctrl->chunksize = -1;
	url = url_new(stream->url);
	stream->streaming_ctrl->url = check4proxies(url);
	url_free(url);

	mp_msg(MSGT_OPEN, MSGL_V, "STREAM_HTTP(1), URL: %s\n", stream->url);
	wiimu_log(1,0,0,0, "STREAM_HTTP(1), URL: %s", stream->url);
...
static int open_s2(stream_t *stream,int mode, void* opts, int* file_format) {
...
#if 0
	if(stream->streaming_ctrl->url &&  stream->streaming_ctrl->url->hostname)
	{
		if(strncasecmp(stream->streaming_ctrl->url->protocol,"http",strlen("http")) == 0)
		{
			sprintf(buf,"sed -i '/%s/d' /tmp/hosts",  stream->streaming_ctrl->url->hostname);
			fprintf(stderr,"%s \r\n",buf);
			system(buf);
		}
	}
	...
	//fprintf(stderr, "STREAM_HTTP(2), URL: %s\n", stream->url);
