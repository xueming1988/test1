...
//#define APP_TRACE_NODEBUG 1
enum {
    ALREADY_CONNECTING = -4,
    BT_DEVICES_NO_HISTORY = -3,
    BT_HISTORY_NOT_FOUND = -2,
    BT_CONNECTED_FAILED = -1,
    BT_CONNECTED_SUCCESSED = 0
};
...
#define APP_DEBUG0(format)                                                                         \
    do {                                                                                           \
        if (g_debug_level & DEBUG_LEVEL_0_ENABLE) {                                                \
...
#define APP_DEBUG1(format, ...)                                                                    \
    do {                                                                                           \
        if (g_debug_level & DEBUG_LEVEL_1_ENABLE) {                                                \
            app_print_debug("%s: " format "\n", __func__, __VA_ARGS__);                            \
        }                                                                                          \
    } while (0)
...
#define APP_INFO0(format)                                                                          \
    do {                                                                                           \
        if (g_debug_level & DEBUG_LEVEL_2_ENABLE) {                                                \
...
#define APP_INFO1(format, ...)                                                                     \
    do {                                                                                           \
        if (g_debug_level & DEBUG_LEVEL_3_ENABLE) {                                                \
            app_print_info(format "\n", __VA_ARGS__);                                              \
        }                                                                                          \
    } while (0)
