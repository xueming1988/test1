...
const char *app_service_id_to_string(tBSA_SERVICE_ID serviceId)
{
    ...
    case BSA_FTP_SERVICE_ID:
        return "FTP";
    case BSA_CTP_SERVICE_ID:
        return "CTP";
    case BSA_ICP_SERVICE_ID:
        return "ICP";
 ...
 **
 *******************************************************************************/
char *app_service_mask_to_string(tBSA_SERVICE_MASK serviceMask, char *profile)
{
    /* Clear the entire string */
    memset(services_string, 0, sizeof(services_string));

    if (serviceMask & BSA_RES_SERVICE_MASK)
        strncat(services_string, "RESERVED!!! ",
                sizeof(services_string) - strlen(services_string) - 1);
    ...
    if (serviceMask & BSA_A2DP_SRC_SERVICE_MASK) {
        strncat(services_string, "A2DP_SRC ",
                sizeof(services_string) - strlen(services_string) - 1);
        if (profile)
            strcpy(profile, "Audio Source");
    }
    if (serviceMask & BSA_AVRCP_TG_SERVICE_MASK)
        strncat(services_string, "AVRCP_TG ",
                sizeof(services_string) - strlen(services_string) - 1);
    ...
    if (serviceMask & BSA_CTP_SERVICE_MASK)
        strncat(services_string, "CTP ", sizeof(services_string) - strlen(services_string) - 1);
    if (serviceMask & BSA_ICP_SERVICE_MASK)
        strncat(services_string, "ICP ", sizeof(services_string) - strlen(services_string) - 1);
    ...
    if (serviceMask & BSA_HID_DEVICE_SERVICE_MASK)
        strncat(services_string, "HID_DEVICE ",
                sizeof(services_string) - strlen(services_string) - 1);
