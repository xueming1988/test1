...
void app_mgr_display_main_menu(void)
{
    ...
    APP_INFO1("\t%d => Change Dual Stack Mode (currently:%s)", APP_MGR_MENU_DUAL_STACK,
              app_mgr_get_dual_stack_mode_desc());
...
#endif
int app_manager_init(const char *bt_name, UINT8 *btaddr)
{
    int choice;
    int i;
    unsigned int afh_ch1, afh_ch2, passkey;
...
int linkplay_bluetooth_unpair_remote_device(BD_ADDR *addr)
{
    ...
    for (index = index; index < (APP_NUM_ELEMENTS(app_xml_remote_devices_db) - 1); index++) {
        if (app_xml_remote_devices_db[index + 1].in_use != FALSE) {
            memcpy(&app_xml_remote_devices_db[index], &app_xml_remote_devices_db[index + 1],
                   sizeof(tAPP_XML_REM_DEVICE));
        } else {
            memset(&app_xml_remote_devices_db[index], 0, sizeof(tAPP_XML_REM_DEVICE));
        }
    }
...
int app_mgr_get_bt_addr(char *addr)
{
    ...
    APP_INFO1("Bdaddr %02x:%02x:%02x:%02x:%02x:%02x", bt_config.bd_addr[0], bt_config.bd_addr[1],
              bt_config.bd_addr[2], bt_config.bd_addr[3], bt_config.bd_addr[4],
              bt_config.bd_addr[5]);
    if (addr) {
        snprintf(addr, 18, "%02x:%02x:%02x:%02x:%02x:%02x", bt_config.bd_addr[0],
                 bt_config.bd_addr[1], bt_config.bd_addr[2], bt_config.bd_addr[3],
                 bt_config.bd_addr[4], bt_config.bd_addr[5]);
    }
    return 0;
}

int linkplay_bluetooth_get_addr(char *addr) { return app_mgr_get_bt_addr(addr); }
