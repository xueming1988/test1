...
void *cmsMem_alloc(int size, int flag) { return malloc(size); }

void cmsMem_free(void *ptr) { free(ptr); }
...
void xmlClose(nxml_t handle) { cmsMem_free(handle); }
...
need for a copy. */
static void nxml_add_namecache(nxml_t handle, char *data, int len, char **data_nocopy,
                               unsigned *len_nocopy)
{
...
int xmlWrite(nxml_t handle, char *data, unsigned len, char **endp)
{
    ...
    while ((data < enddata) && (!treeEnd)) {
        ...
        //        cmsLog_debug(" ---- state %d, %c, %d", handle->state, *data,
        //        handle->namecachesize);
