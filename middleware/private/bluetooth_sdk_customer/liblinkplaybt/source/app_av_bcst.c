 ...
 **
 **  Copyright (c) 2012-2014, Broadcom Corp., All Rights Reserved.
...
static void app_av_bcst_print_statistic(tBSA_AV_HNDL handle, UINT32 *p_byte_sends,
                                        struct timespec *p_timestamp_begin,
                                        struct timespec *p_timestamp_end);
...
int app_av_bcst_init(void)
{
    ...
    for (stream = 0; stream < APP_AV_BCST_CONNECTIONS_MAX; stream++) {
        ...
        app_av_bcst_cb.stream[stream].uipc_config.length =
            sizeof(UINT16) * APP_AV_BCST_AUDIO_BUF_MAX;
    }
...
int app_av_bcst_register(void)
{
    ...
    BSA_AvRegisterInit(&register_param);
    register_param.channel = BSA_AV_CHNL_AUDIO_BCST;
    for (register_param.lt_addr = 1; register_param.lt_addr <= APP_AV_BCST_LT_ADDR_MAX;
         register_param.lt_addr++) {
    ...
    APP_INFO1("Broadcast AV Handle:%d UIPC:%d Registered", register_param.handle,
              register_param.uipc_channel);
...
int app_av_bcst_play_file(void)
{
    ...
    if (app_wav_format(app_av_get_soundfile_path(choice), &wav_format) < 0) {
        ...
        APP_INFO1("    codec(%s) ch(%d) bits(%d) rate(%d)",
                  (wav_format.codec == BSA_AV_CODEC_PCM) ? "PCM" : "apt-X", wav_format.nb_channels,
                  wav_format.bits_per_sample, (int)wav_format.sample_rate);
    }
    ...
    strncpy(p_stream->file_name, app_av_get_soundfile_path(choice),
            sizeof(p_stream->file_name) - 1);
...
void app_av_bcst_start_event_hdlr(tBSA_AV_START_MSG *p_msg)
{
    ...
    if (p_msg->status == BSA_SUCCESS) {
        ...
        default:
            APP_ERROR1("Unsupported feeding format code: %d", p_stream->media_feeding.format);
...
static void app_av_bcst_thread(void)
{
    ...
    if (p_stream->uipc_config.is_blocking) {
        ...
        APP_DEBUG1("UIPC Non Blocking mode Length:%d Period:%d(us)", p_stream->uipc_config.length,
                   p_stream->uipc_config.period);
    }
    ...
    do {
        ...
        case APP_AV_BCST_PLAYTYPE_TONE:
            ...
            app_av_read_tone(p_stream->audio_buf, nb_bytes, p_stream->sinus_type,
                             &p_stream->sinus_index);
            break;

        case APP_AV_BCST_PLAYTYPE_FILE:
            if (file_desc < 0)
                break;
            nb_bytes = app_wav_read_data(file_desc, &wav_format, (char *)p_stream->audio_buf,
...
#ifdef PCM_ALSA
            nb_bytes =
                app_alsa_capture_read((char *)p_stream->audio_buf, p_stream->uipc_config.length);
        ...
        if (nb_bytes <= 0) {
            ...
            if (UIPC_Send(p_stream->uipc_channel, 0, (UINT8 *)p_stream->audio_buf, nb_bytes) ==
                FALSE) {
            ...
            byte_sends += nb_bytes;
            app_av_bcst_print_statistic(p_stream->handle, &byte_sends, &timestamp_begin,
                                        &timestamp_end);
        }
                                 ...
                                 struct timespec *p_timestamp_end)
{
    ...
    if ((p_timestamp_end->tv_sec - p_timestamp_begin->tv_sec) >= APP_AV_BCST_STAT_PERIOD) {
        ...
        delta += (float)diff;
        APP_INFO1("Stream:%d PCM throughput=%d B/s", handle,
                  (int)((float)*p_byte_sends / (float)delta));
...
tAPP_AV_BCST_STREAM *app_av_bcst_stream_get_by_handle(tBSA_AV_HNDL handle)
{
    ...
    for (stream = 0; stream < APP_AV_BCST_CONNECTIONS_MAX; stream++, p_stream++) {
        if ((p_stream->in_use) && (p_stream->handle == handle)) {
            return p_stream;
        }
    }
    return NULL;
...
tAPP_AV_BCST_STREAM *app_av_bcst_stream_get_by_uipc(tUIPC_CH_ID uipc_channel)
{
    ...
    for (stream = 0; stream < APP_AV_BCST_CONNECTIONS_MAX; stream++, p_stream++) {
        if ((p_stream->in_use) && (p_stream->uipc_channel == uipc_channel)) {
            return p_stream;
        }
    }
    return NULL;
...
int app_av_bcst_stream_display(void)
{
    ...
    for (stream = 0; stream < APP_AV_BCST_CONNECTIONS_MAX; stream++, p_stream++) {
        if (p_stream->in_use) {
            APP_INFO1("Stream:%d LtAddr:%d Started:%d UIPC:%d", p_stream->handle, p_stream->lt_addr,
...
int app_av_bcst_ask_uipc_config(void)
{
    ...
    if ((p_stream->media_feeding.cfg.pcm.bit_per_sample == 0) ||
        ...
        APP_INFO0("Broadcast AV not yet started => let's use default values as feeding example "
...
BOOLEAN app_av_bcst_is_audio_started(void)
{
    ...
    for (stream = 0; stream < APP_AV_BCST_CONNECTIONS_MAX; stream++, p_stream++) {
        ...
        if ((p_stream->in_use) && (p_stream->is_started)) {
            return TRUE;
        }
    }
    /* No Stream Started */
