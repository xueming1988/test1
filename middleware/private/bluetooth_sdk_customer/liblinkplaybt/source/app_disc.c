...
static tAPP_VENDOR app_vendor_apple[APP_VENDOR_APPLE_COUNT] = {{76, VENDOR_SOURCE_BT},
 ...
 **
 *******************************************************************************/
static void app_disc_parse_eir_uuid16(char *p_type, UINT8 *p_eir, UINT8 data_length, UINT8 *profile)
{
    UINT16 uuid16;
    char *p_desc;
    ...
    while (data_length >= sizeof(UINT16)) {
        ...
        if (p_desc != NULL) {
            if (((0x110A == uuid16) || (0x110B == uuid16)) && profile)
                strcpy(profile, p_desc);
...
static void app_disc_parse_eir_uuid128(char *p_type, UINT8 *p_eir, UINT8 data_length)
{
    ...
    while (data_length >= sizeof(UINT32)) {
        ...
        snprintf(buffer, sizeof(buffer),
                 ...
                 uuid128[0], uuid128[1], uuid128[2], uuid128[3], uuid128[4], uuid128[5], uuid128[6],
                 uuid128[7], uuid128[8], uuid128[9], uuid128[10], uuid128[11], uuid128[12],
                 uuid128[13], uuid128[14], uuid128[15]);
        APP_INFO1("%s", buffer);
    }
}
...
static void app_disc_parse_eir_ble_appearance(UINT8 *p_eir, UINT8 data_length)
{
    ...
    APP_INFO1("\t    BLE Appearance:0x%04X [%s - %s]", key, p_appearance_desc->desc,
              p_appearance_desc->sub_desc);
}
...
static void app_disc_parse_eir_cod(UINT8 *p_eir, UINT8 data_length)
{
    ...
    APP_INFO1("\t    ClassOfDevice:%02x:%02x:%02x => %s", class_of_device[0], class_of_device[1],
              class_of_device[2], app_get_cod_string(class_of_device));
}
...
static void app_disc_parse_eir_manuf_specific(UINT8 *p_eir, UINT8 data_length)
{
    ...
    while (data_length) {
        ...
        for (index = 0; index < line_length; index++) {
            ...
            data_length--;
            snprintf(hex_buffer, sizeof(hex_buffer), "%02X ", byte);
 ...
 **
 *******************************************************************************/
void app_disc_parse_eir(UINT8 *p_eir, UINT8 *profile)
{
    UINT8 *p = p_eir;
    UINT8 eir_length;
    UINT8 eir_tag;
    ...
    while (1) {
        ...
        case HCI_EIR_MORE_16BITS_UUID_TYPE:
            app_disc_parse_eir_uuid16("Incomplete Service", p, eir_length, profile);
            break;
        case HCI_EIR_COMPLETE_16BITS_UUID_TYPE:
            app_disc_parse_eir_uuid16("Complete Service", p, eir_length, profile);
            break;
...
void app_generic_disc_cback(tBSA_DISC_EVT event, tBSA_DISC_MSG *p_data)
{
    ...
    case BSA_DISC_NEW_EVT:
        ...
        APP_INFO1("\tBdaddr:%02x:%02x:%02x:%02x:%02x:%02x", p_data->disc_new.bd_addr[0],
                  p_data->disc_new.bd_addr[1], p_data->disc_new.bd_addr[2],
                  p_data->disc_new.bd_addr[3], p_data->disc_new.bd_addr[4],
                  p_data->disc_new.bd_addr[5]);
        APP_INFO1("\tName:%s", p_data->disc_new.name);
        APP_INFO1("\tClassOfDevice:%02x:%02x:%02x => %s", p_data->disc_new.class_of_device[0],
                  p_data->disc_new.class_of_device[1], p_data->disc_new.class_of_device[2],
                  app_get_cod_string(p_data->disc_new.class_of_device));
        APP_INFO1("\tServices:0x%08x (%s)", (int)p_data->disc_new.services,
                  app_service_mask_to_string(p_data->disc_new.services, NULL));
        ...
        APP_INFO1("\tinq_result_type:%d ble_addr_type:%d device_type:%d",
                  p_data->disc_new.inq_result_type, p_data->disc_new.ble_addr_type,
    ...
    case BSA_DISC_DEV_INFO_EVT: /* Discovery Device Info */
        ...
        APP_INFO1("\tBdaddr:%02x:%02x:%02x:%02x:%02x:%02x", p_data->dev_info.bd_addr[0],
                  p_data->dev_info.bd_addr[1], p_data->dev_info.bd_addr[2],
                  p_data->dev_info.bd_addr[3], p_data->dev_info.bd_addr[4],
                  p_data->dev_info.bd_addr[5]);
        if (p_data->dev_info.status == BSA_SUCCESS) {
            UINT8 dev_platform =
                app_get_dev_platform(p_data->dev_info.vendor, p_data->dev_info.vendor_id_source);
    ...
    if (app_disc_cb.p_disc_cback) {
        app_disc_cb.p_disc_cback(event, &app_discovery_cb);
    }
}
...
int app_disc_start_regular(tBSA_DISC_CBACK *p_custom_disc_cback)
{
    ...
    disc_start_param.services = BSA_A2DP_SRC_SERVICE_MASK | BSA_A2DP_SERVICE_MASK;

    /* Save the provided custom callback */
    app_disc_cb.p_disc_cback = p_custom_disc_cback;

    memset(&app_discovery_cb, 0, sizeof(tAPP_DISCOVERY_CB));
...
int app_disc_start_ble_regular(tBSA_DISC_CBACK *p_custom_disc_cback)
{
    ...
    disc_start_param.duration = 4;
    disc_start_param.mode = BSA_BLE_GENERAL_INQUIRY;
#if defined(ORION_OS_MODULE)
    disc_start_param.duration = 40;
    disc_start_param.update = BSA_DISC_UPDATE_ANY;
    disc_start_param.cback = p_custom_disc_cback;
#endif
    /* Save the provided custom callback */
    app_disc_cb.p_disc_cback = p_custom_disc_cback;

    memset(&app_discovery_cb, 0, sizeof(app_discovery_cb));
...
int app_disc_start_services(tBSA_SERVICE_MASK services)
{
    ...
    APP_INFO1("Look for (%s) services", app_service_mask_to_string(services, NULL));
    status = BSA_DiscStart(&disc_start_param);
    if (status != BSA_SUCCESS) {
        APP_ERROR1("BSA_DiscStart failed status:%d", status);
        return -1;
                       ...
                       tBSA_DISC_CBACK *p_custom_disc_cback)
{
    ...
    memcpy(disc_start_param.filter_cond.dev_class_cond.dev_class, dev_class, sizeof(DEV_CLASS));
    ...
    APP_INFO1("COD Mask: 0x%02x 0x%02x 0x%02x", dev_class_mask[0], dev_class_mask[1],
              dev_class_mask[2]);
    memcpy(disc_start_param.filter_cond.dev_class_cond.dev_class_mask, dev_class_mask,
           sizeof(DEV_CLASS));
...
int app_disc_start_dev_info(BD_ADDR bd_addr, tBSA_DISC_CBACK *p_custom_disc_cback)
{
    ...
    APP_INFO1("Start Device Info Discovery for %02x-%02x-%02x-%02x-%02x-%02x", bd_addr[0],
 ...
 **
 *******************************************************************************/
int app_disc_start_brcm_filter_cod(tBSA_DISC_BRCM_FILTER brcm_filter, unsigned short services,
                                   ...
                                   tBSA_DISC_CBACK *p_disc_cback)
{
    ...
    APP_INFO1("Look for COD service:%x major:%x minor:%x and BRCM filter:0x%x", services, major,
              minor, brcm_filter);
 ...
 **
 *******************************************************************************/
int app_disc_start_bdaddr(BD_ADDR bd_addr, BOOLEAN name_req, tBSA_DISC_CBACK *p_disc_cback)
{
    int status;
    tBSA_DISC_START disc_start_param;

    APP_INFO0("Start BdAddr filtered Discovery");
    APP_INFO1("Look for BdAddr :%02X-%02X-%02X-%02X-%02X-%02X", bd_addr[0], bd_addr[1], bd_addr[2],
 ...
 **
 *******************************************************************************/
int app_disc_start_bdaddr_services(BD_ADDR bd_addr, tBSA_SERVICE_MASK services,
                                   tBSA_DISC_CBACK *p_disc_cback)
{
    int status;
    tBSA_DISC_START disc_start_param;
    ...
    APP_INFO1("Device BdAddr :%02X-%02X-%02X-%02X-%02X-%02X", bd_addr[0], bd_addr[1], bd_addr[2],
              bd_addr[3], bd_addr[4], bd_addr[5]);
...
tAPP_DISCOVERY_CB *linkplay_bluetooth_get_discovery_cb(void) { return &app_discovery_cb; }
