...
void app_avk_display_connections(void)
{
    ...
    for (index = 0; index < APP_AVK_MAX_CONNECTIONS; index++) {
        ...
        if (conn->in_use) {
            APP_INFO1("    Connection index %d: - Connected to %02X:%02X:%02X:%02X:%02X:%02X",
                      index, conn->bda_connected[0], conn->bda_connected[1], conn->bda_connected[2],
                      conn->bda_connected[3], conn->bda_connected[4], conn->bda_connected[5]);
        }
    }
}
 ...
 ** Function         main
 **
 ** Description      This is the main function to connect to AVK. It is assumed that an other
 *process handle security.
 **
 ** Parameters
 **
 ** Returns          void
...
int linkplay_bluetooth_config_init(const char *bt_name, UINT8 *btaddr)
{
    return app_manager_init(bt_name, btaddr);
}
...
int linkplay_bluetooth_disconnect_server(void) { return app_mgt_close(); }
...
void linkplay_bluetooth_avk_deregister(void) { app_avk_deregister(); }

int linkplay_bluetooth_auto_reconnect(int index) { return app_avk_open(index); }
...
int linkplay_bluetooth_avk_play_stop_all(void) { app_avk_stop_all(); }

int linkplay_bluetooth_avk_close_all(void) { app_avk_close_all(); }
int linkplay_bluetooth_set_abs_vol(UINT8 volume, int index)
{
    ...
    connection = app_avk_find_connection_by_index(index);
    if (connection) {
        app_avk_reg_notfn_rsp(volume, connection->rc_handle, connection->volChangeLabel,
