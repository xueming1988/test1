...
static void app_hs_set_initial_indicator_status(tBSA_HS_CONN_CB *p_conn, char *ind)
{
    ...
    if (p_conn->curr_call_setup_ind < 4)
        APP_DEBUG1("Callsetup: Ind %s,%d", app_hs_callsetup_ind_name[p_conn->curr_call_setup_ind],
...
int app_hs_play_file(char *filename)
{
    ...
    if (fd < 0) {
        ...
        APP_ERROR1(
            "Use the Record audio file function to create an audio file called %s and then try "
            "again",
...
void app_hs_cback(tBSA_HS_EVT event, tBSA_HS_MSG *p_data)
{
    ...
    case BSA_HS_CONN_EVT: /* Service level connection */
        ...
        APP_INFO1("BSA_HS_CONN_EVT: bdaddr: %02x:%02x:%02x:%02x:%02x:%02x, Service: 0x%08x %s ",
...
def PCM_ALSA
    ...
    case BSA_HS_BSIR_EVT:
        APP_DEBUG1("BSA_HS_BSIR_EVT handle: %d, num: %d val: %s", p_data->val.handle,
    ...
    case BSA_HS_OPEN_EVT:
        APP_INFO1("BSA_HS_OPEN_EVT bd_addr %02x:%02x:%02x:%02x:%02x:%02x, handle %d, status %d, "
                  "initiator %d",
                  ...
                  p_data->open.handle, p_data->open.status, p_data->open.initiator);
        app_hs_cb.open_pending = FALSE;
        break;
...
void app_hs_init(void)
{
    ...
    for (index = 0; index < BSA_HS_MAX_NUM_CONN; index++) {
        ...
        app_hs_cb.conn_cb[index].connection_active = FALSE;
    }
}
