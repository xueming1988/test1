...
#define APP_XML_SI_DEVICES_FILE_PATH "/vendor/bsa/si_devices.xml"
...
static pthread_mutex_t mutex_write_xml = PTHREAD_MUTEX_INITIALIZER;

void xml_db_lock(void) { pthread_mutex_lock(&mutex_write_xml); }

void xml_db_unlock(void) { pthread_mutex_unlock(&mutex_write_xml); }
...
int app_read_xml_config(tAPP_XML_CONFIG *p_xml_config)
{
    ...
    if (status < 0) {
        ...
        APP_DEBUG1("\tBdaddr %02x:%02x:%02x:%02x:%02x:%02x", p_xml_config->bd_addr[0],
                   p_xml_config->bd_addr[1], p_xml_config->bd_addr[2], p_xml_config->bd_addr[3],
...
int app_write_xml_si_devices(void)
{
    ...
    status = app_xml_write_si_db(APP_XML_SI_DEVICES_FILE_PATH, app_xml_si_devices_db,
                                 APP_NUM_ELEMENTS(app_xml_si_devices_db));
 ...
 **
 *******************************************************************************/
BOOLEAN app_xml_read_data_length(void *p_data, unsigned int datalen, const char *p_buf,
                                 unsigned int length)
{
    char *p_newbuf;
    unsigned int newlen;
