...
#define APP_XML_DEV_LAST_ACTIVE "last_active"
#define APP_XML_DEV_PROFILE_TYPE "profile_type"
#define APP_XML_DEV_AUDIO_PATH "audio_path"
...
static void app_xml_conf_tagBeginCallbackFunc(nxml_t handle, const char *tagName, unsigned len);
static void app_xml_conf_attrBeginCallbackFunc(nxml_t handle, const char *attrName, unsigned len);
static void app_xml_conf_attrValueCallbackFunc(nxml_t handle, const char *attrValue, unsigned len,
                                               int more);
static void app_xml_conf_dataCallbackFunc(nxml_t handle, const char *data, unsigned len, int more);
static void app_xml_conf_tagEndCallbackFunc(nxml_t handle, const char *tagName, unsigned len);

static void app_xml_dev_db_tagBeginCallbackFunc(nxml_t handle, const char *tagName, unsigned len);
static void app_xml_dev_db_attrBeginCallbackFunc(nxml_t handle, const char *attrName, unsigned len);
static void app_xml_dev_db_attrValueCallbackFunc(nxml_t handle, const char *attrValue, unsigned len,
                                                 int more);
static void app_xml_dev_db_dataCallbackFunc(nxml_t handle, const char *data, unsigned len,
                                            int more);
static void app_xml_dev_db_tagEndCallbackFunc(nxml_t handle, const char *tagName, unsigned len);

static void app_xml_si_dev_db_tagBeginCallbackFunc(nxml_t handle, const char *tagName,
                                                   unsigned len);
static void app_xml_si_dev_db_attrBeginCallbackFunc(nxml_t handle, const char *attrName,
                                                    unsigned len);
static void app_xml_si_dev_db_attrValueCallbackFunc(nxml_t handle, const char *attrValue,
                                                    unsigned len, int more);
static void app_xml_si_dev_db_dataCallbackFunc(nxml_t handle, const char *data, unsigned len,
                                               int more);
static void app_xml_si_dev_db_tagEndCallbackFunc(nxml_t handle, const char *tagName, unsigned len);
 ...
 **
 *******************************************************************************/
int app_xml_init(void) { return 0; }
 ...
 **
 *******************************************************************************/
int app_xml_read_db(const char *p_fname, tAPP_XML_REM_DEVICE *p_xml_rem_devices, int devices_max)
{
    nxml_t nxmlHandle;
    nxml_settings nxmlSettings;
    char *endp = NULL;
 ...
 **
 *******************************************************************************/
int app_xml_read_si_db(const char *p_fname, tAPP_XML_SI_DEVICE *p_xml_si_devices, int si_dev_max)
{
    nxml_t nxmlHandle;
    nxml_settings nxmlSettings;
    char *endp = NULL;
...
int app_xml_write_cfg(const char *p_fname, const tAPP_XML_CONFIG *p_xml_config)
{
    ...
    app_xml_write_data(fd, p_xml_config->class_of_device, sizeof(p_xml_config->class_of_device),
                       FALSE);
 ...
 **
 *******************************************************************************/
int app_xml_write_db(const char *p_fname, const tAPP_XML_REM_DEVICE *p_xml_rem_devices,
                     int devices_max)
{
    int fd, dummy;
    int index;
    ...
    for (index = 0, instance = 0; index < devices_max; index++, p_xml_rem_devices++) {
        ...
        if (p_xml_rem_devices->in_use != FALSE) {
            ...
            app_xml_write_data(fd, p_xml_rem_devices->bd_addr, sizeof(p_xml_rem_devices->bd_addr),
                               FALSE);
            app_xml_close_tag(fd, APP_XML_DEV_BDADDR_KEY, FALSE);

            app_xml_open_tag(fd, APP_XML_DEV_NAME_KEY, FALSE);
            dummy =
                write(fd, (char *)p_xml_rem_devices->name, strlen((char *)p_xml_rem_devices->name));
            app_xml_close_tag(fd, APP_XML_DEV_NAME_KEY, FALSE);

            app_xml_open_tag(fd, APP_XML_DEV_CLASS_KEY, FALSE);
            app_xml_write_data(fd, p_xml_rem_devices->class_of_device,
                               sizeof(p_xml_rem_devices->class_of_device), FALSE);
            app_xml_close_tag(fd, APP_XML_DEV_CLASS_KEY, FALSE);

            app_xml_open_tag(fd, APP_XML_DEV_LINKKEY_KEY, FALSE);
            app_xml_write_data(fd, p_xml_rem_devices->link_key, sizeof(p_xml_rem_devices->link_key),
                               FALSE);
            ...
            app_xml_open_tag(fd, APP_XML_DEV_LAST_ACTIVE, FALSE);
            /* coverity[(PW.IMPLICIT_FUNC_DECL] False-positive: from stdio.h is included */
            dprintf(fd, "%d", p_xml_rem_devices->last_active);
            app_xml_close_tag(fd, APP_XML_DEV_LAST_ACTIVE, FALSE);

            app_xml_open_tag(fd, APP_XML_DEV_PROFILE_TYPE, FALSE);
            dprintf(fd, "%d", p_xml_rem_devices->profile_type);
            app_xml_close_tag(fd, APP_XML_DEV_PROFILE_TYPE, FALSE);

            app_xml_open_tag(fd, APP_XML_DEV_AUDIO_PATH, FALSE);
            dprintf(fd, "%d", p_xml_rem_devices->is_audio_path_on);
            app_xml_close_tag(fd, APP_XML_DEV_AUDIO_PATH, FALSE);
            ...
            app_xml_write_data(fd, p_xml_rem_devices->features, sizeof(p_xml_rem_devices->features),
                               FALSE);
            ...
            app_xml_write_data(fd, p_xml_rem_devices->penc_ltk, sizeof(p_xml_rem_devices->penc_ltk),
                               FALSE);
            app_xml_close_tag(fd, APP_XML_DEV_PENC_LTK, FALSE);

            app_xml_open_tag(fd, APP_XML_DEV_PENC_RAND, FALSE);
            app_xml_write_data(fd, p_xml_rem_devices->penc_rand,
                               sizeof(p_xml_rem_devices->penc_rand), FALSE);
            ...
            app_xml_write_data(fd, p_xml_rem_devices->pid_irk, sizeof(p_xml_rem_devices->pid_irk),
                               FALSE);
            ...
            app_xml_write_data(fd, p_xml_rem_devices->pid_static_addr,
                               sizeof(p_xml_rem_devices->pid_static_addr), FALSE);
            ...
            app_xml_write_data(fd, p_xml_rem_devices->pcsrk_csrk,
                               sizeof(p_xml_rem_devices->pcsrk_csrk), FALSE);
                        ...
                        int si_dev_max)
{
    ...
    for (index = 0, instance = 0; index < si_dev_max; index++, p_xml_si_devices++) {
        ...
        if (p_xml_si_devices->in_use != FALSE) {
            ...
            app_xml_write_data(fd, p_xml_si_devices->bd_addr, sizeof(p_xml_si_devices->bd_addr),
                               FALSE);
...
static void app_xml_conf_tagBeginCallbackFunc(nxml_t handle, const char *tagName, unsigned len)
{
    if (strncmp(tagName, APP_XML_CONF_ENABLE_KEY, strlen(APP_XML_CONF_ENABLE_KEY)) == 0) {
        app_xml_param_cb.cfg_tag = CONF_ENABLE_KEY;
    } else if (strncmp(tagName, APP_XML_CONF_DISCOVERABLE_KEY,
                       strlen(APP_XML_CONF_DISCOVERABLE_KEY)) == 0) {
        app_xml_param_cb.cfg_tag = CONF_DISCOVERABLE_KEY;
    } else if (strncmp(tagName, APP_XML_CONF_CONNECTABLE_KEY,
                       strlen(APP_XML_CONF_CONNECTABLE_KEY)) == 0) {
        app_xml_param_cb.cfg_tag = CONF_CONNECTABLE_KEY;
    } else if (strncmp(tagName, APP_XML_CONF_NAME_KEY, strlen(APP_XML_CONF_NAME_KEY)) == 0) {
        app_xml_param_cb.cfg_tag = CONF_NAME_KEY;
    } else if (strncmp(tagName, APP_XML_CONF_BDADDR_KEY, strlen(APP_XML_CONF_BDADDR_KEY)) == 0) {
        app_xml_param_cb.cfg_tag = CONF_BDADDR_KEY;
    } else if (strncmp(tagName, APP_XML_CONF_CLASS_KEY, strlen(APP_XML_CONF_CLASS_KEY)) == 0) {
        app_xml_param_cb.cfg_tag = CONF_CLASS_KEY;
    } else if (strncmp(tagName, APP_XML_CONF_PINCODE_KEY, strlen(APP_XML_CONF_PINCODE_KEY)) == 0) {
        app_xml_param_cb.cfg_tag = CONF_PINCODE_KEY;
    } else if (strncmp(tagName, APP_XML_CONF_PINLEN_KEY, strlen(APP_XML_CONF_PINLEN_KEY)) == 0) {
        app_xml_param_cb.cfg_tag = CONF_PINLEN_KEY;
    } else if (strncmp(tagName, APP_XML_CONF_IO_CAP_KEY, strlen(APP_XML_CONF_IO_CAP_KEY)) == 0) {
        app_xml_param_cb.cfg_tag = CONF_IOCAP_KEY;
    } else if (strncmp(tagName, APP_XML_CONF_ROOTPATH_KEY, strlen(APP_XML_CONF_ROOTPATH_KEY)) ==
               0) {
...
static void app_xml_conf_dataCallbackFunc(nxml_t handle, const char *data, unsigned len, int more)
{
    ...
    case CONF_BDADDR_KEY:
        if (app_xml_read_data_length(&app_xml_param_cb.config_ptr->bd_addr,
                                     sizeof(app_xml_param_cb.config_ptr->bd_addr), data,
                                     len) == FALSE) {
    ...
    case CONF_CLASS_KEY:
        if (app_xml_read_data_length(&app_xml_param_cb.config_ptr->class_of_device,
                                     sizeof(app_xml_param_cb.config_ptr->class_of_device), data,
                                     len) == FALSE) {
            APP_ERROR0("Failed reading BD address in XML");
        }
        break;
...
static void app_xml_dev_db_tagBeginCallbackFunc(nxml_t handle, const char *tagName, unsigned len)
{
    if (strncmp(tagName, APP_XML_DEV_BDADDR_KEY, strlen(APP_XML_CONF_ENABLE_KEY)) == 0) {
        app_xml_param_cb.dev_tag = DEV_BDADDR_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_NAME_KEY, strlen(APP_XML_DEV_NAME_KEY)) == 0) {
        app_xml_param_cb.dev_tag = DEV_NAME_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_CLASS_KEY, strlen(APP_XML_DEV_CLASS_KEY)) == 0) {
        app_xml_param_cb.dev_tag = DEV_CLASS_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_LINKKEY_KEY, strlen(APP_XML_DEV_LINKKEY_KEY)) == 0) {
        app_xml_param_cb.dev_tag = DEV_LINKKEY_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_LINKKEYP_KEY, strlen(APP_XML_DEV_LINKKEYP_KEY)) == 0) {
        app_xml_param_cb.dev_tag = DEV_LINKKEYP_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_LAST_ACTIVE, strlen(APP_XML_DEV_LAST_ACTIVE)) == 0) {
        app_xml_param_cb.dev_tag = DEV_LAST_ACTIVE_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_PROFILE_TYPE, strlen(APP_XML_DEV_PROFILE_TYPE)) == 0) {
        app_xml_param_cb.dev_tag = DEV_PROFILE_TYPE_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_AUDIO_PATH, strlen(APP_XML_DEV_AUDIO_PATH)) == 0) {
        app_xml_param_cb.dev_tag = DEV_AUDIO_PATH_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_KEYTYPE_KEY, strlen(APP_XML_DEV_KEYTYPE_KEY)) == 0) {
        app_xml_param_cb.dev_tag = DEV_KEYTYPE_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_TRUSTED_KEY, strlen(APP_XML_DEV_TRUSTED_KEY)) == 0) {
        app_xml_param_cb.dev_tag = DEV_TRUSTED_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_PID_KEY, strlen(APP_XML_DEV_PID_KEY)) == 0) {
        app_xml_param_cb.dev_tag = DEV_PID_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_VID_KEY, strlen(APP_XML_DEV_VID_KEY)) == 0) {
        app_xml_param_cb.dev_tag = DEV_VID_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_VERSION_KEY, strlen(APP_XML_DEV_VERSION_KEY)) == 0) {
        app_xml_param_cb.dev_tag = DEV_VERSION_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_FEATURES, strlen(APP_XML_DEV_FEATURES)) == 0) {
        app_xml_param_cb.dev_tag = DEV_FEATURES_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_LMP_VERSION, strlen(APP_XML_DEV_LMP_VERSION)) == 0) {
        app_xml_param_cb.dev_tag = DEV_LMP_VERSION_KEY;
    }
#if (defined(BLE_INCLUDED) && BLE_INCLUDED == TRUE)
    else if (strncmp(tagName, APP_XML_DEV_BLE_ADDR_TYPE, strlen(APP_XML_DEV_BLE_ADDR_TYPE)) == 0) {
        app_xml_param_cb.dev_tag = DEV_BLE_ADDR_TYPE_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_DEVICE_TYPE, strlen(APP_XML_DEV_DEVICE_TYPE)) == 0) {
        app_xml_param_cb.dev_tag = DEV_DEVICE_TYPE_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_INQ_RESULT_TYPE, strlen(APP_XML_DEV_DEVICE_TYPE)) ==
               0) {
        app_xml_param_cb.dev_tag = DEV_INQ_RESULT_TYPE;
    } else if (strncmp(tagName, APP_XML_DEV_BLE_LINKKEYP_KEY,
                       strlen(APP_XML_DEV_BLE_LINKKEYP_KEY)) == 0) {
        app_xml_param_cb.dev_tag = DEV_BLE_LINKKEYP_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_PENC_LTK, strlen(APP_XML_DEV_PENC_LTK)) == 0) {
        app_xml_param_cb.dev_tag = DEV_PENC_LTK_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_PENC_RAND, strlen(APP_XML_DEV_PENC_RAND)) == 0) {
        app_xml_param_cb.dev_tag = DEV_PENC_RAND_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_PENC_EDIV, strlen(APP_XML_DEV_PENC_EDIV)) == 0) {
        app_xml_param_cb.dev_tag = DEV_PENC_EDIV_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_PENC_SEC_LEVEL, strlen(APP_XML_DEV_PENC_SEC_LEVEL)) ==
               0) {
        app_xml_param_cb.dev_tag = DEV_PENC_SEC_LEVEL_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_PENC_KEY_SIZE, strlen(APP_XML_DEV_PENC_KEY_SIZE)) ==
               0) {
        app_xml_param_cb.dev_tag = DEV_PENC_KEY_SIZE_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_PID_IRK, strlen(APP_XML_DEV_PID_IRK)) == 0) {
        app_xml_param_cb.dev_tag = DEV_PID_IRK_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_PID_ADDR_TYPE, strlen(APP_XML_DEV_PID_ADDR_TYPE)) ==
               0) {
        app_xml_param_cb.dev_tag = DEV_PID_ADDR_TYPE_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_PID_STATIC_ADDR, strlen(APP_XML_DEV_PID_STATIC_ADDR)) ==
               0) {
        app_xml_param_cb.dev_tag = DEV_PID_STATIC_ADDR_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_PCSRK_COUNTER, strlen(APP_XML_DEV_PCSRK_COUNTER)) ==
               0) {
        app_xml_param_cb.dev_tag = DEV_PCSRK_COUNTER_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_PCSRK_CSRK, strlen(APP_XML_DEV_PCSRK_CSRK)) == 0) {
        app_xml_param_cb.dev_tag = DEV_PCSRK_CSRK_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_PCSRK_SEC_LEVEL, strlen(APP_XML_DEV_PCSRK_SEC_LEVEL)) ==
               0) {
        app_xml_param_cb.dev_tag = DEV_PCSRK_SEC_LEVEL_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_LCSRK_COUNTER, strlen(APP_XML_DEV_LCSRK_COUNTER)) ==
               0) {
        app_xml_param_cb.dev_tag = DEV_LCSRK_COUNTER_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_LCSRK_DIV, strlen(APP_XML_DEV_LCSRK_DIV)) == 0) {
        app_xml_param_cb.dev_tag = DEV_LCSRK_DIV_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_LCSRK_SEC_LEVEL, strlen(APP_XML_DEV_LCSRK_SEC_LEVEL)) ==
               0) {
        app_xml_param_cb.dev_tag = DEV_LCSRK_SEC_LEVEL_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_LENC_DIV, strlen(APP_XML_DEV_LENC_DIV)) == 0) {
        app_xml_param_cb.dev_tag = DEV_LENC_DIV_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_LENC_KEY_SIZE, strlen(APP_XML_DEV_LENC_KEY_SIZE)) ==
               0) {
        app_xml_param_cb.dev_tag = DEV_LENC_KEY_SIZE_KEY;
    } else if (strncmp(tagName, APP_XML_DEV_LENC_SEC_LEVEL, strlen(APP_XML_DEV_LENC_SEC_LEVEL)) ==
               0) {
...
static void app_xml_dev_db_attrBeginCallbackFunc(nxml_t handle, const char *attrName, unsigned len)
{
    if (strncmp(attrName, APP_XML_DEV_INST_KEY, strlen(APP_XML_DEV_INST_KEY)) == 0) {
 ...
 **
 *******************************************************************************/
static void app_xml_dev_db_dataCallbackFunc(nxml_t handle, const char *data, unsigned len, int more)
{
    tAPP_XML_REM_DEVICE *devices_db_ptr;
    ...
    case DEV_BDADDR_KEY:
        if (app_xml_read_data_length(&devices_db_ptr->bd_addr, sizeof(devices_db_ptr->bd_addr),
                                     data, len) == FALSE) {
    ...
    case DEV_NAME_KEY:
        ...
        app_xml_read_string(devices_db_ptr->name, sizeof(devices_db_ptr->name), data, len);
        break;
    ...
    case DEV_LINKKEY_KEY:
        if (app_xml_read_data_length(&devices_db_ptr->link_key, sizeof(devices_db_ptr->link_key),
                                     data, len) == FALSE) {
    ...
    case DEV_LINKKEYP_KEY:
        ...
        break;
    case DEV_LAST_ACTIVE_KEY:
        devices_db_ptr->last_active = app_xml_read_value(data, len);
        break;
    case DEV_PROFILE_TYPE_KEY:
        devices_db_ptr->profile_type = app_xml_read_value(data, len);
        break;
    case DEV_AUDIO_PATH_KEY:
        devices_db_ptr->is_audio_path_on = app_xml_read_value(data, len);
        break;
    ...
    case DEV_FEATURES_KEY:
        if (app_xml_read_data_length(&devices_db_ptr->features, sizeof(devices_db_ptr->features),
                                     data, len) == FALSE) {
    ...
    case DEV_PENC_LTK_KEY:
        if (app_xml_read_data_length(&devices_db_ptr->penc_ltk, sizeof(devices_db_ptr->penc_ltk),
                                     data, len) == FALSE) {
    ...
    case DEV_PENC_RAND_KEY:
        if (app_xml_read_data_length(&devices_db_ptr->penc_rand, sizeof(devices_db_ptr->penc_rand),
                                     data, len) == FALSE) {
    ...
    case DEV_PID_IRK_KEY:
        if (app_xml_read_data_length(&devices_db_ptr->pid_irk, sizeof(devices_db_ptr->pid_irk),
                                     data, len) == FALSE) {
 ...
 **
 *******************************************************************************/
static void app_xml_si_dev_db_tagBeginCallbackFunc(nxml_t handle, const char *tagName, unsigned len)
{
    if (strncmp(tagName, APP_XML_SI_DEV_BDADDR_KEY, strlen(APP_XML_SI_DEV_BDADDR_KEY)) == 0) {
        app_xml_param_cb.si_dev_tag = SI_DEV_BDADDR_KEY;
    } else if (strncmp(tagName, APP_XML_SI_DEV_PLATFORM_KEY, strlen(APP_XML_SI_DEV_PLATFORM_KEY)) ==
               0) {
...
static void app_xml_si_dev_db_attrBeginCallbackFunc(nxml_t handle, const char *attrName,
                                                    unsigned len)
{
    if (strncmp(attrName, APP_XML_SI_DEV_INST_KEY, strlen(APP_XML_SI_DEV_INST_KEY)) == 0) {
                                               ...
                                               int more)
{
    ...
    case SI_DEV_BDADDR_KEY:
        if (app_xml_read_data_length(&si_dev_db_ptr->bd_addr, sizeof(si_dev_db_ptr->bd_addr), data,
                                     len) == FALSE) {
            APP_ERROR0("Failed reading instance BD address in XML");
        }
 ...
 **
 *******************************************************************************/
int app_xml_add_dev_db(tAPP_XML_REM_DEVICE *p_stored_device_db, int nb_device_max, BD_ADDR bd_addr)
{
    int index;

    for (index = 0; index < nb_device_max; index++) {
        p_stored_device_db[index].last_active = FALSE;
    }
    ...
    for (index = 0; index < nb_device_max; index++) {
        if ((p_stored_device_db[index].in_use != FALSE) &&
            ...
            p_stored_device_db[index].last_active = TRUE;
            return 0;
        }
    }
 ...
 **
 *******************************************************************************/
tAPP_XML_REM_DEVICE *app_xml_find_dev_db(tAPP_XML_REM_DEVICE *p_stored_device_db, int nb_device_max,
                                         BD_ADDR bd_addr)
{
    int index;
 ...
 **
 *******************************************************************************/
int app_xml_update_key_db(tAPP_XML_REM_DEVICE *p_stored_device_db, int nb_device_max,
                          BD_ADDR bd_addr, LINK_KEY link_key, unsigned char key_type)
{
    int index;
    int ret_code = -1;
    ...
    for (index = 0; index < nb_device_max; index++) {
        if ((p_stored_device_db[index].in_use != FALSE) &&
            ...
            memcpy(p_stored_device_db[index].link_key, link_key, sizeof(LINK_KEY));
 ...
 **
 *******************************************************************************/
int app_xml_update_ble_key_db(tAPP_XML_REM_DEVICE *p_stored_device_db, int nb_device_max,
                              BD_ADDR bd_addr, tBSA_LE_KEY_VALUE ble_key,
...
/*******************************************************************************
 **
 ** Function        app_xml_add_audio_path_db
 **
 ** Description     Add a audio path for a device
 **
 ** Returns         0 if successful, error code otherwise
 **
 *******************************************************************************/
int app_xml_add_audio_path_db(tAPP_XML_REM_DEVICE *p_stored_device_db, int nb_device_max,
                              BD_ADDR bd_addr, BOOLEAN audio_path)
{
    int index;
    ...
    for (index = 0; index < nb_device_max; index++) {
        if (bdcmp(p_stored_device_db[index].bd_addr, bd_addr) == 0) {
            printf("add_audio_path\n");
            p_stored_device_db[index].is_audio_path_on = audio_path;
            return 0;
        }
    }
    return -1;
 ...
 **
 *******************************************************************************/
int app_xml_add_trusted_services_db(tAPP_XML_REM_DEVICE *p_stored_device_db, int nb_device_max,
                                    BD_ADDR bd_addr, tBSA_SERVICE_MASK trusted_services,
                                    int profile_type)
{
    int index;
    ...
    for (index = 0; index < nb_device_max; index++) {
        if ((p_stored_device_db[index].in_use != FALSE) &&
            ...
            if (profile_type > 0)
                p_stored_device_db[index].profile_type = profile_type;
            return 0;
        }
    }
    return -1;
 ...
 **
 *******************************************************************************/
int app_xml_update_name_db(tAPP_XML_REM_DEVICE *p_stored_device_db, int nb_device_max,
                           BD_ADDR bd_addr, BD_NAME name)
{
    int index;
 ...
 **
 *******************************************************************************/
int app_xml_update_cod_db(tAPP_XML_REM_DEVICE *p_stored_device_db, int nb_device_max,
                          BD_ADDR bd_addr, DEV_CLASS class_of_device)
{
    int index;
    ...
    for (index = 0; index < nb_device_max; index++) {
        if ((p_stored_device_db[index].in_use != FALSE) &&
            ...
            printf("Update class-of-device [0x%02X-0x%02X-0x%02X]\n", class_of_device[0],
                   class_of_device[1], class_of_device[2]);
 ...
 **
 *******************************************************************************/
int app_xml_display_devices(const tAPP_XML_REM_DEVICE *p_stored_device_db, int nb_device_max)
{
    int index;
    ...
    for (index = 0; index < nb_device_max; index++) {
        if (p_stored_device_db[index].in_use != FALSE) {
            ...
            printf("\tBdaddr:%02x:%02x:%02x:%02x:%02x:%02x\n", p_stored_device_db[index].bd_addr[0],
                   p_stored_device_db[index].bd_addr[1], p_stored_device_db[index].bd_addr[2],
                   p_stored_device_db[index].bd_addr[3], p_stored_device_db[index].bd_addr[4],
            ...
            printf("\tClassOfDevice:%02x:%02x:%02x => %s\n",
                   ...
                   app_get_cod_string(p_stored_device_db[index].class_of_device));
 ...
 **
 *******************************************************************************/
int app_xml_update_pidvid_db(tAPP_XML_REM_DEVICE *p_stored_device_db, int nb_device_max,
                             BD_ADDR bd_addr, UINT16 pid, UINT16 vid)
{
    int index;
 ...
 **
 *******************************************************************************/
int app_xml_update_version_db(tAPP_XML_REM_DEVICE *p_stored_device_db, int nb_device_max,
                              BD_ADDR bd_addr, UINT16 version)
{
    int index;
 ...
 **
 *******************************************************************************/
int app_xml_update_features_db(tAPP_XML_REM_DEVICE *p_stored_device_db, int nb_device_max,
                               BD_ADDR bd_addr, BD_FEATURES features)
{
    int index;
    ...
    for (index = 0; index < nb_device_max; index++) {
        if ((p_stored_device_db[index].in_use != FALSE) &&
            ...
            printf("Update features:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X\n", features[0],
                   features[1], features[2], features[3], features[3], features[4], features[5],
                   features[6]);
 ...
 **
 *******************************************************************************/
int app_xml_update_lmp_version_db(tAPP_XML_REM_DEVICE *p_stored_device_db, int nb_device_max,
                                  BD_ADDR bd_addr, UINT8 lmp_version)
{
    int index;
 ...
 **
 *******************************************************************************/
int app_xml_update_ble_addr_type_db(tAPP_XML_REM_DEVICE *p_stored_device_db, int nb_device_max,
                                    BD_ADDR bd_addr, UINT8 ble_addr_type)
{
    int index;
 ...
 **
 *******************************************************************************/
int app_xml_update_device_type_db(tAPP_XML_REM_DEVICE *p_stored_device_db, int nb_device_max,
                                  BD_ADDR bd_addr, tBT_DEVICE_TYPE device_type)
{
    int index;
 ...
 **
 *******************************************************************************/
int app_xml_update_inq_result_type_db(tAPP_XML_REM_DEVICE *p_stored_device_db, int nb_device_max,
                                      BD_ADDR bd_addr, UINT8 inq_res_type)
{
    int index;
 ...
 **
 *******************************************************************************/
tAPP_XML_SI_DEVICE *app_xml_add_si_dev_db(tAPP_XML_SI_DEVICE *p_stored_device_db, int si_device_max,
                                          BD_ADDR bd_addr)
{
    int index;
 ...
 **
 *******************************************************************************/
int app_xml_update_si_dev_platform_db(tAPP_XML_SI_DEVICE *p_stored_device_db, int si_device_max,
                                      BD_ADDR bd_addr, UINT8 platform)
{
    tAPP_XML_SI_DEVICE *p_si_device =
        app_xml_add_si_dev_db(p_stored_device_db, si_device_max, bd_addr);
