...
int main(int argc, char **argv)
{
    ...
    app_mgt_init();
    if (app_mgt_open("/tmp/", app_hs_mgt_callback) < 0)
    {
...
{
    app_hs_init();           /* Init Headset Application */
    return app_hs_start(cb); /* Start Headset service*/
}

void linkplay_bluetooth_hfp_deinit(void) { app_hs_stop(); /* Start Headset service*/ }

int linkplay_bluetooth_hfp_answer_call(void) { return app_hs_answer_call(); }

int linkplay_bluetooth_hfp_hangup_call(void) { return app_hs_hangup(); }

int linkplay_bluetooth_hfp_dial_num(char *num)
{
    int ret = -1;
    if (num) {
        ret = app_hs_dial_num(num);
    }
    return ret;
}

int linkplay_bluetooth_hfp_dial_last_num(void) { return app_hs_last_num_dial(); }
