...
BT_APP_GATTS_CB_FUNC_T server_profile_callback = {0};
...
void app_ble_server_display(void)
{
    ...
    for (index = 0; index < BSA_BLE_SERVER_MAX; index++) {
        if (app_ble_cb.ble_server[index].enabled) {
            APP_INFO1("%d:BLE Server server_if:%d", index, app_ble_cb.ble_server[index].server_if);
            for (attr_num = 0; attr_num < BSA_BLE_ATTRIBUTE_MAX; attr_num++) {
                memset(uuid128, 0, sizeof(uuid128));
                if (app_ble_cb.ble_server[index].attr[attr_num].attr_UUID.len == LEN_UUID_16 ||
                    app_ble_cb.ble_server[index].attr[attr_num].attr_UUID.len == LEN_UUID_128) {
                    for (i = app_ble_cb.ble_server[index].attr[attr_num].attr_UUID.len - 1; i >= 0;
                         i--) {
                        snprintf(
                            uuid128 + strlen(uuid128), 3, "%02X",
                            app_ble_cb.ble_server[index].attr[attr_num].attr_UUID.uu.uuid128[i]);
                    }
                    if ((app_ble_cb.ble_server[index].attr[attr_num].attr_type ==
                         BSA_GATTC_ATTR_TYPE_SRVC) ||
                        (app_ble_cb.ble_server[index].attr[attr_num].attr_type ==
                         BSA_GATTC_ATTR_TYPE_INCL_SRVC)) {
                        APP_INFO1("\t attr_num:%d:attr_type:%d:is_pri:%d, service_id:%d "
                                  "attr_id:%d, uuid:%s",
                                  ...
                                  app_ble_cb.ble_server[index].attr[attr_num].attr_id, uuid128);
                    } else {
                        APP_INFO1("\t\t attr_num:%d:attr_type:%d:is_pri:%d, service_id:%d "
                                  "attr_id:%d, uuid:%s",
                                  attr_num, app_ble_cb.ble_server[index].attr[attr_num].attr_type,
...
int app_ble_server_register(tBT_UUID *uuid, BT_APP_GATTS_CB_FUNC_T *p_cback)
{
    ...
    memcpy(&ble_register_param.uuid, uuid, sizeof(tBT_UUID));

    ble_register_param.p_cback = app_ble_server_profile_cback;

    memcpy(&server_profile_callback, p_cback, sizeof(server_profile_callback));
                                  ...
                                  BOOLEAN is_primary)
{
    ...
    memcpy(&ble_create_param.service_uuid, service, sizeof(tBT_UUID));
    ...
    memcpy(&(app_ble_cb.ble_server[server_num].attr[attr_num].attr_UUID), service,
           sizeof(tBT_UUID));
                            ...
                            int attribute_permission, int characteristic_property)
{
    ...
    memcpy(&ble_addchar_param.char_uuid, char_uuid, sizeof(tBT_UUID));
    ...
    memcpy(&(app_ble_cb.ble_server[server_num].attr[char_attr_num].attr_UUID), char_uuid,
           sizeof(tBT_UUID));
...
void app_ble_server_profile_cback(tBSA_BLE_EVT event, tBSA_BLE_MSG *p_data)
{
    ...
    case BSA_BLE_SE_CREATE_EVT:
        APP_INFO1("BSA_BLE_SE_CREATE_EVT server_if:%d status:%d service_id:%d",
                  p_data->ser_create.server_if, p_data->ser_create.status,
        ...
        for (attr_index = 0; attr_index < BSA_BLE_ATTRIBUTE_MAX; attr_index++) {
            if (app_ble_cb.ble_server[num].attr[attr_index].wait_flag == TRUE) {
                ...
                if (p_data->ser_create.status == BSA_SUCCESS) {
                    app_ble_cb.ble_server[num].attr[attr_index].service_id =
                        p_data->ser_create.service_id;
                    app_ble_cb.ble_server[num].attr[attr_index].wait_flag = FALSE;
                    break;
                } else { /* if CREATE fail */
                    memset(&app_ble_cb.ble_server[num].attr[attr_index], 0,
                           sizeof(tAPP_BLE_ATTRIBUTE));
                    break;
                }
            }
        }
        ...
        if (server_profile_callback.bt_gatts_server_create_cb)
            server_profile_callback.bt_gatts_server_create_cb(server_profile_callback.context,
                                                              p_data);
    ...
    case BSA_BLE_SE_ADDCHAR_EVT:
        ...
        for (attr_index = 0; attr_index < BSA_BLE_ATTRIBUTE_MAX; attr_index++) {
            if (app_ble_cb.ble_server[num].attr[attr_index].wait_flag == TRUE) {
                ...
                if (p_data->ser_addchar.status == BSA_SUCCESS) {
                    app_ble_cb.ble_server[num].attr[attr_index].service_id =
                        p_data->ser_addchar.service_id;
                    app_ble_cb.ble_server[num].attr[attr_index].attr_id =
        ...
        if (server_profile_callback.bt_gatts_add_char_cb)
            server_profile_callback.bt_gatts_add_char_cb(server_profile_callback.context, p_data);

        break;
    case BSA_BLE_SE_START_EVT:
        APP_INFO1("BSA_BLE_SE_START_EVT status:%d", p_data->ser_start.status);
        if (server_profile_callback.bt_gatts_start_cb)
            server_profile_callback.bt_gatts_start_cb(server_profile_callback.context, p_data);

        break;
    case BSA_BLE_SE_READ_EVT:
        APP_INFO1("BSA_BLE_SE_READ_EVT status:%d", p_data->ser_read.status);
        if (server_profile_callback.bt_gatts_req_read_cb) {
            server_profile_callback.bt_gatts_req_read_cb(server_profile_callback.context, p_data);
        } else {
            ...
            APP_INFO1(
                "BSA_BLE_SE_READ_EVT: send_server_resp.conn_id:%d, send_server_resp.trans_id:%d",
            ...
            APP_INFO1(
                "BSA_BLE_SE_READ_EVT: send_server_resp.handle:%d, send_server_resp.offset:%d, "
    ...
    case BSA_BLE_SE_WRITE_EVT:
        ...
        if (p_data->ser_write.need_rsp) {
            ...
            APP_INFO1(
                "BSA_BLE_SE_WRITE_EVT: send_server_resp.conn_id:%d, send_server_resp.trans_id:%d",
                send_server_resp.conn_id, send_server_resp.trans_id, send_server_resp.status);
            APP_INFO1(
                "BSA_BLE_SE_WRITE_EVT: send_server_resp.status:%d,send_server_resp.auth_req:%d",
                send_server_resp.status, send_server_resp.auth_req);
            APP_INFO1("BSA_BLE_SE_WRITE_EVT: send_server_resp.handle:%d, "
                      "send_server_resp.offset:%d, send_server_resp.len:%d",
                      send_server_resp.handle, send_server_resp.offset, send_server_resp.len);
            BSA_BleSeSendRsp(&send_server_resp);
        }

        if (server_profile_callback.bt_gatts_req_write_cb)
            server_profile_callback.bt_gatts_req_write_cb(server_profile_callback.context, p_data);
        break;
    case BSA_BLE_SE_EXEC_WRITE_EVT:
        ...
        APP_INFO1("BSA_BLE_SE_WRITE_EVT trans_id:%d, conn_id:%d, exec_write:%d",
                  ...
                  p_data->ser_exec_write.exec_write);
        ...
        send_server_resp.len = 0;
        APP_INFO1("BSA_BLE_SE_WRITE_EVT: send_server_resp.conn_id:%d, send_server_resp.trans_id:%d",
                  send_server_resp.conn_id, send_server_resp.trans_id, send_server_resp.status);
    ...
    case BSA_BLE_SE_OPEN_EVT:
        ...
        if (p_data->ser_open.reason == BSA_SUCCESS) {
            ...
            memcpy(conn_param.bd_addr, p_data->ser_open.remote_bda, sizeof(BD_ADDR));
            conn_param.min_conn_int = 30;
            conn_param.max_conn_int = 30;
            conn_param.slave_latency = 0;
            conn_param.supervision_tout = 300;
            conn_param.is_immediate_updating = TRUE;
            app_dm_set_ble_conn_param(&conn_param);

            if (server_profile_callback.bt_gatts_connection_open_cb)
                server_profile_callback.bt_gatts_connection_open_cb(server_profile_callback.context,
                                                                    p_data);
        }
        break;
    case BSA_BLE_SE_CLOSE_EVT:
        ...
        if (server_profile_callback.bt_gatts_connection_close_cb)
            server_profile_callback.bt_gatts_connection_close_cb(server_profile_callback.context,
                                                                 p_data);
        break;
    ...
    case BSA_BLE_SE_MTU_EVT:
        APP_INFO1("BSA_BLE_SE_MTU_EVT conn_id:0x%x, mtu:%d", p_data->ser_mtu.conn_id,
                  p_data->ser_mtu.mtu);
        if (server_profile_callback.bt_gatts_set_mtu_cb)
            server_profile_callback.bt_gatts_set_mtu_cb(server_profile_callback.context, p_data);
        break;
    default:
        break;
    }
...
int app_ble_server_open(void)
{
    ...
    if (device_index == 0) {
        ...
        if ((device_index >= 0) && (device_index < APP_NUM_ELEMENTS(app_xml_remote_devices_db)) &&
