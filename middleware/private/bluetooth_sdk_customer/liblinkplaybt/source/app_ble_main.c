...
//#include "linkplay_bt_interface.h"
...
#define APP_BLE_ADV_VALUE_LEN                                                                      \
    6 /*This is temporary value, Total Adv data including all fields should be <31bytes*/
...
void app_ble_display_menu(void)
{
    ...
    APP_INFO1("\t%d => Search Device Information Service",
              APP_BLECL_MENU_SEARCH_DEVICE_INFORMATION_SERVICE);
    ...
    do {
        ...
        switch(choice) {
            ...
            case APP_BLESE_MENU_CONFIG_BLE_ADV_DATA:
                ...
                adv_conf.adv_data_mask = BSA_DM_BLE_AD_BIT_FLAGS | BSA_DM_BLE_AD_BIT_SERVICE | BSA_DM_BLE_AD_BIT_APPEARANCE |
...
extern int ble_orion_received_broadcast(unsigned char *buff, int len);
extern void ble_orion_init(void);
static int g_orion_ble_start = 0;

void ble_orion_disc_cback(tBSA_DISC_EVT event, tBSA_DISC_MSG *p_data)
{
    if (event == BSA_DISC_NEW_EVT && (p_data->disc_new.eir_data[0])) {
        UINT8 *p = p_data->disc_new.eir_data;
        UINT8 eir_length;
        UINT8 eir_tag;
        ...
        while (1) {
            ...
            case HCI_EIR_COMPLETE_128BITS_UUID_TYPE:
                ...
                ble_orion_received_broadcast(p, eir_length);
                break;
            default:
                printf("Unknown EIR (Tag and data)\n");
                break;
            }
            p += eir_length;
        }
    } else if (event == BSA_DISC_CMPL_EVT && g_orion_ble_start) {
        ble_orion_init();
        app_disc_start_ble_regular(ble_orion_disc_cback);
    }
}
#endif
...
int linkplay_bluetooth_ble_server_register(tBT_UUID *uuid, BT_APP_GATTS_CB_FUNC_T *p_cback)
{
    return app_ble_server_register(uuid, p_cback);
}
int linkplay_bluetooth_ble_server_create_service(ble_create_service_params *params)
{
    int ret = 0;
    ret = app_ble_server_create_service(params->server_num, &params->service_uuid,
                                        params->num_handle, params->is_primary);
    if (ret != -1) {
        params->attr_num = ret;
        ret = 0;
    }
...
{
    int ret = 0;
    ret = app_ble_server_add_char(params->server_num, params->srvc_attr_num, &params->char_uuid,
                                  params->is_descript, params->attribute_permission,
                                  params->characteristic_property);

    if (ret != -1) {
        params->char_attr_num = ret;
        ret = 0;
    }
...
int linkplay_ble_server_send_indication(ble_server_indication *params)
{
    ...
    return app_ble_server_send_indication(params->server_num, params->attr_num,
                                          params->length_of_data, params->value,
                                          params->need_confirm);
}
int linkplay_ble_SeSendRspInit(tBSA_BLE_SE_SENDRSP *p_sendrsp)
{
    return BSA_BleSeSendRspInit(p_sendrsp);
}
int linkplay_ble_SeSendRsp(tBSA_BLE_SE_SENDRSP *p_sendrsp) { return BSA_BleSeSendRsp(p_sendrsp); }

tAPP_BLE_CB *linkplay_bluetooth_ble_get_cb(void) { return &app_ble_cb; }
