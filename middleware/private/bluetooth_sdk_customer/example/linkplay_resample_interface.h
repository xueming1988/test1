...
#endif
/*
filter = [out rate:filtersize:phaseshift:linear:cutoff]
filtersize:The length of each FIR filter in the filter bank relative to the cutoff frequency.
phaseshift:Number of entries in log2's polyphase filter bank.
linear:If 1, then the FIR filter used will be linear interpolated between the closest 2, if 0 will
be used the closest 2.
cutoff:Cutoff frequency, 1.0 corresponds to half of the output sampling rate.
*/
