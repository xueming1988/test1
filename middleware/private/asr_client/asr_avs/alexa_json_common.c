...
int create_system_params(char **buff, long inactive_ts, char *locale, char *err_msg)
{
    json_object *cmd_params = NULL;
    ...
    if (cmd_params) {
        ...
        if (locale) {
            ...
            if (json) {
                json_object_array_add(json, JSON_OBJECT_NEW_STRING(locale));
                json_object_object_add(cmd_params, KEY_locales, json);
            }
        }
 ...
 * Output: int
 */
char *create_system_cmd(int cmd_id, char *cmd_name, long inactive_ts, char *locale, char *err_msg)
{
    char *buff = NULL;
    char *params = NULL;
    ...
    create_system_params(&params, inactive_ts, locale, Val_Obj(err_msg));
    if (params) {
