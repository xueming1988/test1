...
#include <sys/types.h>
#include <sys/stat.h>
#include "amazon_lib/wwe100/inlude/pryon_lite.h"
...
static PryonLiteDecoderConfig config;
...
void loadModel(PryonLiteDecoderConfig *config)
{
    ...
    if (model_path == NULL || model_size <= 0) {
        model_size = file_size_read((char *)WWE_BIN_JP);
        model_path = (char *)(WWE_BIN_JP);
    }
...
int main(int argc, char **argv)
{
    ...
    if (argc < 2) {
        show_cpu_usage();
        fprintf(stderr, "Usage: %s 16khz_1ch_pcm_file [ threshold ]\n", argv[0]);
        return -1;
    }
