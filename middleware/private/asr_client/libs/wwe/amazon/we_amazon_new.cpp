...
PryonLiteResult g_alexa_last_plresult = {0};
int64_t g_cosume_asr_totlen = 0;
int64_t g_cosume_asr_last_totlen = 0;
#endif
...
int get_asr_amazon_enable(void);
int CCaptCosume_Asr_Amazon(char *buf, int size, void *priv);
int CCaptCosume_initAsr_Amazon(void *priv);
int CCaptCosume_deinitAsr_Amazon(void *priv);
...
static locale_info locale_info_tbl[] = {
/* USA - English */
...
#define WWE_BIN_MX WWE_PATH_PREFIX "U_250k.es-ES.alexa.bin"
...
static locale_info locale_info_tbl[] = {
    ...
    /* Australia - English */
    {LOCALE_AU, WWE_BIN_US, 500}, // FAR number is 8 for simulation ?
    /* Canada - English */
...
int CCaptCosume_Asr_Amazon(char *buf, int size, void *priv)
{
    ...
    if (dataLen) {
        ...
        ret = PryonLiteDecoder_PushAudioSamples(sDecoder, (short *)(databuf), len / 2);
...
void detectionCallback(PryonLiteDecoderHandle handle, const PryonLiteResult *result)
{
    ...
    wiimu_log(1, 0, 0, 0, (char *)"Amazon detection Result: %s", result->keyword);
...
#endif
    {
        if (strcmp(result->keyword, "ALEXA") == 0) {
            wiimu_log(1, 0, 0, 0, (char *)"Amazon Result: %s (%d/%d) score:%d len_ms:%d",
                      result->keyword, ++count, g_wiimu_shm->times_talk_on + 1, score, len_ms);

            if (len_ms < ww_len_min_ms) {
                wiimu_log(1, 0, 0, 0, (char *)"wake word too short %d < %d ms", len_ms,
                          ww_len_min_ms);
                return;
            }
#ifdef AVS_CLOUD_BASED_WWV
            printf("[CBV info] beginSampleIndex = %lld,endSampleIndex = %lld, g_cosume_asr_totlen "
                   "= %lld,dt1=%lld,dt2=%lld\n",
                   result->beginSampleIndex, result->endSampleIndex, g_cosume_asr_totlen,
                   (result->endSampleIndex - result->beginSampleIndex),
                   (g_cosume_asr_totlen - result->endSampleIndex));
            g_alexa_last_plresult.beginSampleIndex = result->beginSampleIndex;
            g_alexa_last_plresult.endSampleIndex = result->endSampleIndex;
            g_cosume_asr_last_totlen = g_cosume_asr_totlen;
            ...
            if (detect) {
                SocketClientReadWriteMsg("/tmp/RequestIOGuard",
                                         (char *)"GNOTIFY=WWE_TRIGGED:AMAZON",
                                         strlen("GNOTIFY=WWE_TRIGGED:AMAZON"), NULL, NULL, 0);
#if defined(A98_MOXIE2_256M)
                system("nanomix \"VPU1 Voice Ctrl 1007 VOICETRIGGEREVENT\" 00 00 00 01 &");
#endif
#if defined(WWE_DOUBLE_TRIGGER)
                ...
                SocketClientReadWriteMsg("/tmp/RequestASRTTS", (char *)"talkstart:1",
            ...
            wiimu_log(1, 0, 0, 0, (char *)"!!!!Amazon Result: %s score:%d len_ms:%d",
            ...
            printf("!!!!!!!!!!!!Unknown wakeup word %s score:%d len_ms:%d\n", result->keyword,
                   score, len_ms);
        }
    } else {
        wiimu_log(
            1, 0, 0, 0,
            (char
                 *)"Amazon Result: %s already waked count(%d) asr_ongoing(%d) score:%d len_ms:%d\n",
            result->keyword, count, g_wiimu_shm->asr_ongoing, score, len_ms);
    }
}

// VAD event callback
void vadCallback(const PryonLiteVadEvent *vadEvent)
{
    printf("VAD state %d\n", (int)vadEvent->vadState);
}

int file_size_read(const char *filename)
{
    int file_size = 0;
    if (filename == NULL)
        return -1;
...
{
    int locale_count = sizeof(locale_info_tbl) / sizeof(locale_info_tbl[0]);
    int i = 0;
    for (i = 0; i < locale_count; ++i) {
        printf("%.2d %.30s %10s %d %s\n", i, locale_info_tbl[i].locale_key,
               locale_info_tbl[i].model_path, locale_info_tbl[i].threshold,
               file_exist((char *)locale_info_tbl[i].model_path) ? " " : "(missing)");
    }
}
...
static void loadModel(PryonLiteDecoderConfig *config)
{
    ...
    if (lan) {
        for (i = 0; i < locale_count; ++i) {
            if (strcmp(lan, locale_info_tbl[i].locale_key) == 0 &&
                file_exist(locale_info_tbl[i].model_path)) {
                break;
            }
        }
    }
    ...
    model_size = file_size_read(locale_info_tbl[i].model_path);
    model_path = (char *)locale_info_tbl[i].model_path;
    config->detectThreshold = locale_info_tbl[i].threshold;
    ...
    printf("###loadModel SDK %s lan=%s path=%s size=%d+++++\n", WWE_SDK_VERSION, lan, model_path,
           model_size);
...
int CCaptCosume_deinitAsr_Amazon(void *priv)
{
    wiimu_log(1, 0, 0, 0, (char *)"%s++\n", __FUNCTION__);
    pthread_mutex_lock(&mutex_YDserver_op);
    PryonLiteError err = PryonLiteDecoder_Destroy(&sDecoder);
    if (err != PRYON_LITE_ERROR_OK) {
...
#ifdef AVS_CLOUD_BASED_WWV
    g_cosume_asr_totlen = 0;
    g_cosume_asr_last_totlen = 0;
#endif
    pthread_mutex_unlock(&mutex_YDserver_op);
