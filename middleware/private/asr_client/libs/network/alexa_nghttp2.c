...
#define ALEXA_NGHTTP2_FIFO ("/tmp/nghttp2_fifo")

#define ALEXA_PING ("ping")
#define ALEXA_INSERT ("insert")
#define ALEXA_QUIT ("quit")
...
#define ALEXA_NGHTTP2_DEBUG_TRACE
#if defined(ALEXA_NGHTTP2_DEBUG_INFO)
...
#if defined(ALEXA_NGHTTP2_DEBUG_TRACE) || defined(ALEXA_NGHTTP2_DEBUG_INFO)
...
#if defined(ALEXA_NGHTTP2_DEBUG_TRACE) || defined(ALEXA_NGHTTP2_DEBUG_INFO) ||                     \
    defined(ALEXA_NGHTTP2_DEBUG_ERROR)
...
static int submit_request(alexa_conn_t *conn, req_item_t *req);
...
static int create_local_sock(void)
{
    ...
    strncpy(server_addr.sun_path, ALEXA_NGHTTP2_FIFO, sizeof(server_addr.sun_path) - 1);
    unlink(ALEXA_NGHTTP2_FIFO);
...
static int local_sock_send(char *buf, size_t len)
{
    ...
    strncpy(client_addr.sun_path, ALEXA_NGHTTP2_FIFO, sizeof(client_addr.sun_path) - 1);
...
static req_item_t *create_request(alexa_request_t *req, int time_out)
{
    ...
    if (req_item) {
        ...
        req_item->time_out_ms = (time_out > 0) ? (time_out * 1000) : 0;
...
static void debug_session_status(alexa_conn_t *connection, req_item_t *req_item, int flags)
{
    ...
    int32_t stream_local_window_size = nghttp2_session_get_stream_effective_local_window_size(
        connection->session, req_item->stream_id);
    int32_t effect_recv_len = nghttp2_session_get_effective_recv_data_length(connection->session);
...
static int add_request(alexa_conn_t *connection)
{
    int ret = 0;
...
req_item_t *need_remove_request(alexa_conn_t *connection, nghttp2_session *session,
...
static int check_all_timeout(alexa_conn_t *connection, int remove_old, int flags)
{
    int ret = 0;
    struct lp_list_head *pos = NULL;
    struct lp_list_head *npos = NULL;
...
{
    int rv = 0;
    alexa_conn_t *connection = (alexa_conn_t *)user_data;
...
/* enable debug for mrm temproary by Jason.fang */
#ifdef AVS_MRM_ENABLE
        Debug_Trace("C ----------------------------> S (HEADERS)\n");
                                  ...
                                  void *user_data)
{
    ...
    case NGHTTP2_SETTINGS:
        ...
        break;
    case NGHTTP2_PING:
        Debug_Info("C <---------------------------- S (PING) stream_id(%u)\n", frame->hd.stream_id);
        break;
    case NGHTTP2_WINDOW_UPDATE:
        Debug_Info("C <---------------------------- S (WINDOW UPDATE) stream_id(%u)\n",
                   frame->hd.stream_id);
...
#if defined(TENCENT)
    if (2 >= stream_id) {
#elif defined(BAIDU_DUMI)
    if (1 >= stream_id) {
#else
    if (0 == stream_id) {
#endif
        Debug_Error("stream_id = %d; The ssl connect will be colsed at %lld \n", stream_id,
                    tickSincePowerOn());
                                 ...
                                 int lib_error_code, void *user_data)
{
    ...
    if (connection && connection->session && frame->hd.stream_id > 1) {
        ...
        if (state > NGHTTP2_STREAM_STATE_OPEN) {
            ...
            need_remove_request(connection, connection->session, frame->hd.stream_id, 1);
        }
    }
                             ...
                             int lib_error_code, void *user_data)
{
    ...
    if (lib_error_code == NGHTTP2_ERR_STREAM_CLOSING) {
        return 0;
    }
                     ...
                     void *userp)
{
    ...
    alexa_conn_t *connection = (alexa_conn_t *)userp;
    (void)flags;
    ...
    if (req_item && req_item->request && source) {
        ...
        if (req_item->request->body_left == 0) {
            if (req_item->request->upload_cb && req_item->request->upload_data) {
                upload_cb_t *upload_cb = req_item->request->upload_cb;
                void *upload_data = req_item->request->upload_data;
                pthread_mutex_unlock(&req_item->req_mutex);
                nread = upload_cb(buf, connection, length, upload_data);
                if (nread == 0) {
...
static int set_tcp_sendbuf(int fd, int send_len)
{
    ...
    if (send_len > 0) {
        getsockopt(fd, SOL_SOCKET, SO_SNDBUF, (const char *)&send_len, &optlen);
...
static int connect_to(const char *host, uint16_t port, int conn_tmout)
{
    ...
    for (rp = res; rp; rp = rp->ai_next) {
        ...
        if (conn_tmout > 0) {
            ...
            host_ip = (struct sockaddr_in *)rp->ai_addr;
            char abuf[INET_ADDRSTRLEN] = {0};
            char *addr = inet_ntop(AF_INET, &host_ip->sin_addr, abuf, INET_ADDRSTRLEN);
            Debug_Error("----- host_ip(%s)----- at %lld\n", addr, tickSincePowerOn());
#endif
...
#if defined(BAIDU_DUMI)
    char buf[64] = {0};
    snprintf(buf, sizeof(buf), "%s_%d", g_wiimu_shm->uuid, time(NULL));
    Debug_Trace("SAIYALOGID is %s\n", buf);
    Debug_Trace("deviceid is %s\n", g_wiimu_shm->uuid);
#endif

    if (!strncasecmp(ALEXA_GET, request->method, strlen(ALEXA_GET))) {
        /* Make sure that the last item is NULL */
        const nghttp2_nv nva[] = {MAKE_NV_CS(":method", request->method),
                                  MAKE_NV_CS(":path", request->path),
                                  MAKE_NV(":scheme", "https"),
                                  MAKE_NV(":authority", ALEXA_HOST_URL),
#ifdef BAIDU_DUMI
                                  MAKE_NV_CS("dueros-device-id", g_wiimu_shm->uuid),
#endif
...
#else
        stream_id = nghttp2_submit_request(conn->session, NULL, nva, sizeof(nva) / sizeof(nva[0]),
                                           &req->data_prd, req);
#endif
    }
 ...
 */
static void ctl_poll(struct pollfd *pollfd, alexa_conn_t *conn)
{
    if (pollfd && conn) {
        pollfd->events = 0;
...
{
    int rv = -1;
    alexa_conn_t *conn = NULL;
    struct sigaction act;
    nghttp2_session_callbacks *callbacks = NULL;

    conn = (alexa_conn_t *)calloc(1, sizeof(alexa_conn_t));
    if (conn) {
        ...
        SSL_get0_next_proto_negotiated(conn->ssl, &alpn, &alpnlen);
#if OPENSSL_VERSION_NUMBER >= 0x10002000L
        if (alpn == NULL) {
...
#if 0 // NGHTTP2_SETTINGS_INITIAL_WINDOW_SIZE
...
{
    alexa_conn_t *alexa_conn = *conn;

    if (alexa_conn) {

        if (alexa_conn->session) {
            nghttp2_session_del(alexa_conn->session);
            alexa_conn->session = NULL;
        }

        if (alexa_conn->ssl) {
            SSL_shutdown(alexa_conn->ssl);
            SSL_free(alexa_conn->ssl);
            alexa_conn->ssl = NULL;
        }

        if (alexa_conn->ssl_ctx) {
            SSL_CTX_free(alexa_conn->ssl_ctx);
            alexa_conn->ssl_ctx = NULL;
        }

        if (alexa_conn->ssl_fd > 0) {
            shutdown(alexa_conn->ssl_fd, SHUT_WR);
            close(alexa_conn->ssl_fd);
            alexa_conn->ssl_fd = -1;
        }
...
void AlexaNghttp2CheckRead(alexa_conn_t *conn, int timeout_ms)
{
    ...
    if (conn && timeout_ms > 0) {
        ...
        int nfds = poll(pollfds, sizeof(pollfds) / sizeof(struct pollfd), timeout_ms);
        if (0 > nfds) {
        ...
        while ((nghttp2_session_want_read(conn->session) ||
            ...
            if (0 > nfds) {
                ...
                if (pollfds[1].revents & (POLLIN | POLLOUT)) {
                    ...
                    if (!strncmp(buf, ALEXA_INSERT, strlen(ALEXA_INSERT))) {
                        rv = add_request(conn);
                        if (0 != rv) {
                            break;
                        }
                    } else if (!strncmp(buf, ALEXA_PING, strlen(ALEXA_PING))) {
                        // TODO: if ping failed, restart the connect
                        nghttp2_submit_ping(conn->session, NGHTTP2_FLAG_ACK, NULL);
                    } else if (!strncmp(buf, ALEXA_QUIT, strlen(ALEXA_QUIT))) {
                        break;
                    }
                }
            ...
            if (conn->timeout_cnt >= ALEXA_MAX_TIMEOU_CNT) {
                break;
            }
        }
...
int AlexaNghttp2SendPing(void)
{
    ...
    ret = local_sock_send(ALEXA_PING, strlen(ALEXA_PING));
...
int AlexaNghttp2Exit(void)
{
    ...
    ret = local_sock_send(ALEXA_QUIT, strlen(ALEXA_QUIT));
...
int AlexaNghttp2WaitFin(alexa_conn_t *conn, alexa_request_t *request, int timeout)
{
    ...
    if (req_item) {
        ...
        request_list_unlock();
        ret = local_sock_send(ALEXA_INSERT, strlen(ALEXA_INSERT));
        if (ret < 0) {
            request_list_lock();
            lp_list_del(&req_item->list);
            request_list_unlock();
