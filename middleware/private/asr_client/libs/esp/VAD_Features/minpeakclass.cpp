...
void MinPeakClass::setFrameSize(unsigned int frameSize)
{
    if (frameSize == 256) // 16mS
    {
        //Time Constant for the Smoother
        k_ALPHA            = 25523; //0.882553959f*2^15;
        k_ONE_MINUS_ALPHA  = 7245; //1.0f  - MinPeakClass::k_ALPHA;

        //Time Constant for attack in Min and level
        k_ATTACK           = 3631; //2^15 * 0.332871079f;
        k_ONE_MINUS_ATTACK = 29137; //2^15 * 1.0f  -  MinPeakClass::k_ATTACK;

        //Time Constant for attack in Peak tracking
        k_PEAK_ATTACK           = 4247; //0.36f * 2^15; //tuned 04-08-14
        k_ONE_MINUS_PEAK_ATTACK = 28521; //2^15* (1.0f  -  MinPeakClass::k_PEAK_ATTACK);

        // Time Constant for release in Peak tracking
        k_PEAK_RELEASE           = 32506; //2^15 * 0.996f;
        k_ONE_MINUS_PEAK_RELEASE = 262; //2^15 - (1.0f - MinPeakClass::k_PEAK_RELEASE);

        // Time Constant for release in Min  tracking
        k_MIN_RELEASE           = 32538; // 2^15 * 0.996486187f;
        k_ONE_MINUS_MIN_RELEASE = 230; // 2^15 - (1.0f - MinPeakClass::k_MIN_RELEASE);

        // Time Constant for smoothing initial Threshold
        k_THR_ATTACK           = 32441; // 2^15 * 0.995000000f;
        k_ONE_MINUS_THR_ATTACK = 327; // 2^15 *(1.0f - MinPeakClass::k_THR_ATTACK);

        // Time Constant for smooting ensemble estimation
        k_EST_FAST_ATTACK    = 20972; // 2^15 * 0.8f;
        k_ONE_MINUS_EST_FAST_ATTACK = 11796; // 2^15 * (1.0f - MinPeakClass::k_EST_FAST_ATTACK);

        k_EST_SLOW_ATTACK    = 32116; // 2^15 * 0.99f;
        k_ONE_MINUS_EST_SLOW_ATTACK = 652; // 2^15 * (1.0f - MinPeakClass::k_EST_SLOW_ATTACK);

        // Set num of frames to start adapting after extended release
        k_MAX_FRAMES_AFTER_LAST_ATTACK = 0;

        // Set initial frames to wait for
        k_WAIT_FOR_FRAMES              = 20;
    }
    else if (frameSize == 240) // 15mS
    {
        //Time Constant for the Smoother
        k_ALPHA            = 25925; //0.882553959f*2^15;
        k_ONE_MINUS_ALPHA  = 6843; //1.0f  - MinPeakClass::k_ALPHA;

        //Time Constant for attack in Min and level
        k_ATTACK           = 4166; //2^15 * 0.332871079f;
        k_ONE_MINUS_ATTACK = 28602; //2^15 * 1.0f  -  MinPeakClass::k_ATTACK;

        //Time Constant for attack in Peak tracking
        k_PEAK_ATTACK           = 4825; //0.36f * 2^15; //tuned 04-08-14
        k_ONE_MINUS_PEAK_ATTACK = 27943; //2^15* (1.0f  -  MinPeakClass::k_PEAK_ATTACK);

        // Time Constant for release in Peak tracking
        k_PEAK_RELEASE           = 32523; //2^15 * 0.996f;
        k_ONE_MINUS_PEAK_RELEASE = 245; //2^15 - (1.0f - MinPeakClass::k_PEAK_RELEASE);

        // Time Constant for release in Min  tracking
        k_MIN_RELEASE           = 32552; // 2^15 * 0.996486187f;
        k_ONE_MINUS_MIN_RELEASE = 216; // 2^15 - (1.0f - MinPeakClass::k_MIN_RELEASE);

        // Time Constant for smoothing initial Threshold
        k_THR_ATTACK           = 32461; // 2^15 * 0.995000000f;
        k_ONE_MINUS_THR_ATTACK = 307; // 2^15 *(1.0f - MinPeakClass::k_THR_ATTACK);

        // Time Constant for smooting ensemble estimation
        k_EST_FAST_ATTACK    = 21565; // 2^15 * 0.8f;
        k_ONE_MINUS_EST_FAST_ATTACK = 11203; // 2^15 * (1.0f - MinPeakClass::k_EST_FAST_ATTACK);

        k_EST_SLOW_ATTACK    = 32156; // 2^15 * 0.99f;
        k_ONE_MINUS_EST_SLOW_ATTACK = 612; // 2^15 * (1.0f - MinPeakClass::k_EST_SLOW_ATTACK);

        // Set num of frames to start adapting after extended release
        k_MAX_FRAMES_AFTER_LAST_ATTACK = 67;

        // Set initial frames to wait for
        k_WAIT_FOR_FRAMES              = 67;
    }
    else // assume 8mS
    {
        //Time Constant for the Smoother
        k_ALPHA            = 28920; //0.882553959f*2^15;
        k_ONE_MINUS_ALPHA  = 3848; //1.0f  - MinPeakClass::k_ALPHA;

        //Time Constant for attack in Min and level
        k_ATTACK           = 10908; //2^15 * 0.332871079f;
        k_ONE_MINUS_ATTACK = 21860; //2^15 * 1.0f  -  MinPeakClass::k_ATTACK;

        //Time Constant for attack in Peak tracking
        k_PEAK_ATTACK           = 11796; //0.36f * 2^15; //tuned 04-08-14
        k_ONE_MINUS_PEAK_ATTACK = 20972; //2^15* (1.0f  -  MinPeakClass::k_PEAK_ATTACK);

        // Time Constant for release in Peak tracking
        k_PEAK_RELEASE           = 32637; //2^15 * 0.996f;
        k_ONE_MINUS_PEAK_RELEASE = 131; //2^15 - (1.0f - MinPeakClass::k_PEAK_RELEASE);

        // Time Constant for release in Min  tracking
        k_MIN_RELEASE           = 32653; // 2^15 * 0.996486187f;
        k_ONE_MINUS_MIN_RELEASE = 115; // 2^15 - (1.0f - MinPeakClass::k_MIN_RELEASE);

        // Time Constant for smoothing initial Threshold
        k_THR_ATTACK           = 32604; // 2^15 * 0.995000000f;
        k_ONE_MINUS_THR_ATTACK = 164; // 2^15 *(1.0f - MinPeakClass::k_THR_ATTACK);

        // Time Constant for smooting ensemble estimation
        k_EST_FAST_ATTACK    = 26214; // 2^15 * 0.8f;
        k_ONE_MINUS_EST_FAST_ATTACK = 6554; // 2^15 * (1.0f - MinPeakClass::k_EST_FAST_ATTACK);

        k_EST_SLOW_ATTACK    = 32440; // 2^15 * 0.99f;
        k_ONE_MINUS_EST_SLOW_ATTACK = 328; // 2^15 * (1.0f - MinPeakClass::k_EST_SLOW_ATTACK);

        // Set num of frames to start adapting after extended release
        k_MAX_FRAMES_AFTER_LAST_ATTACK = 125;

        // Set initial frames to wait for
        k_WAIT_FOR_FRAMES              = 125;
    }
}
