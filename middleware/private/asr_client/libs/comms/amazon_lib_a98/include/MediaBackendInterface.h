...
struct MediaBackendConfig {
    ...
    uint32_t    targetReadLatencyInMilliseconds;            //! how far behind the writer should be from the reader after resynchronizing
    uint32_t    readLatencyThresholdInMilliseconds;         //! max read latency in msec before audio input is resynchronized
};
