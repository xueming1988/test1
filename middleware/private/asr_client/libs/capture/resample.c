...
int resample_process(void *handle, Float factor, float *inBuffer, int inBufferLen, int lastFlag,
   ...
   per loop iteration */
        ...
        *inBufferUsed += len;
        ...
        if (factor >= 1) { /* SrcUp() is faster if we can use it */
            Nout =
                lrsSrcUp(hp->X, hp->Y, factor, &hp->Time, Nx, Nwing, LpScl, Imp, ImpD, interpFilt);
        } else {
            Nout =
                lrsSrcUD(hp->X, hp->Y, factor, &hp->Time, Nx, Nwing, LpScl, Imp, ImpD, interpFilt);
        }
