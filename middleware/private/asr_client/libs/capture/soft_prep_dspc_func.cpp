...
#if defined(SOFT_PREP_DSPC_MODULE)
#include <awelib.h>
#include <vector>
#include <mutex>
#include <thread>
    ...
    while (write_running) {
        ...
        if (level >= WRITE_UNIT) {
            ...
            if (err < 0) {
                ...
                if (catpure_alsa_recovery(handle, err, 1000)) {
...
void *initialize(SOFT_PREP_INIT_PARAM input)
{
    ...
    audioData_SDS.resize(PREFERRED_SAMPLES_PER_CALLBACK_FOR_ALSA * 2);
...
int recServer_processs(Soft_prep_dspc_wrapper *handle, int *input, int frame_num, void *recPriv)
{
    ...
    if (handle->record_callback && mic_run_flag) {
        ...
        handle->record_callback((char *)output, outputCount * 4, recPriv);
    }
    m_mutex_enable.unlock();
}
...
void do_dsp_processing_fn(int *in_samples, int *out_samples, int inCount, int outCount, int &result,
                          Sample *startIndex, Sample *endIndex)
{
...
// fill_write_buffer1(&ring_buffer[rp]);
            ...
            if (DSP_state.iVal != prev_state) {
                int ret = pAwelib->SetValues(
                    MAKE_ADDRESS(AWE_VR_State_ID, AWE_VR_State_value_OFFSET), 1, &DSP_state, 0);
                if (ret != 0)
            ...
            do_dsp_processing_fn(&in_samples[0], &out_samples[0], inCount, outCount, result,
                                 &startIndex[0], &endIndex[0]);

            // convert_DSP_2ch(&out_samples[0], &audioData_SDS[0],
            // PREFERRED_SAMPLES_PER_CALLBACK_FOR_DSP);
...
void *do_pcm_read(void *priv)
{
    ...
    while (read_running) {
        ...
        if (err < 0) {
            if (catpure_alsa_recovery(capture_alsa_context, err, 1000)) {
                printf("[AFE/ALSA] ret=%d, recover failed\n", err);
            } else {
                printf("[AFE/ALSA] ret=%d, recover succeed\n", err);
            }
