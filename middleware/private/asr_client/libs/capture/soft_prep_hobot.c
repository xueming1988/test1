...
int Soft_prep_hobot_init(SOFT_PREP_INIT_PARAM input_param)
{
    ...
    Soft_prep_hobot.audio_param.NumChannels = 2;
...
int Soft_prep_hobot_flushState(void)
{
    int ret;
    ret = hobot_flushState();
    return ret;
}
...
int Soft_prep_hobot_getID(char *ID, size_t len1, char *token, size_t len2)
{
    char buf[1024] = {0};
    if (wm_db_get_ex(DB_NVRAM, "hobot_token", buf) >= 0) {
        printf("found token in config nvram\n");
        strncpy(token, buf, len2 - 1);
    }
    if (wm_db_get_ex(FACOTRY_NVRAM, "hobot_id", buf) >= 0) {
        printf("found id in factory nvram\n");
        strncpy(ID, buf, len1 - 1);
    } else {
        printf("get id from license.txt\n");
        FILE *fp;
        char buffer[1024];
        // char buf2[64];
        fp = fopen("/system/workdir/lib/license.txt", "r");
        if (fp) {
            fseek(fp, 0, SEEK_SET);
            while (!feof(fp)) {
...
int Soft_prep_hobot_setToken(char *token)
{
    if (token && strlen(token)) {
        wm_db_set_commit("hobot_token", token);
    }
    return 0;
}
...
SOFT_PRE_PROCESS Soft_prep_hobot = {
    ...
    .cSoft_PreP_flushState = hobot_flushState,
