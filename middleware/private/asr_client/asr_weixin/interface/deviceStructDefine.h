//
//  deviceStructDefine.h
//  deviceSDK
//
//  Created by cppwang(王缘) on 2019/4/25.
...
#include "./app/xiaowei/xiaoweiSDK.h"
#include "./app/voip/voipSDK.h"
...
typedef struct _device_info {
    /*!< Device system type. Select from these Enum {"Android", "Linux", "Windows", "IOS",
     * "Unknown"} */
...
typedef struct _device_notify {
     ...
     * @param status 0:success other:failed
     * @param pBinderList contacts
     * @param nCount contact number
     */
    void (*on_binder_list_change)(int error_code, DEVICE_BINDER_INFO *p_binder_list, int n_count);

    /**
     * @brief someone want to bind this device. If your device doesn't have a screen to show his
     * info, it is not recommended to handle this notify.
     ...
     * @param expireTime qr code expireTime
     */
...
typedef struct _service_calback {
    XIAOWEI_CALLBACK xiaowei_callback;
    XIAOWEI_NOTIFY xiaowei_notify;
#ifndef __NO_VOIP
    VOIP_CALLBACK voip_callback;
#endif
} SERVICE_CALLBACK;

typedef struct _device_init_path {
    /** device will write config file, data and logs in this path. An illegal path will lead to
     * system_err while calling device_init() */
    const char *system_path;

    /** device data capacity limitation. The old data will be overwrited when the data size reaches
     the threshold.
     The capacity must no less than 10M(system_path_capacity = 1024*1024*10), Set 0 to disable
     capacity limitation
     */
...
/**
 * @brief xiaowei custom log callback, call {@see device_set_log_func} before init device SDK.
 * @return SDK will do nothing if reutrn 0. Return 1 to write local log file by SDK automaticly.
 * Then xiaowei cloud can pull that log by internal command. We suggest that return 1 if log level
 * is warn or error(3 or 4). Local log file will be auto cleaned to keep total size less than your
...
typedef struct _device_ota_update_info {
    ...
    /*!< UNKNOWN = 0, SOFTWARE = 1, FIRMWARE = 2, HARDWARE = 3, only SOFTWARE support currently*/
...
typedef struct _device_ota_callback {
    /** ota check result, if new_version == false, update_info and package_info may == nullptr*/
