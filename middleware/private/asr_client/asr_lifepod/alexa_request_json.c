...
#ifdef ASR_OPUS_FORMAT
#define ASR_FORMAT_AUDIO FORMAT_AUDIO_OPUS
#else
#define ASR_FORMAT_AUDIO FORMAT_AUDIO_L16
#endif
    ...
    if (StrEq(name_space, NAMESPACE_SPEECHRECOGNIZER)) {
        if (StrEq(event_name, NAME_RECOGNIZE)) {
            ...
            json_object_object_add(json_event_payload, PAYLOAD_FORMAT,
                                   JSON_OBJECT_NEW_STRING(ASR_FORMAT_AUDIO));
        ...
        } else if (StrEq(event_name, NAME_LocalesChanged)) {
            json_object *tmp = json_object_object_get(cmd_params, KEY_locales);
            if (tmp) {
                json_object *locales = json_object_object_dup(tmp);
                if (locales)
                    json_object_object_add(json_event_payload, PAYLOAD_LOCALES, locales);
            }
        }
...
char *alexa_create_event_body(char *state_json, char *cmd_json, char **outURI)
{
    ...
    need_context = alexa_need_context(name_space, event_name);
    ...
    // ALEXA_PRINT(ALEXA_DEBUG_TRACE, " need_context [%d]\n", need_context);
    ...
    ALEXA_LOGCHECK(ALEXA_DEBUG_TRACE, "[LOG_CHECK (%s)]%.1000s\r\n", event_name, post_body);
