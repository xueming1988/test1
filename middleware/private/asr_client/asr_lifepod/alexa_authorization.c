...
int lifepod_result_get_access_token(char *body)
{
    ...
    if (json_obj) {
        ...
        refresh_token = (char *)JSON_GET_STRING_FROM_OBJECT(json_obj, "refresh_token");
        if (refresh_token) {
            if (g_lifepod_access.alexa_refresh_token) {
                free(g_lifepod_access.alexa_refresh_token);
            }
            g_lifepod_access.alexa_refresh_token = strdup(refresh_token);
        }
        ...
        if (access_token && refresh_token) {
            ...
            char *encrypt_access_token = (char *)calloc(1, strlen(access_token) * 2);
            if (encrypt_access_token) {
                lp_aes_cbc_encrypt((unsigned char *)access_token, strlen(access_token),
                                   (unsigned char *)encrypt_access_token, strlen(access_token) * 2,
                                   &aes_cbc_param);
            } else {
                fprintf(stderr, "%s: malloc encrypt lifepod_access_token fail\n", __func__);
            }
            char *encrypt_refresh_token = (char *)calloc(1, strlen(refresh_token) * 2);
            if (encrypt_refresh_token) {
                lp_aes_cbc_encrypt((unsigned char *)refresh_token, strlen(refresh_token),
                                   (unsigned char *)encrypt_refresh_token,
                                   strlen(refresh_token) * 2, &aes_cbc_param);
            } else {
                fprintf(stderr, "%s: malloc encrypt lifepod_refresh_token fail\n", __func__);
            }

            g_lifepod_access.refresh_time = tickSincePowerOn();
            char *buf = NULL;
            asprintf(&buf, "%s:%s", encrypt_access_token, encrypt_refresh_token);
            if (buf) {
                lifepod_save_cfg(buf, strlen(buf), 1);
                free(buf);
            }
            if (encrypt_access_token) {
            ...
            if (!AlexaDisableMrm()) {
                ...
                SocketClientReadWriteMsg("/tmp/alexa_mrm", "GNOTIFY=AlexaRegistrationStatus:1",
                                         strlen("GNOTIFY=AlexaRegistrationStatus:1"), NULL, NULL,
                                         1);
            }
#endif
        }
...
int pco_get_skill_enable_status(char *data)
{
    ...
    if (data && strlen(data) > 0) {
        ...
        asprintf(&authorization, PCO_AUTHORIZATION, g_lifepod_access.alexa_access_token);
...
int pco_delete_report(void)
{
    ...
    asprintf(&post_body, PCO_DELETE_REPORT, alexa_rand_string_36(message_id), g_wiimu_shm->uuid,
             g_wiimu_shm->uuid, g_lifepod_access.alexa_access_token);
...
int pco_addorupdate_report(void)
{
    ...
    asprintf(&post_body, PCO_ADD_OR_UPDATE_REPORT, alexa_rand_string_36(message_id),
             g_wiimu_shm->uuid, g_wiimu_shm->device_name, g_lifepod_access.alexa_access_token);
...
static int lifepod_refresh_acess_token(char *refresh_access_token)
{
    ...
    char *encode_refresh_token = curl_escape(refresh_access_token, strlen(refresh_access_token));
...
int remove_cfg(void)
{
    ...
    if ((access(LIFEPOD_TOKEN_PATH, 0)) == 0) { // if access_token not exist
        ...
        snprintf(buff, sizeof(buff), "rm -rf %s && sync", LIFEPOD_TOKEN_PATH);
        system(buff);
        snprintf(buff, sizeof(buff), "rm -rf %s && sync", LIFEPOD_TOKEN_PATH_SECURE);
        system(buff);
    } else {
...
int LifepodLogInWithAuthcode(char *data)
{
    ...
    g_lifepod_access.alexa_access_code = strdup(code);
    ...
    asprintf(&post_body, ALEXA_LWA_TOKEN_DATA, g_lifepod_access.alexa_access_code,
             g_lifepod_access.client_id, g_lifepod_access.code_verifier,
             g_lifepod_access.redirect_uri);
    ...
    if (data && strlen(data) > 0) {
        {
            ...
            g_lifepod_access.alexa_refresh_token = strdup(refreshToken);
            g_lifepod_access.alexa_access_token = strdup(data);
            // printf("JiangTao g_lifepod_access refreshToken = %s, accessToken = %s\n",
            // refreshToken,
            // data);
...
void LifepodGetDeviceInfo(void)
{
    ...
    if (asr_config_get(NVRAM_ALEXA_DEVICE_ID, device_id, sizeof(device_id)) >= 0 &&
        ...
        if (strlen(device_id) && strlen(client_id) && strlen(client_secret)) {
            if (g_lifepod_access.device_type_id) {
                free(g_lifepod_access.device_type_id);
            }
            if (g_lifepod_access.client_id) {
                free(g_lifepod_access.client_id);
            }
            if (g_lifepod_access.client_secret) {
                free(g_lifepod_access.client_secret);
            }

            if (g_lifepod_access.server_url) {
                free(g_lifepod_access.server_url);
            }
#if ALEXA_BETA
            g_lifepod_access.device_type_id = strdup(ALEXA_DEFAULT_DEVICE_NAME_BETA);
#else
            g_lifepod_access.device_type_id = strdup(device_id);
#endif
            g_lifepod_access.client_id = strdup(client_id);
            g_lifepod_access.client_secret = strdup(client_secret);
            if (asr_config_get(NVRAM_ALEXA_SERVER_URL, server_url, sizeof(server_url)) >= 0 &&
                strlen(server_url))
                g_lifepod_access.server_url = strdup(server_url);
            else
                g_lifepod_access.server_url = NULL;
        }
    } else {
        ...
        if (g_lifepod_access.device_type_id && g_lifepod_access.client_id &&
            g_lifepod_access.client_secret) {
        ...
        // 3. try get alexa device info from local file
            ...
            g_lifepod_access.device_type_id = strdup(ALEXA_DEFAULT_DEVICE_NAME_BETA);
#else
            g_lifepod_access.device_type_id = strdup(device_info.deviceName);
#endif
            g_lifepod_access.client_id = strdup(device_info.clientId);
            g_lifepod_access.client_secret = strdup(device_info.clientSecret);
            g_lifepod_access.server_url =
                strlen(device_info.serverUrl) ? strdup(device_info.serverUrl) : NULL;
        }
#endif
// 4. hard-coded
#if defined(LIFEPOD_DEFAULT_DEVICE_NAME_BETA) && defined(LIFEPOD_DEFAULT_CLIENT_ID) &&             \
    defined(LIFEPOD_DEFAULT_CLIENT_SECRET)
        else {
#if ALEXA_BETA
            g_lifepod_access.device_type_id = strdup(ALEXA_DEFAULT_DEVICE_NAME_BETA);
#else
            ...
            if (!g_lifepod_access.client_id) {
                ...
                lp_rc4_enc_str_by_key_type((unsigned char *)LIFEPOD_DEFAULT_CLIENT_ID,
                                           ...
                                           (unsigned char *)g_lifepod_access.client_id,
                                           LP_RC4_KEY_TYPE_AVS);
            }

            g_lifepod_access.client_secret =
                (char *)calloc(1, sizeof(LIFEPOD_DEFAULT_CLIENT_SECRET));
            if (!g_lifepod_access.client_secret) {
                ALEXA_PRINT(ALEXA_DEBUG_ERR, "calloc g_lifepod_access.client_secret failed\n");
            } else {
                lp_rc4_enc_str_by_key_type((unsigned char *)LIFEPOD_DEFAULT_CLIENT_SECRET,
                                           sizeof(LIFEPOD_DEFAULT_CLIENT_SECRET) - 1,
                                           (unsigned char *)g_lifepod_access.client_secret,
                                           LP_RC4_KEY_TYPE_AVS);
            }

#ifdef LIFEPOD_DEFAULT_SERVER_URL
            ...
            if (!g_lifepod_access.server_url) {
                ...
                lp_rc4_enc_str_by_key_type((unsigned char *)LIFEPOD_DEFAULT_SERVER_URL,
                                           ...
                                           (unsigned char *)g_lifepod_access.server_url,
                                           LP_RC4_KEY_TYPE_AVS);
            }
#else
            g_lifepod_access.server_url = NULL;
...
char *LifepodGetDeviceInfoJson(void)
{
    ...
    if (g_lifepod_access.device_type_id && g_lifepod_access.client_id) {
        ...
        if (g_lifepod_access.code_verifier) {
            free(g_lifepod_access.code_verifier);
            g_lifepod_access.code_verifier = NULL;
        }
        g_lifepod_access.code_verifier = strdup(code_verifier);
