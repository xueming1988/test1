...
unsigned char dt[7] = {0x03, 0x01, 0x04, 0x07, 0x05, 0x01, 0x00};
...
static void get_key(const unsigned char **key_pwd)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        strncpy((char *)p_tmpkey, (char *)p_passwd, i + 1);
        p_tmpkey += (i + 1);
    }
    tmpkey[31] = '\0';
...
*/
int enc_str(const unsigned char *buff_in, int len_in, unsigned char *buff_out)
{
...
int enc_file(char *file_in, char *file_out)
{
    ...
    while (1) {
        ...
        if (len_src <= 0)
            break;
        enc_str(buff_src, len_src, buff_enc);
...
int enc_file_strout(char *file_in, char *str_out)
{
    ...
    while (1) {
        ...
        if (len_src <= 0)
            break;
        enc_str(buff_src, len_src, buff_enc);
        strncpy(str_out, (char *)buff_enc, len_src);
        str_out += len_src;
    }
