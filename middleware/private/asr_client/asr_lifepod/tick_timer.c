...
tick_item_t *job_is_exist(tick_timer_t *self, int job_id)
{
        ...
        if (!lp_list_empty(&self->job_list)) {
            lp_list_for_each_safe(pos, npos, &self->job_list)
            {
                ...
                if (tick_item && tick_item->job && cur_tick > tick_item->job->start_time &&
                    cur_tick >= tick_item->job->end_time) {
                    start_time = tick_item->job->start_time;
                    do_job = tick_item->job->do_job;
                    job_ctx = tick_item->job->job_ctx;
                    if (tick_item->job->need_repeat) {
                        tick_item->job->start_time = cur_tick;
                        tick_item->job->end_time = cur_tick + tick_item->job->end_time - start_time;
                    } else {
                        lp_list_del(&tick_item->job_item);
                        if (tick_item->job) {
                            os_free(tick_item->job);
...
int tick_timer_add_item(tick_timer_t *self, tick_job_t new_job)
{
    ...
    if (self) {
        tick_item = job_is_exist(self, new_job.job_id);
        if (tick_item) {
            if (tick_item->job) {
#if 0
                tick_item->job->job_id = new_job.job_id;
                tick_item->job->need_repeat = new_job.need_repeat;
                tick_item->job->start_time = new_job.start_time;
                tick_item->job->end_time = new_job.end_time;
                tick_item->job->do_job = new_job.do_job;
                tick_item->job->job_ctx = new_job.job_ctx;
#else
