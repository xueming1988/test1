    ...
    if (js_obj) {
        ...
        if (js_payload) {
            ...
            if (js_mode) {
                ...
                if (strcmp(str_mode, "MOVIE") == 0) {
                    g_wiimu_shm->audio_equalizer = 1;
                } else if (strcmp(str_mode, "MUSIC") == 0) {
                    g_wiimu_shm->audio_equalizer = 2;
                } else if (strcmp(str_mode, "NIGHT") == 0) {
                    g_wiimu_shm->audio_equalizer = 3;
                } else if (strcmp(str_mode, "SPORT") == 0) {
                    g_wiimu_shm->audio_equalizer = 4;
                } else if (strcmp(str_mode, "TV") == 0) {
                    g_wiimu_shm->audio_equalizer = 5;
                } else {
                    g_wiimu_shm->audio_equalizer = 0; // user customize
                }

                snprintf(cmd_buf, sizeof(cmd_buf), "GNOTIFY=EQUALIZER:EQMode:%s", str_mode);
                if (0 >= SocketClientReadWriteMsg("/tmp/RequestIOGuard", cmd_buf, strlen(cmd_buf),
                                                  NULL, NULL, 0)) {
...
/*this function is only work for Marshall projects*/
/*
...
004 007 005 004 005
*/
static int alexa_parse_equalizercontroller_set_mode_for_Marshall(json_object *js_obj)
{
    if (js_obj) {
        json_object *js_payload = json_object_object_get(js_obj, KEY_PAYLOAD);
        if (js_payload) {
            ...
            if (js_mode) {
                ...
                // accept value is: "MOVIE", "MUSIC", "NIGHT", "SPORT", "TV"
                if (strcmp(str_mode, "MOVIE") == 0) { // ELECTRONIC
                    g_wiimu_shm->audio_equalizer = 1;

                    g_wiimu_shm->bass = 7;
                    g_wiimu_shm->eq_low = 4;
                    g_wiimu_shm->eq_midrange = 4; //
                    g_wiimu_shm->eq_upper = 7;
                    g_wiimu_shm->treble = 6;
                } else if (strcmp(str_mode, "MUSIC") == 0) { // Pop
                    g_wiimu_shm->audio_equalizer = 2;

                    g_wiimu_shm->bass = 6;
                    g_wiimu_shm->eq_low = 7;
                    g_wiimu_shm->eq_midrange = 8; //
                    g_wiimu_shm->eq_upper = 4;
                    g_wiimu_shm->treble = 5;
                } else if (strcmp(str_mode, "NIGHT") == 0) { // JAZZ
                    g_wiimu_shm->audio_equalizer = 3;

                    g_wiimu_shm->bass = 4;
                    g_wiimu_shm->eq_low = 7;
                    g_wiimu_shm->eq_midrange = 5; //
                    g_wiimu_shm->eq_upper = 4;
                    g_wiimu_shm->treble = 5;
                } else if (strcmp(str_mode, "SPORT") == 0) { // Rock
                    g_wiimu_shm->audio_equalizer = 4;

                    g_wiimu_shm->bass = 8;
                    g_wiimu_shm->eq_low = 6;
                    g_wiimu_shm->eq_midrange = 3; //
                    g_wiimu_shm->eq_upper = 5;
                    g_wiimu_shm->treble = 7;
                } else if (strcmp(str_mode, "TV") == 0) { // METAL
                    g_wiimu_shm->audio_equalizer = 5;

                    g_wiimu_shm->bass = 8;
                    g_wiimu_shm->eq_low = 3;
                    g_wiimu_shm->eq_midrange = 5; //
                    g_wiimu_shm->eq_upper = 7;
                    g_wiimu_shm->treble = 8;
                } else {
                    g_wiimu_shm->audio_equalizer = 0; // user customize
                }

                // snprintf(cmd_buf, sizeof(cmd_buf), "GNOTIFY=EQUALIZER:EQMode:%s", str_mode);
                snprintf(cmd_buf, sizeof(cmd_buf), "GNOTIFY=EQUALIZER:EQSet");
                if (0 >= SocketClientReadWriteMsg("/tmp/RequestIOGuard", cmd_buf, strlen(cmd_buf),
                                                  NULL, NULL, 0)) {
                    ALEXA_PRINT(ALEXA_DEBUG_ERR, "Error SetEQ send cmd failed!\n");
                }
            ...
            if (js_bands) {
                ...
                for (i = 0; i < length; i++) {
                    ...
                    if (StrEq(str_name, NAME_TREBLE)) {
                        if (StrEq(str_dir, "UP")) {
                            g_wiimu_shm->treble += delta;
                        } else if (StrEq(str_dir, "DOWN")) {
                            g_wiimu_shm->treble -= delta;
                        }
                        alexa_parse_equalizercontroller_add_event(js_event, NAME_TREBLE,
                                                                  g_wiimu_shm->treble);
                    } else if (StrEq(str_name, NAME_MIDRANGE)) {
                        if (StrEq(str_dir, "UP")) {
                            g_wiimu_shm->eq_midrange += delta;
                        } else if (StrEq(str_dir, "DOWN")) {
                            g_wiimu_shm->eq_midrange -= delta;
                        }
                        alexa_parse_equalizercontroller_add_event(js_event, NAME_MIDRANGE,
                                                                  g_wiimu_shm->eq_midrange);
                    } else if (StrEq(str_name, NAME_BASS)) {
                        if (StrEq(str_dir, "UP")) {
                            g_wiimu_shm->bass += delta;
                        } else if (StrEq(str_dir, "DOWN")) {
                            g_wiimu_shm->bass -= delta;
                        }
                        alexa_parse_equalizercontroller_add_event(js_event, NAME_MIDRANGE,
        ...
        if (js_payload) {
            ...
            if (js_bands) {
                ...
                for (i = 0; i < length; i++) {
                    ...
                    if (StrEq(str_name, NAME_TREBLE)) {
                        g_wiimu_shm->treble = 0;
                        alexa_parse_equalizercontroller_add_event(js_event, NAME_TREBLE,
                                                                  g_wiimu_shm->treble);
                    } else if (StrEq(str_name, NAME_MIDRANGE)) {
                        g_wiimu_shm->eq_midrange = 0;
                        alexa_parse_equalizercontroller_add_event(js_event, NAME_MIDRANGE,
                                                                  g_wiimu_shm->eq_midrange);
                    } else if (StrEq(str_name, NAME_BASS)) {
                        g_wiimu_shm->bass = 0;
                        alexa_parse_equalizercontroller_add_event(js_event, NAME_BASS,
                                                                  g_wiimu_shm->bass);
                    } else {
        ...
        if (js_payload) {
            ...
            if (js_bands) {
                ...
                for (i = 0; i < length; i++) {
                    ...
                    str_name = json_object_get_string(js_name);
                    level = json_object_get_int(js_level);
                    if (StrEq(str_name, NAME_TREBLE)) {
                        g_wiimu_shm->treble = level;
                        alexa_parse_equalizercontroller_add_event(js_event, NAME_TREBLE,
                                                                  g_wiimu_shm->treble);
                    } else if (StrEq(str_name, NAME_MIDRANGE)) {
                        g_wiimu_shm->eq_midrange = level;
                        alexa_parse_equalizercontroller_add_event(js_event, NAME_MIDRANGE,
                                                                  g_wiimu_shm->eq_midrange);
                    } else if (StrEq(str_name, NAME_BASS)) {
                        g_wiimu_shm->bass = level;
                        alexa_parse_equalizercontroller_add_event(js_event, NAME_BASS,
                                                                  g_wiimu_shm->bass);
                    } else {
...
static int lifepod_parse_stop_capture(json_object *js_obj)
{
    if (js_obj) {
        json_object *js_header = json_object_object_get(js_obj, KEY_HEADER);
        if (js_header) {
        ...
        // TODO:  close the microphone and stop listening
        if (ASR_IS_SERVER_VAD(lifepod_dump_info.asr_test_flag)) {
            NOTIFY_CMD(ASR_TTS, VAD_FINISHED);
        }
                ...
                if (name && name_space) {
                    ...
                    if (!avs_emplayer_directive_check(name_space)) {
                        ...
                        if (StrEq(name, NAME_SET_BANDS)) {
                            ...
                            if (StrEq(g_wiimu_shm->project_name, "Stanmore_II_Voice") ||
                                StrEq(g_wiimu_shm->project_name, "Acton_II_Voice")) {
                                alexa_parse_equalizercontroller_set_mode_for_Marshall(
                                    json_directive);
                            } else {
...
#endif
                        {
                            alexa_system_cmd(0, NAME_ExceptionEncountered, 0, message);
...
int lifepod_result_json_parse(char *message, int is_next)
{
    json_object *json_message = NULL;
    ...
    if (json_message) {
        ...
        if (json_directive) {
            ...
            if (json_header) {
                ...
                if (name && name_space) {
                    if (StrEq(name_space, NAMESPACE_TEMPLATERUNTIME)) {
                        ALEXA_LOGCHECK(ALEXA_DEBUG_TRACE, "[LFPD_LOG_CHECK <%s>]%.100s\r\n", name,
                                       message);
                    } else {
                    ...
                    if (!avs_emplayer_directive_check(name_space)) {
                        ...
                        lifepod_parse_stop_capture(json_directive);
                        /*add by weiqiang.huang for Routines 2018-11-08 -begin*/
                    } else if (StrEq(name, "NewDialogRequest")) {
                        // do nothing, just avoid send ExceptionEncountered event to AVS
                        ALEXA_PRINT(ALEXA_DEBUG_TRACE, "Rountines command will receive (%s:%s)\n",
