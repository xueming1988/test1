...
bool avs_cp_iot_certs(char *path)
{
    // three certs need to be used(copy to path). devKey, devCrt, rootCa(copeid in genrom)
    bool ret = false;
    char devKey_path[64];
    char devCrt_path[64];
    char proj_devKey_path[64];
    char proj_devCrt_path[64];

    snprintf(devKey_path, sizeof(devKey_path), "%s/%s", COMMS_CERTS_DIR_PATH,
             COMMS_CERTS_DEVKEY_NAME);
    snprintf(devCrt_path, sizeof(devCrt_path), "%s/%s", COMMS_CERTS_DIR_PATH,
             COMMS_CERTS_DEVCRT_NAME);

    // if file exists , do not need to do anything.
    if (access(devKey_path, F_OK) == 0 && access(devCrt_path, F_OK) == 0) {
        printf("CHERYL DEBUG: certs exists\n");
        ret = true;
    } else {
        // get product name, find product's certs directory, copy all files to path
        ...
        snprintf(proj_devKey_path, sizeof(proj_devKey_path), "%s/%s/%s",
                 COMMS_CERTS_PROJECT_DIR_PATH, device_info.deviceName, COMMS_CERTS_DEVKEY_NAME_ENC);
        snprintf(proj_devCrt_path, sizeof(proj_devCrt_path), "%s/%s/%s",
                 COMMS_CERTS_PROJECT_DIR_PATH, device_info.deviceName, COMMS_CERTS_DEVCRT_NAME_ENC);
        if (access(proj_devKey_path, F_OK) == 0 && access(proj_devCrt_path, F_OK) == 0) {
            lp_aes_cbc_decrypt_file(proj_devKey_path, devKey_path, &aes_cbc_param);
            lp_aes_cbc_decrypt_file(proj_devCrt_path, devCrt_path, &aes_cbc_param);
            wiimu_log(1, 0, 0, 0, "COMMS certs file decrypt: %s --> %s\n", proj_devKey_path,
                      devKey_path);
            wiimu_log(1, 0, 0, 0, "                          %s --> %s\n", proj_devCrt_path,
                      devCrt_path);
            ret = true;
        } else {
            wiimu_log(1, 0, 0, 0, "COMMS certs for %s does not exits!!", device_info.deviceName);
            ret = false;
        }
...
{
    pthread_t tid;
    // CP CERTS FILE TO IOT
    if (avs_cp_iot_certs(COMMS_CERTS_DIR_PATH)) {
        pthread_create(&tid, NULL, comms_process_monitor, NULL);
        wiimu_log(1, 0, 0, 0, "%s\n", __FUNCTION__);
    } else {
        g_have_asr_comms = 0;
        wiimu_log(1, 0, 0, 0, "%s ,comms_process_monitor does not start!\n", __FUNCTION__);
    }
}
