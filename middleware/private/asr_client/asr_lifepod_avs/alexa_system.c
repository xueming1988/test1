...
static avs_system_t g_lifepod_system;
...
static int start_ping2(void *ctx)
{
    http2_conn_ping2();
    return 0;
}
...
static int start_inactive_report(void *ctx)
{
    ...
    if (last_inactive_ts > 0 && alexa_access_token) {
        alexa_system_cmd(0, NAME_UserInactivityReport, last_inactive_ts, NULL);
    }
...
{
    memset(&g_lifepod_system, 0x0, sizeof(avs_system_t));
    pthread_mutex_init(&g_lifepod_system.state_lock, NULL);
    pthread_mutex_lock(&g_lifepod_system.state_lock);
    g_lifepod_system.tick_timer = tick_timer_create();
    g_wiimu_shm->client_last_inactive_ts = os_power_on_tick();
    pthread_mutex_unlock(&g_lifepod_system.state_lock);
...
{
    pthread_mutex_lock(&g_lifepod_system.state_lock);
    if (g_lifepod_system.tick_timer) {
        tick_timer_destroy(&g_lifepod_system.tick_timer);
        g_lifepod_system.tick_timer = NULL;
    }
    pthread_mutex_unlock(&g_lifepod_system.state_lock);
    pthread_mutex_destroy(&g_lifepod_system.state_lock);
...
int lifepod_system_add_ping_timer(void)
{
    ...
    ping_job.start_time = os_power_on_tick();
    ping_job.end_time = PING_TIME + os_power_on_tick();
    ping_job.do_job = start_ping2;
    ping_job.job_ctx = NULL;

    return tick_timer_add_item(g_lifepod_system.tick_timer, ping_job);
}
...
static int avs_system_parse_report_soft_version(json_object *js_obj)
{
    if (js_obj) {
        ...
        alexa_system_cmd(0, NAME_SoftwareInfo, 0, NULL);
...
int avs_system_update_soft_version(void)
{
    ...
    if (is_first_boot == 0) {
        alexa_system_cmd(1, NAME_SoftwareInfo, 0, NULL);
    }
