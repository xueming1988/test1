...
alexa_dump_t lifepod_dump_info;
...
void switch_alert_to_background(int val)
{
    char buf[1024];
    if (val)
        strncpy(buf, CMD_ALERT_BACK, sizeof(buf) - 1);
    else
        strncpy(buf, CMD_ALERT_FORE, sizeof(buf) - 1);
...
static int web_getAlexaProfile(SOCKET_CONTEXT *pmsg_socket, int encry)
{
    char *result = AlexaGetDeviceInfoJson();
...
static int parse_http_args(char *src, char *dest, char *name)
{
    char *p_start = NULL;
    char *p_end = NULL;
    int len = 0;
    p_start = strstr(src, name);
    if (!p_start)
        return -1;
...
static int web_setAlexaProfile(char *buf)
{
    char name[64];
    char client_id[256];
    ...
    if (parse_http_args(buf, name, "name=") || parse_http_args(buf, client_id, "client_id=") ||
        parse_http_args(buf, client_secret, "client_secret=") || parse_http_args(buf, url, "url="))
        return -1;

    if (!strlen(name) || !strlen(client_id) || !strlen(client_secret))
        return -1;
    AlexaSetDeviceInfo(name, client_id, client_secret, url);
    return 0;
}
        ...
        if (ret <= 0) {
            ...
            if (recv_num <= 0) {
                ...
                if (strncmp(recv_buf, "CAP_RESET", strlen("CAP_RESET")) == 0) {
                    ...
                    if (req_from == AWAKE_BY_WAKEWORD && g_wiimu_shm->asr_session_start == 1) {
...
//#if defined(RECORD_MODULE_ENABLE)
                    ...
                    CAIMan_YD_enable();
                } else if (strncmp(recv_buf, "getAlexaProfile", strlen("getAlexaProfile")) == 0) {
                    web_getAlexaProfile(&msg_socket, 0);
                } else if (strncmp(recv_buf, "getAvsDevInfo", strlen("getAvsDevInfo")) == 0) {
                    web_getAlexaProfile(&msg_socket, 1);
                } else if (strncmp(recv_buf, "getLifepodDevInfo", strlen("getLifepodDevInfo")) ==
                           0) {
                    web_getLifepodProfile(&msg_socket, 1);
                } else if (strncmp(recv_buf, "setAlexaProfile", strlen("setAlexaProfile")) == 0) {
                    if (!web_setAlexaProfile(recv_buf + strlen("setAlexaProfile")))
                        web_getAlexaProfile(&msg_socket, 0);
                    else
                        UnixSocketServer_write(&msg_socket, "OK", 2);
...
static ASR_ERROR_CODE do_talkstart_check(int req_from)
{
    ...
    if (req_from <= 1) {
        ...
        if (lifepod_dump_info.on_line == 0) {
            return ERR_ASR_HAVE_PROMBLEM;
        }
    }
...
static int do_talkstart_prepare(int req_from)
{
    ...
    if (req_from <= 1) {
        ...
        asprintf(&buf, "lifepodRecognizeStart:%d", req_from);
        if (buf) {
            if (!(g_wiimu_shm->asr_test_mode || g_wiimu_shm->asr_trigger_test_on ||
                  g_wiimu_shm->asr_trigger_mis_test_on))
                SocketClientReadWriteMsg("/tmp/LFPD_ASRTTS", buf, strlen(buf), NULL, NULL, 0);
            // g_wiimu_shm->asr_ongoing = 0;
            if (req_from != 2) {
                ASR_SetRecording_TimeOut(MAX_RECORD_TIMEOUT_MS);
            }
...
static int do_talkcontinue(int errCode, int req_from)
{
    ...
    ret = do_talkstart_check(g_alexa_req_from);
    if (ret) {
        do_talkstop(ret, req_from);
        return -1;
    }
    ...
    if (g_alexa_req_from != 3) {
#endif
        if (g_alexa_req_from <= 1) {
            alexa_dump_info.record_start_ts = tickSincePowerOn();
            time(&alexa_dump_info.last_dialog_ts);
        } else if (g_alexa_req_from == 4) {
            lifepod_dump_info.record_start_ts = tickSincePowerOn();
            time(&lifepod_dump_info.last_dialog_ts);
        }
        // g_alexa_req_from = 2;
        g_wiimu_shm->asr_session_start = 1;
        avs_system_set_expect_speech_timed_out(1);
        if (g_alexa_req_from <= 1) {
            ...
            SocketClientReadWriteMsg("/tmp/LFPD_ASRTTS", "lifepodRecognizeStart:2",
                                     strlen("lifepodRecognizeStart:2"), NULL, NULL, 0);
        }
        if (g_wiimu_shm->asr_mic_needless == 0) {
            if (!IsCCaptStart()) {
                printf("===== %s capture stopped, restart  =====\n", __FUNCTION__);
