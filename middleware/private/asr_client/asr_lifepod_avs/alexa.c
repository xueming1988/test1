...
static volatile int g_retry_cnt2 = 0;
...
pthread_t avs_publish_api_pid = 0; // add by weiqiang.huang for STAB-264 2018-08-13
...
static int alexa_error_code(int http_ret)
{
    ...
    case E_ERROR_TIMEOUT:
        ret = ERR_ASR_NETWORK_LOW_CONTINUE;
        break;
    case E_HTTPRET_200:
        ret = 0;
        break;
...
static int lifepod_send_recognizer(void)
{
    int ret = -1;
    int time_out = 30;
    long long record_time_out = 0;
    ...
    state_json = avs_player_state(g_alexa_req_from);
    Create_Cmd(cmd_js_str, 0, NAMESPACE_SPEECHRECOGNIZER, NAME_RECOGNIZE, Val_Obj(0));
    ret = lifepod_event_exec(state_json, cmd_js_str, (void *)g_ring_buffer, time_out);
...
static int lifepod_send_event(char *cmd_js_str)
{
    int ret = E_ERROR_INIT;
    char *state_json = NULL;
    ...
    state_json = avs_player_state(0);
    if (!g_wiimu_shm->slave_notified) {
        ret = lifepod_event_exec(state_json, cmd_js_str, NULL, 30);
    }
...
void *alexa_rcognize_proc(void *arg)
{
    ...
    if (ret != ERR_ASR_CONTINUE && g_wiimu_shm->asr_ongoing == 0) {
        if (ret == ERR_ASR_USER_LOGIN_FAIL) {
            ...
            if ((ret == ERR_ASR_PROPMT_STOP || ret == ERR_ASR_NOTHING_PLAY || ret == -1 ||
                 ret == -2 || ret == -3 || cid_playing == 0) &&
                g_wiimu_shm->asr_session_start == 0) {
                if (cid_playing == 0 || ret == -1 || ret == -2 || ret == -3) {
...
void *lifepod_rcognize_proc(void *arg)
{
    int ret = 0;
...
static int lifepod_recognize_create(pthread_t *pid)
{
    int ret = 0;
    pthread_attr_t attr;
    struct sched_param param;
    ...
    ret = pthread_create(pid, &attr, lifepod_rcognize_proc, NULL);
    if (ret) {
        ALEXA_PRINT(ALEXA_DEBUG_ERR, "lifepod_rcognize_proc failed\n");
        g_wiimu_shm->asr_ongoing = 0;
        alexa_need_talk_stop(ERR_ASR_PROPMT_STOP);
        *pid = 0;
    }
...
static int lifepod_recognize_destroy(pthread_t *pid)
{
    int ret = -1;
    ALEXA_PRINT(ALEXA_DEBUG_INFO, "start\n");
...
static void lifepod_start_request(pthread_t *pid, int req_from)
{
    lifepod_recognize_destroy(pid);

    // start the recognize request
    lifepod_recognize_create(pid);
}
        ...
        if (ret <= 0) {
            ...
            if (recv_num <= 0) {
                ...
                if (StrInclude(recv_buf, ASR_RECO_START)) {
                    ...
                    g_wiimu_shm->asr_ongoing = 0;
                } else if (StrInclude(recv_buf, ASR_INIT_CFG)) {
                    int ret = alexa_auth_read_cfg(0);
                    if (!ret)
                        UnixSocketServer_write(&msg_socket, CMD_INIT_CFGOK, strlen(CMD_INIT_CFGOK));
                    else
                        UnixSocketServer_write(&msg_socket, CMD_NO_TOKEN, strlen(CMD_NO_TOKEN));
                } else if (StrInclude(recv_buf, ASR_IS_LOGIN)) {
                    int login = 0;
                    if ((access(ALEXA_TOKEN_PATH, F_OK)) == 0) {
                    ...
                    if (cmd && strlen(cmd) > 0) {
                        ...
                        ret = AlexaAuthorizationLogIn(cmd);
                        if (ret == 0) {
                            g_retry_cnt = 0;
#ifdef EN_AVS_COMMS
                            avs_comms_notify(REPORT_ALEXA_LOGIN);
#endif
#ifdef S11_EVB_EUFY_DOT
...
/**add by weiqiang.huang for STAB-264 2018-08-13 -- end*/
#ifdef PURE_RPC /* avs.user, send to pure if login */
                            SocketClientReadWriteMsg(RPC_LOCAL_SOCKET_PARTH, "GNOTIFY=USER_STAT:1",
                                                     strlen("GNOTIFY=USER_STAT:1"), NULL, NULL, 0);
#endif
#ifdef AVS_MRM_ENABLE
...
#ifdef AVS_MRM_ENABLE
                    /* notify mrm to update registration state */
                    SocketClientReadWriteMsg("/tmp/alexa_mrm", "GNOTIFY=AlexaRegistrationStatus:0",
                                             strlen("GNOTIFY=AlexaRegistrationStatus:0"), NULL,
                                             NULL, 1);
#endif
...
void *lifepod_comand_proc(void *arg)
{
    int recv_num;
    char *recv_buf = (char *)malloc(65536);
    ...
    SOCKET_CONTEXT msg_socket;
    msg_socket.path = "/tmp/LFPD_ASRTTS";
    msg_socket.blocking = 1;
...
RE_INIT:
    ret = UnixSocketServer_Init(&msg_socket);
    if (ret <= 0) {
        ALEXA_PRINT(ALEXA_DEBUG_ERR, "LFPD_ASR UnixSocketServer_Init fail\n");
        sleep(5);
        goto RE_INIT;
    }
    ...
    while (1) {
        ret = UnixSocketServer_accept(&msg_socket);
        if (ret <= 0) {
            ALEXA_PRINT(ALEXA_DEBUG_ERR, "LFPD_ASR UnixSocketServer_accept fail\n");
            UnixSocketServer_deInit(&msg_socket);
            sleep(5);
            goto RE_INIT;
        } else {
            ...
            recv_num = UnixSocketServer_read(&msg_socket, recv_buf, 65536);
            if (recv_num <= 0) {
                ALEXA_PRINT(ALEXA_DEBUG_ERR, "LFPD_ASR recv failed\r\n");
                if (recv_num < 0) {
                    UnixSocketServer_close(&msg_socket);
                    UnixSocketServer_deInit(&msg_socket);
                    sleep(5);
                ...
                recv_buf[recv_num] = '\0';
                if (!StrInclude(recv_buf, ASR_LOGIN) && !StrInclude(recv_buf, ASR_SET_ACCESS_TOKEN))
                    ALEXA_PRINT(ALEXA_DEBUG_TRACE, "LFPD_ASR start at %lld recv_buf %s ++++++\n",
                                tickSincePowerOn(), recv_buf);
                if (StrInclude(recv_buf, LFPD_ASR_RECO_START)) {
                    int req_from = atoi(recv_buf + strlen(LFPD_ASR_RECO_START) + 1);
                    cmd_q_remove(evnet_cmd_q, (void *)NAME_PLAYFAILED, event_remove);
                    lifepod_start_request(&record_upload_pid, req_from);
                } else if (StrInclude(recv_buf, ASR_RECO_STOP)) {
                    if (record_upload_pid > 0) {
                        alexa_recognize_destroy(&record_upload_pid);
                        g_wiimu_shm->asr_session_start = 0;
                ...
                } else if (StrInclude(recv_buf, ASR_SET_LIFEPOD_AUTHCODE)) {
                    char *cmd = recv_buf + strlen(ASR_SET_LIFEPOD_AUTHCODE);
                    if (cmd && strlen(cmd) > 0) {
...
void *lifepod_down_channel_proc(void *arg)
{
    // Verify that when the device is unable to connect to the AVS webservice,
    // the device does not try to reconnect more than 50 times in 10 minutes,
    // using an appropriate exponential backoff method.
    ...
    while (1) {
        char *lifepod_access_token = NULL;

        lifepod_dump_info.on_line = 0;
        lifepod_access_token = LifepodGetAccessToken();
        if (lifepod_access_token) {
            int ret = lifepod_event_down_channel();

            if (ret != 0) {
                if (g_retry_cnt2 > 5) {
                    g_retry_cnt2 = 0;
                }
                retry_delay_ms = g_retry_table[g_retry_cnt2];
                tv.tv_sec = retry_delay_ms / 1000;
                tv.tv_usec = retry_delay_ms % 1000 * 1000;
                g_retry_cnt2++;
                ALEXA_PRINT(ALEXA_DEBUG_TRACE, "retry_time_s = %d retry_cnt=%d\n", retry_delay_ms,
...
static int cmd_add_settings_cmd(cmd_q_t *evnet_cmd_q, char *name, char *location)
{
    int ret = -1;

    if (evnet_cmd_q && name) {
        char *local_cmd = create_settings_cmd(0, name, location);
        if (local_cmd) {
            cmd_q_add_tail(evnet_cmd_q, local_cmd, strlen(local_cmd));
            free(local_cmd);
            ret = 0;
...
void *lifepod_msg_parse(void *ctx)
{
    cmd_q_t *evnet_cmd_q = (cmd_q_t *)ctx;
    int rv = 0;
    int to_cnt = 0;
    ...
    while (1) {
        char *cmd = NULL;
        if (lifepod_dump_info.on_line == 1) {
            cmd = (char *)cmd_q_pop(evnet_cmd_q);
        }
        if (cmd) {
            // ALEXA_PRINT(ALEXA_DEBUG_ERR, "cmd:\n%s\n", cmd);
            int ret = lifepod_send_event(cmd);
            if (ret >= E_HTTPRET_200 && ret < E_HTTPRET_300) {
                int flags = avs_system_get_changeendpoint();
                if (ret == E_HTTPRET_204 || flags == 0) {
                    rv = asr_event_findby_name(cmd, NAME_SYNCHORNIZESTATE);
                    if (rv == 0) {
                        // lifepod_event_sync_state();
                    }
                }
                to_cnt = 0;
                last_send = 0;
            ...
            if (recv_num <= 0) {
                ...
                if (0 == asr_event_cmd_type(recv_buf, NULL)) {
                    ...
                    if (1 == asr_event_cmd_type(recv_buf, (char *)Val_emplayer)) {
                        char *local_cmd = avs_emplayer_parse_cmd(recv_buf);
                        if (local_cmd) {
                            cmd_q_add_tail(evnet_cmd_q, local_cmd, strlen(local_cmd));
                            free(local_cmd);
                        }
                        ALEXA_PRINT(ALEXA_DEBUG_TRACE, "mplayer_cmd:\n%.100s\n", recv_buf);
#ifdef EN_AVS_BLE
                    ...
                    /* {alexaPlayPause} send play pause event, by the pause buttom is pushed */
                        ...
                        if (message_len > 0) {
                            ...
                            if (1 == alexa_dump_info.on_line) {
                                ret = cmd_add_settings_cmd(evnet_cmd_q, NAME_SettingUpdated,
                                                           location);
                                if (0 != ret) {
...
void *lifepod_msg_recv(void *ctx)
{
    int ret = 0;
    int init_ok = 0;
    int recv_num = 0;
    ...
    SOCKET_CONTEXT msg_socket;
    msg_socket.path = (char *)LIFEPOD_CMD_FD;
    msg_socket.blocking = 1;
    if (!recv_buf) {
        fprintf(stderr, "%s malloc failed\n", __FUNCTION__);
        exit(-1);
...
RE_INIT:
    ret = UnixSocketServer_Init(&msg_socket);
    if (ret <= 0) {
        ALEXA_PRINT(ALEXA_DEBUG_ERR, "LIFEPOD_EVENT UnixSocketServer_Init fail\n");
        sleep(5);
        goto RE_INIT;
    }
    ...
    while (1) {
        ret = UnixSocketServer_accept(&msg_socket);
        if (ret <= 0) {
            ALEXA_PRINT(ALEXA_DEBUG_ERR, "LIFEPOD_EVENT UnixSocketServer_accept fail, ret=%d\n",
                        ret);
            UnixSocketServer_deInit(&msg_socket);
            sleep(5);
            goto RE_INIT;
            ...
            recv_num = UnixSocketServer_read(&msg_socket, recv_buf, 65536);
            if (recv_num <= 0) {
                ALEXA_PRINT(ALEXA_DEBUG_ERR, "LIFEPOD_EVENT recv failed\r\n");
                UnixSocketServer_close(&msg_socket);
                UnixSocketServer_deInit(&msg_socket);
                sleep(5);
                goto RE_INIT;
...
int AlexaInit(void)
{
    ...
    pthread_t lifepod_pid = 0;
    pthread_t alexa_down_channel = 0;
    pthread_t lifepod_down_channel = 0;
    pthread_t alexa_event_pid = 0;
    pthread_t lifepod_event_pid = 0;
    pthread_t avs_cmd_q = 0;
    pthread_t lifepod_cmd_q = 0;
    cmd_q_t *evnet_cmd_q = NULL;
    cmd_q_t *event_cmd_q2 = NULL;
     ...
     * release supported API version to AVS server
     */
    ret = pthread_create(&avs_publish_api_pid, NULL, alexa_publish_supported_api_proc, NULL);
    if (ret) {
        ALEXA_PRINT(ALEXA_DEBUG_ERR, "pthread_create release api version failed\n");
        return -1;
    }
     ...
     * create a thread to recv comand from others, just recv login command and start recognize
     */
    ret = pthread_create(&lifepod_pid, NULL, lifepod_comand_proc, (void *)event_cmd_q2);
    if (ret) {
        ALEXA_PRINT(ALEXA_DEBUG_ERR, "pthread_create lifepod_comand_proc failed\n");
        return -1;
    }
    ...
    /*
     *  create a thread to exec EVENTS that need to send to LifePod server
     */
    ret = pthread_create(&lifepod_event_pid, NULL, lifepod_msg_recv, (void *)event_cmd_q2);
    if (ret) {
        ALEXA_PRINT(ALEXA_DEBUG_ERR, "pthread_create lifepod_msg_recv failed\n");
        return -1;
    }
    ...
    /*
     * create a thread to KEEP the device online and recv Directive from LifePod send by APP: volume
     * control, play control
     */
    ret = pthread_create(&lifepod_down_channel, NULL, lifepod_down_channel_proc, NULL);
    if (ret) {
        ALEXA_PRINT(ALEXA_DEBUG_ERR, "pthread_create life_down_channel_proc failed\n");
        return -1;
    }
    ...
    /*
     * create a thread to ping LifePod server
     */
    ret = pthread_create(&lifepod_cmd_q, NULL, lifepod_msg_parse, (void *)event_cmd_q2);
    if (ret) {
        ALEXA_PRINT(ALEXA_DEBUG_ERR, "pthread_create lifepod_msg_parse failed\n");
        return -1;
    }

    /*add by weiqiang.huang for MAR-385 Equalizer controller 2018-08-30 -begin*/
    /*get initial EQ settings from MCU*/
    if (0 >= SocketClientReadWriteMsg("/tmp/RequestIOGuard", "GNOTIFY=EQUALIZER:EQGet",
                                      strlen("GNOTIFY=EQUALIZER:EQGet"), NULL, NULL, 0)) {
...
{
    int log_in = 0;
    char *refresh_token = AlexaGetRefreshToken();
    if (refresh_token) {
        log_in = 1;
    } else {
        log_in = 0;
