...
void asr_queue_clear_speech(void)
{
    if (g_event_queue) {
        ...
        cmd_q_remove(g_event_queue, (void *)NAME_SPEECHFINISHED, event_remove);
    }
}
...
int clova_settings_request_sync_cmd(char *cmd_name, char *deviceId, int syncWithStorage)
{
    ...
    buff = create_settings_request_sync_cmd(cmd_name, deviceId, syncWithStorage);
    if (buff) {
        cmd_q_add_tail(g_event_queue, buff, strlen(buff));
        free(buff);
        ret = 0;
...
int clova_settings_report_cmd(char *cmd_name)
{
    ...
    buff = create_settings_report_cmd(cmd_name);
    if (buff) {
        cmd_q_add_tail(g_event_queue, buff, strlen(buff));
        free(buff);
        ret = 0;
...
int clova_settings_request_store_cmd(char *cmd_name)
{
    ...
    buff = create_settings_request_store_cmd(cmd_name);
    if (buff) {
        cmd_q_add_tail(g_event_queue, buff, strlen(buff));
        free(buff);
        ret = 0;
...
int cmd_add_alert_event(char *name, char *alert_token, char *alert_type)
{
    ...
    if (g_event_queue && name && alert_token) {
        char *local_cmd = create_alerts_general_cmd(name, alert_token, alert_type);
        if (local_cmd) {
