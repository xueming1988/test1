                            ...
                            char *asset_url, int need_block, int tts_type)
{
    ...
    if (asr_noti) {
        ...
        if (list_item) {
            ...
            if (get_asr_talk_start() && clova_get_extension_state() != 1) {
                list_item->need_back_ground = 1;
            } else {
                list_item->need_back_ground = 0;
            }
            if (is_exist == 0) {
                if (asset_id) {
...
void *noti_proc(void *args)
{
    ...
    printf(
        "-----------------------------------noti_proc------------------------------------------\n");
    if (!asr_noti) {
        ASR_PRINT(ASR_DEBUG_ERR, "args is NULL\n");
        return NULL;
    }
    ...
    while (asr_noti->is_running) {
        ...
        if (list_item) {
            ...
            if (list_item->asset_url && list_item->need_back_ground == 0 &&
                get_asr_talk_start() == 0) {
                focus_flags = 1;
            ...
            if (list_item->asset_url && list_item->asset_id) {
                if (list_item->tts_type == e_tts_action) {
                    cmd_add_alert_event(NAME_ALERT_STARTED, list_item->asset_id, TYPE_ACTIONTIMER);
                }
                if (StrInclude(list_item->asset_url, "http")) {
                    ...
                    if (list_item->tts_type >= e_tts_connect &&
                        list_item->tts_type < e_tts_action) {
                ...
                if (list_item->tts_type == e_tts_action) {
                    cmd_add_alert_event(NAME_ALERT_STOPPED, list_item->asset_id, TYPE_ACTIONTIMER);
                }
            }
