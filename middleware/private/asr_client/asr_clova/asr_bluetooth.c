...
static int asr_ble_scan(json_object *js_data)
{
    ...
    if (js_data) {
        ...
        if (js_payload) {
            ASR_PRINT(ASR_DEBUG_TRACE, "Bluetooth Process(directive scan) recv. +++++++\n");
...
static int asr_ble_delete(json_object *js_data)
{
    ...
    if (js_data) {
        json_object *js_payload = json_object_object_get(js_data, KEY_PAYLOAD);
        if (js_payload) {
            ASR_PRINT(ASR_DEBUG_TRACE, "Bluetooth Process(directive delete) recv. +++++++\n");
...
static int asr_ble_play(json_object *js_data)
{
    ...
    if (js_data) {
        json_object *js_payload = json_object_object_get(js_data, KEY_PAYLOAD);
        if (js_payload) {
            ASR_PRINT(ASR_DEBUG_TRACE, "Bluetooth Process(directive bt play) recv. +++++++\n");
...
static int asr_ble_notify_connect(char *address, char *role)
{
    ...
    ASR_PRINT(ASR_DEBUG_TRACE, "recv address=%s role=%s +++++++", address, role);
...
static int asr_ble_notify_disconnect(char *address, char *role)
{
    ...
    ASR_PRINT(ASR_DEBUG_TRACE, "recv address=%s role=%s+++++++\n", address, role);
    Create_Bluetooth_Comm_Params(params, Val_Str(address), Val_Str(role));
    if (params) {
        ...
        if (js_payload) {
            ...
            if (need_tts) {
                if (g_asr_ble.active_dev.friendly_name) {
                    ...
                    ASR_PRINT(ASR_DEBUG_TRACE, "!!!!!!!!!! It cannot happened !!!!!!!!!!!!\n");
                }
            }
            pthread_mutex_unlock(&g_asr_ble.state_lock);
            if (need_tts == 0) {
...
static int asr_ble_connect_pin_code(json_object *js_data)
{
    ...
    if (js_data) {
        ...
        if (js_payload) {
            ...
            if (pin_code) {
                ASR_PRINT(ASR_DEBUG_TRACE, "recv pin_code=%s +++++++\n", pin_code);
            }
        }
...
void asr_ble_getpaired_list(void)
{
    ...
    ASR_PRINT(ASR_DEBUG_TRACE, "Bluetooth Process(getpaired_list). +++++++\n");
...
static int asr_ble_del_a_dev(struct lp_list_head *dev_list, char *uuid)
{
    ...
    ASR_PRINT(ASR_DEBUG_ERR, "asr_ble_del_a_dev start\n");
    ...
    ASR_PRINT(ASR_DEBUG_ERR, "asr_ble_del_a_dev end\n");
...
static char *clova_name_btname_rule(char *src_name)
{
    ...
    if (!src_name || (src_name && strlen(src_name) <= 0)) {
        ASR_PRINT(ASR_DEBUG_TRACE, "clova_name_btname_rule name is null\n");
        goto Exit;
    }
    ...
    if (!dst_name) {
        ASR_PRINT(ASR_DEBUG_TRACE, "dst_name calloc failed\n");
        goto Exit;
    }
    ...
    ASR_PRINT(ASR_DEBUG_TRACE, "src_name=%s dst_name=%s\n", src_name, dst_name);
...
static char *asr_bluetooth_find_dev_by_uuid(struct lp_list_head *dev_list, char *uuid)
{
    char *name = NULL;
    struct lp_list_head *pos = NULL;
    struct lp_list_head *npos = NULL;
    ...
    ASR_PRINT(ASR_DEBUG_TRACE, "asr_bluetooth_find_dev_by_uuid start\n");
    if (uuid && (dev_list && !lp_list_empty(dev_list))) {
        ...
        lp_list_for_each_safe(pos, npos, dev_list)
        {
            ...
            if (device_item) {
                if (StrCaseEq(device_item->uuid, uuid)) {
                    name = device_item->friendly_name;
                    ASR_PRINT(ASR_DEBUG_TRACE, "asr_bluetooth_find_dev_by_uuid friendly_name(%s)\n",
                              name);
                    break;
                }
            }
    ...
    ASR_PRINT(ASR_DEBUG_TRACE, "asr_bluetooth_find_dev_by_uuid (%s) end\n", name ? name : "");
                                        ...
                                        char *role_name)
{
    ...
    if (StrEq(event, Val_disconnect)) {
        ...
        if (ret) {
            ...
            if (address && ((name && strlen(name) == 0) || !name)) {
                name = asr_bluetooth_find_dev_by_uuid(&g_asr_ble.scan_list, address);
            }
            dst_name = clova_name_btname_rule(name);
            if (dst_name) {
...
char *asr_bluetooth_parse_cmd(char *json_cmd_str, int *need_tts)
{
    json_bool ret = 0;
    ...
    ASR_PRINT(ASR_DEBUG_TRACE, "cmd json is \n%s\n", json_cmd_str);
    ...
    if (StrEq(event_type, Val_bluetooth)) {
        ...
        if (event) {
            ...
            *need_tts = asr_bluetooth_check_need_tts(ret, event, address, name, role_name);
