...
#if defined(LGUPLUS_IPTV_ENABLE)
int clova_cdk_applications_state(json_object *context_list)
{
    int ret;
    json_object *applications_payload = NULL;
    json_object *applications_array = NULL;
    json_object *applications_array_item1 = NULL;
    json_object *applications_array_item2 = NULL;

    applications_array_item1 = json_object_new_object();
    if (applications_array_item1) {
        json_object_object_add(applications_array_item1, ID,
                               json_object_new_string("ai.clova.lguplus.voiceremote"));
        json_object_object_add(applications_array_item1, ENABLED, json_object_new_boolean(1));
        json_object_object_add(applications_array_item1, AVAILABLE, json_object_new_boolean(1));

        applications_array_item2 = json_object_new_object();
        if (applications_array_item2) {
            json_object_object_add(applications_array_item2, ID,
                                   json_object_new_string("com.linecorp.clova.app"));
            json_object_object_add(applications_array_item2, ENABLED, json_object_new_boolean(1));
            json_object_object_add(applications_array_item2, AVAILABLE, json_object_new_boolean(1));

            applications_array = json_object_new_array();
            if (applications_array) {
                ret = json_object_array_add(applications_array, applications_array_item1);
                if (!ret) {
                    ret = json_object_array_add(applications_array, applications_array_item2);
                    if (!ret) {
                        applications_payload = json_object_new_object();
                        if (applications_payload) {
                            json_object_object_add(applications_payload, APPLICATIONS,
                                                   applications_array);
                        } else {
                            json_object_put(applications_array);
                            return -1;
                        }
                    } else {
                        json_object_put(applications_array_item2);
                        json_object_put(applications_array);
                        return -1;
                    }
                } else {
                    json_object_put(applications_array_item1);
                    json_object_put(applications_array_item2);
                    json_object_put(applications_array);
                    return -1;
                }
    ...
    ret = asr_context_list_add(context_list, IPTV_HEADER_NAMESPACE_CDK,
                               IPTV_HEADER_NAME_APPLICATIONS, applications_payload);
    if (ret) {
        json_object_put(applications_payload);
        return -1;
    }
