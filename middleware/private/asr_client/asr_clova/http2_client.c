...
static int local_sock_send(char *buf, size_t len)
{
    ...
    // if http2 connection has not been established yet, do not send message to it
    if (access(NGHTTP2_FIFO, F_OK))
        return -1;
...
static void destroy_request(req_item_t **req_item)
{
    req_item_t *req = *req_item;

    if (req) {
        Debug_Trace("start free request(%p) ++++++++++\n", req);
        pthread_mutex_destroy(&req->cond_mutex);
...
static req_item_t *create_request(http2_request_t *req, int time_out)
{
    ...
    if (req_item) {
        ...
        req_item->is_notify = 0;
        pthread_condattr_t cattr;
        pthread_condattr_init(&cattr);
        pthread_mutex_init(&req_item->cond_mutex, NULL);
        pthread_condattr_setclock(&cattr, CLOCK_MONOTONIC);
        pthread_cond_init(&req_item->req_cond, &cattr);
        pthread_condattr_destroy(&cattr);
        Debug_Trace("create req_item(%p) request(%p) ++++++++++\n", req_item, req);
    } else {
    ...
    if (!lp_list_empty(&g_request_list)) {
        lp_list_for_each_safe(pos, npos, &g_request_list)
        {
            ...
            if ((req_item && req_item->status >= REQUEST_ADDED) ||
                ...
                pthread_mutex_lock(&req_item->cond_mutex);
                req_item->is_notify = 1;
                pthread_cond_signal(&req_item->req_cond);
                pthread_mutex_unlock(&req_item->cond_mutex);
            }
        }
    }
    Debug_Trace("-------- request number = %d ----------\n", lp_list_number(&g_request_list));
    request_list_unlock();
...
req_item_t *need_remove_request(http2_conn_t *connection, nghttp2_session *session,
    ...
    if (!lp_list_empty(&g_request_list)) {
        lp_list_for_each_safe(pos, npos, &g_request_list)
        {
            ...
            if (req_item && req_item->status < REQUEST_EXIT && req_item->time_out_ms > 0 &&
                ...
                Debug_Trace("request count %d remove_old(%d) remove event_request(%p) streamid "
                            "id(%d) status(%d)\n",
                            lp_list_number(&g_request_list), remove_old, req_item,
                                 ...
                                 int lib_error_code, void *user_data)
{
    ...
    if (connection && connection->session && frame->hd.stream_id > 1) {
        nghttp2_stream *stream =
            nghttp2_session_find_stream(connection->session, frame->hd.stream_id);
        if (stream) {
...
static int submit_request(http2_conn_t *conn, req_item_t *req)
{
    ...
    if (!strncasecmp(HTTP_GET, request->method, strlen(HTTP_GET))) {
        ...
        const nghttp2_nv nva[] = {MAKE_NV_CS(":method", request->method),
                                  ...
                                  MAKE_NV("accept", "*/*"),
                                  MAKE_NV_CS("user-agent", request->user_agent)};
    ...
    req->request->request_id = conn->last_request_id;
...
int http2_conn_keep_alive(http2_conn_t *conn, http2_request_t *request, int timeout)
{
    ...
    if (req_item && ret > 0) {
        ...
        while ((nghttp2_session_want_read(conn->session) ||
               ...
               (!conn->continued) && get_internet_flags()) {
            pollfds[1].events |= POLLIN;
...
int http2_conn_do_request(http2_conn_t *conn, http2_request_t *request, int timeout)
{
    ...
    if (req_item) {
        ...
        if (ret < 0) {
            ...
            ts.tv_sec += (timeout + 2);
            ts.tv_nsec = (ts.tv_nsec + 500000) / 1000000;

            pthread_mutex_lock(&req_item->cond_mutex);
            if (req_item->is_notify == 0) {
                ret = pthread_cond_timedwait(&req_item->req_cond, &req_item->cond_mutex, &ts);
                if (ret == ETIMEDOUT) {
                    Debug_Trace("stream_id %d pthread_cond_timedwait() timed out\n",
                                req_item->stream_id);
                } else {
