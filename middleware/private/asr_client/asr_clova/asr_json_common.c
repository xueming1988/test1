...
int create_settings_request_sync_params(char **buff, int syncWithStorage)
{
    int ret = -1;
    json_object *cmd_params = NULL;
...
char *create_settings_request_sync_event(int syncWithStorage)
{
    char *buff = NULL;
    char *params = NULL;

    create_settings_request_sync_params(&params, syncWithStorage);
    if (params) {
        Create_Cmd(buff, CLOVA_HEADER_NAMESPACE_SETTINGS, CLOVA_HEADER_NAME_REQUEST_SYNCHRONIZATION,
                   params);
        free(params);
    }
...
char *create_settings_general_event(char *cmd_name)
{
    char *buff = NULL;
    char *params = NULL;
