...
/*  call state */
enum { BSA_HS_CALL_NONE, BSA_HS_CALL_INC, BSA_HS_CALL_OUT, BSA_HS_CALL_CONN };
...
#define BSA_HS_ST_SCALLSETUP                                                                       \
    0x0004 /* if value set , outgoingcall and incomming call value are valid*/
...
typedef enum { BTHF_VR_STATE_STOPPED = 0, BTHF_VR_STATE_STARTED = 1 } tBSA_BTHF_VR_STATE_T;

typedef enum { BTHF_VOLUME_TYPE_SPK = 0, BTHF_VOLUME_TYPE_MIC = 1 } tBSA_BTHF_VOLUME_TYPE_T;
...
*******************************************************************************/
BOOLEAN app_is_open_pending(void);
...
/*  linkplay  初始化蓝牙hfp  */
int linkplay_bluetooth_hfp_init(tHsCallback cb);

/*   linkplay  关闭蓝牙hfp  */
void linkplay_bluetooth_hfp_deinit(void);

/* 外部(按键等)  接听电话 */
int linkplay_bluetooth_hfp_answer_call(void);

/* 外部(按键等)  挂断电话  */
int linkplay_bluetooth_hfp_hangup_call(void);

int linkplay_bluetooth_hfp_dial_num(char *num);

int linkplay_bluetooth_hfp_dial_last_num(void);
