...
int linkplay_bluetooth_ble_server_register(tBT_UUID *uuid, tBSA_BLE_CBACK *p_cback)
{
...
{
	int ret = 0;
	ret = app_ble_server_create_service(params->server_num,&(params->service_uuid),params->num_handle,params->is_primary);
	if(ret != -1)
	{
		params->attr_num = ret;
		ret = 0;
...
{
	int ret = 0;
	ret = app_ble_server_add_char(params->server_num,
		params->srvc_attr_num,
		&(params->char_uuid),
		params->is_descript,
		params->attribute_permission,
...
int linkplay_ble_server_send_indication(ble_server_indication *params)
{
	...
	return app_ble_server_send_indication(params->server_num,params->attr_num,params->length_of_data,params->value,params->need_confirm);
