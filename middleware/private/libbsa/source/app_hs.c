...
#define APP_HS_FEATURES                                                                            \
    (BSA_HS_FEAT_ECNR | BSA_HS_FEAT_3WAY | BSA_HS_FEAT_CLIP | BSA_HS_FEAT_VREC |                   \
     BSA_HS_FEAT_RVOL | BSA_HS_FEAT_ECS | BSA_HS_FEAT_ECC | BSA_HS_FEAT_CODEC | BSA_HS_FEAT_UNAT)
...
#define APP_HS_XML_REM_DEVICES_FILE_PATH "/vendor/bsa/bt_devices.xml"
...
const char *app_hs_service_ind_name[] = {"NO SERVICE", "SERVICE AVAILABLE"};

const char *app_hs_call_ind_name[] = {"NO CALL", "ACTIVE CALL"};

const char *app_hs_callsetup_ind_name[] = {"CALLSETUP DONE", "INCOMING CALL", "OUTGOING CALL",
                                           "ALERTING REMOTE"};

const char *app_hs_callheld_ind_name[] = {"NONE ON-HOLD", "ACTIVE+HOLD", "ALL ON-HOLD"};

const char *app_hs_roam_ind_name[] = {"HOME NETWORK", "ROAMING"};
...
static UINT8 app_hs_find_indicator_id(char *ind, char *field)
{
    ...
    for (i = 0; i < string_len; i++) {
        if (ind[i] == '"') {
            if (!skip) {
                id++;
                if (!strncmp(&ind[i + 1], field, strlen(field)) &&
                    (ind[i + 1 + strlen(field)] == '"')) {
...
static void app_hs_set_initial_indicator_status(tBSA_HS_CONN_CB *p_conn, char *ind)
{
    ...
    if (p_conn->curr_service_ind < 2)
        APP_DEBUG1("Service: %s,%d", app_hs_service_ind_name[p_conn->curr_service_ind],
                          p_conn->curr_service_ind);

    if (p_conn->curr_call_ind < 2)
        APP_DEBUG1("Call: %s,%d", app_hs_call_ind_name[p_conn->curr_call_ind],
                          p_conn->curr_call_ind);

    if (p_conn->curr_call_setup_ind < 4)
        APP_DEBUG1("Callsetup: Ind %s,%d",
                          app_hs_callsetup_ind_name[p_conn->curr_call_setup_ind],
                          p_conn->curr_call_setup_ind);

    if (p_conn->curr_callheld_ind < 3)
        APP_DEBUG1("Hold: %s,%d", app_hs_callheld_ind_name[p_conn->curr_callheld_ind],
                          p_conn->curr_callheld_ind);

    if (p_conn->curr_roam_ind < 2)
        APP_DEBUG1("Roam: %s,%d", app_hs_roam_ind_name[p_conn->curr_roam_ind],
...
def PCM_ALSA
    ...
    APP_INFO1("app_hs_sco_uipc_cback len %d", pkt_len);
#if 0
def PCM_ALSA
...
{
    APP_INFO0("app_hs_audio_open");

    tBSA_STATUS status;
    tBSA_HS_AUDIO_OPEN audio_open;
    tBSA_HS_CONN_CB *p_conn = &(app_hs_cb.conn_cb[0]);

    if (app_hs_cb.sco_route == BSA_SCO_ROUTE_HCI && p_conn->uipc_channel == UIPC_CH_ID_BAD) {
...
*******************************************************************************/
BOOLEAN app_is_open_pending(void) { return app_hs_cb.open_pending; }
...
int app_hs_answer_call(void)
{
    ...
    /* sevice is headset, send AT+CKPD=200 to answer call */
    if (p_conn->connected_hs_service_id == BSA_HSP_HS_SERVICE_ID) {
		app_hs_send_keypress_evt("200");
    } else {
...
**
*******************************************************************************/
void app_hs_open_rec_file(char *filename) { app_hs_cb.rec_fd = app_wav_create_file(filename, 0); }
...
int app_hs_play_file(char *filename)
{
    ...
    APP_INFO0("app_hs_play_file");

    fd = app_wav_open_file(filename, &wav_format);

    if (fd < 0) {
        APP_ERROR0("Error could not open wav input file");
        APP_ERROR1("Use the Record audio file function to create an audio file called %s and then try "
               "again", APP_HS_SCO_OUT_SOUND_FILE);
        return -1;
    }

    do {
        nb_bytes =
            read(fd, app_hs_cb.audio_buf, sizeof(app_hs_cb.audio_buf)); /* read audio sample */
        ...
        if (TRUE != UIPC_Send(p_conn->uipc_channel, 0, (UINT8 *)app_hs_cb.audio_buf, nb_bytes)) {
...
void app_hs_cback(tBSA_HS_EVT event, tBSA_HS_MSG *p_data)
{
    ...
    if (!p_data) {
        APP_ERROR1("app_hs_cback p_data=NULL for event:%d", event);
        return;
    }

    /* retrieve the handle of the connection for which this event */
    handle = p_data->hdr.handle;
    APP_DEBUG1("app_hs_cback event:%d for handle: %d", event, handle);

    /* retrieve the connection for this handle */
    p_conn = app_hs_get_conn_by_handle(handle);

    if (!p_conn) {
        APP_ERROR1("app_hs_cback: handle %d not supported", handle);
        return;
    }
    ...
    case BSA_HS_CONN_EVT: /* Service level connection */
        ...
        app_hs_cb.open_pending = FALSE;
        //printf("    - Remote bdaddr: %02x:%02x:%02x:%02x:%02x:%02x\n", p_data->conn.bd_addr[0],
        //       p_data->conn.bd_addr[1], p_data->conn.bd_addr[2], p_data->conn.bd_addr[3],
        //       p_data->conn.bd_addr[4], p_data->conn.bd_addr[5]);
        //printf("    - Service: ");
        switch (p_data->conn.service) {
        case BSA_HSP_HS_SERVICE_ID:
            //printf("Headset\n");
            strncpy(buf, "Headset", sizeof(buf) - 1);
            break;
        case BSA_HFP_HS_SERVICE_ID:
            //printf("Handsfree\n");
            strncpy(buf, "Handsfree", sizeof(buf) - 1);
            break;
        default:
            //printf("Not supported 0x%08x\n", p_data->conn.service);
            strncpy(buf, "Not supported", sizeof(buf) - 1);
            break;
        }
        APP_INFO1("BSA_HS_CONN_EVT: bdaddr: %02x:%02x:%02x:%02x:%02x:%02x, Service: 0x%08x '%s', peer_features 0x%04x ", p_data->conn.bd_addr[0],
               p_data->conn.bd_addr[1], p_data->conn.bd_addr[2], p_data->conn.bd_addr[3],
               p_data->conn.bd_addr[4], p_data->conn.bd_addr[5], p_data->conn.service, buf,
               p_data->conn.peer_features);
    ...
    case BSA_HS_AUDIO_OPEN_EVT: /* Audio Open Event */
        ...
        if (app_hs_cb.sco_route == BSA_SCO_ROUTE_HCI && p_conn->uipc_channel != UIPC_CH_ID_BAD &&
            ...
            if (UIPC_Open(p_conn->uipc_channel, app_hs_sco_uipc_cback) != TRUE) {
                APP_ERROR1("app_hs_register failed to open UIPC channel(%d)", p_conn->uipc_channel);
                break;
            }
...
def PCM_ALSA
    ...
    case BSA_HS_AUDIO_CLOSE_EVT: /* Audio Close event */
        ...
        if (p_conn->uipc_channel != UIPC_CH_ID_BAD && p_conn->uipc_connected) {
            APP_DEBUG0("Closing UIPC Channel");
            UIPC_Close(p_conn->uipc_channel);
            p_conn->uipc_connected = FALSE;
        }
    ...
    case BSA_HS_CIEV_EVT: /* CIEV event */
        APP_INFO1("BSA_HS_CIEV_EVT handle: %d, val: %s", p_data->val.handle, p_data->val.str);
    ...
    case BSA_HS_CIND_EVT: /* CIND event */
        APP_INFO1("BSA_HS_CIND_EVT handle %d, num %d", p_data->val.handle, p_data->val.num);
    ...
    case BSA_HS_RING_EVT:
        APP_DEBUG0("BSA_HS_RING_EVT");
        break;

    case BSA_HS_CLIP_EVT:
        APP_DEBUG0("BSA_HS_CLIP_EVT");
        break;

    case BSA_HS_BSIR_EVT:
        APP_DEBUG1("BSA_HS_BSIR_EVT handle: %d, num: %d val: %s", p_data->val.handle, p_data->val.num, p_data->val.str);
        break;

    case BSA_HS_BVRA_EVT:
        APP_DEBUG0("BSA_HS_BVRA_EVT");
        break;

    case BSA_HS_CCWA_EVT:
        APP_DEBUG1("Call waiting : BSA_HS_CCWA_EVT:%s", p_data->val.str);
        break;

    case BSA_HS_CHLD_EVT:
        APP_DEBUG0("BSA_HS_CHLD_EVT");
        break;

    case BSA_HS_VGM_EVT:
        APP_DEBUG0("BSA_HS_VGM_EVT");
        break;

    case BSA_HS_VGS_EVT:
        APP_DEBUG0("BSA_HS_VGS_EVT");
        break;

    case BSA_HS_BINP_EVT:
        APP_DEBUG0("BSA_HS_BINP_EVT");
        break;

    case BSA_HS_BTRH_EVT:
        APP_DEBUG0("BSA_HS_BTRH_EVT");
        break;

    case BSA_HS_CNUM_EVT:
        APP_DEBUG1("BSA_HS_CNUM_EVT:%s", p_data->val.str);
        break;

    case BSA_HS_COPS_EVT:
        APP_DEBUG1("BSA_HS_COPS_EVT:%s", p_data->val.str);
        break;

    case BSA_HS_CMEE_EVT:
        APP_DEBUG1("BSA_HS_CMEE_EVT:%s", p_data->val.str);
        break;

    case BSA_HS_CLCC_EVT:
        APP_DEBUG1("BSA_HS_CLCC_EVT:%s", p_data->val.str);
        break;

    case BSA_HS_UNAT_EVT:
        APP_DEBUG0("BSA_HS_UNAT_EVT");
        break;

    case BSA_HS_OK_EVT:
        APP_DEBUG1("BSA_HS_OK_EVT: command value %d, %s", p_data->val.num, p_data->val.str);
        break;

    case BSA_HS_ERROR_EVT:
        APP_DEBUG0("BSA_HS_ERROR_EVT");
        break;

    case BSA_HS_BCS_EVT:
        APP_INFO1("BSA_HS_BCS_EVT: codec %d (%s)", p_data->val.num,
                (p_data->val.num == BSA_SCO_CODEC_MSBC) ? "mSBC" : "CVSD");
        app_hs_cb.hs_pcm_capture_rate = (p_data->val.num == BSA_SCO_CODEC_MSBC) ? 16000 : 8000;

        break;

    case BSA_HS_OPEN_EVT:
        APP_INFO1("BSA_HS_OPEN_EVT bd_addr %02x:%02x:%02x:%02x:%02x:%02x, handle %d, status %d, initiator %d", 
                p_data->open.bd_addr[0], p_data->open.bd_addr[1], p_data->open.bd_addr[2], p_data->open.bd_addr[3],
                p_data->open.bd_addr[4], p_data->open.bd_addr[5], p_data->open.handle, p_data->open.status, p_data->open.initiator);
        app_hs_cb.open_pending = FALSE;
        break;

    default:
        APP_INFO1("app_hs_cback unknown event:%d", event);
        break;
    }
    fflush(stdout);
...
int app_hs_hold_call(tBSA_BTHF_CHLD_TYPE_T type)
{
    ...
    APP_INFO1("app_hs_hold_call type %d", type);
...
int app_hs_last_num_dial(void)
{
    ...
    APP_INFO0("app_hs_last_num_dial");
...
int app_hs_dial_num(const char *num)
{
    ...
    APP_INFO1("app_hs_dial_num %s", num);
...
int app_hs_send_unat(char *cCmd)
{
    ...
    APP_INFO1("app_hs_send_unat:Command : %s", cCmd);
...
{
    APP_INFO0("app_hs_send_clcc_cmd");
    tBSA_HS_COMMAND cmd_param;
    tBSA_HS_CONN_CB *p_conn = NULL;
...
int app_hs_send_cops_cmd(char *cCmd)
{
    ...
    if (!cCmd)
        return -1;
    APP_INFO1("app_hs_send_cops_cmd cCmd %s", cCmd);
    /* If no connection exist, error */
    if (NULL == (p_conn = app_hs_get_default_conn())) {
        APP_DEBUG0("no connection available");
        return -1;
...
{
    APP_INFO0("app_hs_send_cind_cmd");
    tBSA_HS_COMMAND cmd_param;
    tBSA_HS_CONN_CB *p_conn = NULL;
...
int app_hs_send_dtmf(char dtmf)
{
    ...
    APP_INFO1("app_hs_send_dtmf:Command : %x", dtmf);
...
int app_hs_send_cnum(void)
{
    ...
    APP_INFO0("app_hs_send_cnum:Command");
...
int app_hs_send_keypress_evt(char *cCmd)
{
    ...
    APP_INFO0("app_hs_send_keypress_evt:Command");
...
int app_hs_start_voice_recognition(void)
{
    ...
    APP_INFO0("app_hs_start_voice_recognition:Command");
...
int app_hs_stop_voice_recognition(void)
{
    ...
    APP_INFO0("app_hs_stop_voice_recognition:Command");
...
int app_hs_set_volume(tBSA_BTHF_VOLUME_TYPE_T type, int volume)
{
    ...
    APP_INFO1("app_hs_set_volume:Command : %d, %d", type, volume);
...
/*  when  BSA_HS_AUDIO_OPEN_EVT    happens , calling start
     when  BSA_HS_AUDIO_CLOSE_EVT happens,  calling  end
*/
int app_hs_is_calling(void)
{
    tBSA_HS_CONN_CB *p_default_connect = 0;

    p_default_connect = app_hs_get_default_conn();

    if (p_default_connect == 0) {
        return 0;
    }
    return (p_default_connect->call_state == BSA_HS_CALL_CONN);
}

/*AT+XAPL=vendorID-productID-version, features
  "AT+XAPL=ABCD-1234-0100,2" .  feature  2 means only support battery indication
   if neccisary ,you can  change vendorID and productID
   for detail , search words:AT+XAPL.
 */
int app_hs_enable_battery_indicate(void)
{
    char str[30];
    memset(str, '\0', sizeof(str));
    strcpy(str, "+XAPL=ABCD-1234-0100,2"); /* "AT" is  not needed */

    return app_hs_send_unat(str);
}

/* level>=0   and level<=9 */
int app_hs_set_battery_indicate(uint level)
{
    char str[30];
    if (level > 9) {
        APP_ERROR1("level>=0&&level<=9, level %d", level);
        return -1;
    }
    sprintf(str, "+IPHONEACCEV=1,1,%d", level);
    return app_hs_send_unat(str);
}

UINT32 app_hs_get_pcm_capture_rate() { return app_hs_cb.hs_pcm_capture_rate; }
