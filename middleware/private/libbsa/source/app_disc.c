...
static void app_disc_parse_eir_uuid16(char *p_type, UINT8 *p_eir, UINT8 data_length,UINT8 * profile)
{
	...
	if(cap_a2dp & 2)
	{
		p_desc = app_disc_get_uuid16_desc(0x110B);
		if(profile && p_desc)
			strcpy(profile,p_desc);
	}else if(cap_a2dp & 1)
	{
		p_desc = app_disc_get_uuid16_desc(0x110A);
		if(profile && p_desc)
			strcpy(profile,p_desc);		
	}
...
tAPP_DISCOVERY_CB * linkplay_bluetooth_get_discovery_cb(void)
{
  return &app_discovery_cb;
}
