/*
    Copyright (C) 2018 Apple Inc. All Rights Reserved. Not to be used or disclosed without
...
static void ramstadFrontEndStereo16Pole(float L,
                                        float R,
                                        float const *a1,
                                        float const *a2,
                                        float *w1,
...
static void ramstadBackEndStereo16Pole(float const *b1,
                                       float const *b2,
                                       float *w1,
                                       float *w2,
                                       float *L,
                                    ...
                                    int inOutputStride)
{
    ...
    for (i = 0; i < inNumOutputSamples; ++i) {
        ...
        *outR = (int16_t)CLAMP(outR_i, -32768, 32767);
        ...
        fracTicks +=
            fracTickRate; //   Undefined behavior, rely on an overflow to generate a negative value
    ...
    if (unconsumedInput) {
        int excessInput = MIN(
            unconsumedInput,
            numInputsBeforeOutput); // this is the input that can be fed into the front end filter.
        int j;
...
static void ramstadPrecalculateCoeffs(double passbandWidth,
                                      ...
                                      float *a2)
{
    ...
    for (i = 0; i < kNumStages; i++) {
        double poleRealT =
            kRamstadASRCPoles16[i][kReal] * aRamstadASRCImpulseInvariantTransformFactor;
        double poleImagT =
            kRamstadASRCPoles16[i][kImag] * aRamstadASRCImpulseInvariantTransformFactor;

        a1[i] = 2.0 * exp(poleRealT) * cos(poleImagT);
        a2[i] = -exp(2.0 * poleRealT);

        double zeroRealT =
            kRamstadASRCResidues16[i][kReal] * aRamstadASRCImpulseInvariantTransformFactor;
        double zeroImagT =
            kRamstadASRCResidues16[i][kImag] * aRamstadASRCImpulseInvariantTransformFactor;
...
static void ramstadBackEndCoeffs(double alpha,
                                 ...
                                 float *b2)
{
    ...
    for (i = 0; i < kNumStages; ++i) {
        ...
        double E = -ric->poleRealExp[i] *
                   cos(ric->zeroAngle[i] - ric->poleImag[i] + (ric->poleImag[i] * alpha));
        b1[i] = K * D;
        b2[i] = K * E;

        // printf("alpha %12.6f  stage %2d   b1 %12.6f   b2 %12.6f   re %12.6f   im %12.6f   mag
        // %12.6f ang %12.6f exp %12.6f\n", alpha, i, K*D, K*E, ric->poleReal[i], ric->poleImag[i],
        // ric->zeroMag2[i], ric->zeroAngle[i], ric->poleRealExp[i]);
    }
}

static void CRamstadKernel_init()
{
...
int CRamstadASRC_inputSamplesForOutputSamples(struct CRamstadASRC *r, int inNumOutputSamples)
{
    ...
    return r->mNumInputsBeforeOutput +
           (int)(((inNumOutputSamples - 1) * r->mTickRate64 + (int64_t)r->mFracTicks) >> 31);
}

int CRamstadASRC_outputSamplesForInputSamples(struct CRamstadASRC *r,
                                              int inNumInputSamples,
                                              int *outActualInputNeeded)
{
