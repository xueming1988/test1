...
int app_wav_read_data(int fd, const tAPP_WAV_FILE_FORMAT *p_format, char *p_data, int len)
{
    ...
    if (((bytes_per_sample == 2) && ((size_t)p_data & 1)) ||
        ...
        APP_DEBUG1("Audio buffer start is not aligned on PCM sample (%d bytes) word boundary, this "
                   "could degrade system perf (%p)",
    ...
    if (nbytes > 0 && p_format->codec == BSA_AV_CODEC_PCM) {
        ...
        case 2:
            if (nbytes & 1) {
                APP_DEBUG1("Number of PCM samples read not multiple of sample size(%d)",
                           bytes_per_sample);
            }
            if (app_wav_cb.is_be) {
        ...
        case 4:
            ...
            if (app_wav_cb.is_be) {
                ...
                for (index = 0; index < nbytes; index += 4) {
                    tmp32 = *tmp32_ptr;
                    *tmp32_ptr = (tmp32 >> 24) | ((tmp32 >> 8) & 0xff00) |
                                 ((tmp32 << 8) & 0xff0000) | ((tmp32 << 24) & 0xff000000);
                    tmp32_ptr++;
                }
            }
            break;
...
int app_wav_open_file(const char *p_fname, tAPP_WAV_FILE_FORMAT *p_format)
{
    ...
    if ((tmpU32 + 8) != (UINT32)file_size) {
        APP_DEBUG1("WARNING: RIFF chunk size (%lu + 8) does not match file size(%lu)", tmpU32,
                   file_size);
    }
    ...
    do {
        ...
        if (!memcmp(&data[0], "fmt ", 4)) {
            if ((tmpU32 != 16) && (tmpU32 != 18) && (tmpU32 != 40)) {
                APP_DEBUG1("WARNING: format chunk size is not supported (%lu != (16, 18, 40))",
                           tmpU32);
                goto app_wav_open_file_error;
            }
            ...
            if ((UINT32)size != tmpU32) {
                APP_ERROR1("Length read does not match FMT params (%d != %d)", (int)size,
                           (int)tmpU32);
                goto app_wav_open_file_error;
            }
            ...
            if (tmpU32 !=
                (p_format->sample_rate * p_format->nb_channels * p_format->bits_per_sample / 8)) {
                APP_DEBUG1("WARNING: byte rate does not match PCM rate calculation (%lu != (%lu * "
                           "%u * %u / 8))",
                           tmpU32, p_format->sample_rate, p_format->nb_channels,
                           p_format->bits_per_sample);
            }
            ...
            if ((unsigned long)size != tmpU32) {
                APP_ERROR1("Length read does not match FACT chunk (%d != %d)", (int)size,
                           sizeof(data));
                goto app_wav_open_file_error;
            }
        } else if (!memcmp(&data[0], "LIST", 4) || !memcmp(&data[0], "cue ", 4)) {
            /* Bypass */
            ...
            if (size != sizeof(data)) {
                APP_ERROR1("Length read does not match LIST chunk (%d != %d)", (int)size,
                           sizeof(data));
                goto app_wav_open_file_error;
            }
            ...
            if (totalsize != file_size) {
                APP_DEBUG1("WARNING: RIFF size does not match file size (%lu != %lu))", file_size,
                           totalsize);
            }
        } else {
