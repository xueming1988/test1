...
static int _ble_add_conn_id(UINT16 add_id, UINT16 *conn_id)
{
	int i = 0;

	for (i = 0; i < MAX_BLE_CONN_NUM; i++) {
		if (conn_id[i] == BSA_BLE_INVALID_CONN) {
			conn_id[i] = add_id;
			break;
		}
	}
	if (i >= MAX_BLE_CONN_NUM) {
		APP_ERROR1("conn_id num: %d > MAX_BLE_CONN_NUM: %d\n", i, MAX_BLE_CONN_NUM);
		return -1;
	}
...
static int _ble_del_conn_id(UINT16 del_id, UINT16 *conn_id)
{
	int i = 0;

	for (i = 0; i < MAX_BLE_CONN_NUM; i++) {
		if (conn_id[i] == del_id) {
			conn_id[i] = BSA_BLE_INVALID_CONN;
			break;
		}
	}
...
tBSA_BLE_CBACK *p_server_profile_callback = NULL;
...
{
    int index, attr_num, i;
    UINT8 uuid128[MAX_UUID_SIZE*2 + 1] = {0};
    ...
    for(index = 0; index < BSA_BLE_SERVER_MAX; index++) {
        if(app_ble_cb.ble_server[index].enabled) {
            ...
            for(attr_num = 0; attr_num < BSA_BLE_ATTRIBUTE_MAX ; attr_num++) {
                ...
                if(app_ble_cb.ble_server[index].attr[attr_num].attr_UUID.len == LEN_UUID_16 || app_ble_cb.ble_server[index].attr[attr_num].attr_UUID.len == LEN_UUID_128) {
                    for(i = app_ble_cb.ble_server[index].attr[attr_num].attr_UUID.len - 1; i >= 0; i--) {
                        snprintf(uuid128 + strlen(uuid128), 3, "%02X", app_ble_cb.ble_server[index].attr[attr_num].attr_UUID.uu.uuid128[i]);
                    }
                    if((app_ble_cb.ble_server[index].attr[attr_num].attr_type == BSA_GATTC_ATTR_TYPE_SRVC) ||
                        ...
                        APP_INFO1("\t attr_num:%d:attr_type:%d:is_pri:%d, service_id:%d attr_id:%d, uuid:%s",
                                  attr_num, app_ble_cb.ble_server[index].attr[attr_num].attr_type,
...
int app_ble_server_find_index_by_conn_id(UINT16 conn_id)
{
    ...
    for(index = 0; index < BSA_BLE_CLIENT_MAX ; index++) {
		for(j= 0; j < MAX_BLE_CONN_NUM; j++)
		{
	        if(app_ble_cb.ble_server[index].conn_id[j] == conn_id) {
	            return index;
	        }
		}
    }
 ...
 **
 *******************************************************************************/
int app_ble_server_register(tBT_UUID *uuid, tBSA_BLE_CBACK *p_cback)
{
    tBSA_STATUS status;
    ...
    ble_register_param.uuid.len = uuid->len;
    if(uuid->len == LEN_UUID_16 || uuid->len == LEN_UUID_32) {
        ble_register_param.uuid.uu.uuid16 = uuid->uu.uuid16;
    } else if(uuid->len == LEN_UUID_128) {
        memcpy(ble_register_param.uuid.uu.uuid128, uuid->uu.uuid128, LEN_UUID_128);
    } else {
        APP_ERROR0("BSA_BleSeAppRegisterInit failed uuid error");
        return -1;
    }
    APP_ERROR1("uuid len1: %d", ble_register_param.uuid.len);
    APP_DUMP("app_ble_server_register1: ", ble_register_param.uuid.uu.uuid128, ble_register_param.uuid.len);
    //  if (p_cback == NULL)
    //  {
    ...
    // else
    // {
    p_server_profile_callback = p_cback;
    //}
 ...
 **
 *******************************************************************************/
int app_ble_server_create_service(int server_num, tBT_UUID *service, UINT16  num_handle, BOOLEAN  is_primary)
{
    tBSA_STATUS status;
...
//   num_handle = app_get_choice("Enter num of handle");
    ...
    ble_create_param.service_uuid.len = service->len;
    if(service->len == LEN_UUID_16 || service->len == LEN_UUID_32) {
        ble_create_param.service_uuid.uu.uuid16 = service->uu.uuid16;
    } else if(service->len == LEN_UUID_128) {
        memcpy(ble_create_param.service_uuid.uu.uuid128, service->uu.uuid128, LEN_UUID_128);
    } else {
        APP_ERROR0("BSA_BleSeCreateService failed uuid");
        return -1;
    }
    ...
    memcpy(&(app_ble_cb.ble_server[server_num].attr[attr_num].attr_UUID), service, sizeof(tBT_UUID));
    APP_ERROR1("BSA_BleSeCreateService uuid len %d attr_num %d ===debug", app_ble_cb.ble_server[server_num].attr[attr_num].attr_UUID.len, attr_num);
    APP_DUMP("service uuid: ", app_ble_cb.ble_server[server_num].attr[attr_num].attr_UUID.uu.uuid128, app_ble_cb.ble_server[server_num].attr[attr_num].attr_UUID.len);
...
//   char_uuid = app_get_choice("Add char UUID(eg. x9999)");
    ...
    ble_addchar_param.char_uuid.len = char_uuid->len;
    if(char_uuid->len == LEN_UUID_16 || char_uuid->len == LEN_UUID_32) {
        ble_addchar_param.char_uuid.uu.uuid16 = char_uuid->uu.uuid16;
    } else if(char_uuid->len == LEN_UUID_128) {
        memcpy(ble_addchar_param.char_uuid.uu.uuid128, char_uuid->uu.uuid128, LEN_UUID_128);
    } else {
        APP_ERROR0("app_ble_server_add_char failed uuid");
        return -1;
    }
    ...
    memcpy(&(app_ble_cb.ble_server[server_num].attr[char_attr_num].attr_UUID), char_uuid, sizeof(tBT_UUID));
    APP_DEBUG1("app_ble_server_add_char uuid len %d ===debug", app_ble_cb.ble_server[server_num].attr[char_attr_num].attr_UUID.len);
    APP_DUMP("app_ble_server_add_char uuid:", app_ble_cb.ble_server[server_num].attr[char_attr_num].attr_UUID.uu.uuid128, app_ble_cb.ble_server[server_num].attr[char_attr_num].attr_UUID.len);
    ...
    if(status != BSA_SUCCESS) {
        ...
        app_ble_cb.ble_server[server_num].attr[char_attr_num].wait_flag = FALSE;
        return -1;
    }
 ...
 **
 *******************************************************************************/
int app_ble_server_send_indication(int num, int attr_num, int length_of_data, UINT8 *value, BOOLEAN need_confirm)
{
    tBSA_STATUS status;
...
//   app_ble_server_display();
    ...
    //      return -1;
	for(index = 0 ; index < MAX_BLE_CONN_NUM; index++)
	{
		if(app_ble_cb.ble_server[num].conn_id[index] == BSA_BLE_INVALID_CONN)
			continue;
	    ...
	    ble_sendind_param.conn_id = app_ble_cb.ble_server[num].conn_id[index];
	    ble_sendind_param.attr_id = app_ble_cb.ble_server[num].attr[attr_num].attr_id;
		//printf("ble_sendind_param.conn_id %d \r\n",ble_sendind_param.conn_id);
		//printf("ble_sendind_param.attr_id %d \r\n",ble_sendind_param.attr_id);
		

	    ble_sendind_param.data_len = length_of_data;
	    memcpy(ble_sendind_param.value, value, length_of_data);


	    ble_sendind_param.need_confirm = need_confirm;
...
void app_ble_server_profile_cback(tBSA_BLE_EVT event,  tBSA_BLE_MSG *p_data)
{
    ...
    switch(event) {
        ...
        case BSA_BLE_SE_ADDCHAR_EVT:
            APP_INFO1("BSA_BLE_SE_ADDCHAR_EVT status:%d, server_if: %d, service_id: %d", p_data->ser_addchar.status, p_data->ser_addchar.server_if, p_data->ser_addchar.service_id);
        ...
        case BSA_BLE_SE_READ_EVT:
#if !defined(ASR_WEIXIN_MODULE) && !defined(NAVER_LINE)
            ...
            APP_INFO1("BSA_BLE_SE_READ_EVT: send_server_resp.conn_id:%d, send_server_resp.trans_id:%d", send_server_resp.conn_id,
                      send_server_resp.trans_id, send_server_resp.status);
            APP_INFO1("BSA_BLE_SE_READ_EVT: send_server_resp.status:%d,send_server_resp.auth_req:%d", send_server_resp.status,
                      send_server_resp.auth_req);
            APP_INFO1("BSA_BLE_SE_READ_EVT: send_server_resp.handle:%d, send_server_resp.offset:%d, send_server_resp.len:%d",
        ...
        case BSA_BLE_SE_WRITE_EVT:
            ...
            if(p_data->ser_write.need_rsp) {
                ...
                APP_INFO1("BSA_BLE_SE_WRITE_EVT: send_server_resp.conn_id:%d, send_server_resp.trans_id:%d", send_server_resp.conn_id,
                          send_server_resp.trans_id, send_server_resp.status);
                APP_INFO1("BSA_BLE_SE_WRITE_EVT: send_server_resp.status:%d,send_server_resp.auth_req:%d", send_server_resp.status,
                          send_server_resp.auth_req);
                APP_INFO1("BSA_BLE_SE_WRITE_EVT: send_server_resp.handle:%d, send_server_resp.offset:%d, send_server_resp.len:%d",
                          send_server_resp.handle, send_server_resp.offset, send_server_resp.len);
                BSA_BleSeSendRsp(&send_server_resp);
            }
            break;
        ...
        case BSA_BLE_SE_OPEN_EVT:
            ...
            if(p_data->ser_open.reason == BSA_SUCCESS) {
				...
				_ble_add_conn_id(p_data->ser_open.conn_id, app_ble_cb.ble_server[num].conn_id);
#if 0
                /* XML Database update */
        ...
        case BSA_BLE_SE_CLOSE_EVT:
			...
			_ble_del_conn_id(p_data->ser_close.conn_id, app_ble_cb.ble_server[num].conn_id);
            break;
    ...
    if(p_server_profile_callback)
        p_server_profile_callback(event, p_data);
}
...
int app_ble_server_open(void)
{
    ...
    if(app_ble_cb.ble_server[server_num].conn_id[0] != BSA_BLE_INVALID_CONN) {
...
//   server_num = app_get_choice("Select");
	...
	{	
	    ...
	    ble_close_param.conn_id = app_ble_cb.ble_server[server_num].conn_id[index];
