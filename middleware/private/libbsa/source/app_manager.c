...
/* Default local Name */
#define APP_DEFAULT_BT_NAME             "Linkplay_BT"

#define APP_DEFAULT_BLE_NAME             "Linkplay_BLE"

/* Default COD SetTopBox (Major Service = none) (MajorDevclass = Audio/Video) (Minor=STB) */
#define APP_DEFAULT_CLASS_OF_DEVICE     {0x20, 0x04, 0x28}
 ...
 **
 *******************************************************************************/
int app_mgr_set_bt_config(BOOLEAN enable,BOOLEAN discoverable,BOOLEAN connectable)
{
    int status;
    ...
    APP_DEBUG1("Config_mask:0x%x", bt_config.config_mask);
...
void app_mgr_set_callback(tsecuritycback spcb)
{
	g_securitycback = spcb;
}
...
void app_mgr_security_callback(tBSA_SEC_EVT event, tBSA_SEC_MSG *p_data)
{
    ...
    case BSA_SEC_LINK_UP_EVT:       /* A device is physically connected (for info) */
		...
		if(remote_device && !remote_device->last_active)
		{
    		app_xml_add_dev_db(app_xml_remote_devices_db,
				APP_NUM_ELEMENTS(app_xml_remote_devices_db),  p_data->link_up.bd_addr);
    ...
    case BSA_SEC_PIN_REQ_EVT:
        APP_DEBUG1("BSA_SEC_PIN_REQ_EVT (Pin Code Request) received from: %02x:%02x:%02x:%02x:%02x:%02x, name: %s",
                p_data->pin_req.bd_addr[0], p_data->pin_req.bd_addr[1],
                p_data->pin_req.bd_addr[2], p_data->pin_req.bd_addr[3],
                p_data->pin_req.bd_addr[4], p_data->pin_req.bd_addr[5], p_data->pin_req.bd_name);
        g_sec_sp_cfm_req = 1;
        break;
         ...
         */
...
#endif
         {
	        ...
	        app_xml_add_trusted_services_db(app_xml_remote_devices_db,
	                ...
	                1 << p_data->authorize.service,PROFILE_TYPE_UNKNOWN);
...
/*******************************************************************************
 **
 ** Function         app_mgr_sec_disconnect
 **
 ...
 **
 *******************************************************************************/
int app_mgr_sec_disconnect(BD_ADDR bd_addr)
{
    int status;
    int device_index;
    tBSA_SEC_DISCONNECT_DEV sec_disconnect;

    APP_INFO0("app_mgr_sec_disconnect");

    /* Remove the device from Security database (BSA Server) */
    BSA_SecDisconnectDeviceInit(&sec_disconnect);
#if 0
    /* Read the XML file which contains all the bonded devices */
    app_read_xml_remote_devices();
    ...
    device_index = app_get_choice("Select device to disconnect");
    if ((device_index >= 0) &&
        (device_index < APP_MAX_NB_REMOTE_STORED_DEVICES) &&
        (app_xml_remote_devices_db[device_index].in_use != FALSE))
    {
        bdcpy(sec_disconnect.bd_addr, app_xml_remote_devices_db[device_index].bd_addr);
        status = BSA_SecDisconnectDevice(&sec_disconnect);
    }
    else
    {
        APP_ERROR1("Wrong index [%d]",device_index);
...
#else
    bdcpy(sec_disconnect.bd_addr, bd_addr);
    status = BSA_SecDisconnectDevice(&sec_disconnect);
#endif
    if (status != BSA_SUCCESS)
    {
        APP_ERROR1("app_mgr_sec_disconnect Operation Failed with status [%d]",status);
...
int app_mgr_config(const char *bt_name,UINT8 *btaddr)
{
    ...
    if (status < 0)
    {
        ...
        strncpy((char *)app_xml_config.ble_name, bt_name, sizeof(app_xml_config.ble_name));
    ...
    app_mgr_set_bt_config(app_xml_config.enable,app_xml_config.discoverable,app_xml_config.connectable);
...
int app_mgr_set_local_name(char *name)
{
   /* Example of function to get the Local Bluetooth configuration */
   int status;
   tBSA_DM_GET_CONFIG bt_config;
   ...
   /* Update bt name and ble name together */
   strncpy((char *)app_xml_config.name, name, sizeof(app_xml_config.name));
   app_xml_config.name[sizeof(app_xml_config.name) - 1] = '\0';
   strncpy((char *)app_xml_config.ble_name, name, sizeof(app_xml_config.ble_name));
   app_xml_config.ble_name[sizeof(app_xml_config.ble_name) - 1] = '\0';

    /* Example of function to set the Local Bluetooth configuration */
    app_mgr_set_bt_config(app_xml_config.enable,bt_config.discoverable,bt_config.connectable);
...
int app_mgr_get_bt_addr(char *addr)
{
    int status;
    tBSA_DM_GET_CONFIG bt_config;
    ...
    APP_INFO1("Bdaddr %02x:%02x:%02x:%02x:%02x:%02x",
                  ...
                  bt_config.bd_addr[4], bt_config.bd_addr[5]);
    if(addr) {
        snprintf(addr, 18, "%02x:%02x:%02x:%02x:%02x:%02x", bt_config.bd_addr[0], bt_config.bd_addr[1],
                 bt_config.bd_addr[2], bt_config.bd_addr[3],
                 bt_config.bd_addr[4], bt_config.bd_addr[5]);
    }
    return 0;
