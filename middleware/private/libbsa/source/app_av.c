...
#ifndef APP_AV_FEAT_MASK/* Default: Target (receive command) and Controller (send command) enabled and Copy Protection Enabled*/
...
tAPP_AV_RC_PEER_INFO g_peer_avrcp_info[APP_AV_MAX_CONNECTIONS];
...
static tBSA_SEC_AUTH g_av_sec_mask = BSA_SEC_AUTHENTICATION; // need passkey for pairing
/*
 * Local functions
 */
...
/*******************************************************************************
 **
 ** Function         app_av_find_connection_index_by_bd_addr
 **
 ** Description      This function finds the connection structure by its handle
 **
 ** Returns          index
 **
 *******************************************************************************/
static int app_av_find_connection_index_by_bd_addr(BD_ADDR bd_addr)
{
    int index;
    for (index = 0; index < APP_AV_MAX_CONNECTIONS; index++)
    {
...
/*******************************************************************************
 **
 ** Function         app_av_is_peer_support_abs_volume
 **
 ** Description      This function finds the connection structure by its handle
 **
 ** Returns          index
 **
 *******************************************************************************/
static BOOLEAN app_av_is_peer_support_abs_volume(BD_ADDR bd_addr)
{
    int index;
    for (index = 0; index < APP_AV_MAX_CONNECTIONS; index++)
    {
        if (bdcmp(g_peer_avrcp_info[index].peer_addr, bd_addr) == 0)
            return g_peer_avrcp_info[index].support_abs_volume;
    }
    return FALSE;
}
...
void app_av_cback(tBSA_AV_EVT event, tBSA_AV_MSG *p_data)
{
        ...
        else
        {
            if (p_data->open.status == BSA_SUCCESS)
            {
                ...
                app_xml_add_trusted_services_db(app_xml_remote_devices_db,
                        ...
                        BSA_A2DP_SERVICE_MASK | BSA_AVRCP_SERVICE_MASK,PROFILE_TYPE_A2DP_SINK);
    ...
    case BSA_AV_RC_OPEN_EVT:
        APP_DEBUG1("BSA_AV_RC_OPEN_EVT status:%d handle:%d, peer_feature=0x%x", p_data->rc_open.status, p_data->rc_open.rc_handle, p_data->rc_open.peer_features);
        ...
        else if (p_data->rc_open.status == BSA_SUCCESS)
        {
            ...
            index = app_av_find_connection_index_by_bd_addr(p_data->rc_open.peer_addr);
            if (index >= APP_AV_MAX_CONNECTIONS)
                break;
            g_peer_avrcp_info[index].rc_handle = p_data->rc_open.rc_handle;
            if (p_data->rc_open.peer_features & BTA_AV_FEAT_ADV_CTRL)
                g_peer_avrcp_info[index].support_abs_volume = TRUE;
            else
                g_peer_avrcp_info[index].support_abs_volume = FALSE;
            memcpy(g_peer_avrcp_info[index].peer_addr, p_data->rc_open.peer_addr, 6);
            if(p_data->rc_open.peer_features & BTA_AV_FEAT_ADV_CTRL &&
                p_data->rc_open.peer_features & BTA_AV_FEAT_RCTG)
                app_av_rc_send_register_volume_change_notify_vd_command(p_data->rc_open.rc_handle);
        }
        break;
    ...
    case BSA_AV_META_MSG_EVT:
        ...
        case BSA_AV_FEAT_EVT:
            ...
            for (index = 0; index < APP_AV_MAX_CONNECTIONS; index++)
            {
                if (g_peer_avrcp_info[index].rc_handle == p_data->rc_feat.rc_handle) {
                    if (p_data->rc_feat.peer_features & BTA_AV_FEAT_ADV_CTRL)
                        g_peer_avrcp_info[index].support_abs_volume = TRUE;
                    break;
                }
...
int app_av_open(BD_ADDR *bd_addr_in)
{
	...
	printf("Connecting bdaddr %02x:%02x:%02x:%02x:%02x:%02x\n",
    ...
    if (connect)
    {
        ...
        /* Indicate if AVRC must be used */
        open_param.use_rc = APP_AV_USE_RC;
        open_param.sec_mask = g_av_sec_mask;
        status = BSA_AvOpen(&open_param);
        if (status != BSA_SUCCESS)
        {
            APP_ERROR1("BSA_AvOpen(%02X:%02X:%02X:%02X:%02X:%02X) failed: %d",
		...
		while(app_av_cb.open_pending == TRUE && (try_count-- > 0 || g_sec_sp_cfm_req)) {
			if(g_sec_sp_cfm_req) try_count = 200;
			usleep(20000);
		}
...
int app_av_play_pcm_data(tBSA_AV_MEDIA_FEED_CFG_PCM *pcm_config)
{
	...
	BSA_AvStartInit(&start_param);
	start_param.media_feeding.format = BSA_AV_CODEC_PCM;
	start_param.media_feeding.cfg.pcm.sampling_freq = pcm_config->sampling_freq;  /* 44100, 48000 etc */
	start_param.media_feeding.cfg.pcm.num_channel = pcm_config->num_channel;	/* 1 for mono or 2 stereo */
...
static int app_av_rc_send_register_volume_change_notify_vd_command(UINT8 rc_handle)
{
    ...
    APP_INFO1("app_av_rc_send_register_volume_change_notify_vd_command rc_handle %d", rc_handle);
    return 0;
}
/*******************************************************************************
 **
...
int app_av_rc_send_absolute_volume_vd_command(int index, int volume)
{
    ...
    if (app_av_is_peer_support_abs_volume(app_av_cb.connections[index].bd_addr) == FALSE) {
        APP_ERROR0("The peer does NOT support absolute volume!");
        return -1;
    }
...
int app_av_rc_send_get_element_attributes_meta_response(int index)
{
   ...
   pthread_mutex_lock(&media_mutex);
    ...
    for (xx=0; xx<attr_count && xx<BSA_AVRC_MAX_ATTR_COUNT ; xx++)
    {
        ...
        {
            memset((char*) p_attr[xx].str,0x0,BSA_AV_META_INFO_LEN_MAX);
...
int app_av_rc_send_play_status_meta_response(int index)
{
    ...
    if(app_av_cb.play_state == APP_AV_PLAY_STARTED)
    bsa_av_meta_rsp_cmd.param.get_play_status.play_status = p_bsa_av_cb->meta_info.play_status.play_status ;
    else if(app_av_cb.play_state == APP_AV_PLAY_PAUSED )
    {
    	p_bsa_av_cb->meta_info.play_status.play_status = BSA_AVRC_PLAYSTATE_PAUSED;
    ...
    if(bsa_av_meta_rsp_cmd.param.get_items_status.scope == AVRC_SCOPE_PLAYER_LIST)
    {
        ...
        for (xx=pMetaMsg->param.get_folder_items.start_item; xx<BSA_AV_NUM_MPLAYERS && xx <=pMetaMsg->param.get_folder_items.end_item ; xx++)
        {
            if (bsa_av_players[xx])
            {
                ...
                if (bsa_av_players[xx]->name.p_str) {
                ...
                APP_INFO1("Media player ID %d, name %s", bsa_av_players[xx]->player_id, bsa_av_players[xx]->name.p_str ? bsa_av_players[xx]->name.p_str : "null");
            }
        }
    }
    ...
    else if(bsa_av_meta_rsp_cmd.param.get_items_status.scope == AVRC_SCOPE_FILE_SYSTEM)
    {
        ...
        for (xx=pMetaMsg->param.get_folder_items.start_item; xx<BSA_AV_NUMSONGS && xx <=pMetaMsg->param.get_folder_items.end_item ; xx++)
        {
            if (bsa_av_songs[xx])
            {
                ...
                if (bsa_av_songs[xx]->name.str_len != 0 && bsa_av_songs[xx]->name.p_str) {
                    memset((char *)bsa_av_meta_rsp_cmd.param.get_items_status.item_list[xx].item_str, 0,BSA_AV_ITEM_NAME_LEN_MAX);
...
int app_av_get_meta_info_play_status(void)
{
        return p_bsa_av_cb->meta_info.play_status.play_status;

}

int app_av_set_cur_meta_data(char *title, char *artist, char *album, char *genre, char *trackIndex, char *totaltrack, char *playtime)
{
    int i = 0;
    int index = 0;
    int attr_count = BSA_AVRC_MAX_ATTR_COUNT;
  //  if (p_bsa_av_cb) {
  //      p_bsa_av_cb->cur_play = 0;
  //  }
     pthread_mutex_lock(&media_mutex);
    ...
    for (i =0; i < attr_count; i++) {
        if (bsa_av_songs[index]->p_attr_list[i].attr_id == AVRC_MEDIA_ATTR_ID_TITLE && title) {
            bsa_av_songs[index]->p_attr_list[i].name.charset_id = AVRC_CHARSET_ID_UTF8;
            bsa_av_songs[index]->p_attr_list[i].name.str_len = strlen(title);
	      if( bsa_av_songs[index]->p_attr_list[i].name.p_str)free( bsa_av_songs[index]->p_attr_list[i].name.p_str);
            bsa_av_songs[index]->p_attr_list[i].name.p_str = strdup(title);
        }
        if (bsa_av_songs[index]->p_attr_list[i].attr_id == AVRC_MEDIA_ATTR_ID_ARTIST && artist) {
            bsa_av_songs[index]->p_attr_list[i].name.charset_id = AVRC_CHARSET_ID_UTF8;
            bsa_av_songs[index]->p_attr_list[i].name.str_len = strlen(artist);
	     if( bsa_av_songs[index]->p_attr_list[i].name.p_str)free( bsa_av_songs[index]->p_attr_list[i].name.p_str);
            bsa_av_songs[index]->p_attr_list[i].name.p_str = strdup(artist);
        }
        if (bsa_av_songs[index]->p_attr_list[i].attr_id == AVRC_MEDIA_ATTR_ID_ALBUM && album) {
            bsa_av_songs[index]->p_attr_list[i].name.charset_id = AVRC_CHARSET_ID_UTF8;
            bsa_av_songs[index]->p_attr_list[i].name.str_len = strlen(album);
	     if( bsa_av_songs[index]->p_attr_list[i].name.p_str)free( bsa_av_songs[index]->p_attr_list[i].name.p_str);
            bsa_av_songs[index]->p_attr_list[i].name.p_str = strdup(album);
        }
        if (bsa_av_songs[index]->p_attr_list[i].attr_id == AVRC_MEDIA_ATTR_ID_TRACK_NUM&& trackIndex) {
            bsa_av_songs[index]->p_attr_list[i].name.charset_id = AVRC_CHARSET_ID_UTF8;
            bsa_av_songs[index]->p_attr_list[i].name.str_len = strlen(trackIndex);
	     if( bsa_av_songs[index]->p_attr_list[i].name.p_str)free( bsa_av_songs[index]->p_attr_list[i].name.p_str);
            bsa_av_songs[index]->p_attr_list[i].name.p_str = strdup(trackIndex);
        }
        if (bsa_av_songs[index]->p_attr_list[i].attr_id == AVRC_MEDIA_ATTR_ID_NUM_TRACKS&& totaltrack) {
            bsa_av_songs[index]->p_attr_list[i].name.charset_id = AVRC_CHARSET_ID_UTF8;
            bsa_av_songs[index]->p_attr_list[i].name.str_len = strlen(totaltrack);
	     if( bsa_av_songs[index]->p_attr_list[i].name.p_str)free( bsa_av_songs[index]->p_attr_list[i].name.p_str);
            bsa_av_songs[index]->p_attr_list[i].name.p_str =strdup(totaltrack);
        }
        if (bsa_av_songs[index]->p_attr_list[i].attr_id == AVRC_MEDIA_ATTR_ID_GENRE && genre) {
            bsa_av_songs[index]->p_attr_list[i].name.charset_id = AVRC_CHARSET_ID_UTF8;
            bsa_av_songs[index]->p_attr_list[i].name.str_len = strlen(genre);
	      if( bsa_av_songs[index]->p_attr_list[i].name.p_str)free( bsa_av_songs[index]->p_attr_list[i].name.p_str);
            bsa_av_songs[index]->p_attr_list[i].name.p_str = strdup(genre);
        }
        if (bsa_av_songs[index]->p_attr_list[i].attr_id == AVRC_MEDIA_ATTR_ID_PLAYING_TIME && playtime) {
            bsa_av_songs[index]->p_attr_list[i].name.charset_id = AVRC_CHARSET_ID_UTF8;
            bsa_av_songs[index]->p_attr_list[i].name.str_len = strlen(playtime);
	      if( bsa_av_songs[index]->p_attr_list[i].name.p_str)free( bsa_av_songs[index]->p_attr_list[i].name.p_str);
            bsa_av_songs[index]->p_attr_list[i].name.p_str = strdup(playtime);
        }
