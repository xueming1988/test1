...
int linkplay_bluetooth_av_close(void)
{
	return app_av_close();
}

int linkplay_bluetooth_av_start_service(tAvCallback pcb)
{
	   /* Init XML state machine */
    app_xml_init();
    app_av_init(pcb);
...
int linkplay_bluetooth_av_stop_service(void)
{
    return app_av_end();
}

int linkplay_bluetooth_disc_services_by_addr(BD_ADDR bd_addr,
        tBSA_SERVICE_MASK services, tBSA_DISC_CBACK *p_custom_disc_cback)
{
	return app_disc_start_bdaddr_services(bd_addr,services,p_custom_disc_cback);
}
int linkplay_bluetooth_disc_start_regular(tBSA_DISC_CBACK *p_custom_disc_cback,int duration)
{
	app_disc_set_nb_devices(APP_DISC_NB_DEVICES);
	return app_disc_start_regular(p_custom_disc_cback);
}

int linkplay_bluetooth_disc_stop_regular(void)
{
	return app_disc_abort();
}

int linkplay_bluetooth_av_stop_play()
{
	return app_av_stop();
}

int linkplay_bluetooth_av_pause_play()
{
	return app_av_pause();
}

int linkplay_bluetooth_av_resume_play()
{
	return app_av_resume();
}

int linkplay_bluetooth_av_rc_send_abs_vol_command(int volume)
{
	return app_av_rc_send_absolute_volume_vd_command(0, volume);
}

int linkplay_bluetooth_av_open_by_addr(BD_ADDR *bd_addr_in)
{
	return app_av_open(bd_addr_in);
}

int linkplay_bluetooth_av_send_stream(void *audio_buf, int length)
{
	return app_av_tx_send(audio_buf,length);
}
int linkplay_bluetooth_av_start_stream(tBSA_AV_MEDIA_FEED_CFG_PCM *pcm_config)
{
	int ret = 0;
	ret = app_av_play_pcm_data(pcm_config);
	if (ret != 0) {
		printf("app_av_play_pcm_data failed!\n");
		return -1;
	}
	/* Start the timer management */
	return 0;
}
int linkplay_bluetooth_av_stop_stream(void)
{
	int ret = 0;
	ret = app_av_stop_current();
	if (ret != 0) {
		printf("app_av_stop_current failed!\n");
		return -1;
	}
	return 0;
}

int linkplay_bluetooth_av_path_set_by_addr(BD_ADDR bd_addr_in,int enable)
{
	return app_av_set_audio_path_by_addr(bd_addr_in,enable);
}

int linkplay_bluetooth_av_path_get_by_addr(BD_ADDR bd_addr_in,int *enable)
{
	return app_av_get_audio_path_by_addr(bd_addr_in,enable);
}

int linkplay_bluetooth_av_send_change_track(void)
{
    app_av_rc_change_track();
    return 0;
}
