...
#define APP_XML_CONF_BLE_NAME_KEY       "local_ble_name"
...
/*   将s 中的s1  字符串替换为s2 ,
若s2  长于s1  确保s  有足够缓存空间 */
char* strrpl(char *s, const char *s1, const char *s2)
{
	char *ptr=s;
	while (ptr = strstr(ptr, s1)) /* 如果在s中找到s1 */
	{
...
{"&amp;","&"}  放第一个 避免和后面的冲突
*/
char* replace_all_escape_word(const char *data,unsigned len, int direction)
{
    char* escape_word[5][2] = {{"&amp;","&"},{"&lt;","<"},{"&gt;",">"},{"&apos;","\'"},{"&quot;","\""}};
    char* ptr = NULL;
    int   asgn_len = len;
    if(direction==1) 
    {
        asgn_len = len*6;    /* 可能存在 一个字符替换成多 6 个字符的字符串*/
    }
	
    ptr = malloc(asgn_len+1);
    if(ptr==NULL)
    {
        return NULL;
    }
    memset(ptr,0,asgn_len+1);
    memcpy(ptr,data,len);
    ...
    {	
        for(int i=0;i<5;i++) 
        {
           strrpl(ptr,escape_word[i][1],escape_word[i][0]);
        }
    }
    else
    {
        for(int i=0;i<5;i++)
        {
           strrpl(ptr,escape_word[i][0],escape_word[i][1]);
        }
    }
...
int app_xml_write_db(const char *p_fname, const tAPP_XML_REM_DEVICE *p_xml_rem_devices, int devices_max)
{
    ...
    for (index = 0, instance = 0; index < devices_max; index++, p_xml_rem_devices++)
    {
        ...
        if (p_xml_rem_devices->in_use != FALSE)
        {
            ...
            replace = replace_all_escape_word((char *) p_xml_rem_devices->name,strlen((char *) p_xml_rem_devices->name),1);
            if(replace==0)  
            {
            ...
            {
                dummy = write(fd, replace,strlen(replace));
                free(replace);
            }
        ...
        const char *tagName, unsigned len)
{
	...
	else if (strncmp(tagName, APP_XML_DEV_LAST_ACTIVE, strlen(
            APP_XML_DEV_LAST_ACTIVE)) == 0)
    {
        app_xml_param_cb.dev_tag = DEV_LAST_ACTIVE_KEY;
    }
	else if (strncmp(tagName, APP_XML_DEV_PROFILE_TYPE, strlen(
            APP_XML_DEV_PROFILE_TYPE)) == 0)
    {
        app_xml_param_cb.dev_tag = DEV_PROFILE_TYPE_KEY;
    }
	else if (strncmp(tagName, APP_XML_DEV_AUDIO_PATH, strlen(
            APP_XML_DEV_AUDIO_PATH)) == 0)
    {
        ...
        unsigned len, int more)
{
    ...
    case DEV_NAME_KEY:
        /* Copy the name in the instance */
        replace = replace_all_escape_word(data,len,0);
        if(replace==0)
        {
 ...
 **
 *******************************************************************************/
int app_xml_add_audio_path_db(tAPP_XML_REM_DEVICE *p_stored_device_db,
        int nb_device_max, BD_ADDR bd_addr,BOOLEAN audio_path)
{
    int index;
...
int app_xml_add_trusted_services_db(tAPP_XML_REM_DEVICE *p_stored_device_db,
        int nb_device_max, BD_ADDR bd_addr, tBSA_SERVICE_MASK trusted_services,int profile_type)
{
    int index;
