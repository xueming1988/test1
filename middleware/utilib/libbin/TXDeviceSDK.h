...
{
    online_off    = 0, //离线
    online_wifi   = 1, //wifi在线, 对于设备好友来说，在线状态的网络类型无法判断，总是online_wifi
    online_2G     = 2, //2G在线
    online_3G     = 3, //3G在线
    online_4G     = 4, //4G在线
    online_unknow = 5  //未知
...
typedef struct tag_tx_binder_info
{
    ...
    unsigned long long  uin;			//绑定者uin, 出于保护用户隐私的考虑，这个值总是0，如果确实有获取uin的需求，请联系小微helper
    int                 online_status;   //在线状态， 请参见: tx_online_type
...
{
    unsigned long long  tinyid;			//绑定者tinyid
    char                remark_name[128];	//绑定者昵称
} tx_binder_remark_info;
...
* 参数说明:
* pBinderList - 返回的绑定者信息
* nCount      - 返回pBinderList的个数
* callback    - 获取绑定者信息的回调，注意此回调不总是被触发，只有本地缓存的绑定者信息拉取不到时，去Server端获取的时候，才会触发此回调
*
* 返回值:
* err_buffer_notenough         - pBinderList分配的size不够
* err_fetching_buff_not_enough - 分配的size不足，且此次正在从server端获取数据中
* err_fetching                 - 正在从server端获取数据中
*/
...
/**
* 接口说明：更新绑定者的在线状态
* on_binder_list_online_status_update: 通知绑定者在线状态的结果回调, pBinderList只中只有online_status和tinyid有值
*/
typedef void (*on_binder_list_online_status_update)(int result, tx_binder_info * pBinderList, int nCount);
SDK_API int tx_update_binder_list_online_status(on_binder_list_online_status_update callback);
 ...
 */
SDK_API int tx_device_reconnect();

/**
 * 接口说明: 获取小微app登陆的qq号
 */
SDK_API unsigned long long tx_device_get_login_uin();
 ...
 * 接口说明，设置绑定者的备注
*/
typedef void (*on_set_binder_remark_result)(int cookie, int error);
SDK_API int tx_set_binder_remark(tx_binder_remark_info* info, on_set_binder_remark_result callback);
