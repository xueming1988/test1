...
#define MAX_COMMAND_BUFFER_LEN	(4096 + 9)	//WORDS - space for 4096 samples + header
#endif
...
struct CAWELib
{
	 ...
	 * @param [in] blocksize		default 0, pass blocksize target info must report
	 * @param [in] bufferlen		default 0, pass command buffer length target info must report
	 * @param [in] inchans          default 0, pass number of input channels target info must report
	 * @param [in] outchans         default 0, pass number of output channels target info must report
	 * @return						0 or error code
	 *
	 * Creates the AWE instance. All other APIs will fail until you call this.
	 * You must specify a name for this instance (max 8 chars), the CPU clock frequency
	...
	virtual int CreateEx(const char *name, float cpu_clock_speed, float cpu_cycle_speed,
		bool profile = false, UINT32 blocksize = 0, UINT32 bufferlen = 0, UINT32 inchans = 0, UINT32 outchans = 0) = 0;
...
#ifndef NO_LOAD_FILE
	/**
	 * @brief Load a DSPC encrypted AWB file into AWE.
	 * @param [in] filename			file to load
	 * @param [out] pPos			if non-NULL, return failing word offset
	 * @return						0 or error code
	 *
	...
	/**
	 * @brief Load an unencrypted AWB file into AWE.
	 * @param [in] filename			file to load
	 * @param [out] pPos			if non-NULL, return failing word offset
	 * @return						0 or error code
	 *
	...
	/**
	 * @brief Load a DSPC encrypted AWB file from memory into AWE.
	 * @param [in] buffer			buffer containing words to load
	 * @param [in] len				number of words in buffer
	 * @param [out] pPos			if non-NULL, return failing word offset
	 * @return						0 or error code
	...
	/**
	 * @brief Load an unencrypted AWB file from memory into AWE.
	 * @param [in] buffer			buffer containing words to load
	 * @param [in] len				number of words in buffer
	 * @param [out] pPos			if non-NULL, return failing word offset
	 * @return						0 or error code
