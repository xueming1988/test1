...
*/
typedef int (*auto_download_controll_callback)(unsigned long long uFileSize, unsigned char uChannelType);

/**
* 设置自动下载文件控制的回调
* FTN文件传输通道默认是自动下载文件的，调用者可以通过设置回调来决定是否要自动下载
*/
SDK_API void tx_set_auto_download_callbak(auto_download_controll_callback cb);
