...
mDNSlocal int SetupSocket(struct sockaddr *intfAddr, mDNSIPPort port, int interfaceIndex, int *sktPtr)
{
	...
	//printf("mDNS SetupSocket \r\n");
	fcntl(*sktPtr, F_SETFD, FD_CLOEXEC);
	...
	if (err < 0) { err = errno; perror("setsockopt - SO_REUSEPORT"); }
	#endif

        #if defined(SO_REUSEADDR)
        err = setsockopt(*sktPtr, SOL_SOCKET, SO_REUSEADDR, &kOn, sizeof(kOn));
        if (err < 0) { err = errno; perror("setsockopt - SO_REUSEADDR"); }
	#endif
    }
...
mDNSlocal mStatus WatchForInterfaceChange(mDNS *const m)
{
    ...
    if (mStatus_NoError != err) 
    	goto out_free_pChgRec;
    
    err = mDNSPosixAddFDToEventLoop(pChgRec->NotifySD, InterfaceChangeCallback, pChgRec);
    if (mStatus_NoError != err)
    	goto out_free_NotifySD;
...
//	if (m->p->unicastSocket4 != -1) assert(shutdown(m->p->unicastSocket4,SD_BOTH) == 0);
//#if HAVE_IPV6
//	if (m->p->unicastSocket6 != -1) assert(shutdown(m->p->unicastSocket6,SD_BOTH) == 0);
//#endif
