...
int snd_pcm_fade_vol(snd_pcm_t *pcm, int vol)
{
	...
	//fprintf(stderr,"snd_pcm_fade_vol 0x%x  %d\r\n",(unsigned int )pcm,pcm->snd_vol_cur);
...
void snd_pcm_fade(snd_pcm_t *pcm,
							  signed short *dst,
							  snd_pcm_uframes_t tsize,snd_pcm_mix_t *mix_vol_t,int *silence_size)
{
//	g_mix_vol_t.mix_vol_max = DITHER_MAX_VALUE;
//	g_mix_vol_t.mix_vol_min = DITHER_MIN_VALUE;
//	snd_pcm_mix_t *mix_vol_t =&g_mix_vol_t;// (snd_pcm_mix_t *)(dmix->u.dmix.vol_buffer);
    ...
    if(pcm->snd_ext_plugin == 2 || pcm->snd_ctrl_index == 2) {
        int count = tsize;
        signed short *silence_dst = dst;
        while(ext_silence_size > 0) {
            *silence_dst = 0;
            if(!--count)
                break;
            silence_dst++;
			...
			for(;;) {
				*fixed_volume += snd_ctrl_fadeStep;
				...
				if(!*dst) {
					...
					if(!snd_ctrl_fadeFlag) {
						*dst = dithered_vol(*dst, DITHER_MAX_VALUE);
					} else {
						*dst = dithered_vol(*dst, *fixed_volume);
					}
				}
...
snd_pcm_sframes_t snd_pcm_writei(snd_pcm_t *pcm, const void *buffer, snd_pcm_uframes_t size)
{
	...
	if(snd_info_get_bt_transfer() && (snd_pcm_is_music(pcm) || (!snd_info_get_bt_audio_path() && pcm->snd_ctrl_index == SNDSRC_NOTIFY))) {
        snd_pcm_fade(pcm,buffer,size * pcm->channels,&g_mix_vol_t,&silence_size);
