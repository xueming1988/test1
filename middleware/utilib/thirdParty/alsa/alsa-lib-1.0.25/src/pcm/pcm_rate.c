...
void snd_pcm_fade(snd_pcm_t *pcm, void *dst, snd_pcm_uframes_t tsize,snd_pcm_mix_t *mix_vol_t,int *silence_size, snd_pcm_format_t format);
...
snd_pcm_rate_write_areas1(snd_pcm_t *pcm,
			 ...
			 snd_pcm_uframes_t slave_offset,snd_pcm_uframes_t slave_size)
{
	snd_pcm_rate_t *rate = pcm->private_data;

	if(snd_info_get_bt_transfer() && !snd_info_get_bt_audio_path())
	{
		static int btav_res = -1;    
		int mode = 0;
	       int length = 0;
		length = pcm->channels * rate->gen.slave->period_size * 2;
		char *dst_buf = calloc(1,length);
		snd_pcm_channel_area_t *running_areas = (snd_pcm_channel_area_t *)calloc(1, pcm->channels*sizeof(snd_pcm_channel_area_t));
		snd_pcm_areas_from_buf(pcm, running_areas, dst_buf);
		do_convert(running_areas, 0, rate->gen.slave->period_size,
    	...
    	if(btav_res >= 0 )
    	{
            // Mix the stream's volume
            int silence_size = 0;
	     	int send_frames = rate->gen.slave->period_size;
            snd_pcm_mix_t *mix_vol = snd_pcm_get_mix_vol();

			//in this case we think it should be SND_PCM_FORMAT_S16_LE
           	snd_pcm_fade(pcm, dst_buf, rate->gen.slave->period_size* pcm->channels, mix_vol,&silence_size, SND_PCM_FORMAT_S16_LE);
		if(slave_size <= rate->gen.slave->period_size)
		{
			send_frames = slave_size; 
		}
            int ret = write(btav_res,dst_buf,send_frames * pcm->channels  * 2);
            if(ret < 0)
            {
       ...
       free(dst_buf);
       free(running_areas);
	memset(slave_areas->addr + slave_offset * 2 * pcm->channels,0x0,pcm->channels * rate->gen.slave->period_size * 2);
    }else
    {
				    ...
				    snd_pcm_uframes_t slave_size)
{
	...
	if (cont >= size) {
		...
		if (slave_frames < slave_size) {
			snd_pcm_rate_write_areas1(pcm, areas, appl_offset, rate->sareas, 0,slave_size);
			goto __partial;
		}
		snd_pcm_rate_write_areas1(pcm, areas, appl_offset,
					  slave_areas, slave_offset,slave_size);
		...
		snd_pcm_rate_write_areas1(pcm, rate->pareas, 0, rate->sareas, 0,slave_size);
...
static void snd_pcm_rate_vol_info(snd_pcm_t *pcm, int info_index)
{
	snd_pcm_rate_t *rate = pcm->private_data;
	pcm->snd_ctrl_index = info_index;	
	snd_pcm_vol_info(rate->gen.slave, info_index);
}
...
static int snd_pcm_rate_close(snd_pcm_t *pcm)
{
	...
	if(pcm->mix_vol)
	{
		if(rate->gen.slave)
		{
			if(rate->gen.slave->ops->vol_ctl)
				rate->gen.slave->ops->vol_ctl(rate->gen.slave,0,0,0);
		}
	}
...
static void snd_pcm_rate_vol_ctl(snd_pcm_t *pcm, int enable, int min_vol, int max_vol)
{
	snd_pcm_rate_t *rate = pcm->private_data;
	if(rate->gen.slave)
	{
		if(rate->gen.slave->ops->vol_ctl)
			rate->gen.slave->ops->vol_ctl(rate->gen.slave, enable, min_vol, max_vol);
	}
}
...
static const snd_pcm_fast_ops_t snd_pcm_rate_fast_ops = {
	...
	.poll_descriptors = snd_pcm_rate_poll_descriptors,//snd_pcm_generic_poll_descriptors,
...
static const snd_pcm_ops_t snd_pcm_rate_ops = {
	...
	.vol_ctl = snd_pcm_rate_vol_ctl,
	.vol_info = snd_pcm_rate_vol_info,
};
		      ...
		      snd_pcm_t *slave, int close_slave)
{
...
#endif
	fprintf(stderr,"snd_pcm_rate_open  %d \r\n",srate);
	...
	pcm->snd_ctrl_index = slave->snd_ctrl_index;
