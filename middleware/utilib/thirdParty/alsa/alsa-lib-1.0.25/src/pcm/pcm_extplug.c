			    ...
			    snd_pcm_uframes_t *slave_sizep)
{
	...
	*slave_sizep = size;
#if	defined(HARMAN)	
	if(pcm->snd_ctrl_index>=0 && strstr(pcm->name, "harman") && ext->plug.gen.slave && strstr(ext->plug.gen.slave->name, "dmixer"))
	{
		ext->plug.gen.slave->snd_ext_plugin = pcm->snd_ctrl_index;
	}
#endif	
	return size;
}
...
static void snd_pcm_extplug_vol_info(snd_pcm_t *pcm, int info_index)
{
	extplug_priv_t *ext = pcm->private_data;
	pcm->snd_ctrl_index = info_index;	
	snd_pcm_vol_info(ext->plug.gen.slave, info_index);
}
...
static const snd_pcm_ops_t snd_pcm_extplug_ops = {
	...
	.vol_ctl = NULL,
	.vol_info = snd_pcm_extplug_vol_info,
};
