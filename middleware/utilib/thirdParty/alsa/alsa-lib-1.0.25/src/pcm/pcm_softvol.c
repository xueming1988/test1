...
#define PRESET_MIN_DB          -65.0
#define ZERO_DB                  0.0
#define MAX_DB_UPPER_LIMIT      64
#else /*defined(A98_TEUFEL)*/
#define PRESET_MIN_DB		-51.0
#define ZERO_DB                  0.0
#define MAX_DB_UPPER_LIMIT      50
#endif

#if defined(A98_SPRINT_V03)
#define SCALE_MAX              3
#elif defined(DEWALT_BEHAVE)
#define SCALE_MAX              0
#else /*defined(DEWALT_BEHAVE)*/
#define SCALE_MAX              5
#endif 
...
static const unsigned int HM_preset_dB_value[33] = {
	0,
	17,
	34,   
	52,   
...
#elif defined(A98_NAVER) || defined(A98_DONUT)
static const unsigned int HM_preset_dB_value[33] = {
    0    ,
    735  ,
    1039 ,
...
#define CONVERT_AREA(TYPE, swap) do {	\
	...
	for (ch = 0; ch < channels; ch++) { \
		...
		GET_VOL_SCALE; \
		fr = frames; \
		if(svol->last_vol != vol_scale) { \
		    long long tm_now = tickSincePowerOn(); \
		    if(tm_now - svol->last_pts > 500) { \
		        svol->last_vol = vol_scale; \
		    } \
		} \
		if (! vol_scale) { \
			while (fr--) { \
		...
		} else if (vol_scale == 0xffff && svol->last_vol == vol_scale) { \
			while (fr--) { \
				*dst = *src; \
				src += src_step; \
				dst += dst_step; \
				...
				*dst = (TYPE) MULTI_DIV_##TYPE(*src, vol_scale, swap); \
				...
				{\
					if(temp == SCALE_MAX) { \
                		...
                		else if(svol->last_vol < vol_scale)\
                		{\
                    			if(temp == SCALE_MAX) { \
						temp = 0; \
                        			svol->last_vol ++;\
                		...
                		temp ++;\
				*dst = (TYPE) MULTI_DIV_##TYPE(*src, svol->last_vol, swap); \
                	} \
				src += src_step; \
				dst += dst_step; \
			} \
				       ...
				       snd_pcm_uframes_t frames)
{
	...
	if (svol->cur_vol[0] == 0 && svol->cur_vol[1] == 0) {
		snd_pcm_areas_silence(dst_areas, dst_offset, channels, frames,
				      svol->sformat);
	    svol->last_pts = tickSincePowerOn();
	    svol->last_vol = 0;
		return;
	} else if (svol->zero_dB_val && svol->cur_vol[0] == svol->zero_dB_val &&
		   svol->cur_vol[1] == svol->zero_dB_val && (svol->last_vol == svol->dB_value[svol->zero_dB_val])) {
	...
	//printf("soft volume softvol_convert_stereo_vol [%d %d] [%d %d]\r\n", vol[0], vol[1], svol->cur_vol[0], svol->cur_vol[1]);
	switch (svol->sformat) {
...
static void get_current_volume(snd_pcm_softvol_t *svol)
{
	...
	for (i = 0; i < svol->cchannels; i++) {
		...
		if (val > svol->max_val)
			val = svol->max_val;
		svol->cur_vol[i] = val;
		//printf("soft volume get_current_volume svol->cur_vol[%d] %d \r\n", i, val);
	}
}
...
static void snd_pcm_softvol_vol_info(snd_pcm_t *pcm, int info_index)
{
	snd_pcm_softvol_t *svol = pcm->private_data;
	pcm->snd_ctrl_index = info_index;	
	snd_pcm_vol_info(svol->plug.gen.slave, info_index);
}
...
//#if	defined(HARMAN)
		...
		svol->min_dB = min_dB;
		svol->max_dB = max_dB;
		memcpy(svol->dB_value, HM_preset_dB_value, sizeof(HM_preset_dB_value));
		if (svol->zero_dB_val)
...
static const snd_pcm_ops_t snd_pcm_softvol_ops = {
	...
	.vol_ctl = NULL,
	.vol_info = snd_pcm_softvol_vol_info,
};
