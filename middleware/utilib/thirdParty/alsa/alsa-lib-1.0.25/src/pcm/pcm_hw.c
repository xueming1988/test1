...
{
#ifdef DEBUG_RW
        fprintf(stderr, " g_wiimu_shm->privacy_mode %d\n", g_wiimu_shm?g_wiimu_shm->privacy_mode:-1);
#endif
	int err;
	snd_pcm_hw_t *hw = pcm->private_data;
	int fd = hw->fd;
...
static void snd_pcm_hw_vol_info(snd_pcm_t *pcm, int info_index)
{
