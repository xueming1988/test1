...
#define DMIX_SILENCE_PATCH_SIZE  120

const char *BTAVFIFONAME = "/tmp/wiimu_bt_av_share_pcm";
...
#ifdef PURE_RPC
int snd_pcm_mixer_capture(void *pcmPtr, unsigned int size)
{
    int capture_fd = -1;

    if(snd_info_get_audio_capture_flag()
            && ((capture_fd = open(AUDIO_CAPTURE_FIFO_NAME, O_WRONLY | O_NONBLOCK)) > 0)) {
        write(capture_fd, pcmPtr, size);
        close(capture_fd);
    }
...
//#if   defined(HARMAN)
void snd_pcm_HM_plugin_dither(snd_pcm_t *pcm,
                              const snd_pcm_channel_area_t *dst_areas,
                              snd_pcm_uframes_t dst_ofs,
                              snd_pcm_uframes_t size)
{
    ...
    switch(dmix->shmptr->s.format) {
        case SND_PCM_FORMAT_S16_LE:
        case SND_PCM_FORMAT_S16_BE:
            dst = (short*)dst_areas[0].addr + dst_ofs * dmix->channels;
            break;
        case SND_PCM_FORMAT_S32_LE:
        case SND_PCM_FORMAT_S32_BE:
            dst = (int*)dst_areas[0].addr + dst_ofs * dmix->channels;
            break;
        case SND_PCM_FORMAT_S24_LE:
            dst = (int*)dst_areas[0].addr + dst_ofs * dmix->channels;
            break;
...
#ifdef PURE_RPC
		if(pcm->snd_ctrl_index == SNDSRC_MPLAYER ||
				pcm->snd_ctrl_index == SNDSRC_SPOTIFY ||
				pcm->snd_ctrl_index == SNDSRC_AIRPLAY) {
			snd_pcm_mixer_capture(dst, tsize * 2);
		}
#endif

	snd_pcm_fade(pcm, dst, tsize, mix_vol_t, &dmix->silence_size, dmix->shmptr->s.format);
...
//#if   defined(HARMAN)
        if(pcm->snd_ext_plugin >= 0) {
            pcm->snd_ctrl_index = pcm->snd_ext_plugin;
        }
        if(pcm->snd_ctrl_index >= 0) {
            snd_pcm_HM_plugin_dither(pcm, src_areas, appl_ptr, transfer);
        }
//#endif
...
static snd_pcm_sframes_t snd_pcm_dmix_readi(snd_pcm_t *pcm ATTRIBUTE_UNUSED, void *buffer ATTRIBUTE_UNUSED,
        snd_pcm_uframes_t size ATTRIBUTE_UNUSED)
{
    return -ENODEV;
}

static snd_pcm_sframes_t snd_pcm_dmix_readn(snd_pcm_t *pcm ATTRIBUTE_UNUSED, void **bufs ATTRIBUTE_UNUSED,
        snd_pcm_uframes_t size ATTRIBUTE_UNUSED)
{
    return -ENODEV;
}
...
static void snd_pcm_dmix_vol_info(snd_pcm_t *pcm, int info_index)
{
    snd_pcm_direct_t *dmix = pcm->private_data;
    pcm->snd_ctrl_index = info_index;
    if(dmix->spcm)
        snd_pcm_vol_info(dmix->spcm, info_index);
}
static void snd_pcm_dmix_vol_ctl(snd_pcm_t *pcm, int enable, int min_vol, int max_vol)
{
    snd_pcm_direct_t *dmix = pcm->private_data;
//  fprintf(stderr,"snd_pcm_dmix_vol_ctl \r\n");
...
snd_pcm_sframes_t snd_pcm_dmix_writei(snd_pcm_t *pcm, const void *buffer, snd_pcm_uframes_t size)
{
    ...
    unsigned int tsize = size * dmix->channels;
	snd_pcm_fade(pcm, buffer, tsize, mix_vol_t,&dmix->silence_size, dmix->shmptr->s.format);
...
static const snd_pcm_ops_t snd_pcm_dmix_ops = {
    ...
    .vol_info = snd_pcm_dmix_vol_info,
};
...
//  printf("snd_pcm_dmix_open \r\n");
    ...
    dmix->silence_size = DMIX_SILENCE_PATCH_SIZE;
#endif
