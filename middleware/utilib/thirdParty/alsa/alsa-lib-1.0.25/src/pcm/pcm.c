...
#define DMIX_TTS_POPNOISE_PATCH  1

#ifdef	ALSA_PLUGIN_DIR
#undef ALSA_PLUGIN_DIR
#define ALSA_PLUGIN_DIR	"/system/workdir/lib"
#endif
#define MAX_BT_FIFO_NUM        9


WIIMU_CONTEXT* g_wiimu_shm = NULL;
typedef struct snd_pcm_mix_t_
{
	int enable_mix_vol;
	int mix_vol_enable;
	int mix_vol_min;
...
static snd_pcm_mix_t g_mix_vol_t;

int snd_info_get_bt_transfer(void);
int snd_info_mute_flag_get(int info_index);
int snd_info_vol_get(int info_index);
int snd_info_selfVol_get(int info_index);
int snd_info_fade_flag_get(int info_index);
int snd_info_fade_step_get(int info_index);
int snd_info_get_bt_audio_path(void);

//static int g_bMonoSpeaker = 0;
 ...
 * \return 0 on success otherwise a negative error code
 */
int snd_pcm_mix_vol(snd_pcm_t *pcm,int enable, int min_vol, int max_vol)
{
	int err;
	assert(pcm);

	if(pcm->ops->vol_ctl)
		pcm->ops->vol_ctl(pcm->op_arg, enable, min_vol, max_vol);
	else if(pcm->fast_op_arg->ops->vol_ctl)
		pcm->fast_op_arg->ops->vol_ctl(pcm->fast_op_arg, enable, min_vol, max_vol);

	pcm->mix_vol = 1;
	pcm->fast_op_arg->mix_vol = 1;

	return 0;
}

void snd_pcm_vol_info(snd_pcm_t *pcm, int info_index)
{
	assert(pcm);
	pcm->ops->vol_info(pcm->op_arg, info_index);
}

/**
 * \brief set opened pcm dev info index
 * \param pcm PCM handle
...
//#if	defined(HARMAN)
	if(pcm->ops->vol_info)
		pcm->ops->vol_info(pcm->op_arg, info_index);
	else if(pcm->fast_op_arg->ops->vol_info)
		pcm->fast_op_arg->ops->vol_info(pcm->fast_op_arg, info_index);
//#endif

	pcm->snd_ctrl_index = info_index;
	pcm->fast_op_arg->snd_ctrl_index = info_index;
	//fprintf(stderr,"snd_pcm_info_set 0x%x \r\n",(unsigned int )pcm);
	if( g_wiimu_shm)
	{
		g_wiimu_shm->snd_ctrl[info_index].pause = 0;
	}
 ...
 * \brief set opened pcm dev info index
 * \param pcm PCM handle
 * \param vol first init
 * \return 0 on success otherwise a negative error code
 */

int snd_pcm_fade_vol(snd_pcm_t *pcm, int vol)
{
	int err;
	assert(pcm);
	pcm->fast_op_arg->snd_vol_cur = vol;
	return 0;
}
...
int snd_pcm_info(snd_pcm_t *pcm, snd_pcm_info_t *info)
{
	...
	{
		int err;
		if (ioctl(i2s_fd, SNDRV_PCM_IOCTL_INFO, info) < 0) {
			err = -errno;
			SYSMSG("SNDRV_PCM_IOCTL_INFO x failed");
		return err;
	}
	return 0;
		}
...
//Haiyong:
//I can't understand why Huaijing needs this pre-condition
//Anyway, we need to conside not only MPLAYER but also some other streams.
//I think Huaijing needs to make more comments on this and may discuss out a better solution than this.
static int snd_pcm_is_music(snd_pcm_t *pcm)
{
    return (pcm->snd_ctrl_index == SNDSRC_MPLAYER) || (pcm->snd_ctrl_index == SNDSRC_AIRPLAY) || (pcm->snd_ctrl_index == SNDSRC_MULTIROOM) || (pcm->snd_ctrl_index == SNDSRC_AVS_MRM) || (pcm->snd_ctrl_index == SNDSRC_SPOTIFY) ; 
}
...
int snd_pcm_delay(snd_pcm_t *pcm, snd_pcm_sframes_t *delayp)
{
	...
	if(snd_info_get_bt_transfer() && snd_pcm_is_music(pcm)) {
		*delayp = 0;
		return 0;
	}
...
int snd_pcm_prepare(snd_pcm_t *pcm)
{
	...
	if( g_wiimu_shm && pcm->snd_ctrl_index >= 0)
	{
		g_wiimu_shm->snd_ctrl[pcm->snd_ctrl_index].pause = 0;
	}
...
int snd_pcm_drop(snd_pcm_t *pcm)
{
	...
	if( g_wiimu_shm && pcm->snd_ctrl_index >= 0)
	{
		g_wiimu_shm->snd_ctrl[pcm->snd_ctrl_index].pause = 1;
	}
...
static inline void dithered_vol(void* dst, unsigned int fix_volume, int sample_size)
{
	if(sample_size == 2) {
		//keep original algrithm
		short sample = *(short*)dst;
		...
		*(short*)dst = (out >> 16);
		...
		//TODO: we need find better dither algrithm
		if(fix_volume && fix_volume < 0x10000) {
			int sample = *(int*)dst;
		//	int mid = 0x8000; //32768
		//	int x = fix_volume;
		//	const double a = -0.25;
		//	const double b = 0.75;
		//	double radio = x / mid - 1;
		//	double out = (double) sample *  ( a * radio * radio * radio + b * radio + 0.5);
		//	*(int*)dst = (int)out;
			long long out = (long long)sample * (long long)fix_volume;
			sample = (int)(out>>32);
			*(int*)dst = sample;
		}
		else if(fix_volume == 0) 
			*(int*)dst = 0;
	}
}

snd_pcm_mix_t *snd_pcm_get_mix_vol(void)
{
	return &g_mix_vol_t;
}

void snd_pcm_fade(snd_pcm_t *pcm, void *dst, snd_pcm_uframes_t tsize,
							  snd_pcm_mix_t *mix_vol_t,int *silence_size, snd_pcm_format_t format)
{
	unsigned int *fixed_volume = &pcm->snd_vol_cur;
	int vol_reset = 1;
	int vol_dec = 0;
	int vol_inc = 0;
	int mute_flag = 0;
	int snd_ctrl_vol = 100;
	int snd_ctrl_selfVol = 100;
	int snd_ctrl_fadeFlag = 1;
	int snd_ctrl_fadeStep = 5;
	int btav_res;
	int ext_silence_size = *silence_size;
	int sample_size = 2;
	...
	if(pcm->snd_ctrl_index >= 0) {
		mute_flag = snd_info_mute_flag_get(pcm->snd_ctrl_index);
		snd_ctrl_vol = snd_info_vol_get(pcm->snd_ctrl_index);
		snd_ctrl_selfVol = snd_info_selfVol_get(pcm->snd_ctrl_index);
		snd_ctrl_fadeFlag = snd_info_fade_flag_get(pcm->snd_ctrl_index);
		snd_ctrl_fadeStep = snd_info_fade_step_get(pcm->snd_ctrl_index);

#ifndef NAVER_LINE
		//for alexa tts / smplayer, do not dither with other source setting. Only decide by self
		if(pcm->snd_ctrl_index == 2 || pcm->snd_ctrl_index == 1)
			snd_ctrl_vol = snd_ctrl_selfVol > 60 ? 60 : snd_ctrl_selfVol;			//tts, smplayer 60% max
		else
			snd_ctrl_vol = snd_ctrl_vol < snd_ctrl_selfVol ? snd_ctrl_vol : snd_ctrl_selfVol;
#else
		// TODO: need fixed for CLOVA
		snd_ctrl_vol = snd_ctrl_selfVol;
#endif
	}

#ifdef  DMIX_TTS_POPNOISE_PATCH
    if(pcm->snd_ext_plugin == 2 || pcm->snd_ctrl_index == 2) {
		int silent_count = tsize > *silence_size ? *silence_size : tsize;
        memset(dst, 0, silent_count * sample_size);
    	*silence_size -= silent_count;
    }

#endif //DMIX_TTS_POPNOISE_PATCH
	if(snd_ctrl_vol < 100 || mute_flag) {
		if(mute_flag) {
			mix_vol_t->mix_vol_max = DITHER_MAX_VALUE;
			mix_vol_t->mix_vol_min = DITHER_MIN_VALUE;
		}
		if(snd_ctrl_vol < 100) {
			mix_vol_t->mix_vol_max = DITHER_MAX_VALUE;
			mix_vol_t->mix_vol_min = DITHER_MAX_VALUE * snd_ctrl_vol / 100;
		}
		vol_dec = 1;
		vol_inc = 0;
	} else {
	...
	if(snd_ctrl_vol < 100 || mute_flag) {
		...
		if(*fixed_volume < DITHER_MIN_VALUE) {
			*fixed_volume = DITHER_MIN_VALUE;
		} else if(*fixed_volume >= DITHER_MAX_VALUE) {
			*fixed_volume = DITHER_MAX_VALUE;
			vol_reset = 0;
		}
	}
	if(vol_reset) {
		if(vol_dec) {
			for(;;) {
				*fixed_volume -= snd_ctrl_fadeStep;
				if(snd_ctrl_vol < 100 || mute_flag) {
					if(*fixed_volume < mix_vol_t->mix_vol_min)
						*fixed_volume = mix_vol_t->mix_vol_min;
				} else if(*fixed_volume < DITHER_MIN_VALUE) {
					*fixed_volume = DITHER_MIN_VALUE;
				}

				if(!snd_ctrl_fadeFlag) {
					dithered_vol(dst, mix_vol_t->mix_vol_min, sample_size);
				} else {
					dithered_vol(dst, *fixed_volume, sample_size);
				}
				if(!--tsize)
					break;
				dst = (char*)dst + sample_size;
			}
		} else {
			for(;;) {
				*fixed_volume += snd_ctrl_fadeStep;
				if(*fixed_volume > DITHER_MAX_VALUE) {
					*fixed_volume = DITHER_MAX_VALUE;
				}

				if(!snd_ctrl_fadeFlag) {
					dithered_vol(dst, DITHER_MAX_VALUE, sample_size);
				} else {
					dithered_vol(dst, *fixed_volume, sample_size);
				}
				if(!--tsize)
...
/*  get the  bluetooth  av  FIFO file descripter */
int snd_pcm_get_bt_fifo_fd(int mode)
{
    int   i  = 0;
    int   fd = -1;
    int   ret = 0;
    struct flock lock;
    char bt_av_fifo_name[50] = {0};  
    
    for(i=0;i<MAX_BT_FIFO_NUM;i++)
    {
        sprintf(bt_av_fifo_name,"%s_%d","/tmp/wiimu_bt_av_share_pcm",i);
        if(access(bt_av_fifo_name, F_OK) == -1) {
            printf("%s not exit\n",bt_av_fifo_name);
            continue;
        }
        fd = open(bt_av_fifo_name,mode);
        if(fd<0){
            continue; 
        }
...
snd_pcm_sframes_t snd_pcm_writei(snd_pcm_t *pcm, const void *buffer, snd_pcm_uframes_t size)
{
	...
	if(snd_info_get_bt_transfer() && (snd_pcm_is_music(pcm) || (!snd_info_get_bt_audio_path() && pcm->snd_ctrl_index == SNDSRC_NOTIFY))) {
		//in this case we think it should be SND_PCM_FORMAT_S16_LE
        snd_pcm_fade(pcm, buffer, size * pcm->channels, &g_mix_vol_t,&silence_size, SND_PCM_FORMAT_S16_LE);
        if(pcm->mode &  SND_PCM_NONBLOCK)
            mode =  O_WRONLY | O_NONBLOCK ;
        else
            mode =  O_WRONLY ;
        if(btav_res==-1){
            btav_res = snd_pcm_get_bt_fifo_fd(mode);
        }
        if(btav_res >= 0) {
            int ret = write(btav_res, buffer, size * pcm->channels*sizeof(short));
            if(ret <= size * pcm->channels*sizeof(short))
                return ret / (pcm->channels* sizeof(short));
            else
            {
                close(btav_res);
                btav_res = -1;
		 ...
		 snd_pcm_stream_t stream, int mode)
{
	...
	if(g_wiimu_shm == 0)
	{
		int shmid=shmget( WIIMU_CONTEXT_SHM_ID, 0, 0);
		if(shmid != -1)
			g_wiimu_shm = shmat( shmid,( const void* )0,0 );
	}
	g_mix_vol_t.mix_vol_max = DITHER_MAX_VALUE;
	g_mix_vol_t.mix_vol_min = DITHER_MIN_VALUE;
		...
		snd_pcm_stream_t stream, int mode)
{
	...
	pcm->mix_vol = 0;
	pcm->snd_vol_cur = 0;
	pcm->snd_ctrl_index = -1;
	pcm->snd_ext_plugin = -1;

//	fprintf(stderr,"snd_pcm_new 0x%x  %d\r\n",(unsigned int )pcm,pcm->snd_vol_cur);
...
int snd_pcm_recover(snd_pcm_t *pcm, int err, int silent)
{
        ...
        if (err == -EPIPE) {
                ...
                if (!silent)
               	{
               			if(!strcmp(s,"underrun"))
               				g_wiimu_shm->alsa_underrun++;
                    SNDERR("%s occurred", s);
               	}
                err = snd_pcm_prepare(pcm);
                if (err < 0) {
...
int snd_info_mute_flag_get(int info_index)
{
	...
	if( g_wiimu_shm)
	{
		ret =  g_wiimu_shm->snd_ctrl[info_index].mute;;
	}
	return ret;
}
...
int snd_info_fade_flag_get(int info_index)
{
	...
	if( g_wiimu_shm)
	{
		ret =  g_wiimu_shm->snd_ctrl[info_index].fadeFlag;
	}
	return ret;
}
...
int snd_info_fade_step_get(int info_index)
{
	...
	if( g_wiimu_shm)
	{
		ret =  g_wiimu_shm->snd_ctrl[info_index].fadeStep;
	}
	if(ret == 0)
		ret = 5;
	return ret;
...
int snd_info_selfVol_get(int info_index)
{
	...
	if( g_wiimu_shm)
	{
		ret =  g_wiimu_shm->snd_ctrl[info_index].selfVolume;
	}
	return ret;
}
...
//#endif
			...
			ret = g_wiimu_shm->snd_ctrl[info_index].volume[i] < ret ? g_wiimu_shm->snd_ctrl[info_index].volume[i] : ret;
		}
	}
	return ret;
}
int snd_info_get_bt_transfer(void)
{
	return  g_wiimu_shm->bt_av_source;
}

void snd_info_set_bt_audio_path(int enable)
{
	g_wiimu_shm->bt_av_audio_path = enable;
}
int snd_info_get_bt_audio_path(void)
{
...
#ifdef PURE_RPC
int snd_info_get_audio_capture_flag(void)
{
	return g_wiimu_shm->starting_audio_capture;
}
#endif
