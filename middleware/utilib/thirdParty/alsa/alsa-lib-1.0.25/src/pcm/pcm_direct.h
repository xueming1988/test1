...
struct snd_pcm_direct {
	...
	union {
		struct {
			...
			int shmid_vol;			/* IPC global sum ring buffer memory identification */
			signed int *vol_buffer;		/* shared sum buffer */
...
static inline int snd_pcm_direct_semaphore_down(snd_pcm_direct_t *dmix, int sem_num)
{
	...
	timeout.tv_sec = 2;
	timeout.tv_nsec = 0;
	ret=semtimedop(dmix->semid, op, 2,&timeout);
	if(EAGAIN == ret)
	{
      fprintf(stderr,"\r\n snd_picm_direct_semaphore_down timeout \r\n");
	}
