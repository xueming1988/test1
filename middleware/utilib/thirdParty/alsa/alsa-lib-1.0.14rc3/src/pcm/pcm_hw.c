...
static int snd_pcm_hw_start(snd_pcm_t *pcm)
{
	...
	snd_pcm_info_malloc( (snd_pcm_info_t*)&info );
	snd_pcm_hw_info(pcm, (snd_pcm_info_t*)info);
	memcpy(&reserve, info->reserved+8, 8);
	snd_pcm_info_free( (snd_pcm_info_t*)info);

	fprintf(stderr, "ALSA %s %d\n", __func__, (int)reserve);
#endif
	sync_ptr(hw, 0);
