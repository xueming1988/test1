...
snd_pcm_sframes_t snd_pcm_writei(snd_pcm_t *pcm, const void *buffer, snd_pcm_uframes_t size)
{
	...
	static int ch = 0;
	static int pcm_duration = 44100;
	if(pcm_duration >= 44100)
	{
		if(g_wiimu_shm)
			ch = g_wiimu_shm->audio_channel;
		pcm_duration = 0;
	}
	pcm_duration += size;
	...
	else if(g_wiimu_shm && g_wiimu_shm->sterro_mono ) //sterro mono
	{
		int i;
		int dat;
		short* buf = (short*)buffer;
	...
	_again:
		...
		if (state == SND_PCM_STATE_PREPARED) {
			...
			if (state == SND_PCM_STATE_PREPARED &&
				...
				err = snd_pcm_start(pcm);
				fprintf(stderr, "ALSA %s (hw_avail=%d, threshold=%d) \n", __func__, (int)hw_avail, (int)pcm->start_threshold);
				if (err < 0)
					goto _end;
			}
		}
...
/* add by wiimu */
int snd_pause_flag_get(void)
{
	if(g_wiimu_shm)
     	return g_wiimu_shm->pause_flag_for_alexa;
	return 0;
}

int snd_mute_flag_get(void)
{
	static int mute_flag = -1;
	int ret = 0;
	if( g_wiimu_shm && (!g_wiimu_shm->volume || g_wiimu_shm->mute))
		ret = 1;
	if( mute_flag != ret )
		printf("\n**********%s mute flag change from %d to %d ********\n",	__FUNCTION__, mute_flag, ret);
	mute_flag = ret;
	return ret;
}

int snd_pause_flag_set(int pause)
{
	if(g_wiimu_shm)
	 	g_wiimu_shm->pause_flag_for_alexa = pause;
	else return -1;
	return 0;
}

extern volatile int g_pause_local;
extern volatile int g_dump_request;
void snd_local_pause_flag_set(int pause)
{
	g_pause_local = pause;
}

int snd_local_pause_flag_get(void)
{
	return g_pause_local;
}

void snd_flag_dump(void)
{
	g_dump_request = 1;
}
/*add end*/
