...
int snd_pcm_direct_initialize_slave(snd_pcm_direct_t *dmix, snd_pcm_t *spcm, struct slave_params *params)
{
	...
	if (dmix->type == SND_PCM_TYPE_DSHARE) {
		...
		fprintf(stderr, "snd_pcm_areas_silence!\n");
	}

#if 0
	fprintf(stderr, "snd_pcm_direct_initialize_slave ! Type: %d \n", dmix->type);
	if (dmix->type != SND_PCM_TYPE_DMIX) {
		fprintf(stderr, "SND_PCM_TYPE_DMIX !!! snd_pcm_start %d  \n", dmix->type);
	...
	fprintf(stderr, "snd_pcm_poll_descriptors!\n");
	dmix->hw_fd = fd.fd;

	save_slave_setting(dmix, spcm);
	fprintf(stderr, "save_slave_setting!\n");
	...
	fprintf(stderr, "snd_pcm_direct_initialize_slave end !!! \n");
	return 0;
}
