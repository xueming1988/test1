						  ...
						  snd_pcm_uframes_t size)
{
	...
	if (snd_pcm_mmap_playback_avail(pcm) < size) {
		fprintf(stderr, "too short avail %ld to size %ld", snd_pcm_mmap_playback_avail(pcm), size);
		return -EPIPE;
	}
	while (size > 0) {
		...
		//fprintf(stderr, "w pcm_offset=%d, offset=%d, frames=%d, size=%d\n", (int)pcm_offset, (int)offset, (int)frames, (int)size);
		if (result < 0)
