...
{
	snd_pcm_direct_t *dmix = pcm->private_data;
	int mix_vol_max; int mix_vol_min;
	...
	if (snd_ctrl_vol < 100 || mute_flag)
	{
        if (mute_flag)
        {
          mix_vol_max = DITHER_MAX_VALUE;
           mix_vol_min = DITHER_MIN_VALUE;
        }
        if(snd_ctrl_vol < 100)
        {
         mix_vol_max = DITHER_MAX_VALUE;
            mix_vol_min = DITHER_MAX_VALUE * snd_ctrl_vol / 100;
        }
		vol_dec = 1;
		vol_inc = 0;
	}
	...
	if (snd_ctrl_vol < 100 || mute_flag)
	{
		if(*fixed_volume < mix_vol_min)
			*fixed_volume = mix_vol_min;
		else if (*fixed_volume > mix_vol_max)
			*fixed_volume = mix_vol_max;
	}
	else
	{
...
//fprintf(stderr,"%s-%d: pcm->snd_ctrl_index %d, vol_reset %d, fixed_volume %d, snd_ctrl_vol %d\n",__func__,__LINE__,pcm->snd_ctrl_index,vol_reset,*fixed_volume,snd_ctrl_vol);
	if(vol_reset)
	{
		if(vol_dec)
		{
			for (;;)
			{
				if (!*dst) {
					...
					*fixed_volume -= 0x5;
                    ...
                    {
                        *dst = dithered_vol(*dst, mix_vol_min);
                    }
                    else
                    {
	...
	//{
	//	fprintf(stderr,"snd_pcm_mmap_writei  \r\n");
